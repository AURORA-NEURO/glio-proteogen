...
The runtime requires all seven execution controls, emits scalar/interval baseline estimates only for evaluable locked requests, and rejects unsupported controls before input traversal. Results carry seven uncertainty dimensions, diagnostics, provenance, evidence, limitations, and a canonical replay digest.
