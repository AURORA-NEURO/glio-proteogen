# GLIO-PROTEOGEN-M12-04 module manifest

| Property | Provisional locked value |
| --- | --- |
| Module | GLIO-PROTEOGEN-M12-04 |
| Responsibility | Network, state, or mechanism inference beneath the Driver-to-protein consequence map |
| Owner / safety / gate | Data engineering / S2 / G2 |
...
| Operation | `infer_biomarker_panel_mechanism` |
| Input boundary | Opaque M12-01 hypothesis result reference, locked configuration, model/calibration references, mass-spectrometry/genome/transcriptome/PTM source references, and seven caller controls |
...
| API | `GET /v1/m12-04/schema/{name}`; `POST /v1/modules/M12-04/mechanism`; `POST /v1/modules/M12-04/verify` |
| CLI | `m1204_app export-schema NAME`; `m1204_app infer REQUEST [--output RESULT]`; `m1204_app verify RESULT` |
...
M12-04 lines 4128–4171. The dossier does not freeze endpoint names, wire media
...
M12-04 accepts only caller-declared references. It never opens raw proteome,
genome/transcriptome, PTM, model, calibration, or upstream result content. The
M12-01 result is bound by media type and digest as an opaque artifact. Identity,
