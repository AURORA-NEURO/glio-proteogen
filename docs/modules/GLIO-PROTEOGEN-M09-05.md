# GLIO-PROTEOGEN-M09-05 — mechanism and constraint integrator

M09-05 owns mechanism and constraint integration beneath Complex stoichiometry. The
implementation is deterministic and content-addressed while the dossier ABI, ontology catalogue,
estimator family, and endpoint media types remain provisional. It accepts caller-declared
mass-spectrometry, genome/transcriptome, PTM, configuration, identity, provenance, consent,
quality, support, and intended-use references and emits only a typed constraint-aware estimate
plus satisfaction report for the parent `complex_activity` target. It does not emit a parent
claim.

## Authority and safety boundary

- Authority: `GLIO-PROTEOGEN_240_Module_Dossier.md`, SHA-256
  `0a6b200cbe073db13a4bcf315edc23ab97edfe6f500bc7ea2785f5e1c70da181`, lines 3092–3135.
- Owner/safety/gate: Quality engineering / S2 / G2.
...
- Hard boundaries: no kinase-state ownership (KINOPHOS), generic all-omics fusion, direct
  treatment recommendation, identity or consent inference, upstream mutation, evidence
  relabeling, or unsupported-to-negative conversion.

## Contract and runtime behavior

The request binds the provisional M09-04 result by content-addressed artifact reference, an
immutable constraint policy, and seven caller-declared control decisions. Preflight requires
