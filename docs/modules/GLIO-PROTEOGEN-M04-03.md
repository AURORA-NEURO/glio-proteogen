# GLIO-PROTEOGEN-M04-03 - raw input ingestion

M04-03 validates four canonical raw-input manifest documents beneath C04 Proteoform/isoform
inference. It consumes the exact full M04-02 identity-lineage resolution and a separate in-memory
mapping of four roles to immutable bytes, then emits only validated manifest objects and typed
parse diagnostics supporting the parent `protein_rna_discordance` workflow. It never opens an
external scientific content artifact, parses spectra or scientific rows, computes a quality
metric, executes a model, or infers identity, consent, protein, proteoform, PTM, discordance,
...
2. Bind the operation `ingest_proteoform_raw_inputs`, contract version 1.0.0, and one identical
   opaque request identifier across the request and context. Revalidate the exact full public
   M04-02 `ProteoformIdentityLineageResolution`; a compact receipt or re-signed substitute is not
   accepted.
3. Bind context identity to the lineage identity digest, quality evidence to the M04-02 result
   digest, support evidence to the M04-02 receipt digest, intended-use evidence to the lineage
   receipt, and approved-configuration evidence to the M04-03 configuration digest. Close
   upstream completion and policy-review chronology before M04-03 completion.
4. Generate the four canonical documents before M04-02 construction. Bind their manifest
   references into a genuine public M04-02 request, execute genuine public M01-02, M04-01, and
   M04-02 operations, then construct M04-03. Handwritten upstream outputs are prohibited.
5. If the replayed M04-02 result is quarantined, emit typed M04-03 quarantine with
...
6. Require the exact four roles `mass_spectrometry_proteome`, `genome`, `transcriptome`, and
   `ptm_annotations`. Project them exactly to the four corresponding M04-02 manifest claims.
   Missing or extra mapping entries, arbitrary mappings, non-`bytes` values, and `bytes`
   subclasses reject before parsing.
7. Require each submitted M04-02 manifest reference, claim identifier, declared size, content
...
11. Embed exactly one strict document type for each role. Common metadata binds identity,
    protocol, reference bundle, coordinate policy, intended use, assay protocol, specimen
...
15. Return exactly seven local control-evidence records, one policy record, four through 32 parser
    profile records, four manifest-reference records, and four content-reference records: 20
    through 48 total. Embedded upstream evidence remains inside the replayed lineage result.
...
18. Use opaque local identifiers only in the namespaces `request`, `actor`, `decision`, `policy`,
    ...
    digest as `result.m0403.*` and `activity.m0403.*`.
...
20. Expose CLI `proteoform-raw export-schema NAME` and
    `proteoform-raw ingest REQUEST SOURCE --output RESULT`. `SOURCE` must contain exactly four
...
Every receipt and result retains `protein_rna_discordance` only as parent context and sets all
authority flags false. M04-03 emits no parent output, proteogenomic state, proteotype,
protein-level subtype, identity, consent, protein, proteoform, kinase state, CN-to-protein
regression, all-omics fusion, treatment recommendation, upstream mutation, or model execution.

## Architecture and authority boundary

The dossier lists an event-sourced quality service with stoichiometric factorization, a
schema-first batch service with a pathway activity network, and a quarantine-first deterministic
pipeline with a protein-complex graph. Gate G0 installs only the deterministic schema-first
manifest-validation boundary and quarantine-first failure semantics. It installs no scientific
graph, factorization, pathway model, anomaly learner, estimator, event log, database, object
...
carry a content reference to an external scientific artifact, but M04-03 never opens, fetches,
...
See the [module manifest](M04-03.manifest.md),
[evidence inventory](../evidence/M04-03.md), and
[traceability matrix](../traceability/GLIO-PROTEOGEN-M04-03.csv).
