...
frozen ABI or published runtime in this lane, so this implementation binds the
exact upstream artifact identity (ID, version, digest, and media type) without
importing M25-06 symbols or payloads.
...
- strict contracts close all seven operational dimensions, target/tolerance
  ...
  paths, source-artifact uniqueness, request-context identity, exact upstream
  artifact binding, and exact M25-06 media binding;
