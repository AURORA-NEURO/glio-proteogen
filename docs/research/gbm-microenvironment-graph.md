...
The source and graph receipts are nested in the bridge result and replayed
independently. The profile binds both child profile digests and the projection
policy. The synthetic demo adds disjoint MES and OPC markers to the AC-like
Neftel fixture so the graph surface demonstrates both supported directions;
those values are synthetic and contain no patient data.
