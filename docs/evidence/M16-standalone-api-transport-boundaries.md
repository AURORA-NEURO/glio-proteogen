# M16 standalone API transport boundaries

The standalone M16 adapters now enforce their declared byte ceilings at the
HTTP transport boundary. This is a resource-safety correction and does not
change the provisional M16 schemas, replay semantics, or scientific claims.

## Covered surfaces

M16-01, M16-02, M16-04, M16-05, M16-07, and M16-08 each expose a FastAPI
...
Before this correction, all six standalone M16 applications called
...
`tests/interfaces/test_m16_api_transport.py` covers both request and verify
routes for every standalone M16 adapter with oversized `Content-Length` values.
