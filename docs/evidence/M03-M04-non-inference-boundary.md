...
- All 12 executable M03/M04 evaluators pass (`m03_01` through `m03_08` and `m04_01`
  through `m04_04`).
...
- Combined contract+interface coverage is 94.14% (12,919 statements, 525 missed; 3,178
  branches, 370 partial) across the broad legacy M03/M04 surface. The 95% project threshold is
  not met by this broad scope because many pre-existing parser/plugin edge paths are outside this
  boundary change; the dedicated non-inference adversarial suite and all changed flag/replay
  paths pass. The contracts-only run is 92.37% for the same reason.
- `uv build --wheel --sdist` succeeds. The wheel is 3,201,226 bytes with SHA-256
  `673de530b72b01db30ff5c053f748453eefbb634d55f2f7cdcf719e02d72466f`; the sdist is
