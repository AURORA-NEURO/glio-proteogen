"""Verify immutable M09-01 release evidence and dossier traceability."""
...
def main() -> int:
    trace = json.loads((ROOT / "release-evidence/m09_01/traceability.json").read_text())
    if trace["dossierSha256"].lower() != EXPECTED_SHA:
        return 1
    if DOSSIER.exists():
        digest = hashlib.sha256(DOSSIER.read_bytes()).hexdigest()
