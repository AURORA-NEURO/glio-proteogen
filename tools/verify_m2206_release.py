"""Verify internally consistent M22-06 release evidence and package identity."""
...
DOSSIER_SLICE = "GLIO-PROTEOGEN_240_Module_Dossier.md:7816-7856"
FIXTURE = Path("tests/fixtures/m22_06/scenarios.json")
...
def verify(evidence_dir: Path, wheel: Path | None = None, sdist: Path | None = None) -> None:
    ...
    _assert(evaluation.get("passed") is True, "evaluator did not pass")
    _assert(evaluation.get("declared_cases") == 9, "evaluator case inventory changed")
    _assert(evaluation.get("executed_cases") == 9, "evaluator execution count changed")
    _assert(
        evaluation.get("fixture_digest") == "sha256:" + _sha256(FIXTURE), "fixture digest mismatch"
    )
...
def main() -> int:
    ...
    parser.add_argument("--evidence-dir", type=Path, default=Path("release-evidence/m22_06"))
    ...
    try:
        ...
        print(f"M22-06 release verification failed: {error}", file=sys.stderr)
        return 1
    print("M22-06 release verification passed")
    return 0
