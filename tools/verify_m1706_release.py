"""Verify the reproducible M17-06 release evidence bundle."""
...
FIXTURE_DIGEST = "sha256:37f794e2cefa2900aae00acc66b1f8d949bf269f3ee63bbc31d25c3910b4ac6d"
CASE_IDS = (
...
AUTHORITY_LINES = "6016-6056"


class M1706ReleaseEvidenceError(ValueError):
    """Raised when M17-06 release evidence is incomplete or inconsistent."""


def _mapping(value: object, label: str) -> Mapping[str, object]:
    if not isinstance(value, dict):
        raise M1706ReleaseEvidenceError(f"{label} must be an object")
    return value
...
def _load(path: Path, label: str) -> Mapping[str, object]:
    try:
        ...
        raise M1706ReleaseEvidenceError(f"{label} is not valid UTF-8 JSON") from error


def _require(document: Mapping[str, object], field: str, expected: object, label: str) -> None:
    if document.get(field) != expected:
        raise M1706ReleaseEvidenceError(f"{label} has unexpected {field}")


def verify_evaluation(path: Path) -> None:
    report = _load(path, "M17-06 evaluation report")
    ...
    if report.get("passed") is not True:
        raise M1706ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list):
        raise M1706ReleaseEvidenceError("evaluation checks must be an array")
    names: list[str] = []
    for check in checks:
        item = _mapping(check, "evaluation check")
        name = item.get("name")
        if not isinstance(name, str):
            raise M1706ReleaseEvidenceError("evaluation check has no name")
        names.append(name)
        if item.get("passed") is not True:
            raise M1706ReleaseEvidenceError("evaluation check did not pass")
    if tuple(names) != CASE_IDS:
        raise M1706ReleaseEvidenceError("evaluation report lacks exact scenario closure")


def verify_benchmark(path: Path) -> None:
    report = _load(path, "M17-06 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1706ReleaseEvidenceError("benchmark report did not pass")

    def numeric(value: object, field: str) -> float:
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise M1706ReleaseEvidenceError(f"benchmark {field} is not numeric")
        if not math.isfinite(value) or value < 0:
            raise M1706ReleaseEvidenceError(f"benchmark {field} is invalid")
        return float(value)

    if numeric(report.get("mean_ns"), "mean_ns") > MEAN_BUDGET_NS:
        raise M1706ReleaseEvidenceError("benchmark exceeds mean budget")
    if numeric(report.get("p95_ns"), "p95_ns") > P95_BUDGET_NS:
        raise M1706ReleaseEvidenceError("benchmark exceeds p95 budget")


def verify_coverage(path: Path) -> None:
    report = _load(path, "M17-06 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1706ReleaseEvidenceError("coverage report is not branch-enabled")
    percent = report.get("percent")
    threshold = report.get("fail_under")
    if isinstance(percent, bool) or not isinstance(percent, (int, float)):
        raise M1706ReleaseEvidenceError("coverage percentage is not numeric")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)):
        raise M1706ReleaseEvidenceError("coverage fail-under is not numeric")
    if not math.isfinite(percent) or percent < threshold or threshold < FAIL_UNDER:
        raise M1706ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1706ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: Mapping[str, object], label: str) -> None:
    if not path.is_file():
        raise M1706ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    report = _load(path, "M17-06 package report")
    ...
    if not isinstance(wheel_name, str) or not wheel_name.endswith(".whl"):
        raise M1706ReleaseEvidenceError("wheel filename is invalid")
    if not isinstance(sdist_name, str) or not sdist_name.endswith(".tar.gz"):
        raise M1706ReleaseEvidenceError("sdist filename is invalid")
    ...
    if report.get("isolated_import") is not True:
        raise M1706ReleaseEvidenceError("isolated import evidence is missing")
