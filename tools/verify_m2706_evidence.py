"""Verify internally consistent M27-06 machine evidence."""
...
EVIDENCE_ROOT = Path(__file__).parents[1] / "docs" / "evidence" / "m27_06"
EXPECTED_CHECKS = 8
EXPECTED_ITERATIONS = 10


class M2706EvidenceError(ValueError):
...
def _load(name: str) -> dict[str, Any]:
    try:
        ...
        raise M2706EvidenceError(f"unable to read {name}") from error
    if not isinstance(value, dict):
        raise M2706EvidenceError(f"{name} must contain an object")
    return value


def _require(condition: bool, message: str) -> None:  # noqa: FBT001
    if not condition:
        raise M2706EvidenceError(message)
...
def verify() -> dict[str, Any]:
    ...
    _require(
        evaluation.get("checks_declared") == evaluation.get("checks_passed") == EXPECTED_CHECKS,
        "evaluator matrix incomplete",
    )
    _require(benchmark.get("passed") is True, "benchmark failed")
    _require(benchmark.get("iterations") == EXPECTED_ITERATIONS, "iteration count mismatch")
    _require(benchmark.get("warmup") is True, "warmup missing")
    _require(
    ...
    for label in ("wheel", "sdist"):
        ...
        if not isinstance(artifact, dict):
            raise M2706EvidenceError(f"{label} package record missing")
        _require(
            SHA256_PATTERN.fullmatch(str(artifact.get("sha256", ""))) is not None,
            f"{label} package digest invalid",
        )
        _require(int(artifact.get("bytes", 0)) > 0, f"{label} package size missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except M2706EvidenceError as error:
        print(f"M27-06 evidence verification failed: {error}", file=sys.stderr)
        return 1
    return 0
