"""Verify the M14-04 executable and package release evidence."""
...
MODULE_ID: Final = "GLIO-PROTEOGEN-M14-04"
ROOT: Final = Path(__file__).parents[1]
EVIDENCE: Final = ROOT / "release-evidence" / "m14_04"
FIXTURE: Final = ROOT / "tests" / "fixtures" / "m14_04" / "scenarios.json"
FIXTURE_DIGEST: Final = "sha256:36403b923926f0c1de7eda06fae922d2ee9a63253b3ea09f697d6e2dc3b1b9e6"
CASE_IDS: Final = (
    "posterior_inference",
    "state_inference",
    "explicit_abstention",
...
class M1404ReleaseVerificationError(ValueError):
...
def _read(name: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise M1404ReleaseVerificationError(f"{name} must contain an object")
...
def _verify_evaluation(value: dict[str, Any]) -> None:
    ...
    if value.get("module_id") != MODULE_ID or value.get("fixture_sha256") != FIXTURE_DIGEST:
        raise M1404ReleaseVerificationError("evaluation module or fixture digest mismatch")
    if actual_ids != CASE_IDS or tuple(value.get("case_ids", ())) != CASE_IDS:
        raise M1404ReleaseVerificationError("evaluation case IDs mismatch")
    if not (
        ...
        raise M1404ReleaseVerificationError("evaluation is not a complete pass")


def _verify_benchmark(value: dict[str, Any]) -> None:
    if value.get("module_id") != MODULE_ID or value.get("iterations", 0) < 1:
        raise M1404ReleaseVerificationError("benchmark identity or iteration count invalid")
    if not (
        value.get("mean_ns", 0) <= value.get("mean_budget_ns", 0)
        and value.get("p95_ns", 0) <= value.get("p95_budget_ns", 0)
        and value.get("passed") is True
    ):
        raise M1404ReleaseVerificationError("benchmark exceeds declared provisional budget")


def _verify_package(value: dict[str, Any]) -> None:
    if value.get("module_id") != MODULE_ID or value.get("passed") is not True:
        raise M1404ReleaseVerificationError("package evidence identity or pass flag invalid")
    for kind in ("wheel", "sdist"):
        package = cast("dict[str, Any]", value.get(kind, {}))
        path = next((ROOT / "dist-m14-04").glob(str(package.get("filename", ""))), None)
        if path is None or not path.is_file():
            raise M1404ReleaseVerificationError(f"missing {kind} artifact")
        if _sha256(path) != package.get("sha256") or path.stat().st_size != package.get("bytes"):
            raise M1404ReleaseVerificationError(f"{kind} hash or size mismatch")
        if kind == "wheel":
        ...
        if members != package.get("members"):
            raise M1404ReleaseVerificationError(f"{kind} member count mismatch")
    isolated = cast("dict[str, Any]", value.get("isolated_import", {}))
    if isolated.get("passed") is not True:
        raise M1404ReleaseVerificationError("isolated import check did not pass")
...
def verify_release() -> dict[str, object]:
    ...
    if coverage.get("coverage_percent", 0) < coverage.get("fail_under_percent", 95):
        raise M1404ReleaseVerificationError("coverage evidence is below fail-under")
...
def main() -> int:
    verify_release()
    print("M14-04 release evidence verified")  # noqa: T201
    return 0
