"""Verify internally consistent machine-readable M27-04 release evidence."""
...
EVIDENCE_ROOT = Path(__file__).parents[1] / "docs" / "evidence" / "m27_04"
EXPECTED_CHECKS = 10
EXPECTED_ITERATIONS = 10


class M2704EvidenceError(ValueError):
...
def _load(name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise M2704EvidenceError(f"unable to read {path}") from error
    if not isinstance(value, dict):
        raise M2704EvidenceError(f"{name} must contain a JSON object")
    return value


def _require(condition: bool, message: str) -> None:  # noqa: FBT001
    if not condition:
        raise M2704EvidenceError(message)
...
def verify() -> dict[str, Any]:
    ...
    _require(
        ...
        "benchmark exceeds its declared budgets",
    )
    ...
    if not isinstance(wheel, dict) or not isinstance(sdist, dict):
        raise M2704EvidenceError("package records missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except M2704EvidenceError as error:
        print(f"M27-04 evidence verification failed: {error}", file=sys.stderr)
        return 1
    return 0
