"""Verify the reproducible M19-01 release evidence bundle."""
...
def _number(value: object, field: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise M1901ReleaseEvidenceError(f"benchmark {field} is not numeric")
    if not math.isfinite(value) or value < 0:
        raise M1901ReleaseEvidenceError(f"benchmark {field} is invalid")
    return float(value)
...
def verify_coverage(path: Path) -> None:
    ...
    if percent < threshold or threshold < FAIL_UNDER:
        raise M1901ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1901ReleaseEvidenceError(f"coverage {field} is invalid")
