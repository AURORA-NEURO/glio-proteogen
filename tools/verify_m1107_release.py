"""Verify M11-07 release evidence, fixture binding, and optional artifacts."""
...
_MODULE_ID: Final = "GLIO-PROTEOGEN-M11-07"
_FIXTURE = Path(__file__).parents[1] / "tests" / "fixtures" / "m11_07" / "scenarios.json"
_EVIDENCE = Path(__file__).parents[1] / "release-evidence" / "m11_07"
_CASE_IDS: Final = (
    "supported",
    "failed_control",
    "not_evaluable",
...
class M1107ReleaseVerificationError(ValueError):
    """Raised when release evidence does not close the M11-07 gate."""
...
def _fail(code: str) -> NoReturn:
    raise M1107ReleaseVerificationError(code)
...
def _read(name: str) -> dict[str, object]:
    ...
    try:
        ...
        raise M1107ReleaseVerificationError("read") from error
...
def verify_release(*, wheel: Path | None = None, sdist: Path | None = None) -> dict[str, object]:
    """Verify all committed M11-07 evidence and optional built artifacts."""
...
def main() -> int:
    ...
    except M1107ReleaseVerificationError as error:
        sys.stderr.write(f"M11-07 release verification failed: {error}\n")
        return 1
...
__all__ = ["M1107ReleaseVerificationError", "verify_release"]
