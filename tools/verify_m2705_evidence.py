"""Verify internally consistent M27-05 machine evidence."""
...
EVIDENCE_ROOT = Path(__file__).parents[1] / "docs" / "evidence" / "m27_05"
EXPECTED_CHECKS = 7
EXPECTED_ITERATIONS = 10


class M2705EvidenceError(ValueError):
...
def _load(name: str) -> dict[str, Any]:
    try:
        ...
        raise M2705EvidenceError(f"unable to read {name}") from error
    if not isinstance(value, dict):
        raise M2705EvidenceError(f"{name} must contain an object")
    return value


def _require(condition: bool, message: str) -> None:  # noqa: FBT001
    if not condition:
        raise M2705EvidenceError(message)
...
def verify() -> dict[str, Any]:
    ...
    for label in ("wheel", "sdist"):
        ...
        if not isinstance(artifact, dict):
            raise M2705EvidenceError(f"{label} package record missing")
        _require(
...
def main(argv: list[str] | None = None) -> int:
    ...
    except M2705EvidenceError as error:
        print(f"M27-05 evidence verification failed: {error}", file=sys.stderr)
        return 1
    return 0
