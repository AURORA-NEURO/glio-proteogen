"""Verify M24-06 release-evidence manifests and package records."""
...
UPSTREAM_MEDIA_TYPE = "application/vnd.glio-proteogen.m24-05+json"
...
def verify(root: Path, wheel: Path | None = None, sdist: Path | None = None) -> dict[str, object]:
    evidence = root / "release-evidence" / "m24_06"
    ...
    checks: dict[str, bool] = {
        ...
        "dependency": evaluation.get("upstream_media_type") == UPSTREAM_MEDIA_TYPE,
    ...
    if wheel is not None and sdist is not None:
        ...
        if isinstance(wheel_record, dict) and isinstance(sdist_record, dict):
            ...
            if checks["package"]:
                try:
                    checks["wheel_members"] = len(ZipFile(wheel).namelist()) == cast(
