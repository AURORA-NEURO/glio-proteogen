"""Verify fixture-bound M14-01 release evidence and package metadata."""
...
MODULE_ID: Final = "GLIO-PROTEOGEN-M14-01"
...
class M1401ReleaseVerificationError(ValueError):
    """M14-01 release evidence is not internally locked."""

    def __init__(self) -> None:
        super().__init__("M14-01 release evidence verification failed")
...
def _verify_evaluation(evaluation: dict[str, Any]) -> None:
    if evaluation["module_id"] != MODULE_ID or evaluation["fixture_digest"] != FIXTURE_DIGEST:
        raise M1401ReleaseVerificationError
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        sys.stderr.write("usage: verify_m1401_release.py RELEASE_EVIDENCE_DIR\n")
        return 2
    try:
        sys.stdout.write(json.dumps(verify_release(Path(args[0])), sort_keys=True) + "\n")
    except (OSError, KeyError, TypeError, M1401ReleaseVerificationError) as error:
        sys.stderr.write(f"M14-01 release verification failed: {error}\n")
        return 1
    return 0
