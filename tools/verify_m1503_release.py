"""Verify M15-03 release-evidence closure without network access."""
...
# The verifier intentionally reports concise protocol diagnostics.
# ruff: noqa: PLR2004, TRY003, TRY004, T201

ROOT: Final = Path(__file__).parents[1]
EVIDENCE: Final = ROOT / "release-evidence" / "m15_03"
FIXTURE: Final = ROOT / "tests" / "fixtures" / "m15_03" / "scenarios.json"
HISTORICAL_ARTIFACTS: Final = ROOT / "dist-m15-03"
EXPECTED_FIXTURE_DIGEST: Final = (
    "sha256:5c43f9072e6ef78cb60b3525fb8140612a1da21e9c079f4013c4a5c6960f0d3b"
)
...
def verify_release() -> dict[str, object]:
    ...
    _require(FIXTURE.exists(), "M15-03 fixture is missing")
    _require(_sha256(FIXTURE) == EXPECTED_FIXTURE_DIGEST, "fixture digest mismatch")
    _require(evaluation.get("module_id") == "GLIO-PROTEOGEN-M15-03", "evaluation module mismatch")
    _require(
    ...
    _require(
        benchmark.get("module_id") == "GLIO-PROTEOGEN-M15-03"
    ...
    _require(
        coverage.get("module_id") == "GLIO-PROTEOGEN-M15-03"
    ...
    _require(package.get("module_id") == "GLIO-PROTEOGEN-M15-03", "package module mismatch")
    _require(
    ...
    traceability = ROOT / "docs" / "traceability" / "GLIO-PROTEOGEN-M15-03.csv"
    _require(
    ...
    return {
        ...
        "declared_cases": EXPECTED_CASE_COUNT,
        "executed_cases": EXPECTED_CASE_COUNT,
