"""Verify the reproducible M15-04 release evidence bundle."""
...
AUTHORITY_LINES = "5208-5248"


class M1504ReleaseEvidenceError(ValueError):
    """Raised when M15-04 release evidence is incomplete or inconsistent."""


def _mapping(value: object, label: str) -> Mapping[str, object]:
    if not isinstance(value, dict):
        raise M1504ReleaseEvidenceError(f"{label} must be an object")
    return value
...
def verify_evaluation(path: Path) -> None:
    ...
    if report.get("passed") is not True:
        raise M1504ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list):
        raise M1504ReleaseEvidenceError("evaluation checks must be an array")
    names: list[str] = []
    for check in checks:
        item = _mapping(check, "evaluation check")
        name = item.get("name")
        if not isinstance(name, str):
            raise M1504ReleaseEvidenceError("evaluation check has no name")
        names.append(name)
        if item.get("passed") is not True:
            raise M1504ReleaseEvidenceError("evaluation check did not pass")
    if tuple(names) != CASE_IDS:
        raise M1504ReleaseEvidenceError("evaluation report lacks exact scenario closure")


def verify_benchmark(path: Path) -> None:
    """Require finite timing values within the provisional budgets."""
    report = _load(path, "M15-04 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1504ReleaseEvidenceError("benchmark report did not pass")
    mean_ns = report.get("mean_ns")
    p95_ns = report.get("p95_ns")
    if isinstance(mean_ns, bool) or not isinstance(mean_ns, (int, float)):
        raise M1504ReleaseEvidenceError("benchmark mean is not numeric")
    if isinstance(p95_ns, bool) or not isinstance(p95_ns, (int, float)):
        raise M1504ReleaseEvidenceError("benchmark p95 is not numeric")
    if (
        ...
        raise M1504ReleaseEvidenceError("benchmark exceeds provisional timing budgets")


def verify_coverage(path: Path) -> None:
    """Require branch-enabled coverage above the governed threshold."""
    report = _load(path, "M15-04 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1504ReleaseEvidenceError("coverage report is not branch-enabled")
    percent = report.get("percent")
    threshold = report.get("fail_under")
    if isinstance(percent, bool) or not isinstance(percent, (int, float)):
        raise M1504ReleaseEvidenceError("coverage percentage is not numeric")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)):
        raise M1504ReleaseEvidenceError("coverage fail-under is not numeric")
    if not math.isfinite(percent) or percent < threshold or threshold < FAIL_UNDER:
        raise M1504ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1504ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: Mapping[str, object], label: str) -> None:
    if not path.is_file():
        raise M1504ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    ...
    report = _load(path, "M15-04 package report")
    ...
    if not isinstance(wheel_name, str) or not wheel_name.endswith(".whl"):
        raise M1504ReleaseEvidenceError("wheel filename is invalid")
    if not isinstance(sdist_name, str) or not sdist_name.endswith(".tar.gz"):
        raise M1504ReleaseEvidenceError("sdist filename is invalid")
    ...
    if report.get("isolated_import") is not True:
        raise M1504ReleaseEvidenceError("isolated import evidence is missing")


def verify_release(evidence_dir: Path, dist_dir: Path) -> None:
    """Verify every M15-04 release evidence component."""
