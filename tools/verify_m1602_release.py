"""Verify the reproducible M16-02 release evidence bundle."""
...
FIXTURE_DIGEST = "sha256:555ef5945bf3c70d69e05ce71d828545f664958113308fd2c5ac6bfbd08d4af6"
CASE_IDS = (
    "aligned_reconciled",
...
AUTHORITY_LINES = "5480-5520"


class M1602ReleaseEvidenceError(ValueError):
    """Raised when M16-02 release evidence is incomplete or inconsistent."""


def _mapping(value: object, label: str) -> Mapping[str, object]:
    if not isinstance(value, dict):
        raise M1602ReleaseEvidenceError(f"{label} must be an object")
    return value
...
def _load(path: Path, label: str) -> Mapping[str, object]:
    try:
        ...
        raise M1602ReleaseEvidenceError(f"{label} is not valid UTF-8 JSON") from error


def _require(document: Mapping[str, object], field: str, expected: object, label: str) -> None:
    if document.get(field) != expected:
        raise M1602ReleaseEvidenceError(f"{label} has unexpected {field}")


def verify_evaluation(path: Path) -> None:
    report = _load(path, "M16-02 evaluation report")
    ...
    if report.get("passed") is not True:
        raise M1602ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list):
        raise M1602ReleaseEvidenceError("evaluation checks must be an array")
    names: list[str] = []
    for check in checks:
        item = _mapping(check, "evaluation check")
        name = item.get("name")
        if not isinstance(name, str):
            raise M1602ReleaseEvidenceError("evaluation check has no name")
        names.append(name)
        if item.get("passed") is not True:
            raise M1602ReleaseEvidenceError("evaluation check did not pass")
    if tuple(names) != CASE_IDS:
        raise M1602ReleaseEvidenceError("evaluation report lacks exact scenario closure")


def verify_benchmark(path: Path) -> None:
    report = _load(path, "M16-02 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1602ReleaseEvidenceError("benchmark report did not pass")
    mean_ns = report.get("mean_ns")
    p95_ns = report.get("p95_ns")
    if isinstance(mean_ns, bool) or not isinstance(mean_ns, (int, float)):
        raise M1602ReleaseEvidenceError("benchmark mean is not numeric")
    if isinstance(p95_ns, bool) or not isinstance(p95_ns, (int, float)):
        raise M1602ReleaseEvidenceError("benchmark p95 is not numeric")
    if (
        ...
        raise M1602ReleaseEvidenceError("benchmark exceeds provisional timing budgets")


def verify_coverage(path: Path) -> None:
    report = _load(path, "M16-02 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1602ReleaseEvidenceError("coverage report is not branch-enabled")
    percent = report.get("percent")
    threshold = report.get("fail_under")
    if isinstance(percent, bool) or not isinstance(percent, (int, float)):
        raise M1602ReleaseEvidenceError("coverage percentage is not numeric")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)):
        raise M1602ReleaseEvidenceError("coverage fail-under is not numeric")
    if not math.isfinite(percent) or percent < threshold or threshold < FAIL_UNDER:
        raise M1602ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1602ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: Mapping[str, object], label: str) -> None:
    if not path.is_file():
        raise M1602ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    report = _load(path, "M16-02 package report")
    ...
    if not isinstance(wheel_name, str) or not wheel_name.endswith(".whl"):
        raise M1602ReleaseEvidenceError("wheel filename is invalid")
    if not isinstance(sdist_name, str) or not sdist_name.endswith(".tar.gz"):
        raise M1602ReleaseEvidenceError("sdist filename is invalid")
    ...
    if report.get("isolated_import") is not True:
        raise M1602ReleaseEvidenceError("isolated import evidence is missing")
