"""Verify the reproducible M14-02 release evidence bundle."""
...
FIXTURE_DIGEST = "sha256:a6222caae33611ae49f087793ee6aa7ec5762867585a2f9ed594ee4999e17ea4"
CASE_IDS = (
...
class M1402ReleaseEvidenceError(ValueError):
    """Raised when M14-02 release evidence is incomplete or inconsistent."""


def _mapping(value: object, label: str) -> Mapping[str, object]:
    if not isinstance(value, dict):
        raise M1402ReleaseEvidenceError(f"{label} must be an object")
    return value
...
def verify_evaluation(path: Path) -> None:
    ...
    if report.get("passed") is not True:
        raise M1402ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list):
        raise M1402ReleaseEvidenceError("evaluation checks must be an array")
    names: list[str] = []
    for check in checks:
        item = _mapping(check, "evaluation check")
        name = item.get("name")
        if not isinstance(name, str):
            raise M1402ReleaseEvidenceError("evaluation check has no name")
        names.append(name)
        if item.get("passed") is not True:
            raise M1402ReleaseEvidenceError("evaluation check did not pass")
    if tuple(names) != CASE_IDS:
        raise M1402ReleaseEvidenceError("evaluation report lacks exact scenario closure")


def verify_benchmark(path: Path) -> None:
    """Verify the ten-call benchmark and provisional timing budgets."""

    report = _load(path, "M14-02 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1402ReleaseEvidenceError("benchmark report did not pass")
    mean = report.get("mean_ns")
    p95 = report.get("p95_ns")
    if isinstance(mean, bool) or not isinstance(mean, (int, float)):
        raise M1402ReleaseEvidenceError("benchmark mean is not numeric")
    if isinstance(p95, bool) or not isinstance(p95, int):
        raise M1402ReleaseEvidenceError("benchmark p95 is not an integer")
    if (
        ...
        raise M1402ReleaseEvidenceError("benchmark exceeds provisional timing budgets")


def verify_coverage(path: Path) -> None:
    """Verify branch-enabled scoped coverage meets the release threshold."""

    report = _load(path, "M14-02 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1402ReleaseEvidenceError("coverage report is not branch-enabled")
    threshold = report.get("fail_under")
    percent = report.get("branch_percent")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)):
        raise M1402ReleaseEvidenceError("coverage fail-under is not numeric")
    if isinstance(percent, bool) or not isinstance(percent, (int, float)):
        raise M1402ReleaseEvidenceError("coverage percentage is not numeric")
    if threshold < FAIL_UNDER or not math.isfinite(percent) or percent < threshold:
        raise M1402ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1402ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: Mapping[str, object], label: str) -> None:
    if not path.is_file():
        raise M1402ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    """Verify wheel/sdist hashes, sizes, members, and isolated import evidence."""

    report = _load(path, "M14-02 package report")
    ...
    if not isinstance(wheel_name, str) or not wheel_name.endswith(".whl"):
        raise M1402ReleaseEvidenceError("wheel filename is invalid")
    if not isinstance(sdist_name, str) or not sdist_name.endswith(".tar.gz"):
        raise M1402ReleaseEvidenceError("sdist filename is invalid")
    ...
    if report.get("isolated_import") is not True:
        raise M1402ReleaseEvidenceError("isolated import evidence is missing")


def verify_release(evidence_dir: Path, dist_dir: Path) -> None:
    """Verify every M14-02 release-evidence document against the candidate build."""
