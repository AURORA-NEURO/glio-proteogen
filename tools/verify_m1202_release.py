"""Verify fixture-bound M12-02 release evidence and package metadata."""
...
MODULE_ID: Final = "GLIO-PROTEOGEN-M12-02"
...
FIXTURE_DIGEST: Final = "sha256:b0b925512c9ebcf106b2cf4879193ef9c2f5b873deca91a91f4cae13a693e9c8"
MIN_COVERAGE_PERCENT: Final = 95.0
_SHA256 = re.compile(r"^[0-9a-f]{64}$")


class M1202ReleaseVerificationError(ValueError):
    """M12-02 release evidence is not internally locked."""

    def __init__(self) -> None:
        super().__init__("M12-02 release evidence verification failed")
...
def _verify_evaluation(evaluation: dict[str, Any]) -> None:
    if (
        ...
        raise M1202ReleaseVerificationError
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        sys.stderr.write("usage: verify_m1202_release.py RELEASE_EVIDENCE_DIR\n")
        return 2
    try:
        sys.stdout.write(json.dumps(verify_release(Path(args[0])), sort_keys=True) + "\n")
    except (OSError, KeyError, TypeError, ValueError, M1202ReleaseVerificationError) as error:
        sys.stderr.write(f"M12-02 release verification failed: {error}\n")
        return 1
    return 0
