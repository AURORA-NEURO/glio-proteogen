"""Verify M26-03 release-evidence manifests and optional package records."""
...
def verify(root: Path, wheel: Path | None = None, sdist: Path | None = None) -> dict[str, object]:
    """Return independent consistency checks for committed M26-03 evidence."""

    evidence = root / "release-evidence" / "m26_03"
    ...
    checks: dict[str, bool] = {
        ...
        "dependency": evaluation.get("parent_target") == PARENT
        and evaluation.get("upstream_dependency") == "M26-01 and M26-02 media-only boundaries"
        and evaluation.get("media_only_boundary")
        == "M26-02 runtime ABI unpublished; no upstream imports",
        "evaluation": evaluation.get("passed") == SCENARIO_COUNT
