"""Verify the reproducible M19-03 release evidence bundle."""
...
AUTHORITY_LINES = "GLIO-PROTEOGEN_240_Module_Dossier.md:6604-6644"
SCENARIO_COUNT = 8
...
class M1903ReleaseEvidenceError(ValueError):
    """Raised when M19-03 release evidence is incomplete or inconsistent."""


def _mapping(value: object, label: str) -> Mapping[str, object]:
    if not isinstance(value, dict):
        raise M1903ReleaseEvidenceError(f"{label} must be an object")
    return value
...
def _load(path: Path, label: str) -> Mapping[str, object]:
    try:
        ...
        raise M1903ReleaseEvidenceError(f"{label} is not valid UTF-8 JSON") from error


def _require(document: Mapping[str, object], field: str, expected: object, label: str) -> None:
    if document.get(field) != expected:
        raise M1903ReleaseEvidenceError(f"{label} has unexpected {field}")


def verify_evaluation(path: Path) -> None:
    report = _load(path, "M19-03 evaluation report")
    ...
    if report.get("passed") is not True:
        raise M1903ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list) or not checks:
        raise M1903ReleaseEvidenceError("evaluation checks must be a non-empty array")
    for check in checks:
        item = _mapping(check, "evaluation check")
        if not isinstance(item.get("name"), str) or item.get("passed") is not True:
            raise M1903ReleaseEvidenceError("evaluation check did not pass")


def verify_benchmark(path: Path) -> None:
    report = _load(path, "M19-03 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1903ReleaseEvidenceError("benchmark report did not pass")

    def numeric(value: object, field: str) -> float:
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise M1903ReleaseEvidenceError(f"benchmark {field} is not numeric")
        if not math.isfinite(value) or value < 0:
            raise M1903ReleaseEvidenceError(f"benchmark {field} is invalid")
        return float(value)

    if numeric(report.get("mean_ns"), "mean_ns") > MEAN_BUDGET_NS:
        raise M1903ReleaseEvidenceError("benchmark exceeds mean budget")
    if numeric(report.get("p95_ns"), "p95_ns") > P95_BUDGET_NS:
        raise M1903ReleaseEvidenceError("benchmark exceeds p95 budget")


def verify_coverage(path: Path) -> None:
    report = _load(path, "M19-03 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1903ReleaseEvidenceError("coverage report is not branch-enabled")
    percent = report.get("percent")
    threshold = report.get("fail_under")
    if isinstance(percent, bool) or not isinstance(percent, (int, float)):
        raise M1903ReleaseEvidenceError("coverage percentage is not numeric")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)):
        raise M1903ReleaseEvidenceError("coverage fail-under is not numeric")
    if not math.isfinite(percent) or percent < threshold or threshold < FAIL_UNDER:
        raise M1903ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1903ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: Mapping[str, object], label: str) -> None:
    if not path.is_file():
        raise M1903ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    report = _load(path, "M19-03 package report")
    ...
    if not isinstance(wheel_name, str) or not wheel_name.endswith(".whl"):
        raise M1903ReleaseEvidenceError("wheel filename is invalid")
    if not isinstance(sdist_name, str) or not sdist_name.endswith(".tar.gz"):
        raise M1903ReleaseEvidenceError("sdist filename is invalid")
    ...
    if report.get("isolated_import") is not True:
        raise M1903ReleaseEvidenceError("isolated import evidence is missing")
