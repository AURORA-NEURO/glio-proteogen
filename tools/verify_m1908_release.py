"""Verify the reproducible M19-08 release-evidence bundle."""
...
AUTHORITY_SLICE = "GLIO-PROTEOGEN_240_Module_Dossier.md:6824-6864"
SCENARIOS = 8
ADVERSARIAL = 8
...
class M1908ReleaseEvidenceError(ValueError):
    """Raised when M19-08 release evidence is incomplete or inconsistent."""
...
def _load(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise M1908ReleaseEvidenceError(f"{label} is not valid UTF-8 JSON") from error
    if not isinstance(value, dict):
        raise M1908ReleaseEvidenceError(f"{label} must be an object")
    return value


def _require(document: dict[str, Any], field: str, expected: object, label: str) -> None:
    if document.get(field) != expected:
        raise M1908ReleaseEvidenceError(f"{label} has unexpected {field}")


def _number(value: object, field: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise M1908ReleaseEvidenceError(f"{field} is not numeric")
    if not math.isfinite(value) or value < 0:
        raise M1908ReleaseEvidenceError(f"{field} is invalid")
    return float(value)


def verify_evaluation(path: Path) -> None:
    report = _load(path, "M19-08 evaluation report")
    ...
    if report.get("passed") is not True or _number(
        report.get("adversarial_coverage_percent"), "adversarial coverage"
    ) < ADVERSARIAL_TARGET:
        raise M1908ReleaseEvidenceError("evaluation report did not pass")
    checks = report.get("checks")
    if not isinstance(checks, list) or not checks:
        raise M1908ReleaseEvidenceError("evaluation checks must be a non-empty array")
    if any(not isinstance(item, dict) or item.get("passed") is not True for item in checks):
        raise M1908ReleaseEvidenceError("evaluation check did not pass")


def verify_benchmark(path: Path) -> None:
    report = _load(path, "M19-08 benchmark report")
    ...
    if report.get("passed") is not True:
        raise M1908ReleaseEvidenceError("benchmark report did not pass")
    if _number(report.get("mean_ns"), "mean_ns") > MEAN_BUDGET_NS:
        raise M1908ReleaseEvidenceError("benchmark exceeds mean budget")
    if _number(report.get("p95_ns"), "p95_ns") > P95_BUDGET_NS:
        raise M1908ReleaseEvidenceError("benchmark exceeds p95 budget")
    for field in ("request_digest", "result_digest"):
        if not isinstance(report.get(field), str) or not report[field].startswith("sha256:"):
            raise M1908ReleaseEvidenceError(f"benchmark {field} is not a digest")


def verify_coverage(path: Path) -> None:
    report = _load(path, "M19-08 coverage report")
    _require(report, "module_id", MODULE_ID, "coverage report")
    if report.get("branch") is not True:
        raise M1908ReleaseEvidenceError("coverage report is not branch-enabled")
    if _number(report.get("percent"), "coverage percent") < _number(
        report.get("fail_under"), "coverage fail-under"
    ) or _number(report.get("fail_under"), "coverage fail-under") < FAIL_UNDER:
        raise M1908ReleaseEvidenceError("coverage is below fail-under")
    for field in ("statements", "covered_statements", "branches", "covered_branches"):
        value = report.get(field)
        if type(value) is not int or value < 0:
            raise M1908ReleaseEvidenceError(f"coverage {field} is invalid")
...
def _verify_file(path: Path, item: dict[str, Any], label: str) -> None:
    if not path.is_file():
        raise M1908ReleaseEvidenceError(f"{label} is missing")
...
def verify_package(path: Path, dist_dir: Path) -> None:
    report = _load(path, "M19-08 package report")
    _require(report, "module_id", MODULE_ID, "package report")
    if report.get("reproducible_builds") is not True or report.get("isolated_import") is not True:
        raise M1908ReleaseEvidenceError("package reproducibility/import evidence is missing")
    wheel = report.get("wheel")
    sdist = report.get("sdist")
    if not isinstance(wheel, dict) or not isinstance(sdist, dict):
        raise M1908ReleaseEvidenceError("package wheel/sdist records are required")
    _verify_file(dist_dir / str(wheel.get("filename")), wheel, "package wheel")
    _verify_file(dist_dir / str(sdist.get("filename")), sdist, "package sdist")
