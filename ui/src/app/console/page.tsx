...
export default function ConsolePage() {
  ...
  async function prepareConsole() {
    ...
    try {
      ...
      setConsoleUrl("");
      setError(pairingError instanceof Error ? pairingError.message : "Unable to pair the console.");
      setStatus("");
    }
  }
  ...
  return (
    <main className="console-shell">
      ...
      <section className="console-heading">
        ...
        <div className="console-status"><span className={`status-pill ${error ? "offline" : consoleUrl ? "online" : "checking"}`}><i /> {error ? "Offline" : consoleUrl ? "Connected" : "Pairing"}</span><span>{status}</span></div>
      </section>
