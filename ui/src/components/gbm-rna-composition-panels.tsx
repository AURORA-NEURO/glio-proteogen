import { formatNumber, formatSigned, shortDigest, textAt, type JsonObject } from "@/lib/research-state";
...
export function GbmRnaCompositionEvidencePanel({ request, result }: { request: JsonObject; result: JsonObject }) {
  ...
  return <div className="panel-stack"><section className="result-panel zero-fill-panel"><div className="panel-title-row"><div><p className="eyebrow">CALLER-OWNED COUNT INPUT</p><h3>GBM RNA composition request</h3></div><span className="count-chip">{stats.features}</span></div><div className="metric-strip"><article><span>FEATURES</span><b>{stats.features}</b><small>shared gene axis</small></article><article><span>NONZERO COUNTS</span><b>{stats.nonzero}</b><small>raw counts retained</small></article><article><span>DEPTH</span><b>{stats.depth.toLocaleString("en-US")}</b><small>positive sequencing depth</small></article><article><span>REFERENCES</span><b>{stats.references}</b><small>caller-supplied lineage signatures</small></article></div></section><section className="result-panel"><div className="panel-title-row"><div><p className="eyebrow">EXECUTED REQUEST</p><h3>Feature and reference ledger</h3></div><span className="boundary-chip">exact axis · no implicit remapping</span></div><pre>{JSON.stringify({ feature_ids: request.feature_ids, references: request.references, unknown_background: request.unknown_background, source_digests: request.source_digests, provenance_note: request.provenance_note }, null, 2)}</pre></section><section className="result-panel"><div className="panel-title-row"><div><p className="eyebrow">PROVENANCE</p><h3>Source digests and solver profile</h3></div></div><pre>{JSON.stringify({ source_digests: request.source_digests, provenance, profile_id: result.profile_id, profile_digest: result.profile_digest }, null, 2)}</pre></section></div>;
}
