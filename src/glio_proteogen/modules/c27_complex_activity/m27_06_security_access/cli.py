...
def _read_request(path: Path) -> EvaluateComplexActivitySecurityAccessRequest:
    try:
        data = read_bounded(path, M2706_MAX_CANONICAL_REQUEST_BYTES)
...
def _read_result(path: Path) -> ComplexActivitySecurityAccessResult:
    try:
        data = read_bounded(path, M2706_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(data, max_bytes=M2706_MAX_CANONICAL_RESULT_BYTES)
