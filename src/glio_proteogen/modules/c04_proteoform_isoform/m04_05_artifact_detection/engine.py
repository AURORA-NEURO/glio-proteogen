...
def prepare_artifact_request_candidate(candidate: object) -> dict[str, object]:
    ...
    policy = _POLICY_ADAPTER.validate_python(_plain_value(policy_raw), strict=True)
    ...
    ledger = _plain_value(ledger_raw) if may_traverse and ledger_raw is not _MISSING else None
    payload: dict[str, object] = {
        "request_id": _plain_value(_member(candidate, "request_id")),
        "context": _plain_value(_member(candidate, "context")),
...
def _compute_result(
    ...
    return ProteoformArtifactDetectionResult.model_validate(payload, strict=True)
