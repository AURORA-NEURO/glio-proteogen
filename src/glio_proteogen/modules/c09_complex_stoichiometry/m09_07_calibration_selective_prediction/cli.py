"""Typer commands matching the M09-07 FastAPI validation boundary."""
...
from glio_proteogen.contracts.m09_07 import contract_json_schema, contract_json_schemas
from glio_proteogen.kernel.strict_json import (
    StrictJsonError,
    sanitized_validation_errors,
    strict_json_loads,
)
from glio_proteogen.modules.c09_complex_stoichiometry.m09_07_calibration_selective_prediction.service import (  # noqa: E501
    M0907Service,
)
...
def _load(path: Path) -> object:
    try:
        return strict_json_loads(read_bounded(path))
...
def validate(path: Annotated[Path, typer.Argument(exists=True, dir_okay=False)]) -> None:
    ...
    try:
        request = M0907Service.validate_request(_load(path))
...
def calibrate(path: Annotated[Path, typer.Argument(exists=True, dir_okay=False)]) -> None:
    ...
    try:
        result = M0907Service().execute(_load(path))
    except ValidationError as error:
        _emit({"valid": False, "errors": sanitized_validation_errors(error)})
        raise typer.Exit(code=2) from error
