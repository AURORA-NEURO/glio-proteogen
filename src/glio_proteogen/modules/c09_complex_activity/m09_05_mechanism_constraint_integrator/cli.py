"""Typer CLI for strict M09-05 validation and integration."""
...
from glio_proteogen.modules.c09_complex_activity.m09_05_mechanism_constraint_integrator.engine import (  # noqa: E501
    M0905AuthorizationError,
)
from glio_proteogen.modules.c09_complex_activity.m09_05_mechanism_constraint_integrator.service import (  # noqa: E501
    M0905Service,
)
...
app = typer.Typer(help="M09-05 mechanism and constraint integrator.")


def _read(path: Path) -> bytes:
    try:
        body = read_bounded(path, max_bytes=M0905_MAX_CANONICAL_REQUEST_BYTES)
...
def _request_from_file(path: Path) -> IntegrateComplexActivityConstraintsRequest:
    body = _read(path)
    try:
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except ValidationError as error:
        raise typer.BadParameter("request does not match the M09-05 contract") from error


@app.command("export-schema")
def export_schema(contract: Annotated[str, typer.Argument(help="M09-05 contract name.")]) -> None:
    """Export one strict JSON Schema 2020-12 contract."""

    if contract not in _CONTRACT_NAMES:
        raise typer.BadParameter("unknown M09-05 contract")
...
def integrate(
    ...
    try:
        built = M0905Service().integrate(_request_from_file(request))
    except (
        M0905AuthorizationError,
        ValidationError,
        ValueError,
        ...
        raise typer.BadParameter("M09-05 integration input is invalid") from error
