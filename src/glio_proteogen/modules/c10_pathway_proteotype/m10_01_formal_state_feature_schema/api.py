"""FastAPI interface for strict M10-01 validation and execution."""
...
from .engine import M1001AuthorizationError, M1001InputError
...
def create_app(service: M1001Service | None = None) -> FastAPI:
    ...
    def export_schema(contract: str) -> dict[str, object]:
        if contract not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M10-01 contract")
        return contract_json_schema(contract)  # type: ignore[arg-type]

    @app.post("/v1/modules/M10-01/validate")
    async def validate_request(request: Request) -> JSONResponse:
        body = await _strict_body(request)
        try:
            typed = state_service.validate_request(
                _REQUEST_ADAPTER.validate_json(body, strict=True)
            )
        except ValidationError as error:
            raise _validation_error(error) from error
        except M1001AuthorizationError as error:
            raise HTTPException(status_code=403, detail="M10-01 authorization denied") from error
        return JSONResponse(content=typed.model_dump(mode="json"))

    @app.post("/v1/modules/M10-01/execute")
    async def execute(request: Request) -> JSONResponse:
        ...
        try:
            ...
            built = state_service.execute(typed)
        except ValidationError as error:
            raise _validation_error(error) from error
        except M1001AuthorizationError as error:
            raise HTTPException(status_code=403, detail="M10-01 authorization denied") from error
    ...
    @app.exception_handler(M1001InputError)
    async def input_error(_request: Request, _error: M1001InputError) -> JSONResponse:
        return JSONResponse(status_code=422, content={"detail": "M10-01 input rejected"})
