    ...
    input_digests = tuple(
        dict.fromkeys(
            (
                ...
                *(item.reference.digest for entry in request.entries for item in entry.evidence),
                *(
...
class M1906Engine:
    ...
    def replay(self, result: ProteotypeAdjudicationResult) -> ProteotypeAdjudicationResult:
        ...
        if expected.model_dump(mode="json") != validated.model_dump(mode="json"):
            raise M1906ReplayError("M19-06 deterministic replay result mismatch")  # noqa: TRY003
        return validated
