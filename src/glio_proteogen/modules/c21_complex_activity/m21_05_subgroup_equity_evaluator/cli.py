"""Typer adapter for provisional M21-05 subgroup equity evaluation."""
...
from .engine import M2105AuthorizationError
from .service import M2105Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2105Service()
_REQUEST_ADAPTER = TypeAdapter(EvaluateComplexActivitySubgroupEquityRequest)
_RESULT_ADAPTER = TypeAdapter(ComplexActivitySubgroupEvaluationResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "report",
...
class M2105CliError(typer.BadParameter):
    """Sanitized M21-05 command-line validation error."""
...
def _read_request(path: Path) -> EvaluateComplexActivitySubgroupEquityRequest:
    try:
        ...
        raise M2105CliError("input must satisfy the strict M21-05 request contract") from error
...
def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2105CliError("output already exists; refusing to overwrite")
...
def export_schema(
    ...
    """Export one strict provisional M21-05 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2105CliError("unknown M21-05 contract")
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2105AuthorizationError) as error:
        raise M2105CliError("request does not satisfy the M21-05 contract") from error
...
def evaluate(
    ...
    """Evaluate one request and emit the canonical immutable result."""

    try:
        result = _SERVICE.execute(_read_request(path))
    except (ValidationError, ValueError, M2105AuthorizationError) as error:
        raise M2105CliError("request was rejected by the M21-05 service") from error
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2105CliError("result replay is invalid") from error
