"""FastAPI adapter for the provisional M21-05 evaluation boundary."""
...
def create_app(service: M2105Service | None = None) -> FastAPI:
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2105_MAX_CANONICAL_REQUEST_BYTES)
    ...
    @app.get("/v1/modules/M21-05/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M21-05 contract")
    ...
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2105_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
