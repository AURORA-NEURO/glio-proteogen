...
The dossier leaves the public estimator ABI provisional.  This implementation
therefore evaluates a deliberately small caller-declared objective grammar and
keeps every artifact reference opaque.  A request outside that grammar is
quarantined rather than being silently interpreted as a biological negative.
"""
...
_SUPPORTED_OBJECTIVES: Final = frozenset(
    {"stable", "alternating", "territory", "treatment_era", "time_course"}
)
...
def preflight_longitudinal_authorization(candidate: object) -> None:
    """Check all seven controls before traversing typed or opaque request data."""
...
class M1205LongitudinalEngine:
    ...
    def _result(  # noqa: C901 - explicit modeled/abstained closure is intentionally branch-rich.
