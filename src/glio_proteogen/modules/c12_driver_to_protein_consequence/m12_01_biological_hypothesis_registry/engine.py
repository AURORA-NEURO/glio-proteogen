"""Deterministic, replay-bound M12-01 biological hypothesis registry runtime."""
...
_RESULT_ADAPTER: Final = TypeAdapter(BiomarkerPanelHypothesisRegistryResult)
...
class M1201HypothesisAuthorizationError(PermissionError):
    """Caller controls do not authorize hypothesis registry evaluation."""

    def __init__(self) -> None:
        super().__init__(
            "M12-01 requires accepted controls, resolved identity, and granted consent"
        )


class M1201ReplayVerificationError(ValueError):
    """A registry result cannot be reconstructed from its exact request."""

    def __init__(self) -> None:
        super().__init__("M12-01 replay verification failed")
...
def preflight_hypothesis_authorization(candidate: object) -> None:
    """Check seven controls before traversing opaque source or hypothesis material."""
    ...
    try:
        ...
        raise M1201HypothesisAuthorizationError from None
    if states != expected:
        raise M1201HypothesisAuthorizationError
...
def _evidence(
    ...
    return tuple(
        KernelEvidenceReference(reference=artifact, role="evidence", claim=M1201_EVIDENCE_CLAIM)
...
class M1201HypothesisEngine:
    """Evaluate explicit caller-declared hypotheses without hidden interpretation."""

    __slots__ = ()

    def register(self, request: object) -> BiomarkerPanelHypothesisRegistryResult:
    ...
    def _result(
        ...
        for hypothesis in request.hypotheses:
            ...
            for rule, outcome in zip(hypothesis.falsification_rules, rule_outcomes, strict=True):
                ...
                if outcome is not FalsificationOutcome.PASSED:
                    findings.append(
                        HypothesisFinding(
                            finding_id=f"finding.{hypothesis.hypothesis_id}.{rule.rule_id}",
                            code=HypothesisFindingCode.FALSIFICATION_NOT_EVALUABLE,
                            message="falsification rule failed or is not evaluable",
                            evidence=rule.required_evidence,
                        )
                    )
        ...
        payload: dict[str, object] = {
            "output_type": "biomarker_panel_hypothesis_registry",
            "result_id": f"result.{request_hash.removeprefix('sha256:')}",
            "result_version": M1201_CONTRACT_VERSION,
            ...
            "parent_target": M1201_PARENT,
            "emits_parent": False,
            "support_decision": SupportDecision(
                status=SupportStatus.SUPPORTED if safe else SupportStatus.UNSUPPORTED,
                reason_code="m1201_registry_supported" if safe else "m1201_registry_abstained",
                rationale=(
        ...
        constructed = BiomarkerPanelHypothesisRegistryResult.model_construct(
    ...
    def verify(
        ...
        *,
        ...
        try:
            ...
            raise M1201ReplayVerificationError from error
        if validated.result_digest != result_payload_digest(validated):
            raise M1201ReplayVerificationError
        if replay:
...
def register_biomarker_panel_hypotheses(
    request: object,
) -> BiomarkerPanelHypothesisRegistryResult:
    """Public provisional M12-01 operation."""

    return M1201HypothesisEngine().register(request)
