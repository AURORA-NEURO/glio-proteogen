...
def _initial_measurement_objective(
    ...
    """Evaluate the robust paired-delta loss for one perturbation program."""
...
def _robust_initial_center(terms: tuple[_TypedTerm, ...]) -> float:
    """Fit a deterministic Huber center before signed perturbation propagation.

    A failed assay batch can otherwise dominate a quality-weighted mean of
    baseline-to-perturbed deltas and create a spurious downstream response.
    Inverse-variance Huber IRLS down-weights that replicate and accepts only
    finite, non-increasing proposals. The coupled graph fit still determines
    the final perturbation effect.
    """
    ...
    for _ in range(_INITIAL_HUBER_ITERATIONS):
        ...
        proposal = sum(
            weight * term.delta for weight, term in zip(robust_weights, terms, strict=True)
        ) / robust_denominator
        ...
        if not math.isfinite(objective) or objective > previous + _OBJECTIVE_TOLERANCE:
            ...
            for _ in range(_BACKTRACKING_STEPS):
                ...
                trial = [
                    max(
                        -M1406_MAX_EFFECT,
                        min(M1406_MAX_EFFECT, before + step * change),
                    )
