"""Strict parse-once M14-01 plugin boundary."""
...
class ValidatedM1401Request:
    request: RegisterProteinSubtypeHypothesesRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1401Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M14-01 execution requires a validated request token")


class M1401Plugin(
    ModulePlugin[object, ValidatedM1401Request, ProteinSubtypeHypothesisRegistryResult]
):
    """Expose M14-01 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1401Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1401Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1401_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1401Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1401Request) -> ProteinSubtypeHypothesisRegistryResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1401Request
