"""Strict parse-once plugin boundary for provisional M14-08."""
...
class ValidatedM1408Request:
    request: PublishProteinSubtypeMechanismDossierRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1408Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M14-08 execution requires a validated request token")


class M1408Plugin(
    ModulePlugin[object, ValidatedM1408Request, ProteinSubtypeMechanismEvidenceDossierResult]
):
    """Expose M14-08 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1408Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1408Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1408_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1408Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1408Request) -> ProteinSubtypeMechanismEvidenceDossierResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1408Request
...
__all__ = ["M1408Plugin", "ValidatedM1408Request"]
