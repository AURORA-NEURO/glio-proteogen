"""Strict parse-once M14-02 plugin boundary."""
...
from glio_proteogen.contracts.m14_02.canonical import canonical_request_digest
...
if TYPE_CHECKING:
    from .service import M1402Service

_TOKEN_SEAL: Final = object()
_REQUEST_ADAPTER: Final = TypeAdapter(StratifyProteinSubtypeContextRequest)
...
class ValidatedM1402Request:
    """Opaque capability proving strict M14-02 request acceptance."""

    request: StratifyProteinSubtypeContextRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1402Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M14-02 execution requires a validated request token")


class M1402Plugin(
    ModulePlugin[object, ValidatedM1402Request, ProteinSubtypeContextStratificationResult]
):
    """Expose M14-02 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1402Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1402Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1402_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1402Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1402Request) -> ProteinSubtypeContextStratificationResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1402Request
...
__all__ = ["M1402Plugin", "ValidatedM1402Request"]
