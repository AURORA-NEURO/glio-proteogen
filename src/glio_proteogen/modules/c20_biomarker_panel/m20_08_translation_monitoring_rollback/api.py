"""FastAPI adapter for the provisional M20-08 monitoring boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M20-08 contract")


def _parse_request(body: bytes) -> MonitorProteinSubtypeTranslationHealthRequest:
    try:
        strict_json_loads(body, max_bytes=M2008_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2008Service | None = None) -> FastAPI:
    """Create a strict API with sanitized validation and replay errors."""

    boundary = service or M2008Service()
    app = FastAPI(title="GLIO-PROTEOGEN M20-08", version="0.1.0-provisional")
    # Enforce the request ceiling before Starlette materializes any body;
    # /verify applies the larger result ceiling in its bounded reader.
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2008_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M20-08/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M20-08/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M20-08 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M20-08/validate")
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2008_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M20-08/monitor")
    ...
    @app.post("/v1/modules/M20-08/verify")
    async def verify(request: Request) -> dict[str, object]:
        envelope = _parse_object(
            await _read_bounded(request, max_bytes=M2008_MAX_CANONICAL_RESULT_BYTES)
        )
