"""Typer adapter for provisional M20-05 workflow presentation."""
...
class M2005CliError(typer.BadParameter):
    """Sanitized M20-05 command-line validation error."""


def _read_request(path: Path) -> PresentProteinSubtypeHumanReviewWorkspaceRequest:
    try:
        data = read_bounded(path, M2005_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2005_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2005CliError(  # noqa: TRY003
            "input must satisfy the strict M20-05 request contract"
        ) from error


def _read_result(path: Path) -> ProteinSubtypeHumanReviewWorkspaceResult:
    try:
        data = read_bounded(path, M2005_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(data, max_bytes=M2005_MAX_CANONICAL_RESULT_BYTES)
        return _RESULT_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2005CliError("input must be a valid M20-05 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2005CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    name: Annotated[
        ...
        typer.Argument(
            help=(
                "request, output, review-item, next-action, workspace, configuration, policy, "
                "or finding"
            )
        ),
    ],
    ...
    """Export one strict provisional M20-05 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2005CliError("unknown M20-05 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    """Validate one request without presenting it."""

    try:
        request = _SERVICE.validate_request(_read_request(path))
    except (ValidationError, ValueError, M2005AuthorizationError) as error:
        raise M2005CliError("request does not satisfy the M20-05 contract") from error  # noqa: TRY003
...
def present(
    ...
    """Present and emit one canonical human-review workspace result."""

    try:
        result = _SERVICE.present(_read_request(path))
    except (ValidationError, ValueError, M2005AuthorizationError) as error:
        raise M2005CliError("request was rejected by the M20-05 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2005CliError("result replay is invalid") from error  # noqa: TRY003
