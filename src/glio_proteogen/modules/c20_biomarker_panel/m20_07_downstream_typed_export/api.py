"""FastAPI adapter for the provisional M20-07 export boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M20-07 contract")


def _parse_request(body: bytes) -> ExportProteinSubtypeDownstreamContractRequest:
    try:
        strict_json_loads(body, max_bytes=M2007_MAX_CANONICAL_REQUEST_BYTES)
...
async def _read_bounded(request: Request, *, max_bytes: int) -> bytes:
    """Read a request stream without retaining bytes beyond the declared cap."""
    chunks: list[bytes] = []
    total = 0
...
def create_app(service: M2007Service | None = None) -> FastAPI:
    """Create a strict API app with sanitized validation and replay errors."""

    boundary = service or M2007Service()
    app = FastAPI(title="GLIO-PROTEOGEN M20-07", version="0.1.0-provisional")
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2007_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M20-07/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M20-07/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M20-07 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M20-07/validate")
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2007_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M20-07/export")
    async def export(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2007_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M20-07/verify")
    async def verify(request: Request) -> dict[str, object]:
        envelope = _parse_object(
            await _read_bounded(request, max_bytes=M2007_MAX_CANONICAL_RESULT_BYTES)
        )
