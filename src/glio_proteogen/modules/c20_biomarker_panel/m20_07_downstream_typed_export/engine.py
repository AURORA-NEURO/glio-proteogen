"""Deterministic, consent-aware M20-07 downstream export engine.

The engine consumes caller-declared M20-06 references only. It never traverses
raw artifacts, authenticates an issuer, infers identity or consent, or turns
unsupported material into a negative finding.
"""
...
def preflight_m2007_authorization(candidate: object) -> None:
    ...
    try:
        ...
        raise M2007AuthorizationError from error
    if references is None:
        raise M2007AuthorizationError("M20-07 requires all seven upstream controls")
    for role, expected in _EXPECTED_CONTROLS.items():
        decision = _member(references, role)
        actual = _state(decision)
        if actual != expected:
            raise M2007AuthorizationError(
                f"M20-07 control {role} must be {expected}; received {actual}"
            )
...
def _evidence(
    ...
    return tuple(
        EvidenceReference(reference=artifact, role="evidence", claim=M2007_EVIDENCE_CLAIM)
        for artifact in unique.values()
    )


def _declared_text(request: ExportProteinSubtypeDownstreamContractRequest) -> str:
    values = [
        request.request_id,
...
def _provenance(
    ...
    return ProvenanceRecord(
        ...
        module_version=M2007_CONTRACT_VERSION,
...
def _limitations(*, exported: bool) -> tuple[Limitation, ...]:
    return (
        Limitation(
            code="m2007_no_kinase_or_treatment",
            statement=(
                "M20-07 never emits kinase state, generic all-omics fusion or treatment advice."
            ),
        ),
        Limitation(
            code="m2007_caller_declared_authority",
            statement=(
                "Upstream issuer authority and raw artifact contents are not authenticated here."
            ),
        ),
        Limitation(
            code="m2007_exported" if exported else "m2007_abstained",
            statement=(
...
class M2007Engine:
    ...
    def validate_request(self, candidate: object) -> ExportProteinSubtypeDownstreamContractRequest:
        preflight_m2007_authorization(candidate)
        try:
            return _REQUEST_ADAPTER.validate_python(candidate, strict=True)
        except Exception as error:
            raise M2007ExportError("M20-07 request is invalid") from error

    def export(self, candidate: object) -> ProteinSubtypeDownstreamExportResult:
        ...
        if prohibited:
            findings.append(
                _finding(
                    ...
                    "A prohibited responsibility is outside the M20-07 export boundary.",
                    evidence,
                )
            )
        if unsupported:
        ...
        if not findings:
            findings.append(
                _finding(
                    ...
                    "The M20-07 ABI remains provisional pending owner confirmation.",
                    evidence,
                )
            )
        ownership = ExportOwnershipBinding(
            ...
            ownership_statement="M20-07 exclusively owns this signed downstream typed export.",
            evidence=evidence[:1],
        )
        signature = SignedContractEnvelope(
            ...
            signature_digest=sha256_digest(
                {"request_digest": request_digest, "owner": M2007_MODULE_ID}
            ),
            evidence=evidence[:1],
        )
        ...
        support = (
            ...
            if supported
                ...
                reason_code="m2007_abstained",
                rationale="Safe abstention preserves ownership, consent and support semantics.",
            )
        )
        payload: dict[str, Any] = {
            "output_type": "protein_subtype_downstream_contract",
            "result_id": f"result.{request_digest.removeprefix('sha256:')}",
            "result_version": M2007_CONTRACT_VERSION,
            "request_digest": request_digest,
            ...
            "abstention_reason": (
                None if supported else "M20-07 export is outside the safe support envelope."
            ),
        ...
        constructed = ProteinSubtypeDownstreamExportResult.model_construct(**payload)
        payload["result_digest"] = result_payload_digest(constructed)
        try:
            return _RESULT_ADAPTER.validate_python(payload, strict=True)
        except Exception as error:
            raise M2007ExportError("M20-07 result construction failed safely") from error
    ...
    def verify(
        ...
        *,
        ...
        try:
            ...
            raise M2007ReplayError("M20-07 result is invalid") from error
        if validated.result_digest != result_payload_digest(validated):
            raise M2007ReplayError("M20-07 result digest mismatch")
        if replay:
            expected = self.export(validated.request)
            if expected.model_dump(mode="json") != validated.model_dump(mode="json"):
                raise M2007ReplayError("M20-07 deterministic replay mismatch")
        return validated


def export_protein_subtype_downstream_contract(
    candidate: object,
) -> ProteinSubtypeDownstreamExportResult:
    """Public M20-07 export operation."""

    return M2007Engine().export(candidate)
