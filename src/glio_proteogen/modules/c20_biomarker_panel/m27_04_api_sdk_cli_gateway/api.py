...
def _parse_object(body: bytes) -> dict[str, Any]:
    try:
        value = strict_json_loads(body, max_bytes=M2704_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2704Service | None = None) -> FastAPI:
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2704_MAX_CANONICAL_REQUEST_BYTES)
    ...
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2704_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
