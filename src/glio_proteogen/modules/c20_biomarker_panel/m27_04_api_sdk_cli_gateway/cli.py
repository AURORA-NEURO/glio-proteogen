"""Typer commands for strict M27-04 gateway operations."""
...
class M2704CliError(typer.BadParameter):
    """Sanitized M27-04 command-line validation error."""
...
def _read_request(path: Path) -> PublishComplexActivityAccessSurfaceRequest:
    try:
        ...
        raise M2704CliError("input must satisfy the strict M27-04 request contract") from error  # noqa: TRY003
...
def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2704CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    name: Annotated[str, typer.Argument(help="name of an M27-04 contract schema")],
    output: Annotated[Path | None, typer.Option("--output", "-o")] = None,
) -> None:
    """Export one strict provisional M27-04 schema."""

    if name not in _CONTRACT_NAMES:
        raise M2704CliError("unknown M27-04 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2704CliError("request does not satisfy the M27-04 contract") from error  # noqa: TRY003
...
def publish(
    ...
    try:
        ...
        raise M2704CliError("request was rejected by the M27-04 gateway") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    """Verify an M27-04 result by canonical replay."""

    result = _read_result(path)
    try:
        replay = _SERVICE.replay(result)
    except (TypeError, ValueError, ValidationError) as error:
        raise M2704CliError("result replay is invalid") from error  # noqa: TRY003
