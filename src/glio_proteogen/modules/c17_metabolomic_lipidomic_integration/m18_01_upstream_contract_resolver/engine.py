"""Replay-safe typed upstream contract resolver for M18-01.

The engine consumes only caller-declared, typed metadata. It never opens source
artifacts, infers consent or identity, or turns an unknown compatibility state
into a negative biological finding.
"""
...
_REQUEST_ADAPTER: Final = TypeAdapter(ResolveBiomarkerPanelUpstreamContractsRequest)
...
class M1801AuthorizationError(ValueError):
    """Raised before candidate traversal when a required control is unsafe."""


class M1801ReplayError(ValueError):
...
def preflight_m1801_authorization(candidate: object) -> None:
    ...
    if references is None:
        raise M1801AuthorizationError("M18-01 requires all seven upstream controls")  # noqa: TRY003
    for name, expected in _CONTROL_STATES.items():
        ...
        if actual_value != expected:
            raise M1801AuthorizationError(  # noqa: TRY003
                f"M18-01 control {name} must be {expected}; received {actual_value}"
            )
...
def _uncertainty() -> UncertaintyProfile:
    estimate = UncertaintyEstimate(
        ...
        rationale="M18-01 resolves declared contracts; it does not estimate biology.",
    )
...
def _provenance(request: ResolveBiomarkerPanelUpstreamContractsRequest) -> ProvenanceRecord:
    refs = request.context.references
    return ProvenanceRecord(
        activity_id=f"activity.{request.request_id}",
        actor_id=request.context.actor_id,
        module_id=M1801_MODULE_ID,
        module_version=M1801_CONTRACT_VERSION,
...
def _evidence(
    ...
    return tuple(
        EvidenceReference(reference=artifact, role="evidence", claim=M1801_EVIDENCE_CLAIM)
...
def _limitations() -> tuple[Limitation, ...]:
    return (
        ...
        Limitation(
            code="no_biomarker_panel_claim",
            statement="The parent biomarker panel remains outside this resolver's output ceiling.",
        ),
        Limitation(
...
class M1801Engine:
    ...
    def validate_request(
        ...
        preflight_m1801_authorization(candidate)
    ...
    def resolve(self, candidate: object) -> BiomarkerPanelUpstreamResolutionResult:
        ...
        report = CompatibilityReport(
            ...
            version=M1801_CONTRACT_VERSION,
        ...
        payload: dict[str, Any] = {
            "output_type": "biomarker_panel_upstream_resolution",
            "result_id": f"result.{request_digest.removeprefix('sha256:')}",
            "result_version": M1801_CONTRACT_VERSION,
        ...
        return BiomarkerPanelUpstreamResolutionResult.model_validate(payload, strict=True)
    ...
    def replay(
        ...
        if result.request_digest != canonical_request_digest(result.request):
            raise M1801ReplayError("M18-01 result request digest mismatch")  # noqa: TRY003
        if result.result_id != f"result.{result.request_digest.removeprefix('sha256:')}":
            raise M1801ReplayError("M18-01 result identifier mismatch")  # noqa: TRY003
        if result.result_digest != result_payload_digest(result):
            raise M1801ReplayError("M18-01 result payload digest mismatch")  # noqa: TRY003
        try:
            expected = self.resolve(result.request)
        except Exception as exc:
            raise M1801ReplayError from exc
        if expected.model_dump(mode="json") != result.model_dump(mode="json"):
            raise M1801ReplayError("M18-01 deterministic replay result mismatch")  # noqa: TRY003
        return result


def resolve_biomarker_panel_upstream_contracts(
    candidate: object,
) -> BiomarkerPanelUpstreamResolutionResult:
    """Resolve one strict request through the M18-01 engine."""

    return M1801Engine().resolve(candidate)
