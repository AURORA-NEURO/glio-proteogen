"""Replay-safe typed upstream contract resolver for M20-01.

The engine consumes only caller-declared, typed metadata. It never opens
source artifacts, infers consent or identity, or turns an unknown
compatibility state into a negative biological finding.
"""
...
_REQUEST_ADAPTER: Final = TypeAdapter(ResolveProteinSubtypeUpstreamContractsRequest)
...
class M2001AuthorizationError(ValueError):
    """Raised before candidate traversal when a required control is unsafe."""


class M2001ReplayError(ValueError):
...
def preflight_m2001_authorization(candidate: object) -> None:
    ...
    if references is None:
        raise M2001AuthorizationError("M20-01 requires all seven upstream controls")  # noqa: TRY003
    for name, expected in _CONTROL_STATES.items():
        ...
        if actual_value != expected:
            raise M2001AuthorizationError(  # noqa: TRY003
                f"M20-01 control {name} must be {expected}; received {actual_value}"
            )
...
def _uncertainty() -> UncertaintyProfile:
    estimate = UncertaintyEstimate(
        ...
        rationale="M20-01 resolves declared contracts; it does not estimate biology.",
    )
...
def _evidence(
    ...
    return tuple(
        EvidenceReference(reference=artifact, role="evidence", claim=M2001_EVIDENCE_CLAIM)
...
def _limitations() -> tuple[Limitation, ...]:
    return (
        ...
        Limitation(
            code="no_protein_subtype_claim",
            statement="The parent protein subtype remains outside this resolver's output ceiling.",
        ),
        Limitation(
        ...
        Limitation(
            code="no_kinase_or_all_omics_fusion",
            statement=(
                "Kinase activity, generic all-omics fusion, and treatment recommendation "
                "remain outside M20-01."
            ),
        ),
    )
...
class M2001Engine:
    ...
    def validate_request(
        ...
        preflight_m2001_authorization(candidate)
    ...
    def resolve(self, candidate: object) -> ProteinSubtypeUpstreamResolutionResult:
        ...
        report = CompatibilityReport(
            ...
            version=M2001_CONTRACT_VERSION,
        ...
        payload: dict[str, Any] = {
            ...
            "result_version": M2001_CONTRACT_VERSION,
    ...
    def replay(
        ...
        if result.request_digest != canonical_request_digest(result.request):
            raise M2001ReplayError("M20-01 result request digest mismatch")  # noqa: TRY003
        ...
        try:
            validated = ProteinSubtypeUpstreamResolutionResult.model_validate_json(
                canonical_json_bytes(result), strict=True
            )
            expected = self.resolve(validated.request)
        except M2001ReplayError:
            raise
        except Exception as error:
            raise M2001ReplayError("M20-01 replay result validation failed") from error  # noqa: TRY003
        if canonical_json_bytes(expected) != canonical_json_bytes(validated):
            raise M2001ReplayError("M20-01 deterministic replay mismatch")  # noqa: TRY003
        return validated


def resolve_protein_subtype_upstream_contracts(
    candidate: object,
) -> ProteinSubtypeUpstreamResolutionResult:
    """Resolve one strict request through the M20-01 engine."""

    return M2001Engine().resolve(candidate)
