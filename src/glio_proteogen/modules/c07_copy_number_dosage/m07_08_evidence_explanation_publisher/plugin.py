"""Strict parse-once plugin boundary for provisional M07-08."""
...
class ValidatedM0708Request:
    request: PublishProteotypeEvidenceRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM0708Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M07-08 execution requires a validated request token")


class M0708Plugin(
    ModulePlugin[object, ValidatedM0708Request, ProteotypeEvidencePublicationResult]
):
    """Expose M07-08 through the common validate-then-run plugin ABI."""

    __slots__ = ("_service",)

    def __init__(self, service: M0708Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM0708Request:
        candidate = request
        if type(candidate) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", candidate)
            parsed = strict_json_loads(serialized, max_bytes=M0708_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM0708Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM0708Request) -> ProteotypeEvidencePublicationResult:
        snapshot = _ISSUED_TOKENS.get(request)
        if (
            type(request) is not ValidatedM0708Request
...
__all__ = ["M0708Plugin", "ValidatedM0708Request"]
