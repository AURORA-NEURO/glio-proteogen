"""Typer CLI for strict M07-05 validation and integration."""
...
from glio_proteogen.modules.c07_copy_number_dosage.m07_05_mechanism_constraint_integrator.service import (  # noqa: E501
    M0705Service,
)
...
app = typer.Typer(help="M07-05 mechanism and constraint integrator.")


def _read(path: Path) -> bytes:
    try:
        body = read_bounded(path, max_bytes=M0705_MAX_CANONICAL_REQUEST_BYTES)
...
def _request_from_file(path: Path) -> IntegrateProteotypeConstraintsRequest:
    body = _read(path)
    try:
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except ValidationError as error:
        raise typer.BadParameter("request does not match the M07-05 contract") from error


@app.command("export-schema")
def export_schema(
    contract: Annotated[str, typer.Argument(help="M07-05 contract name.")],
) -> None:
    """Export one strict JSON Schema 2020-12 contract."""

    if contract not in _CONTRACT_NAMES:
        raise typer.BadParameter("unknown M07-05 contract")
...
def integrate(
    ...
    try:
        built = M0705Service().integrate(_request_from_file(request))
    except (ValidationError, ValueError, TypeError, StrictJsonError) as error:
        raise typer.BadParameter("M07-05 integration input is invalid") from error
