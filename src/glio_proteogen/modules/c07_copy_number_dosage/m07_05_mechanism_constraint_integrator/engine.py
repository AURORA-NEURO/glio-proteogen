...
def _initial_typed_program_state(members: tuple[GliomaDosageObservation, ...]) -> float:
    ...
    if observed:
        ...
        state = _robust_initial_dosage_center(terms)
        if limits:
...
def _initial_dosage_measurement_objective(
    center: float,
...
def _robust_initial_dosage_center(
    terms: tuple[tuple[float, float, float], ...],
) -> float:
    ...
    for _ in range(_TYPED_INITIAL_HUBER_ITERATIONS):
        ...
        baseline_objective = _initial_dosage_measurement_objective(estimate, terms)
        proposal_objective = _initial_dosage_measurement_objective(proposal, terms)
        accepted = proposal
        if not isfinite(proposal_objective) or (
            ...
            for _ in range(_TYPED_BACKTRACKING_STEPS):
                ...
                trial_objective = _initial_dosage_measurement_objective(trial, terms)
