"""Strict parse-once plugin boundary for provisional M15-06."""
...
class ValidatedM1506Request:
    request: SimulateComplexActivityPerturbationsRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1506Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M15-06 execution requires a validated request token")


class M1506Plugin(
    ModulePlugin[object, ValidatedM1506Request, ComplexActivitySensitivitySimulationResult]
):
    """Expose M15-06 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1506Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1506Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1506Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1506Request) -> ComplexActivitySensitivitySimulationResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1506Request
...
__all__ = ["M1506Plugin", "ValidatedM1506Request"]
