"""Strict parse-once plugin boundary for provisional M22-03."""
...
from .engine import preflight_m2203_authorization
...
@dataclass(frozen=True, slots=True)
class ValidatedM2203Request:
    """Opaque capability proving strict M22-03 request validation."""
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M22-03 execution requires a validated request token")


class _InvalidSubmissionError(TypeError):
    def __init__(self) -> None:
        super().__init__("M22-03 validation requires a benchmark submission")


class M2203Plugin(
    ModulePlugin[object, ValidatedM2203Request, ProteinRnaDiscordanceInternalBenchmarkResult]
):
    """Expose validate-then-run without an authority or parse bypass."""

    __slots__ = ("_service",)

    def __init__(self, service: M2203Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM2203Request:
        if not isinstance(request, BenchmarkSubmission):
            raise _InvalidSubmissionError
        candidate = request.request
        if isinstance(candidate, bytes | bytearray | str):
            decoded = strict_json_loads(candidate, max_bytes=M2203_MAX_CANONICAL_REQUEST_BYTES)
            preflight_m2203_authorization(decoded)
            candidate = _REQUEST_ADAPTER.validate_json(candidate, strict=True)
        return ValidatedM2203Request(request=self._service.validate_request(candidate))

    def run(
        self,
        request: ValidatedM2203Request,
    ) -> ProteinRnaDiscordanceInternalBenchmarkResult:
        if not isinstance(request, ValidatedM2203Request):
