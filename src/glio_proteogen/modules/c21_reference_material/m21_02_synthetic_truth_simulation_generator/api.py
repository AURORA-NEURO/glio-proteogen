"""FastAPI adapter for the provisional M21-02 generation boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M21-02 contract")


def _parse_request(body: bytes) -> GenerateComplexActivitySyntheticTruthRequest:
    try:
        strict_json_loads(body, max_bytes=M2102_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2102Service | None = None) -> FastAPI:
    """Create a strict API app with sanitized errors and replay verification."""
    ...
    @app.get("/v1/modules/M21-02/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M21-02 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M21-02/validate")
    ...
    @app.post("/v1/modules/M21-02/generate")
    ...
    @app.post("/v1/modules/M21-02/verify")
