"""Typer adapter for provisional M24-06 robustness challenges."""
...
from .engine import M2406AuthorizationError
from .service import M2406Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2406Service()
...
class M2406CliError(typer.BadParameter):
    """Sanitized M24-06 command-line validation error."""


def _read_request(path: Path) -> ChallengeBiomarkerPanelRobustnessRequest:
    try:
        data = path.read_bytes()
        strict_json_loads(data, max_bytes=M2406_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2406CliError("input must satisfy the strict M24-06 request contract") from error  # noqa: TRY003


def _read_result(path: Path) -> BiomarkerPanelRobustnessChallengeResult:
    try:
        return _RESULT_ADAPTER.validate_json(path.read_bytes(), strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2406CliError("input must be a valid M24-06 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2406CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M24-06 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2406CliError("unknown M24-06 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2406AuthorizationError) as error:
        raise M2406CliError("request does not satisfy the M24-06 contract") from error  # noqa: TRY003
...
def challenge(
    ...
    """Challenge one request and emit a canonical result envelope."""

    try:
        result = _SERVICE.challenge(_read_request(path))
    except (ValidationError, ValueError, M2406AuthorizationError) as error:
        raise M2406CliError("request was rejected by the M24-06 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2406CliError("result replay is invalid") from error  # noqa: TRY003
