...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M24-06 contract")
...
def create_app(service: M2406Service | None = None) -> FastAPI:
    """Create strict validation/challenge/replay routes."""
    ...
    async def challenge(request: Request) -> dict[str, object]:
        payload = _parse_request(await request.body())
        try:
            result = boundary.challenge(payload)
        except (ValidationError, ValueError, M2406AuthorizationError) as error:
            raise _safe_validation(error) from error
    ...
    async def verify(request: Request) -> dict[str, object]:
        ...
        return {"verified": verified, "result_digest": replay.result_digest}
