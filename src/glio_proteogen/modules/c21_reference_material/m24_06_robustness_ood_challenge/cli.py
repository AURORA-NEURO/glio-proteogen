...
class M2406CliError(typer.BadParameter):
    """Sanitized M24-06 CLI error."""


def _read_request(path: Path) -> ChallengeBiomarkerPanelRobustnessRequest:
    try:
        data = read_bounded(path, M2406_MAX_CANONICAL_REQUEST_BYTES)
        ...
        raise M2406CliError(  # noqa: TRY003
            "input must satisfy the strict M24-06 request contract"
        ) from error


def _read_result(path: Path) -> BiomarkerPanelRobustnessChallengeResult:
    try:
        data = read_bounded(path, M2406_MAX_CANONICAL_RESULT_BYTES)
        value = strict_json_loads(data, max_bytes=M2406_MAX_CANONICAL_RESULT_BYTES)
