"""FastAPI adapter for provisional M22-02 synthetic-truth generation."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M22-02 contract")


def _parse_request(body: bytes) -> GenerateProteinRnaDiscordanceSyntheticTruthRequest:
    try:
        strict_json_loads(body, max_bytes=M2202_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2202Service | None = None) -> FastAPI:
    """Create strict validation/generation/replay routes."""

    boundary = service or M2202Service()
    app = FastAPI(title="GLIO-PROTEOGEN M22-02", version="0.1.0-provisional")
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2202_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M22-02/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M22-02/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M22-02 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M22-02/validate")
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2202_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M22-02/generate")
    ...
    @app.post("/v1/modules/M22-02/verify")
