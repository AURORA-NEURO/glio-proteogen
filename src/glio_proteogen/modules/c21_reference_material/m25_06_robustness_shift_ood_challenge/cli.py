"""Typer adapter for provisional M25-06 robustness challenges."""
...
class M2506CliError(typer.BadParameter):
    """Sanitized M25-06 command-line validation error."""


def _read_request(path: Path) -> ChallengeProteotypeRobustnessRequest:
    try:
        data = path.read_bytes()
        strict_json_loads(data, max_bytes=M2506_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2506CliError(  # noqa: TRY003
            "input must satisfy the strict M25-06 request contract"
        ) from error


def _read_result(path: Path) -> ProteotypeRobustnessChallengeResult:
    try:
        return _RESULT_ADAPTER.validate_json(path.read_bytes(), strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2506CliError("input must be a valid M25-06 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2506CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M25-06 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2506CliError("unknown M25-06 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    """Validate one robustness challenge request without executing it."""

    try:
        request = _SERVICE.validate_request(_read_request(path))
    except (ValidationError, ValueError, M2506AuthorizationError) as error:
        raise M2506CliError("request does not satisfy the M25-06 contract") from error  # noqa: TRY003
...
def challenge(
    ...
    """Run one caller-declared robustness challenge set."""

    try:
        result = _SERVICE.execute(_read_request(path))
    except (ValidationError, ValueError, M2506AuthorizationError) as error:
        raise M2506CliError("request was rejected by the M25-06 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2506CliError("result replay is invalid") from error  # noqa: TRY003
