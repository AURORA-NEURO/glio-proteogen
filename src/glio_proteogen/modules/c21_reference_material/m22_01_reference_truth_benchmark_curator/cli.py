"""Typer adapter for provisional M22-01 reference curation."""
...
class M2201CliError(typer.BadParameter):
    """Sanitized M22-01 command-line validation error."""


def _read_request(path: Path) -> CurateProteinRnaDiscordanceReferenceTruthRequest:
    try:
        data = read_bounded(path, M2201_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2201_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2201CliError(  # noqa: TRY003
            "input must satisfy the strict M22-01 request contract"
        ) from error


def _read_result(path: Path) -> ProteinRnaDiscordanceReferenceTruthResult:
    try:
        return _RESULT_ADAPTER.validate_json(
            read_bounded(path, M2201_MAX_CANONICAL_RESULT_BYTES), strict=True
        )
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2201CliError("input must be a valid M22-01 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2201CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M22-01 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2201CliError("unknown M22-01 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2201CliError("request does not satisfy the M22-01 contract") from error  # noqa: TRY003
...
def curate(
    ...
    try:
        result = _SERVICE.curate(_read_request(path))
    except (ValidationError, ValueError, M2201AuthorizationError) as error:
        raise M2201CliError("request was rejected by the M22-01 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2201CliError("result replay is invalid") from error  # noqa: TRY003
