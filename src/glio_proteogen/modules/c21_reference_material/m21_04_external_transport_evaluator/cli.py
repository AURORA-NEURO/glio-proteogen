"""Typer adapter for provisional M21-04 transport evaluation."""
...
from .engine import M2104AuthorizationError
from .service import M2104Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2104Service()
_REQUEST_ADAPTER = TypeAdapter(EvaluateComplexActivityExternalTransportRequest)
_RESULT_ADAPTER = TypeAdapter(ComplexActivityExternalTransportResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "validation",
...
class M2104CliError(typer.BadParameter):
    """Sanitized M21-04 command-line validation error."""


def _read_request(path: Path) -> EvaluateComplexActivityExternalTransportRequest:
    try:
        data = read_bounded(path, M2104_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2104_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2104CliError(  # noqa: TRY003
            "input must satisfy the strict M21-04 request contract"
        ) from error


def _read_result(path: Path) -> ComplexActivityExternalTransportResult:
    try:
        return _RESULT_ADAPTER.validate_json(
            read_bounded(path, M2104_MAX_CANONICAL_RESULT_BYTES), strict=True
        )
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2104CliError("input must be a valid M21-04 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2104CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M21-04 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2104CliError("unknown M21-04 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2104AuthorizationError) as error:
        raise M2104CliError("request does not satisfy the M21-04 contract") from error  # noqa: TRY003
...
def evaluate(
    ...
    try:
        ...
        raise M2104CliError("request was rejected by the M21-04 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2104CliError("result replay is invalid") from error  # noqa: TRY003
