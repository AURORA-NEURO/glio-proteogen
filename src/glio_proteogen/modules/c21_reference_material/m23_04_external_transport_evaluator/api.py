"""FastAPI adapter for the provisional M23-04 transport evaluator."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M23-04 contract")


def _parse_request(body: bytes) -> EvaluateVariantPeptideExternalTransportRequest:
    try:
        parsed = strict_json_loads(body, max_bytes=M2304_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2304Service | None = None) -> FastAPI:
    ...
    boundary = service or M2304Service()
    app = FastAPI(title="GLIO-PROTEOGEN M23-04", version="0.1.0-provisional")
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2304_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M23-04/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M23-04/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M23-04 contract")
