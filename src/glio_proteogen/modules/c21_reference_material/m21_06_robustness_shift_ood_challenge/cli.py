"""Typer adapter for provisional M21-06 robustness challenges."""
...
class M2106CliError(typer.BadParameter):
    """Sanitized M21-06 command-line validation error."""


def _read_request(path: Path) -> ChallengeComplexActivityRobustnessRequest:
    try:
        data = read_bounded(path, M2106_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2106_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2106CliError(  # noqa: TRY003
            "input must satisfy the strict M21-06 request contract"
        ) from error


def _read_result(path: Path) -> ComplexActivityRobustnessChallengeResult:
    try:
        return _RESULT_ADAPTER.validate_json(
            read_bounded(path, M2106_MAX_CANONICAL_RESULT_BYTES), strict=True
        )
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2106CliError("input must be a valid M21-06 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2106CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M21-06 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2106CliError("unknown M21-06 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2106CliError("request does not satisfy the M21-06 contract") from error  # noqa: TRY003
...
def challenge(
    ...
    """Run one robustness challenge and emit its canonical result."""

    try:
        result = _SERVICE.generate(_read_request(path))
    except (ValidationError, ValueError, M2106AuthorizationError) as error:
        raise M2106CliError("request was rejected by the M21-06 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    """Verify one immutable robustness result by replaying its digest."""

    result = _read_result(path)
    try:
        replay = _SERVICE.replay(result)
    except (TypeError, ValueError, ValidationError) as error:
        raise M2106CliError("result replay is invalid") from error  # noqa: TRY003
