"""Typer adapter for provisional M25-08 evidence adjudication."""
...
class M2508CliError(typer.BadParameter):
    """Sanitized M25-08 command-line validation error."""


def _read_request(path: Path) -> AdjudicateProteotypeEvidenceGateRequest:
    try:
        data = read_bounded(path, M2508_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2508_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2508CliError("input must satisfy the strict M25-08 request contract") from error


def _read_result(path: Path) -> ProteotypeEvidenceGateResult:
    try:
        data = read_bounded(path, M2508_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(data, max_bytes=M2508_MAX_CANONICAL_RESULT_BYTES)
        return _RESULT_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2508CliError("input must be a valid M25-08 result") from error


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2508CliError("output already exists; refusing to overwrite")
...
def export_schema(
    ...
    """Export one strict provisional M25-08 contract schema."""
    if name not in _CONTRACT_NAMES:
        raise M2508CliError("unknown M25-08 contract")
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2508CliError("request does not satisfy the M25-08 contract") from error
...
def adjudicate(
    ...
    except (ValidationError, ValueError, M2508AuthorizationError, M2508EvaluationError) as error:
        raise M2508CliError("request was rejected by the M25-08 service") from error
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2508CliError("result replay is invalid") from error
