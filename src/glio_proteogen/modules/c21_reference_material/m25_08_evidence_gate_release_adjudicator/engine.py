"""Deterministic caller-declared M25-08 evidence-gate runtime."""
...
class M2508AuthorizationError(ValueError):
    """Caller-declared controls do not authorize evidence adjudication."""


class M2508EvaluationError(ValueError):
...
def preflight_m2508_authorization(candidate: object) -> None:
    ...
    try:
        ...
        raise M2508AuthorizationError("M25-08 controls are malformed") from error
    if not authorized:
        raise M2508AuthorizationError("M25-08 requires all seven accepted controls")
...
def _evidence(
    ...
    return tuple(
        EvidenceReference(reference=artifact, role="evidence", claim=M2508_EVIDENCE_CLAIM)
...
def _uncertainty() -> UncertaintyProfile:
    estimate = UncertaintyEstimate(
        ...
        rationale=(
            "M25-08 adjudicates caller-declared evidence and does not estimate scientific error."
        ),
    )
...
def _provenance(
    ...
    return ProvenanceRecord(
        activity_id=f"M2508.activity.{request_digest.removeprefix('sha256:')}",
        actor_id=request.context.actor_id,
        module_id=M2508_MODULE_ID,
        module_version=M2508_CONTRACT_VERSION,
...
def _limitations(*, abstained: bool) -> tuple[Limitation, ...]:
    return (
        Limitation(
            code="M2508_caller_declared_evidence",
            statement=(
        ...
        Limitation(
            code="M2508_abstention" if abstained else "M2508_provisional",
            statement=(
...
def _release_record(
    ...
    return SignedReleaseRecord(
        ...
        limitations=("M25-08 remains provisional pending owner confirmation.",),
        signature_digest=sha256_digest(f"M2508.release:{request_digest}"),
        evidence=evidence,
    )


class M2508Engine:
    """Stateless deterministic M25-08 evidence-gate adjudicator."""

    def validate_request(self, candidate: object) -> AdjudicateProteotypeEvidenceGateRequest:
        preflight_m2508_authorization(candidate)
        try:
            return _REQUEST_ADAPTER.validate_python(candidate, strict=True)
        except Exception as error:
            raise M2508EvaluationError("M25-08 request is invalid") from error

    def evaluate(self, candidate: object) -> ProteotypeEvidenceGateResult:
        ...
        support = SupportDecision(
            ...
            reason_code="M2508_gate_passed" if adjudicated else "M2508_review_required",
            rationale=(
        ...
        payload: dict[str, Any] = {
            "output_type": "proteotype_evidence_gate",
            "result_id": result_identifier(request),
            "result_version": M2508_CONTRACT_VERSION,
            ...
            "abstention_reason": (
                None
                if adjudicated
                else "M25-08 abstained because release evidence is not safely adjudicable."
            ),
        ...
        constructed = ProteotypeEvidenceGateResult.model_construct(**payload)
        payload["result_digest"] = result_payload_digest(constructed)
        try:
            return _RESULT_ADAPTER.validate_python(payload, strict=True)
        except Exception as error:
            raise M2508EvaluationError("M25-08 result construction failed safely") from error
...
def adjudicate_proteotype_evidence_gate(
    candidate: object,
) -> ProteotypeEvidenceGateResult:
    """Public stateless M25-08 adjudication entry point."""

    return M2508Engine().evaluate(candidate)
