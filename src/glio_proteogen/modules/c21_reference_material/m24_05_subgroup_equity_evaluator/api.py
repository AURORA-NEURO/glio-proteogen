"""FastAPI adapter for provisional M24-05 subgroup equity evaluation."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M24-05 contract")


def _parse_request(body: bytes) -> EvaluateBiomarkerPanelSubgroupEquityRequest:
    try:
        strict_json_loads(body, max_bytes=M2405_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2405Service | None = None) -> FastAPI:
    """Create strict validation/evaluation/replay routes."""

    boundary = service or M2405Service()
    app = FastAPI(title="GLIO-PROTEOGEN M24-05", version="0.1.0-provisional")

    @app.get("/v1/modules/M24-05/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M24-05/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M24-05 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M24-05/validate")
    ...
    @app.post("/v1/modules/M24-05/evaluate")
    ...
    @app.post("/v1/modules/M24-05/verify")
