"""Typer adapter for provisional M22-05 subgroup evaluation."""
...
from .engine import M2205AuthorizationError
from .service import M2205Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2205Service()
_REQUEST_ADAPTER = TypeAdapter(EvaluateProteinRnaDiscordanceSubgroupEquityRequest)
_RESULT_ADAPTER = TypeAdapter(ProteinRnaDiscordanceSubgroupEvaluationResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "report",
...
class M2205CliError(typer.BadParameter):
    """Sanitized M22-05 command-line validation error."""


def _read_request(path: Path) -> EvaluateProteinRnaDiscordanceSubgroupEquityRequest:
    try:
        data = read_bounded(path, M2205_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2205_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2205CliError(  # noqa: TRY003
            "input must satisfy the strict M22-05 request contract"
        ) from error


def _read_result(path: Path) -> ProteinRnaDiscordanceSubgroupEvaluationResult:
    try:
        return _RESULT_ADAPTER.validate_json(
            read_bounded(path, M2205_MAX_CANONICAL_RESULT_BYTES), strict=True
        )
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2205CliError("input must be a valid M22-05 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2205CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M22-05 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2205CliError("unknown M22-05 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2205AuthorizationError) as error:
        raise M2205CliError("request does not satisfy the M22-05 contract") from error  # noqa: TRY003
...
def evaluate(
    ...
    try:
        ...
        raise M2205CliError("request was rejected by the M22-05 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2205CliError("result replay is invalid") from error  # noqa: TRY003
