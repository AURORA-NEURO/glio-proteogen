"""Typer adapter for provisional M25-02 synthetic-truth generation."""
...
class M2502CliError(typer.BadParameter):
    """Sanitized M25-02 command-line validation error."""
...
def _read_request(path: Path) -> GenerateProteotypeSyntheticTruthRequest:
    try:
        ...
        raise M2502CliError("input must satisfy the strict M25-02 request contract") from error  # noqa: TRY003
...
def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2502CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M25-02 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2502CliError("unknown M25-02 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2502CliError("request does not satisfy the M25-02 contract") from error  # noqa: TRY003
...
def generate(
    ...
    try:
        ...
        raise M2502CliError("request was rejected by the M25-02 service") from error  # noqa: TRY003
