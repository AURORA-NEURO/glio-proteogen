"""FastAPI boundary for strict M23-08 evidence-gate adjudication."""
...
def _invalid_request(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M23-08 contract")


def _parse_request(body: bytes) -> AdjudicateVariantPeptideEvidenceGateRequest:
    try:
        strict_json_loads(body, max_bytes=M2308_MAX_CANONICAL_REQUEST_BYTES)
...
def _parse_object(body: bytes) -> dict[str, Any]:
    try:
        value = strict_json_loads(body, max_bytes=M2308_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2308Service | None = None) -> FastAPI:
    """Create strict schema, validation, adjudication, and replay routes."""

    boundary = service or M2308Service()
    app = FastAPI(title="GLIO-PROTEOGEN M23-08", version="0.1.0-provisional")
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2308_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M23-08/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M23-08/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M23-08 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M23-08/validate")
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2308_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M23-08/adjudicate")
    ...
    @app.post("/v1/modules/M23-08/verify")
