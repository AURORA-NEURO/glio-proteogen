"""FastAPI adapter for the provisional M25-04 transport boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M25-04 contract")


def _parse_request(body: bytes) -> EvaluateProteotypeExternalTransportRequest:
    try:
        strict_json_loads(body, max_bytes=M2504_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2504Service | None = None) -> FastAPI:
    """Create strict validate/evaluate/replay routes with sanitized errors."""

    boundary = service or M2504Service()
    app = FastAPI(title="GLIO-PROTEOGEN M25-04", version="0.1.0-provisional")

    @app.get("/v1/modules/M25-04/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M25-04/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M25-04 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M25-04/validate")
    ...
    @app.post("/v1/modules/M25-04/evaluate")
    ...
    @app.post("/v1/modules/M25-04/verify")
