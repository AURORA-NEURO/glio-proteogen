"""Typer adapter for provisional M25-04 transport evaluation."""
...
from .engine import M2504AuthorizationError
from .service import M2504Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2504Service()
_REQUEST_ADAPTER = TypeAdapter(EvaluateProteotypeExternalTransportRequest)
_RESULT_ADAPTER = TypeAdapter(ProteotypeExternalTransportResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "validation",
...
class M2504CliError(typer.BadParameter):
    """Sanitized M25-04 command-line validation error."""


def _read_request(path: Path) -> EvaluateProteotypeExternalTransportRequest:
    try:
        data = path.read_bytes()
        strict_json_loads(data, max_bytes=M2504_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2504CliError(  # noqa: TRY003
            "input must satisfy the strict M25-04 request contract"
        ) from error


def _read_result(path: Path) -> ProteotypeExternalTransportResult:
    try:
        return _RESULT_ADAPTER.validate_json(path.read_bytes(), strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2504CliError("input must be a valid M25-04 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2504CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M25-04 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2504CliError("unknown M25-04 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2504AuthorizationError) as error:
        raise M2504CliError("request does not satisfy the M25-04 contract") from error  # noqa: TRY003
...
def evaluate(
    ...
    """Evaluate one caller-declared external transport request."""

    try:
        result = _SERVICE.execute(_read_request(path))
    except (ValidationError, ValueError, M2504AuthorizationError) as error:
        raise M2504CliError("request was rejected by the M25-04 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2504CliError("result replay is invalid") from error  # noqa: TRY003
