"""FastAPI adapter for the provisional M21-01 service boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M21-01 contract")


def _parse_request(body: bytes) -> CurateComplexActivityReferenceTruthRequest:
    try:
        strict_json_loads(body, max_bytes=M2101_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2101Service | None = None) -> FastAPI:
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2101_MAX_CANONICAL_REQUEST_BYTES)
    ...
    @app.get("/v1/modules/M21-01/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M21-01 contract")
