"""FastAPI adapter for provisional M24-02 synthetic truth generation."""
...
from .engine import AuthorizationError, M2402ReplayError
...
def _parse_request(body: bytes) -> GenerateBiomarkerPanelSyntheticTruthRequest:
    try:
        strict_json_loads(body, max_bytes=M2402_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except (StrictJsonError, ValueError, ValidationError) as error:
        raise HTTPException(
            status_code=422, detail="request does not satisfy the M24-02 contract"
        ) from error
...
def create_app(service: M2402Service | None = None) -> FastAPI:
    ...
    @app.get("/v1/modules/M24-02/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M24-02 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M24-02/validate")
    ...
    @app.post("/v1/modules/M24-02/generate")
    async def generate(request: Request) -> dict[str, object]:
        try:
            result = boundary.evaluate(_parse_request(await request.body()))
        except (ValidationError, ValueError, AuthorizationError) as error:
            raise HTTPException(
                status_code=422, detail="request was rejected by the M24-02 service"
            ) from error
        return cast("dict[str, object]", result.model_dump(mode="json"))

    @app.post("/v1/modules/M24-02/verify")
    async def verify(request: Request) -> dict[str, object]:
        ...
        except (ValidationError, ValueError, TypeError, M2402ReplayError) as error:
            raise HTTPException(status_code=422, detail="replay envelope is invalid") from error
        return {
