"""FastAPI adapter for the provisional M21-03 benchmark boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M21-03 contract")


def _parse_request(body: bytes) -> RunComplexActivityInternalBenchmarkRequest:
    try:
        strict_json_loads(body, max_bytes=M2103_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2103Service | None = None) -> FastAPI:
    """Create strict validate/run/replay API routes with sanitized errors."""

    boundary = service or M2103Service()
    app = FastAPI(title="GLIO-PROTEOGEN M21-03", version="0.1.0-provisional")
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2103_MAX_CANONICAL_REQUEST_BYTES)

    @app.get("/v1/modules/M21-03/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M21-03/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M21-03 contract")
        return contract_json_schema(name)  # type: ignore[arg-type]

    @app.post("/v1/modules/M21-03/validate")
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2103_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
    ...
    @app.post("/v1/modules/M21-03/benchmark")
    ...
    @app.post("/v1/modules/M21-03/verify")
    async def verify(request: Request) -> dict[str, object]:
        envelope = _parse_object(
            await _read_bounded(request, max_bytes=M2103_MAX_CANONICAL_RESULT_BYTES)
        )
