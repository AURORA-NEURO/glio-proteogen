"""Strict parse-once plugin boundary for provisional M21-03."""
...
from .engine import preflight_m2103_authorization
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M21-03 execution requires a validated request token")


class _InvalidSubmissionError(TypeError):
    def __init__(self) -> None:
        super().__init__("M21-03 validation requires a benchmark submission")


class M2103Plugin(
    ModulePlugin[object, ValidatedM2103Request, ComplexActivityInternalBenchmarkResult]
):
    """Expose validate-then-run without an authority or parse bypass."""

    __slots__ = ("_service",)

    def __init__(self, service: M2103Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM2103Request:
