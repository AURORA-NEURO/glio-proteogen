...
def create_app(service: M2307Service | None = None) -> FastAPI:
    ...
    app.add_middleware(RequestSizeLimitMiddleware, max_bytes=M2307_MAX_CANONICAL_REQUEST_BYTES)
    ...
    async def validate(request: Request) -> dict[str, object]:
        payload = _parse_request(
            await _read_bounded(request, max_bytes=M2307_MAX_CANONICAL_REQUEST_BYTES)
        )
        try:
