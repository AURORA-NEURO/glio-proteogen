"""Deterministic, caller-declared M23-07 operational evaluation runtime."""
...
_AUTHORIZATION_MESSAGE: Final = (
    "M23-07 operational evaluation requires accepted configuration, resolved identity, granted "
...
_LIMITATIONS: Final = (
    Limitation(
        ...
        statement=(
            "M23-06 is accepted only as a caller-declared media boundary; issuer authority and "
...
class M2307AuthorizationError(ValueError):
...
class M2307ReplayError(ValueError):
    """Raised when an evaluation result fails canonical replay verification."""


class M2307OperationalEngine:
    """Build and replay one deterministic metadata-only operational evaluation."""

    __slots__ = ()

    def generate(self, request: object) -> VariantPeptideHumanFactorsResult:
        preflight_m2307_authorization(request)
        ...
        report = HumanFactorsOperationalReport(
            report_id="m2307.report." + request_digest.removeprefix("sha256:"),
        ...
        if unsupported is None:
            ...
            findings = (
                OperationalFinding(
                    finding_id="m2307.finding.not-evaluable",
        ...
        payload: dict[str, Any] = {
            "output_type": "variant_peptide_human_factors_operational",
            "result_id": result_identifier(canonical),
            "result_version": M2307_CONTRACT_VERSION,
        ...
        provisional = VariantPeptideHumanFactorsResult.model_construct(**payload)
...
def evaluate_variant_peptide_human_factors_operational(
    ...
    """Public stateless M23-07 evaluation entry point."""

    return M2307OperationalEngine().generate(request)


def preflight_m2307_authorization(candidate: object) -> None:
    """Reject denied controls before reading caller-declared operational material."""

    try:
        context = (
            candidate.context
            if isinstance(candidate, EvaluateVariantPeptideHumanFactorsRequest)
        ...
        raise M2307AuthorizationError from None
    if not authorized:
        raise M2307AuthorizationError
...
def _findings(
    ...
    metric_findings = [
        OperationalFinding(
            finding_id="m2307.finding." + metric.metric_id,
    ...
    fallback_findings = [
        OperationalFinding(
            finding_id="m2307.finding." + fallback.scenario_id,
...
def _support() -> SupportDecision:
    return SupportDecision(
        ...
        rationale=(
            "Caller-declared M23-07 operational material satisfied the provisional evaluation "
...
def _uncertainty() -> UncertaintyProfile:
    def unavailable(dimension: str) -> UncertaintyEstimate:
        return UncertaintyEstimate(
            ...
            rationale=(
                f"M23-07 does not estimate {dimension} uncertainty from caller-declared inputs."
            ),
        )
...
def _evidence(
    ...
    return tuple(
        EvidenceReference(
            ...
            role="evidence",
            claim=(
                "Caller-declared M23-07 operational evidence; issuer authority is not "
                "authenticated."
            ),
        )
...
def _provenance(
    ...
    return ProvenanceRecord(
        activity_id="m2307.activity." + request_digest.removeprefix("sha256:"),
        actor_id=request.context.actor_id,
        module_id=M2307_MODULE_ID,
        module_version=M2307_CONTRACT_VERSION,
