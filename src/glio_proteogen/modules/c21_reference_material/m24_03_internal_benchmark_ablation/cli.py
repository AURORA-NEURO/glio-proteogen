"""Typer adapter for provisional M24-03 benchmark execution."""
...
from .engine import M2403AuthorizationError
from .service import M2403Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2403Service()
_REQUEST_ADAPTER = TypeAdapter(RunBiomarkerPanelInternalBenchmarkRequest)
_RESULT_ADAPTER = TypeAdapter(BiomarkerPanelInternalBenchmarkResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "dossier",
...
class M2403CliError(typer.BadParameter):
    """Sanitized M24-03 command-line validation error."""


def _read_request(path: Path) -> RunBiomarkerPanelInternalBenchmarkRequest:
    try:
        data = path.read_bytes()
        strict_json_loads(data, max_bytes=M2403_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2403CliError(  # noqa: TRY003
            "input must satisfy the strict M24-03 request contract"
        ) from error


def _read_result(path: Path) -> BiomarkerPanelInternalBenchmarkResult:
    try:
        return _RESULT_ADAPTER.validate_json(path.read_bytes(), strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2403CliError("input must be a valid M24-03 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2403CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M24-03 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2403CliError("unknown M24-03 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2403AuthorizationError) as error:
        raise M2403CliError("request does not satisfy the M24-03 contract") from error  # noqa: TRY003
...
def benchmark(
    ...
    try:
        ...
        raise M2403CliError("request was rejected by the M24-03 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2403CliError("result replay is invalid") from error  # noqa: TRY003
