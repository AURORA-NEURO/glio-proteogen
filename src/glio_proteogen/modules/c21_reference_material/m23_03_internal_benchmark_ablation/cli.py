"""Typer adapter for provisional M23-03 benchmark execution."""
...
from .engine import M2303AuthorizationError
from .service import M2303Service

app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
_SERVICE = M2303Service()
_REQUEST_ADAPTER = TypeAdapter(RunVariantPeptideInternalBenchmarkRequest)
_RESULT_ADAPTER = TypeAdapter(VariantPeptideInternalBenchmarkResult)
_CONTRACT_NAMES = {
    "request",
    "output",
    "dossier",
...
class M2303CliError(typer.BadParameter):
    """Sanitized M23-03 command-line validation error."""


def _read_request(path: Path) -> RunVariantPeptideInternalBenchmarkRequest:
    try:
        data = read_bounded(path, M2303_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2303_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2303CliError(  # noqa: TRY003
            "input must satisfy the strict M23-03 request contract"
        ) from error


def _read_result(path: Path) -> VariantPeptideInternalBenchmarkResult:
    try:
        return _RESULT_ADAPTER.validate_json(
            read_bounded(path, M2303_MAX_CANONICAL_RESULT_BYTES), strict=True
        )
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2303CliError("input must be a valid M23-03 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2303CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M23-03 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2303CliError("unknown M23-03 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    except (ValidationError, ValueError, M2303AuthorizationError) as error:
        raise M2303CliError("request does not satisfy the M23-03 contract") from error  # noqa: TRY003
...
def benchmark(
    ...
    try:
        ...
        raise M2303CliError("request was rejected by the M23-03 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2303CliError("result replay is invalid") from error  # noqa: TRY003
