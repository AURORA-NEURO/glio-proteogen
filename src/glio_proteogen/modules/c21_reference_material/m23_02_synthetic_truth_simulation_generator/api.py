"""FastAPI adapter for the provisional M23-02 generation boundary."""
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M23-02 contract")


def _parse_request(body: bytes) -> GenerateVariantPeptideSyntheticTruthRequest:
    try:
        strict_json_loads(body, max_bytes=M2302_MAX_CANONICAL_REQUEST_BYTES)
...
def _parse_object(body: bytes) -> dict[str, Any]:
    try:
        value = strict_json_loads(body, max_bytes=M2302_MAX_CANONICAL_RESULT_BYTES)
...
def create_app(service: M2302Service | None = None) -> FastAPI:
    ...
    @app.get("/v1/modules/M23-02/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M23-02 contract")
    ...
    async def verify(request: Request) -> dict[str, object]:
        ...
        try:
            ...
            replay = boundary.verify(result, typed_request)
