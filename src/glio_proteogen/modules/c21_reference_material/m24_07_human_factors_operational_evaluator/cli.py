"""Typer adapter for provisional M24-07 operational evaluation."""
...
class M2407CliError(typer.BadParameter):
    """Sanitized M24-07 command-line validation error."""


def _read_request(path: Path) -> EvaluateBiomarkerPanelHumanFactorsRequest:
    try:
        data = read_bounded(path, M2407_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(data, max_bytes=M2407_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2407CliError("input must satisfy the strict M24-07 request contract") from error  # noqa: TRY003


def _read_result(path: Path) -> BiomarkerPanelHumanFactorsResult:
    try:
        data = read_bounded(path, M2407_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(data, max_bytes=M2407_MAX_CANONICAL_RESULT_BYTES)
        return _RESULT_ADAPTER.validate_json(data, strict=True)
    except (OSError, StrictJsonError, ValueError, ValidationError) as error:
        raise M2407CliError("input must be a valid M24-07 result") from error  # noqa: TRY003


def _write_new(path: Path, data: bytes) -> None:
    if path.exists():
        raise M2407CliError("output already exists; refusing to overwrite")  # noqa: TRY003
...
def export_schema(
    ...
    """Export one strict provisional M24-07 contract schema."""

    if name not in _CONTRACT_NAMES:
        raise M2407CliError("unknown M24-07 contract")  # noqa: TRY003
...
def validate(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2407CliError("request does not satisfy the M24-07 contract") from error  # noqa: TRY003
...
def evaluate(
    ...
    """Evaluate one request and emit a canonical operational result envelope."""

    try:
        result = _SERVICE.evaluate(_read_request(path))
    except (ValidationError, ValueError, M2407AuthorizationError) as error:
        raise M2407CliError("request was rejected by the M24-07 service") from error  # noqa: TRY003
...
def verify(path: Annotated[Path, typer.Argument(exists=True, readable=True)]) -> None:
    ...
    try:
        ...
        raise M2407CliError("result replay is invalid") from error  # noqa: TRY003
