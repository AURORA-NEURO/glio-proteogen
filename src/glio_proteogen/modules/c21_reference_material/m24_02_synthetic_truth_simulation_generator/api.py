...
from .engine import M2402AuthorizationError
...
def _safe_validation(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M24-02 contract")
...
def create_app(service: M2402Service | None = None) -> FastAPI:
    ...
    async def validate(request: Request) -> dict[str, object]:
        ...
        except (ValidationError, ValueError, M2402AuthorizationError) as error:
            raise _safe_validation(error) from error
