"""Deterministic, caller-declared M24-08 evidence-gate runtime."""
...
_RESULT_ADAPTER: Final = TypeAdapter(BiomarkerPanelEvidenceGateResult)
_ZERO_DIGEST: Final = "sha256:" + ("0" * 64)


class M2408AuthorizationError(ValueError):
    """Caller-declared controls do not authorize adjudication."""

    def __init__(self) -> None:
        super().__init__(
            "M24-08 evidence adjudication requires accepted configuration, resolved identity, "
            "granted consent, and accepted provenance/quality/support/intended-use controls"
        )


class M2408ReplayError(ValueError):
...
def preflight_m2408_authorization(candidate: object) -> None:
    ...
    try:
        ...
        raise M2408AuthorizationError from None
    if not authorized:
        raise M2408AuthorizationError


def _gate_decision(request: AdjudicateBiomarkerPanelEvidenceGateRequest) -> GateDecision:
...
def _findings(
    ...
    findings.extend(
        GateFinding(
            finding_id="m2408.finding." + item.requirement_id,
    ...
    findings.extend(
        GateFinding(
            finding_id="m2408.finding." + item.benchmark_id,
    ...
    findings.extend(
        GateFinding(
            finding_id="m2408.finding." + item.risk_id,
    ...
    findings.extend(
        GateFinding(
            finding_id="m2408.finding." + item.approval_id,
    ...
    if not findings:
        findings.append(
            GateFinding(
                finding_id="m2408.finding.provisional-review",
...
def _release_record(
    ...
    record = SignedReleaseRecord(
        release_id="m2408.release." + request_digest.removeprefix("sha256:"),
        ...
        signature_digest=_ZERO_DIGEST,
        evidence=evidence,
    )
    return record.model_copy(
        update={
            "signature_digest": release_record_signature_digest(request_digest, record),
        }
    )
...
def _uncertainty() -> UncertaintyProfile:
    def unavailable(dimension: str) -> UncertaintyEstimate:
        return UncertaintyEstimate(
            ...
            rationale=f"M24-08 does not estimate {dimension} uncertainty from gate material.",
        )
...
def _provenance(
    ...
    return ProvenanceRecord(
        activity_id="m2408.activity." + request_digest.removeprefix("sha256:"),
        actor_id=request.context.actor_id,
        module_id=M2408_MODULE_ID,
        module_version=M2408_CONTRACT_VERSION,
...
class M2408EvidenceGateEngine:
    ...
    def adjudicate(self, request: object) -> BiomarkerPanelEvidenceGateResult:
        ...
        payload: dict[str, Any] = {
            "output_type": "biomarker_panel_evidence_gate",
            "result_id": result_identifier(request_digest),
            "result_version": M2408_CONTRACT_VERSION,
        ...
        provisional = BiomarkerPanelEvidenceGateResult.model_construct(**payload)
        payload["result_digest"] = result_payload_digest(provisional)
        return _RESULT_ADAPTER.validate_python(payload, strict=True)

    def replay(self, result: BiomarkerPanelEvidenceGateResult) -> BiomarkerPanelEvidenceGateResult:
        try:
            validated = _RESULT_ADAPTER.validate_python(result, strict=True)
            if validated.request_digest != canonical_request_digest(validated.request):
                raise M2408ReplayError  # noqa: TRY301
            ...
            if expected.model_dump(mode="json") != validated.model_dump(mode="json"):
                raise M2408ReplayError  # noqa: TRY301
        except M2408ReplayError:
            raise
        except Exception as error:
            raise M2408ReplayError from error
        return validated


def adjudicate_biomarker_panel_evidence_gate(
    request: object,
) -> BiomarkerPanelEvidenceGateResult:
    """Public stateless M24-08 evidence gate entry point."""

    return M2408EvidenceGateEngine().adjudicate(request)
