"""FastAPI boundary for strict M28-04 gateway publication."""
...
def _invalid_request(error: Exception) -> HTTPException:
    del error
    return HTTPException(status_code=422, detail="request does not satisfy the M28-04 contract")


def _parse_request(body: bytes) -> PublishProteinRnaDiscordanceAccessSurfaceRequest:
    try:
        strict_json_loads(body, max_bytes=M2804_MAX_CANONICAL_REQUEST_BYTES)
...
def _parse_object(body: bytes) -> dict[str, Any]:
    try:
        value = strict_json_loads(body, max_bytes=M2804_MAX_CANONICAL_REQUEST_BYTES)
...
def create_app(service: M2804Service | None = None) -> FastAPI:
    """Create strict schema, validation, publication, and replay routes."""

    boundary = service or M2804Service()
    app = FastAPI(title="GLIO-PROTEOGEN M28-04", version="0.1.0-provisional")

    @app.get("/v1/modules/M28-04/schemas")
    async def schemas() -> dict[str, dict[str, object]]:
        return cast("dict[str, dict[str, object]]", contract_json_schemas())

    @app.get("/v1/modules/M28-04/schemas/{name}")
    async def schema(name: str) -> dict[str, object]:
        if name not in _CONTRACT_NAMES:
            raise HTTPException(status_code=404, detail="unknown M28-04 contract")
