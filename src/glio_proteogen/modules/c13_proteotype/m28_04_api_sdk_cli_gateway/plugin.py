...
_TOKENS: WeakKeyDictionary[ValidatedM2804Request, tuple[object, str]] = WeakKeyDictionary()
...
class M2804Plugin:
    ...
    def validate(self, submission: GatewaySubmission) -> ValidatedM2804Request:
        ...
        _TOKENS[token] = (self._seal, canonical_request_digest(validated))
        return token
    ...
    def run(self, token: ValidatedM2804Request) -> ProteinRnaDiscordanceAccessSurfaceResult:
        binding = _TOKENS.get(token) if isinstance(token, ValidatedM2804Request) else None
        if binding is None or binding[0] is not self._seal:
            raise M2804TokenError
        if token._seal is not self._seal:
            raise M2804TokenError
        if canonical_request_digest(token.request) != binding[1]:
            raise M2804TokenError
