"""Strict parse-once plugin boundary for provisional M15-04."""
...
from glio_proteogen.contracts.m15_04.canonical import canonical_request_digest
...
_REQUEST_ADAPTER: Final = TypeAdapter(InferComplexActivityMechanismRequest)
...
class ValidatedM1504Request:
    """Opaque capability proving strict M15-04 request acceptance."""

    request: InferComplexActivityMechanismRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1504Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M15-04 execution requires a validated request token")
...
class M1504Plugin(
    ...
    """Expose M15-04 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1504Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1504Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1504_MAX_CANONICAL_REQUEST_BYTES)
            ...
            preflight_mechanism_authorization(typed)
        token = ValidatedM1504Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1504Request) -> ComplexActivityMechanismInferenceResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1504Request
...
__all__ = ["M1504Plugin", "ValidatedM1504Request"]
