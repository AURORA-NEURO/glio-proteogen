...
app = typer.Typer(help="M08-05 transcript-protein mechanism and constraint integrator.")


def _read(path: Path) -> bytes:
    try:
        body = read_bounded(path, max_bytes=M0805_MAX_CANONICAL_REQUEST_BYTES)
...
def _request_from_file(path: Path) -> IntegrateTranscriptProteinConstraintsRequest:
    body = _read(path)
    try:
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except ValidationError as error:
        raise typer.BadParameter("request does not match the M08-05 contract") from error


@app.command("export-schema")
def export_schema(
    contract: Annotated[str, typer.Argument(help="M08-05 contract name.")],
) -> None:
    """Export one strict JSON Schema 2020-12 contract."""

    if contract not in _CONTRACT_NAMES:
        raise typer.BadParameter("unknown M08-05 contract")
...
def integrate(
    ...
    try:
        built = M0805Service().integrate(_request_from_file(request))
    except (
        M0805AuthorizationError,
        ValidationError,
        ValueError,
        ...
        raise typer.BadParameter("M08-05 integration input is invalid") from error
