"""Typer CLI for strict M08-02 validation and construction."""
...
from glio_proteogen.modules.c08_transcript_protein_discordance.m08_02_representation_feature_constructor.service import (  # noqa: E501
    M0802Service,
)
...
app = typer.Typer(help="M08-02 transcript-protein representation constructor.")
...
def _request_from_file(path: Path) -> ConstructTranscriptProteinRepresentationRequest:
    body = _read(path)
    try:
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except ValidationError as error:
        raise typer.BadParameter("request does not match the M08-02 contract") from error


@app.command("export-schema")
def export_schema(
    contract: Annotated[str, typer.Argument(help="M08-02 contract name.")],
) -> None:
    """Export one strict JSON Schema 2020-12 contract."""

    if contract not in _CONTRACT_NAMES:
        raise typer.BadParameter("unknown M08-02 contract")
    typer.echo(
...
def construct(
    ...
    """Construct a representation; abstention exits nonzero after writing the result."""

    try:
        built = M0802Service().construct(_request_from_file(request))
    except (ValidationError, ValueError, TypeError, StrictJsonError) as error:
        raise typer.BadParameter("M08-02 construction input is invalid") from error
