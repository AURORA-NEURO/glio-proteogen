"""Strict parse-once M13-05 plugin boundary."""
...
from glio_proteogen.contracts.m13_05.canonical import canonical_request_digest
from glio_proteogen.kernel.canonical import canonical_json_bytes
from glio_proteogen.kernel.plugin import ModuleDescriptor, ModulePlugin
from glio_proteogen.kernel.strict_json import strict_json_loads

from .engine import preflight_longitudinal_authorization

if TYPE_CHECKING:
    from .service import M1305Service
...
class ValidatedM1305Request:
    """Opaque capability proving strict M13-05 request acceptance."""

    request: ModelProteotypeLongitudinalEvolutionRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1305Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M13-05 execution requires a validated request token")


class M1305Plugin(
    ModulePlugin[object, ValidatedM1305Request, ProteotypeLongitudinalEvolutionResult]
):
    """Expose M13-05 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1305Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1305Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1305_MAX_CANONICAL_REQUEST_BYTES)
            preflight_longitudinal_authorization(parsed)
            typed = _REQUEST_ADAPTER.validate_json(canonical_json_bytes(parsed), strict=True)
        else:
            typed = self._service.validate_request(request)
        token = ValidatedM1305Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1305Request) -> ProteotypeLongitudinalEvolutionResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1305Request
...
__all__ = ["M1305Plugin", "ValidatedM1305Request"]
