"""Strict parse-once M11-05 plugin boundary."""
...
class ValidatedM1105Request:
    request: ModelVariantPeptideLongitudinalEvolutionRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1105Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M11-05 execution requires a validated request token")


class M1105Plugin(
    ModulePlugin[object, ValidatedM1105Request, VariantPeptideLongitudinalEvolutionResult]
):
    """Expose M11-05 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1105Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1105Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1105_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1105Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1105Request) -> VariantPeptideLongitudinalEvolutionResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1105Request
...
__all__ = ["M1105Plugin", "ValidatedM1105Request"]
