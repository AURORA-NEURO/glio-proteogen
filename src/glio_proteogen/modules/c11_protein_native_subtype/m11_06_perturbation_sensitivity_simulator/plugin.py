"""Strict parse-once M11-06 plugin boundary."""
...
if TYPE_CHECKING:
    from .service import M1106Service
...
_DESCRIPTOR: Final = ModuleDescriptor(
    ...
    gate="G2",
    prohibited_outputs=(
        "kinase state, generic all-omics fusion, treatment recommendation",
        "identity or consent inference, unsupported-to-negative conversion",
...
class ValidatedM1106Request:
    request: SimulateVariantPeptidePerturbationsRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1106Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M11-06 execution requires a validated request token")


class M1106Plugin(
    ModulePlugin[object, ValidatedM1106Request, VariantPeptideSensitivitySimulationResult]
):
    """Expose M11-06 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1106Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1106Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1106_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1106Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1106Request) -> VariantPeptideSensitivitySimulationResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1106Request
...
__all__ = ["M1106Plugin", "ValidatedM1106Request"]
