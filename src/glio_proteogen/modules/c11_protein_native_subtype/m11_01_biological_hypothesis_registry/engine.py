"""Deterministic, replay-bound M11-01 hypothesis registry runtime."""
...
_RESULT_ADAPTER: Final = TypeAdapter(VariantPeptideHypothesisRegistryResult)
...
class M1101HypothesisAuthorizationError(PermissionError):
    """Caller-owned controls are not authorized for registry evaluation."""

    def __init__(self) -> None:
        super().__init__(
            "M11-01 requires accepted controls, resolved identity, and granted consent"
        )


class M1101ReplayVerificationError(ValueError):
    """A registry result cannot be reconstructed from its exact request."""

    def __init__(self) -> None:
        super().__init__("M11-01 replay verification failed")
...
def preflight_hypothesis_authorization(candidate: object) -> None:
    ...
    try:
        ...
        raise M1101HypothesisAuthorizationError from None
    if states != expected:
        raise M1101HypothesisAuthorizationError
...
def _evidence(
    ...
    return tuple(
        KernelEvidenceReference(reference=artifact, role="evidence", claim=M1101_EVIDENCE_CLAIM)
...
class M1101HypothesisEngine:
    """Evaluate explicit caller-declared hypotheses without hidden interpretation."""

    __slots__ = ()

    def register(self, request: object) -> VariantPeptideHypothesisRegistryResult:
    ...
    def _result(
        ...
        payload: dict[str, object] = {
            "output_type": "variant_peptide_hypothesis_registry",
            "result_id": f"result.{request_hash.removeprefix('sha256:')}",
            "result_version": M1101_CONTRACT_VERSION,
            ...
            "parent_target": M1101_PARENT,
            "emits_parent": False,
            "support_decision": SupportDecision(
                status=SupportStatus.SUPPORTED if supported else SupportStatus.UNSUPPORTED,
                reason_code="m1101_registry_supported" if supported else "m1101_registry_abstained",
                rationale=(
        ...
        constructed = VariantPeptideHypothesisRegistryResult.model_construct(**payload)  # type: ignore[arg-type]
    ...
    def verify(
        ...
        *,
        ...
        try:
            ...
            raise M1101ReplayVerificationError from error
        if validated.result_digest != result_payload_digest(validated):
            raise M1101ReplayVerificationError
        if replay:
...
def register_variant_peptide_hypotheses(
    request: object,
) -> VariantPeptideHypothesisRegistryResult:
    """Public provisional M11-01 operation."""

    return M1101HypothesisEngine().register(request)
