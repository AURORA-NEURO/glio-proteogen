"""Strict parse-once M13-01 plugin boundary."""
...
class ValidatedM1301Request:
    request: RegisterProteotypeHypothesesRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1301Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M13-01 execution requires a validated request token")


class M1301Plugin(ModulePlugin[object, ValidatedM1301Request, ProteotypeHypothesisRegistryResult]):
    """Expose M13-01 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1301Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1301Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1301_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1301Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1301Request) -> ProteotypeHypothesisRegistryResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1301Request
...
__all__ = ["M1301Plugin", "ValidatedM1301Request"]
