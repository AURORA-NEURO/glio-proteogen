"""Strict parse-once plugin boundary for provisional M11-04."""
...
class ValidatedM1104Request:
    request: InferVariantPeptideMechanismRequest
    _seal: object


_ISSUED_TOKENS: Final[WeakKeyDictionary[ValidatedM1104Request, tuple[object, str]]] = (
    WeakKeyDictionary()
)
...
class _InvalidExecutionTokenError(TypeError):
    def __init__(self) -> None:
        super().__init__("M11-04 execution requires a validated request token")


class M1104Plugin(
    ModulePlugin[object, ValidatedM1104Request, VariantPeptideMechanismInferenceResult]
):
    """Expose M11-04 through validate-then-run token semantics."""

    __slots__ = ("_service",)

    def __init__(self, service: M1104Service) -> None:
        self._service = service

    def descriptor(self) -> ModuleDescriptor:
        return _DESCRIPTOR

    def validate(self, request: object) -> ValidatedM1104Request:
        if type(request) in {bytes, bytearray, str}:
            serialized = cast("bytes | bytearray | str", request)
            parsed = strict_json_loads(serialized, max_bytes=M1104_MAX_CANONICAL_REQUEST_BYTES)
        ...
        token = ValidatedM1104Request(request=typed, _seal=_TOKEN_SEAL)
        _ISSUED_TOKENS[token] = (typed, canonical_request_digest(typed))
        return token

    def run(self, request: ValidatedM1104Request) -> VariantPeptideMechanismInferenceResult:
        try:
            snapshot = _ISSUED_TOKENS.get(request)
        except TypeError as error:
            raise _InvalidExecutionTokenError from error
        if (
            type(request) is not ValidatedM1104Request
...
__all__ = ["M1104Plugin", "ValidatedM1104Request"]
