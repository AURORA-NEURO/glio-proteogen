...
from .errors import NeftelConditionalInferenceError
...
def _validate_inputs(
    ...
    if design.ndim != 2 or design.shape[0] == 0 or design.shape[1] == 0:
        raise NeftelConditionalInferenceError("solver design must be a non-empty matrix")
    if not np.all(np.isfinite(design)):
        raise NeftelConditionalInferenceError("solver design contains non-finite values")
    if not evidence:
        raise NeftelConditionalInferenceError("solver requires active evidence")
    positions = tuple(item.feature_position for item in evidence)
    if len(set(positions)) != len(positions):
        raise NeftelConditionalInferenceError("solver evidence positions are duplicated")
    for item in evidence:
        if not 0 <= item.feature_position < design.shape[0]:
            raise NeftelConditionalInferenceError("solver evidence position is out of range")
        if (
        ...
        if item.semantics not in {"exact_delta", "upper_bound", "lower_bound"}:
            raise NeftelConditionalInferenceError("unsupported solver evidence semantics")
...
def solve_conditional_coordinates(
    ...
    *,
    ...
    if (
        ...
        raise NeftelConditionalInferenceError("solver constants are outside their domain")
    ...
    if initial_coordinates is None:
        ...
        if coordinates.shape != (active_design.shape[1],) or not np.all(np.isfinite(coordinates)):
            raise NeftelConditionalInferenceError(
                "initial coordinates must be a finite vector matching the design width"
            )
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        system = active_design.T @ (weights[:, None] * active_design) + ridge_lambda * penalty
