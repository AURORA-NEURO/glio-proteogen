"""JSON Schema 2020-12 exports for provisional M21-08 contracts."""

from __future__ import annotations

from typing import Final, Literal, cast

from pydantic import TypeAdapter

from glio_proteogen.contracts.m21_08.v1 import (
    M2108_CONTRACT_VERSION,
    M2108_GATE,
    M2108_MAX_CANONICAL_REQUEST_BYTES,
    M2108_MODULE_ID,
    M2108_OUTPUT_MEDIA_TYPE,
    M2108_OWNER,
    M2108_PARENT,
    M2108_PROVISIONAL_ABI,
    M2108_SAFETY_CLASS,
    AdjudicateComplexActivityEvidenceGateRequest,
    ApprovalRecord,
    BenchmarkOutcome,
    ComplexActivityEvidenceGateResult,
    GateConfiguration,
    GateFinding,
    GateRequirement,
    ResidualRisk,
    SignedReleaseRecord,
)

SCHEMA_ID_PREFIX: Final = (
    "urn:aurora-neuro:glio-proteogen:GLIO-PROTEOGEN-M21-08:0.1.0-provisional"
)
CONTRACT_VERSION: Final = M2108_CONTRACT_VERSION
ContractName = Literal[
    "request",
    "output",
    "requirement",
    "benchmark",
    "risk",
    "approval",
    "release-record",
    "configuration",
    "finding",
]
_CONTRACTS: Final = {
    "request": AdjudicateComplexActivityEvidenceGateRequest,
    "output": ComplexActivityEvidenceGateResult,
    "requirement": GateRequirement,
    "benchmark": BenchmarkOutcome,
    "risk": ResidualRisk,
    "approval": ApprovalRecord,
    "release-record": SignedReleaseRecord,
    "configuration": GateConfiguration,
    "finding": GateFinding,
}


def contract_json_schema(name: ContractName) -> dict[str, object]:
    """Return one strict, metadata-only provisional M21-08 schema."""

    schema = TypeAdapter(_CONTRACTS[name]).json_schema(mode="validation")
    schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    schema["$id"] = f"{SCHEMA_ID_PREFIX}:{name}"
    schema["x-glio-contract"] = {
        "moduleId": M2108_MODULE_ID,
        "contractVersion": CONTRACT_VERSION,
        "owner": M2108_OWNER,
        "safetyClass": M2108_SAFETY_CLASS,
        "gate": M2108_GATE,
        "strict": True,
        "provisionalAbi": M2108_PROVISIONAL_ABI,
        "abiStatus": "dossier-behavioral-brief-only",
        "pendingOwnerConfirmation": True,
        "externalContentTraversal": False,
        "rawPayload": False,
        "allOmicsFusion": False,
        "kinaseActivity": False,
        "treatmentRecommendation": False,
        "parentTarget": M2108_PARENT,
        "unsupportedToNegative": False,
        "outputMediaType": M2108_OUTPUT_MEDIA_TYPE,
        "primaryArchitecture": "distributed_simulation_continuous_challenge",
        "alternateArchitecture": "locked_offline_benchmark_harness",
        "fallbackArchitecture": "independent_dual_run_validation",
        "traceabilityRequired": True,
        "riskControlsRequired": True,
        "benchmarkOutcomesRequired": True,
        "claimCeilingRequired": True,
        "residualRiskRequired": True,
        "approvalRequired": True,
        "postReleaseObligationsRequired": True,
        "signedReleaseRecordRequired": True,
        "noUnresolvedCriticalRequirements": True,
        "uncertaintyRequired": True,
        "explicitAbstentionRequired": True,
    }
    if name == "request":
        schema["x-glio-contract"]["maxRequestBytes"] = M2108_MAX_CANONICAL_REQUEST_BYTES
    return cast("dict[str, object]", schema)


def contract_json_schemas() -> dict[ContractName, dict[str, object]]:
    """Return all nine provisional M21-08 schemas in ABI order."""

    names = cast("tuple[ContractName, ...]", tuple(_CONTRACTS))
    return {name: contract_json_schema(name) for name in names}


__all__ = [
    "CONTRACT_VERSION",
    "SCHEMA_ID_PREFIX",
    "ContractName",
    "contract_json_schema",
    "contract_json_schemas",
]
