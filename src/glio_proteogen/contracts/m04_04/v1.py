"""Strict M04-04 fixed-point proteoform quality contracts."""
...
from glio_proteogen.contracts.m04_01 import (  # noqa: TC001 - Pydantic resolves models.
...
from glio_proteogen.contracts.m04_03 import (
    normalized_result as normalized_m0403_result,
)
...
M0404_MODULE_ID: Final = "GLIO-PROTEOGEN-M04-04"
M0404_OPERATION: Final = "compute_proteoform_quality_metrics"
M0404_CONTRACT_VERSION: Final = "1.0.0"
M0404_PARENT: Final = "protein_rna_discordance"
M0404_OWNER: Final = "Data engineering"
M0404_SAFETY_CLASS: Final = "S2"
M0404_GATE: Final = "G1"
M0404_ROLE_COUNT: Final = 4
M0404_METRIC_COUNT: Final = 8
M0404_COMPUTED_METRIC_COUNT: Final = 32
M0404_RATE_SCALE: Final = 1_000_000
M0404_MAX_COUNT: Final = 9_223_372_036_854_775_807
M0404_MIN_PROFILES: Final = 4
...
_REQUEST_CAPABILITY_CONTEXT_KEY: Final = "_m0404_validated_request_capability"
...
_ISSUED_RAW_CAPABILITIES: Final[
    WeakKeyDictionary[
        ...
        tuple[bytes, bytes, ProteoformRawInputValidationResult],
    ]
] = WeakKeyDictionary()
...
type ProteoformQualityOpaqueNamespace = Literal[
    "request", "actor", "decision", "policy", "profile", "ledger", "fact", "evidence", "reviewer"
]


def opaque_proteoform_quality_identifier(
    namespace: ProteoformQualityOpaqueNamespace, value: Identifier
) -> Identifier:
    """Validate an M04-04 caller-reflected opaque identifier."""

    if _OPAQUE_IDENTIFIER.fullmatch(value) is None or not value.startswith(f"{namespace}."):
        raise ValueError("M04-04 identifiers must use their exact opaque local namespace")
    return value


class ProteoformQualityMetricCode(StrEnum):
...
class ProteoformQualityObservationState(StrEnum):
    OBSERVED = "observed"
    CENSORED = "censored"
    MISSING = "missing"
...
class ProteoformQualityMetricStatus(StrEnum):
...
class ProteoformQualityMetricDirection(StrEnum):
    AT_LEAST = "at_least"
    AT_MOST = "at_most"


class ProteoformQualityDisposition(StrEnum):
    QUALIFIED = "qualified"
    QUARANTINED = "quarantined"
    ABSTAINED = "abstained"


class ProteoformQualityFindingAction(StrEnum):
    RECORD = "record"
    QUARANTINE = "quarantine"
    ABSTAIN = "abstain"


class ProteoformQualityFindingCode(StrEnum):
    ...
    UNIT_DEFINITION_VERSION_MISMATCH = "unit_definition_version_mismatch"
...
_DIRECTION_BY_METRIC: Final = dict.fromkeys(
    ProteoformQualityMetricCode, ProteoformQualityMetricDirection.AT_LEAST
)
_DIRECTION_BY_METRIC[ProteoformQualityMetricCode.DETECTION_LIMIT_BURDEN] = (
...
_ACTION_BY_FINDING: Final = {
    ...
    ProteoformQualityFindingCode.ASSAY_PROFILE_UNSUPPORTED: ProteoformQualityFindingAction.ABSTAIN,
    ...
    ProteoformQualityFindingCode.REQUIRED_METRIC_MISSING: ProteoformQualityFindingAction.ABSTAIN,
    ProteoformQualityFindingCode.REQUIRED_METRIC_UNSUPPORTED: (
        ProteoformQualityFindingAction.ABSTAIN
    ),
    ProteoformQualityFindingCode.REQUIRED_METRIC_NOT_EVALUABLE: (
        ProteoformQualityFindingAction.ABSTAIN
    ),
    ProteoformQualityFindingCode.REQUIRED_METRIC_WARNING: ProteoformQualityFindingAction.QUARANTINE,
    ProteoformQualityFindingCode.METRIC_THRESHOLD_FAILED: ProteoformQualityFindingAction.QUARANTINE,
    ProteoformQualityFindingCode.OPTIONAL_METRIC_WARNING: ProteoformQualityFindingAction.RECORD,
    ProteoformQualityFindingCode.CROSS_METRIC_INCONSISTENCY: (
        ProteoformQualityFindingAction.QUARANTINE
    ),
}
_MESSAGE_BY_FINDING: Final = {
    code: code.value.replace("_", " ").capitalize() + "." for code in ProteoformQualityFindingCode
}
...
def _owned_artifact(value: ArtifactReference, media_type: str) -> ArtifactReference:
    opaque_proteoform_quality_identifier("evidence", value.artifact_id)
    if value.media_type != media_type:
        raise ValueError("M04-04 artifact media type is not owned by this contract region")
    return value
...
def _require_consistent_evidence_identities(
    ...
    for artifact in artifacts:
        ...
        if previous != content:
            raise ValueError("one M04-04 evidence identity cannot declare conflicting content")


class ProteoformQualityThreshold(FrozenModel):
    metric_code: ProteoformQualityMetricCode
    direction: ProteoformQualityMetricDirection
    pass_threshold_ppm: int = Field(ge=0, le=M0404_RATE_SCALE)
    warning_threshold_ppm: int = Field(ge=0, le=M0404_RATE_SCALE)
    required: bool

    @model_validator(mode="after")
    def thresholds_are_directionally_closed(self) -> ProteoformQualityThreshold:
        ...
        if self.direction is ProteoformQualityMetricDirection.AT_LEAST:
...
class ProteoformAssayQualityProfile(FrozenModel):
    ...
    approved_assay_protocol_versions: tuple[SemanticVersion, ...] = Field(
        min_length=1, max_length=M0404_MAX_APPROVED_VERSIONS
    )
    approved_specimen_processing_versions: tuple[SemanticVersion, ...] = Field(
        min_length=1, max_length=M0404_MAX_APPROVED_VERSIONS
    )
    approved_unit_definition_versions: tuple[SemanticVersion, ...] = Field(
        min_length=1, max_length=M0404_MAX_APPROVED_VERSIONS
    )
    controls_applicable: bool
    thresholds: tuple[ProteoformQualityThreshold, ...] = Field(
        min_length=M0404_METRIC_COUNT, max_length=M0404_METRIC_COUNT
    )
    ...
    def thresholds_are_canonical(
        cls, values: tuple[ProteoformQualityThreshold, ...]
    ) -> tuple[ProteoformQualityThreshold, ...]:
    ...
    @model_validator(mode="after")
    def profile_is_closed(self) -> ProteoformAssayQualityProfile:
        opaque_proteoform_quality_identifier("profile", self.profile_id)
        if (self.role is ProteoformRawInputRole.MASS_SPECTROMETRY_PROTEOME) != (
            self.applicability is not None
        ):
            raise ValueError("applicability is required only for the proteome role")
        codes = tuple(item.metric_code for item in self.thresholds)
        if len(codes) != len(set(codes)) or set(codes) != set(ProteoformQualityMetricCode):
            raise ValueError("quality profile requires every metric exactly once")
        return self


class ProteoformQualityPolicy(FrozenModel):
    policy_id: Identifier
    version: SemanticVersion
    max_count: int = Field(gt=0, le=M0404_MAX_COUNT)
    profiles: tuple[ProteoformAssayQualityProfile, ...] = Field(
        min_length=M0404_MIN_PROFILES, max_length=M0404_MAX_PROFILES
    )
    ...
    def profiles_are_canonical(
        cls, values: tuple[ProteoformAssayQualityProfile, ...]
    ) -> tuple[ProteoformAssayQualityProfile, ...]:
        return tuple(sorted(values, key=canonical_json_bytes))
    ...
    @model_validator(mode="after")
    def profiles_are_total_and_disjoint(self) -> ProteoformQualityPolicy:
        opaque_proteoform_quality_identifier("policy", self.policy_id)
        opaque_proteoform_quality_identifier("reviewer", self.reviewed_by)
        ...
        if {item.role for item in self.profiles} != set(ProteoformRawInputRole):
        ...
        for index, left in enumerate(self.profiles):
            for right in self.profiles[index + 1 :]:
                overlap = (
                    ...
                    and left.applicability is right.applicability
                    ...
                    and bool(
                        set(left.approved_unit_definition_versions)
                        & set(right.approved_unit_definition_versions)
                    )
                )
                if overlap:
...
class ProteoformQualityRoleCounts(FrozenModel):
    declared_record_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    parsed_record_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    valid_record_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    expected_feature_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    observed_feature_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    reference_eligible_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    reference_mapped_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    detection_eligible_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    above_detection_limit_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    below_detection_limit_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    control_expected_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    control_recovered_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    context_applicable_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    context_coherent_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    cross_input_applicable_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    cross_input_coherent_count: int = Field(ge=0, le=M0404_MAX_COUNT)

    @model_validator(mode="after")
    def count_partitions_close(self) -> ProteoformQualityRoleCounts:
        if not (
...
class ProteoformQualityRoleFactStates(FrozenModel):
    raw_input_completeness: ProteoformQualityObservationState
    valid_record_coverage: ProteoformQualityObservationState
    assay_feature_coverage: ProteoformQualityObservationState
    reference_mapping_coverage: ProteoformQualityObservationState
    detection_limit_burden: ProteoformQualityObservationState
    control_material_recovery: ProteoformQualityObservationState
    sample_context_binding_coherence: ProteoformQualityObservationState
    cross_input_consistency: ProteoformQualityObservationState
...
_COUNT_FIELDS_BY_METRIC: Final = {
    ...
    ProteoformQualityMetricCode.VALID_RECORD_COVERAGE: (
        "valid_record_count",
        "parsed_record_count",
    ),
    ProteoformQualityMetricCode.ASSAY_FEATURE_COVERAGE: (
        "observed_feature_count",
        "expected_feature_count",
    ),
    ProteoformQualityMetricCode.REFERENCE_MAPPING_COVERAGE: (
    ...
    ProteoformQualityMetricCode.CONTROL_MATERIAL_RECOVERY: (
...
class ProteoformQualityRoleFacts(FrozenModel):
    fact_id: Identifier
    role: ProteoformRawInputRole
    input_id: Identifier
    validated_input_digest: Sha256Digest
    document_digest: Sha256Digest
    counts: ProteoformQualityRoleCounts
    states: ProteoformQualityRoleFactStates
    evidence: ArtifactReference
    ...
    @model_validator(mode="after")
    def fact_shape_is_closed(self) -> ProteoformQualityRoleFacts:
        opaque_proteoform_quality_identifier("fact", self.fact_id)
        opaque_proteoform_raw_input_identifier("input", self.input_id)
        states = {
            code: cast("ProteoformQualityObservationState", getattr(self.states, code.value))
            for code in ProteoformQualityMetricCode
        }
        for code, state in states.items():
            if state is ProteoformQualityObservationState.CENSORED and (
                code is not ProteoformQualityMetricCode.DETECTION_LIMIT_BURDEN
            ):
        ...
        detection_censored = (
            states[ProteoformQualityMetricCode.DETECTION_LIMIT_BURDEN]
            is ProteoformQualityObservationState.CENSORED
        )
...
class ProteoformQualityFactLedger(FrozenModel):
    ledger_id: Identifier
    version: SemanticVersion
    raw_input_result_digest: Sha256Digest
    raw_input_receipt_digest: Sha256Digest
    role_facts: tuple[ProteoformQualityRoleFacts, ...] = Field(
        min_length=M0404_ROLE_COUNT, max_length=M0404_ROLE_COUNT
    )
    evidence: ArtifactReference
    ...
    def role_facts_are_canonical(
        cls, values: tuple[ProteoformQualityRoleFacts, ...]
    ) -> tuple[ProteoformQualityRoleFacts, ...]:
        return tuple(sorted(values, key=canonical_json_bytes))
    ...
    @model_validator(mode="after")
    def ledger_is_closed(self) -> ProteoformQualityFactLedger:
        opaque_proteoform_quality_identifier("ledger", self.ledger_id)
        ...
        if self.ledger_digest in {_M0404_ZERO_DIGEST, M0403_ZERO_DIGEST}:
...
class ComputeProteoformQualityMetricsRequest(FrozenModel):
    operation: Literal["compute_proteoform_quality_metrics"] = M0404_OPERATION
    contract_version: Literal["1.0.0"] = M0404_CONTRACT_VERSION
    request_id: Identifier
    context: ExecutionContext
    raw_input_result: ProteoformRawInputValidationResult
    policy: ProteoformQualityPolicy
    fact_ledger: ProteoformQualityFactLedger | None = None
    ...
    def raw_input_result_is_fully_replayed(
        ...
        if type(value) is ProteoformRawInputValidationResult:
            ...
            return cast("ProteoformRawInputValidationResult", handler(value))
        if _M0404_ZERO_DIGEST in (result_digest, receipt_digest_value) or M0403_ZERO_DIGEST in (
            result_digest,
            receipt_digest_value,
        ):
            raise ValueError("embedded M04-03 derived digests must be final")
        parsed = (
            value
            if type(value) is ProteoformRawInputValidationResult
            else ProteoformRawInputValidationResult.model_validate_json(
                _raw_input_semantic_bytes(value), strict=True
            )
        )
        return ProteoformRawInputValidationResult.model_validate_json(
            canonical_json_bytes(normalized_m0403_result(parsed)), strict=True
        )

    @model_validator(mode="after")
    def request_is_closed(self) -> ComputeProteoformQualityMetricsRequest:
        opaque_proteoform_quality_identifier("request", self.request_id)
        opaque_proteoform_quality_identifier("request", self.context.request_id)
        opaque_proteoform_quality_identifier("actor", self.context.actor_id)
        ...
        if upstream.disposition is ProteoformRawInputDisposition.VALIDATED:
            if self.fact_ledger is None:
                raise ValueError("validated M04-03 input requires a fact ledger")
        elif self.fact_ledger is not None:
            raise ValueError("nonvalidated M04-03 input prohibits fact-ledger traversal")
        ...
        if upstream.completed_at > self.context.occurred_at:
            raise ValueError("M04-03 result cannot postdate quality computation")
        if self.fact_ledger is not None and not (
            upstream.completed_at <= self.fact_ledger.recorded_at <= self.context.occurred_at
        ):
            raise ValueError("fact ledger chronology must follow M04-03 and precede computation")
        ...
        if (
            ...
            raise ValueError("proteoform quality computation is not authorized")
        if (
            ...
            raise ValueError("M04-04 context does not bind the exact upstream result and policy")
        controls = (
            refs.approved_configuration,
        ...
        for control in controls:
            opaque_proteoform_quality_identifier("decision", control.decision_id)
            _owned_artifact(control.evidence, _CONTROL_MEDIA_TYPE)
        quality_evidence_index(self)
        if len(canonical_json_bytes(normalized_request(self))) > M0404_MAX_CANONICAL_REQUEST_BYTES:
            raise ValueError("canonical M04-04 request exceeds the 4 MiB ingress bound")
        return self


class ProteoformQualityMetricProvenance(FrozenModel):
    raw_input_result_digest: Sha256Digest
    raw_input_receipt_digest: Sha256Digest
...
class ProteoformQualityMetric(FrozenModel):
    role: ProteoformRawInputRole
    metric_code: ProteoformQualityMetricCode
    observation_state: ProteoformQualityObservationState
    status: ProteoformQualityMetricStatus
    required: bool
    numerator: int | None = Field(default=None, ge=0, le=M0404_MAX_COUNT)
    denominator: int | None = Field(default=None, ge=0, le=M0404_MAX_COUNT)
    value_ppm: int | None = Field(default=None, ge=0, le=M0404_RATE_SCALE)
    unit: Literal["ppm_fraction"] = "ppm_fraction"
    censored_count: int = Field(ge=0, le=M0404_MAX_COUNT)
    provenance: ProteoformQualityMetricProvenance

    @model_validator(mode="after")
    def value_shape_is_closed(self) -> ProteoformQualityMetric:
        ...
        if self.denominator == 0:
            if (
                ...
                or self.status is not ProteoformQualityMetricStatus.NOT_EVALUABLE
            ):
                raise ValueError("zero-denominator quality metric must remain not evaluable")
        else:
            expected_value = (
                self.numerator * M0404_RATE_SCALE + self.denominator // 2
            ) // self.denominator
            if self.value_ppm != expected_value:
                raise ValueError("quality metric value must use exact round-half-up integer ppm")
        if (self.observation_state is ProteoformQualityObservationState.CENSORED) != (
            self.censored_count > 0
        ):
            raise ValueError("censored state must exactly retain a positive censored count")
        if self.observation_state is ProteoformQualityObservationState.CENSORED and (
            self.metric_code is not ProteoformQualityMetricCode.DETECTION_LIMIT_BURDEN
...
class ProteoformQualityFinding(FrozenModel):
    finding_id: Identifier
    code: ProteoformQualityFindingCode
    action: ProteoformQualityFindingAction
    roles: tuple[ProteoformRawInputRole, ...] = Field(default=(), max_length=M0404_ROLE_COUNT)
    metric_codes: tuple[ProteoformQualityMetricCode, ...] = Field(
        default=(), max_length=M0404_METRIC_COUNT
    )
    message: NonEmptyStr

    @model_validator(mode="after")
    def finding_is_closed(self) -> ProteoformQualityFinding:
        ...
        if self != finding_for(self.code, self.roles, self.metric_codes):
            raise ValueError("M04-04 finding contradicts its closed vocabulary")
        return self


class ProteoformAssayQualityResult(FrozenModel):
    role: ProteoformRawInputRole
    input_id: Identifier
    validated_input_digest: Sha256Digest
    document_digest: Sha256Digest
    ...
    profile_digest: Sha256Digest
    role_fact_digest: Sha256Digest
    metrics: tuple[ProteoformQualityMetric, ...] = Field(
        min_length=M0404_METRIC_COUNT, max_length=M0404_METRIC_COUNT
    )
    finding_codes: tuple[ProteoformQualityFindingCode, ...] = Field(
        default=(), max_length=M0404_MAX_FINDINGS
    )
    ...
    def metrics_are_canonical(
        cls, values: tuple[ProteoformQualityMetric, ...]
    ) -> tuple[ProteoformQualityMetric, ...]:
        return tuple(sorted(values, key=canonical_json_bytes))

    @field_validator("finding_codes")
    @classmethod
    def finding_codes_are_canonical(
        cls, values: tuple[ProteoformQualityFindingCode, ...]
    ) -> tuple[ProteoformQualityFindingCode, ...]:
        if len(values) != len(set(values)):
            raise ValueError("assay quality finding codes must be unique")
        return tuple(sorted(values))

    @model_validator(mode="after")
    def assay_quality_is_closed(self) -> ProteoformAssayQualityResult:
        opaque_proteoform_raw_input_identifier("input", self.input_id)
        opaque_proteoform_quality_identifier("profile", self.profile_id)
...
class ProteoformQualityComputationReceipt(FrozenModel):
    raw_input_result_digest: Sha256Digest
    raw_input_receipt_digest: Sha256Digest
    ...
    selected_profile_digests: tuple[Sha256Digest, ...] = Field(
        default=(), max_length=M0404_ROLE_COUNT
    )
    assay_quality_digests: tuple[Sha256Digest, ...] = Field(default=(), max_length=M0404_ROLE_COUNT)
    finding_codes: tuple[ProteoformQualityFindingCode, ...] = Field(
        default=(), max_length=M0404_MAX_FINDINGS
    )
    parent_target: Literal["protein_rna_discordance"] = M0404_PARENT
    ...
    @model_validator(mode="after")
    def receipt_is_closed(self) -> ProteoformQualityComputationReceipt:
        expected_count = M0404_ROLE_COUNT if self.fact_ledger_digest is not None else 0
        ...
        if self.receipt_digest != receipt_digest(self):
            raise ValueError("M04-04 receipt digest does not match its canonical content")
        if self.disposition is not _disposition_for_finding_codes(self.finding_codes):
...
class ProteoformQualityResult(FrozenModel):
    ...
    assay_quality: tuple[ProteoformAssayQualityResult, ...] = Field(
        default=(), max_length=M0404_ROLE_COUNT
    )
    findings: tuple[ProteoformQualityFinding, ...] = Field(
        default=(), max_length=M0404_MAX_FINDINGS
    )
    disposition: ProteoformQualityDisposition
    parent_target: Literal["protein_rna_discordance"] = M0404_PARENT
    ...
    evidence: tuple[EvidenceReference, ...] = Field(
        min_length=M0404_MIN_EVIDENCE, max_length=M0404_MAX_EVIDENCE
    )
    limitations: tuple[Limitation, ...] = Field(
        min_length=M0404_LIMITATION_COUNT, max_length=M0404_LIMITATION_COUNT
    )
    ...
    def request_may_reuse_sealed_validation(
        ...
        if info.mode == "json":
            return TypeAdapter(ComputeProteoformQualityMetricsRequest).validate_json(
                canonical_json_bytes(value), strict=True
            )
        return cast("ComputeProteoformQualityMetricsRequest", handler(value))
    ...
    @model_validator(mode="after")
    def result_is_exact_replay(self, info: ValidationInfo) -> ProteoformQualityResult:
        capability = (
            info.context.get(_REQUEST_CAPABILITY_CONTEXT_KEY)
            if isinstance(info.context, dict)
            else None
...
def _issue_raw_input_replay_capability(candidate: object) -> _RawInputReplayCapability:
    ...
    if type(raw_value) is not ProteoformRawInputValidationResult and dict not in raw_value_mro:
        raise TypeError("M04-04 raw-input replay requires an exact result or built-in dict family")
    adapter = TypeAdapter(ProteoformRawInputValidationResult)
    if type(raw_value) is ProteoformRawInputValidationResult:
        replayed = adapter.validate_python(raw_value, strict=True)
        normalized_bytes = canonical_json_bytes(normalized_m0403_result(replayed))
    else:
...
def _validate_request_with_raw_capability(
    ...
    if (
        ...
        raise TypeError("invalid or mismatched M04-04 raw-input replay capability")
    context = {_RAW_CAPABILITY_CONTEXT_KEY: raw_capability}
    adapter = TypeAdapter(ComputeProteoformQualityMetricsRequest)
...
def _validate_result_with_capability(
    ...
    if capability.seal is not _VALIDATION_CAPABILITY_SEAL or not _request_capability_is_issued(
        ...
        raise TypeError("invalid M04-04 request-validation capability")
    return TypeAdapter(ProteoformQualityResult).validate_python(
        value,
        strict=True,
...
def _disposition_for_finding_codes(
    codes: tuple[ProteoformQualityFindingCode, ...],
) -> ProteoformQualityDisposition:
    actions = {_ACTION_BY_FINDING[code] for code in codes}
    if ProteoformQualityFindingAction.QUARANTINE in actions:
        return ProteoformQualityDisposition.QUARANTINED
    if ProteoformQualityFindingAction.ABSTAIN in actions:
        return ProteoformQualityDisposition.ABSTAINED
    return ProteoformQualityDisposition.QUALIFIED


def finding_for(
    code: ProteoformQualityFindingCode,
    roles: tuple[ProteoformRawInputRole, ...] = (),
    metric_codes: tuple[ProteoformQualityMetricCode, ...] = (),
    ...
    return ProteoformQualityFinding.model_construct(
        finding_id=(f"finding.m0404.{code.value}.{digest.removeprefix('sha256:')[:16]}"),
        code=code,
...
def _validated_input_by_role(
    request: ComputeProteoformQualityMetricsRequest,
) -> dict[ProteoformRawInputRole, object]:
    return {item.role: item for item in request.raw_input_result.validated_inputs}


def _ledger_bindings_close(request: ComputeProteoformQualityMetricsRequest) -> bool:
    from glio_proteogen.contracts.m04_03 import validated_input_digest  # noqa: PLC0415
    ...
    if set(inputs) != set(ProteoformRawInputRole):
        return False
...
def _profile_candidates(
    ...
    source = inputs[role]
    applicability = cast(
        "ProteoformApplicability | None",
        getattr(source.document, "applicability", None),
    )
    ...
    return tuple(
        ...
        and profile.applicability is applicability
...
def matching_quality_profiles(
    ...
    if (
        request.raw_input_result.disposition is not ProteoformRawInputDisposition.VALIDATED
        or not _ledger_bindings_close(request)
    ):
        return ()
    inputs = {item.role: item for item in request.raw_input_result.validated_inputs}
    selected: list[ProteoformAssayQualityProfile] = []
    for role in ProteoformRawInputRole:
        ...
        matches = tuple(
            ...
            and source.document.unit_definition_version in profile.approved_unit_definition_versions
        )
        if len(matches) != 1:
            return ()
...
def _status_for_ratio(  # noqa: PLR0911 - direct closed threshold matrix.
    ...
    if denominator == 0:
        return ProteoformQualityMetricStatus.NOT_EVALUABLE
    value_ppm = (numerator * M0404_RATE_SCALE + denominator // 2) // denominator
    if threshold.direction is ProteoformQualityMetricDirection.AT_LEAST:
        if value_ppm >= threshold.pass_threshold_ppm:
            return ProteoformQualityMetricStatus.PASS
        if value_ppm >= threshold.warning_threshold_ppm:
            return ProteoformQualityMetricStatus.WARNING
        return ProteoformQualityMetricStatus.FAIL
    if value_ppm <= threshold.pass_threshold_ppm:
        return ProteoformQualityMetricStatus.PASS
    if value_ppm <= threshold.warning_threshold_ppm:
        return ProteoformQualityMetricStatus.WARNING
...
def _derive_quality_metrics(
    ...
    ledger = request.fact_ledger
    if ledger is None or len(profiles) != M0404_ROLE_COUNT or not _ledger_bindings_close(request):
        return ()
    profile_by_role = {item.role: item for item in profiles}
    facts_by_role = {item.role: item for item in ledger.role_facts}
    metrics: list[ProteoformQualityMetric] = []
    ...
    for role in ProteoformRawInputRole:
        ...
        for code in ProteoformQualityMetricCode:
            ...
            state = cast("ProteoformQualityObservationState", getattr(fact.states, code.value))
            numerator_field, denominator_field = _COUNT_FIELDS_BY_METRIC[code]
            ...
            value_ppm = (
                (raw_numerator * M0404_RATE_SCALE + raw_denominator // 2) // raw_denominator
            ...
            metrics.append(
                ProteoformQualityMetric(
                    ...
                    censored_count=(
                        ...
                        if state is ProteoformQualityObservationState.CENSORED
                        else 0
                    ),
                    provenance=ProteoformQualityMetricProvenance(
...
def _version_mismatch_findings(
    request: ComputeProteoformQualityMetricsRequest,
) -> tuple[ProteoformQualityFinding, ...]:
    if request.fact_ledger is None or not _ledger_bindings_close(request):
        return ()
    inputs = {item.role: item for item in request.raw_input_result.validated_inputs}
    findings: list[ProteoformQualityFinding] = []
    for role in ProteoformRawInputRole:
...
def _derive_quality_findings(  # noqa: PLR0912 - explicit closed finding matrix.
    request: ComputeProteoformQualityMetricsRequest,
    metrics: tuple[ProteoformQualityMetric, ...],
    profiles: tuple[ProteoformAssayQualityProfile, ...],
) -> tuple[ProteoformQualityFinding, ...]:
    upstream = request.raw_input_result.disposition
    if upstream is ProteoformRawInputDisposition.QUARANTINED:
        return (finding_for(ProteoformQualityFindingCode.UPSTREAM_RAW_INPUTS_QUARANTINED),)
    if upstream is ProteoformRawInputDisposition.ABSTAINED:
        return (finding_for(ProteoformQualityFindingCode.UPSTREAM_RAW_INPUTS_ABSTAINED),)
    if not _ledger_bindings_close(request):
        return (finding_for(ProteoformQualityFindingCode.FACT_LEDGER_BINDING_MISMATCH),)
    if not profiles:
        versions = _version_mismatch_findings(request)
        return versions or (finding_for(ProteoformQualityFindingCode.ASSAY_PROFILE_UNSUPPORTED),)
    findings: list[ProteoformQualityFinding] = []
    ...
    for metric in metrics:
        code: ProteoformQualityFindingCode | None = None
        if (
            metric.required
            and metric.observation_state is ProteoformQualityObservationState.MISSING
        ):
            code = ProteoformQualityFindingCode.REQUIRED_METRIC_MISSING
        elif (
            metric.required
            and metric.observation_state is ProteoformQualityObservationState.UNSUPPORTED
        ):
            code = ProteoformQualityFindingCode.REQUIRED_METRIC_UNSUPPORTED
        elif metric.required and metric.status in {
            ProteoformQualityMetricStatus.NOT_EVALUABLE,
            ProteoformQualityMetricStatus.NOT_APPLICABLE,
        }:
            code = ProteoformQualityFindingCode.REQUIRED_METRIC_NOT_EVALUABLE
        elif metric.required and metric.status is ProteoformQualityMetricStatus.WARNING:
            code = ProteoformQualityFindingCode.REQUIRED_METRIC_WARNING
        elif metric.status is ProteoformQualityMetricStatus.FAIL:
            code = ProteoformQualityFindingCode.METRIC_THRESHOLD_FAILED
        elif not metric.required and metric.status is ProteoformQualityMetricStatus.WARNING:
            code = ProteoformQualityFindingCode.OPTIONAL_METRIC_WARNING
        if code is not None:
            findings.append(
                finding_for(
                    code,
...
def _derive_disposition(
    findings: tuple[ProteoformQualityFinding, ...] = (),
) -> ProteoformQualityDisposition:
...
def _derive_assay_quality(
    ...
    findings: tuple[ProteoformQualityFinding, ...],
    profiles: tuple[ProteoformAssayQualityProfile, ...],
) -> tuple[ProteoformAssayQualityResult, ...]:
    ledger = request.fact_ledger
    if (
        ledger is None
        or len(metrics) != M0404_COMPUTED_METRIC_COUNT
        or len(profiles) != M0404_ROLE_COUNT
    ):
        return ()
    by_role = {item.role: item for item in request.raw_input_result.validated_inputs}
    facts = {item.role: item for item in ledger.role_facts}
    profile_by_role = {item.role: item for item in profiles}
    values: list[ProteoformAssayQualityResult] = []
    for role in ProteoformRawInputRole:
...
def _derive_receipt(
    request: ComputeProteoformQualityMetricsRequest,
    assay_quality: tuple[ProteoformAssayQualityResult, ...],
    findings: tuple[ProteoformQualityFinding, ...],
    disposition: ProteoformQualityDisposition,
    profiles: tuple[ProteoformAssayQualityProfile, ...],
    ...
    payload: dict[str, object] = {
        ...
        "receipt_digest": _M0404_ZERO_DIGEST,
    }
    payload["receipt_digest"] = receipt_digest(payload)
    return ProteoformQualityComputationReceipt.model_validate(payload, strict=True)
...
def expected_quality_metrics(
    request: ComputeProteoformQualityMetricsRequest,
    profiles: tuple[ProteoformAssayQualityProfile, ...] | None = None,
) -> tuple[ProteoformQualityMetric, ...]:
    ...
    if profiles is not None and _canonical_region(profiles) != _canonical_region(bundle.profiles):
        raise ValueError("supplied M04-04 profiles do not match exact profile selection")
...
def expected_quality_findings(
    ...
    if metrics and _canonical_region(metrics) != _canonical_region(bundle.metrics):
        raise ValueError("supplied M04-04 metrics do not match exact metric projection")
...
def expected_disposition(
    ...
    if findings and _canonical_region(findings) != _canonical_region(bundle.findings):
        raise ValueError("supplied M04-04 findings do not match exact finding projection")
...
def expected_receipt(
    ...
    if _canonical_region(assay_quality) != _canonical_region(bundle.assay_quality):
        raise ValueError("supplied M04-04 assay quality does not match exact projection")
    if _canonical_region(findings) != _canonical_region(bundle.findings):
        raise ValueError("supplied M04-04 findings do not match exact finding projection")
    if disposition is not bundle.disposition:
        raise ValueError("supplied M04-04 disposition does not match exact precedence")
...
def _derive_support(
    ...
    if disposition is ProteoformQualityDisposition.QUALIFIED and optional_warning:
        return SupportDecision(
            status=SupportStatus.LIMITED,
            reason_code="proteoform_quality_qualified_with_optional_warning",
            rationale="Required metrics passed; an optional warning limits support.",
        )
    if disposition is ProteoformQualityDisposition.QUALIFIED:
        return SupportDecision(
            status=SupportStatus.SUPPORTED,
            reason_code="proteoform_quality_qualified",
            rationale="All required aggregate proteoform quality metrics passed.",
        )
    if disposition is ProteoformQualityDisposition.QUARANTINED:
        return SupportDecision(
            status=SupportStatus.REVIEW_REQUIRED,
            reason_code="proteoform_quality_quarantined",
            rationale="A binding, version, consistency, or threshold outcome requires review.",
        )
    return SupportDecision(
        status=SupportStatus.UNSUPPORTED,
        reason_code="proteoform_quality_abstained",
        rationale="Upstream evidence or a required quality dimension is not evaluable.",
    )


def expected_support(request: ComputeProteoformQualityMetricsRequest) -> SupportDecision:
...
def expected_uncertainty() -> UncertaintyProfile:
    rationales = (
        ...
        "No protein, proteoform, or isoform identification is performed.",
...
def quality_evidence_index(
    ...
    if not M0404_MIN_EVIDENCE <= len(artifacts) <= M0404_MAX_EVIDENCE:
        raise ValueError("M04-04 evidence index exceeds its exact installed shape")
    _require_consistent_evidence_identities(artifacts)
    return tuple(
        sorted(
            (
                EvidenceReference(reference=item, role="evidence", claim=M0404_EVIDENCE_CLAIM)
...
def _derive_provenance(  # noqa: PLR0913 - optional sealed digest cache is explicit.
    ...
    *,
    ...
    return ProvenanceRecord(
        activity_id=f"activity.m0404.{request_hash.removeprefix('sha256:')}",
        actor_id=request.context.actor_id,
        module_id=M0404_MODULE_ID,
        module_version=M0404_CONTRACT_VERSION,
...
def expected_provenance(
    request: ComputeProteoformQualityMetricsRequest,
    metrics: tuple[ProteoformQualityMetric, ...] = (),
    receipt: ProteoformQualityComputationReceipt | None = None,
) -> ProvenanceRecord:
    ...
    if receipt is not None and receipt != bundle.receipt:
        raise ValueError("supplied M04-04 receipt does not match exact receipt projection")
    ...
    return tuple(
        sorted(
            (
                ...
                Limitation(
                    code="no_proteoform_discordance_or_clinical_inference",
                    statement=(
                        "No proteoform, discordance, treatment, or clinical inference is produced."
                    ),
                ),
            ),
            key=canonical_json_bytes,
...
def _validate_result_replay(
    ...
    expected_result_id = f"result.m0404.{request_hash.removeprefix('sha256:')}"
    if (
        ...
        raise ValueError("M04-04 result envelope does not replay from its full request")
    if self.completed_at != self.request.context.occurred_at:
        raise ValueError("M04-04 completion time must equal execution time")
    if self.support != bundle.support:
        raise ValueError("M04-04 support does not replay")
    if canonical_json_bytes(_normalized_model(self.uncertainty)) != canonical_json_bytes(
        _normalized_model(expected_uncertainty())
    ):
        raise ValueError("M04-04 uncertainty does not replay")
    if canonical_json_bytes(_normalized_model(self.provenance)) != canonical_json_bytes(
        ...
        raise ValueError("M04-04 provenance does not replay")
    if tuple(sorted(self.evidence, key=canonical_json_bytes)) != quality_evidence_index(
        self.request
    ):
        raise ValueError("M04-04 evidence index does not replay")
    if tuple(sorted(self.limitations, key=canonical_json_bytes)) != expected_limitations():
        raise ValueError("M04-04 limitations do not replay")
    optional_warning = any(
        item.status is ProteoformQualityMetricStatus.WARNING and not item.required
        for item in replay_metrics
    )
    if self.human_review_required != (
        replay_disposition is not ProteoformQualityDisposition.QUALIFIED or optional_warning
    ):
...
__all__ = [name for name in globals() if name.startswith("M0404_")] + [
    "ComputeProteoformQualityMetricsRequest",
