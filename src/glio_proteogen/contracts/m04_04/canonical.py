"""Semantic canonicalization for M04-04 proteoform quality computation."""
...
def _python(value: Any) -> Any:  # noqa: ANN401 - recursive canonical JSON shape.
    ...
    if dict in value_mro:
        ...
        if any(type(key) is not str for key in keys):
            raise TypeError("canonical M04-04 object keys must be exact strings")
...
def configuration_digest(value: BaseModel | dict[str, Any]) -> Sha256Digest:
    return sha256_digest({"proteoform_quality_policy": normalized_policy(value)})
...
def normalized_raw_input_result(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    return normalized_m0403_result(_dump(value))
