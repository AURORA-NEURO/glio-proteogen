...
M1204_MAX_OBSERVATIONS: Final = 512
M1204_MAX_RELATIONS: Final = 1024
M1204_DEFAULT_BOOTSTRAP_REPLICATES: Final = 64
M1204_MAX_BOOTSTRAP_REPLICATES: Final = 256
M1204_GLIOMA_MODEL_FAMILY: Final = "glioma-panel-mechanism-evidence-graph/1.0.0"
...
class MechanismObservation(FrozenModel):
    ...
    def observation_shape_is_closed(self) -> MechanismObservation:
        ...
        if (
            self.state in {MechanismObservationState.MISSING, MechanismObservationState.UNSUPPORTED}
            and measured
        ):
...
class MechanismInferenceConfiguration(FrozenModel):
    ...
    bootstrap_replicates: int = Field(
        default=M1204_DEFAULT_BOOTSTRAP_REPLICATES,
        ge=16,
        le=M1204_MAX_BOOTSTRAP_REPLICATES,
    )
...
class InferBiomarkerPanelMechanismRequest(FrozenModel):
    ...
    typed_observations: tuple[MechanismObservation, ...] = Field(
        default=(), max_length=M1204_MAX_OBSERVATIONS
    )
    typed_relations: tuple[MechanismRelation, ...] = Field(
        default=(), max_length=M1204_MAX_RELATIONS
    )
...
__all__ = [
    ...
    "M1204_DEFAULT_BOOTSTRAP_REPLICATES",
