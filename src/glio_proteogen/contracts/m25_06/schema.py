"""JSON Schema 2020-12 exports for provisional M25-06 contracts."""
...
_CONTRACTS: Final = {
    ...
    "output": ProteotypeRobustnessChallengeResult,
...
def contract_json_schema(name: ContractName) -> dict[str, object]:
    """Return one strict, metadata-only provisional M25-06 schema."""

    schema = TypeAdapter(_CONTRACTS[name]).json_schema(mode="validation")
    schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    schema["$id"] = f"{SCHEMA_ID_PREFIX}:{name}"
    schema["x-glio-contract"] = {
        "moduleId": M2506_MODULE_ID,
        "authoritySha256": M2506_DOSSIER_SHA256,
        "authoritySlice": M2506_DOSSIER_SLICE,
        "contractVersion": CONTRACT_VERSION,
        "owner": M2506_OWNER,
        "safetyClass": M2506_SAFETY_CLASS,
        "gate": M2506_GATE,
        "strict": True,
        "provisionalAbi": M2506_PROVISIONAL_ABI,
    ...
    if name == "request":
        schema["x-glio-contract"]["maxRequestBytes"] = M2506_MAX_CANONICAL_REQUEST_BYTES
    return cast("dict[str, object]", schema)


def contract_json_schemas() -> dict[ContractName, dict[str, object]]:
    """Return all eight provisional M25-06 schemas in ABI order."""
