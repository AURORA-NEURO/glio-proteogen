...
M1303_EVIDENCE_CLAIM: Final = (
    "Caller-declared M13-03 mechanistic feature evidence; issuer authority "
...
class MechanisticFeatureObject(FrozenModel):
    ...
    features: tuple[MechanisticFeature, ...] = Field(
        min_length=1, max_length=M1303_MAX_FEATURES
    )
    relations: tuple[MechanisticRelation, ...] = Field(
        default=(), max_length=M1303_MAX_RELATIONS
    )
...
class ProteotypeMechanisticFeatureResult(FrozenModel):
    ...
    findings: tuple[MechanisticFindingCode, ...] = Field(
        default=(), max_length=M1303_MAX_FINDINGS
    )
