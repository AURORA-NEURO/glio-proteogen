"""Semantic canonicalization for M03-07 protein-inference support routing."""
...
from glio_proteogen.contracts.m03_04 import normalized_result as normalized_m0304_result
from glio_proteogen.contracts.m03_06 import normalized_result as normalized_m0306_result
...
def _dump(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):  # pragma: no cover - contract roots are objects.
        raise TypeError("M03-07 canonical object root must be a mapping")
    return decoded
...
def normalized_harmonization_support_receipt(
    ...
    data = _dump(value)
    data["platform_level_ids"] = tuple(sorted(data["platform_level_ids"]))
    return data
...
def normalized_prerequisites(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    ...
    data["quality_result"] = normalized_m0304_result(data["quality_result"])
    data["quality_result"]["result_digest"] = quality_result_digest
    data["harmonization_result"] = normalized_m0306_result(data["harmonization_result"])
...
def configuration_digest(
    ...
    return sha256_digest(
        {
            "protein_inference_support_profile": normalized_profile(profile),
            "protein_inference_support_policy": normalized_policy(policy),
        }
    )
