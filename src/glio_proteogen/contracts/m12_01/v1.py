"""Provisional M12-01 biological hypothesis registry contracts.

The M12-01 dossier requires versioned hypotheses, competing explanations,
...
the ABI below is intentionally provisional pending ML engineering owner
confirmation.
"""
...
from glio_proteogen.contracts.m12_01.canonical import (
...
# PROVISIONAL ABI: inferred solely from the M12-01 dossier slice.
M1201_MODULE_ID: Final = "GLIO-PROTEOGEN-M12-01"
M1201_OPERATION: Final = "register_biomarker_panel_hypotheses"
M1201_CONTRACT_VERSION: Final = "0.1.0-provisional"
M1201_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m12-01+json"
M1201_PARENT: Final = "biomarker_panel"
M1201_OWNER: Final = "ML engineering"
M1201_SAFETY_CLASS: Final = "S2"
M1201_GATE: Final = "G0"
M1201_PROVISIONAL_ABI: Final = True
M1201_MAX_HYPOTHESES: Final = 256
M1201_MAX_EXPLANATIONS: Final = 16
M1201_MAX_RULES: Final = 16
M1201_MAX_EVIDENCE_TIERS: Final = 8
M1201_MAX_EVALUATIONS: Final = M1201_MAX_HYPOTHESES
M1201_MAX_FALSIFICATION_EVALUATIONS: Final = M1201_MAX_HYPOTHESES * M1201_MAX_RULES
M1201_MAX_EVIDENCE: Final = 64
M1201_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1201_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1201_EVIDENCE_CLAIM: Final = (
    "Caller-declared M12-01 hypothesis, falsification, and evidence-tier material; "
...
class HypothesisFindingCode(StrEnum):
    ...
    SUPPORT_DOMAIN_FAILURE = "support_domain_failure"
    UNRESOLVED_BIOLOGICAL_CONFLICT = "unresolved_biological_conflict"
...
class CompetingExplanation(FrozenModel):
    ...
    required_evidence: tuple[EvidenceReference, ...] = Field(
        min_length=1, max_length=M1201_MAX_EVIDENCE
    )
...
class EvidenceTier(FrozenModel):
    tier: int = Field(ge=1, le=M1201_MAX_EVIDENCE_TIERS)
    label: NonEmptyStr
    rationale: NonEmptyStr
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1201_MAX_EVIDENCE)
...
class BiologicalHypothesis(FrozenModel):
    ...
    target_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M1201_MAX_HYPOTHESES)
    competing_explanations: tuple[CompetingExplanation, ...] = Field(
        min_length=1, max_length=M1201_MAX_EXPLANATIONS
    )
    falsification_rules: tuple[FalsificationRule, ...] = Field(
        min_length=1, max_length=M1201_MAX_RULES
    )
    evidence_tiers: tuple[EvidenceTier, ...] = Field(
        min_length=1, max_length=M1201_MAX_EVIDENCE_TIERS
    )
    prohibited_interpretations: tuple[NonEmptyStr, ...] = Field(min_length=1, max_length=32)
    status: HypothesisStatus = HypothesisStatus.PROPOSED
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1201_MAX_EVIDENCE)
...
class HypothesisRegistry(FrozenModel):
    ...
    hypotheses: tuple[BiologicalHypothesis, ...] = Field(
        min_length=1, max_length=M1201_MAX_HYPOTHESES
    )
...
class RegisterBiomarkerPanelHypothesesRequest(FrozenModel):
    """Provisional request for registering a complete hypothesis registry."""

    operation: Literal["register_biomarker_panel_hypotheses"] = M1201_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1201_CONTRACT_VERSION
    ...
    @model_validator(mode="after")
    def request_is_closed(self) -> RegisterBiomarkerPanelHypothesesRequest:
        ids = tuple(item.hypothesis_id for item in self.hypotheses)
        if len(ids) != len(set(ids)):
...
class BiomarkerPanelHypothesisRegistryResult(FrozenModel):
    """Versioned registry with complete evaluations and explicit abstention."""

    output_type: Literal["biomarker_panel_hypothesis_registry"] = (
        "biomarker_panel_hypothesis_registry"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1201_CONTRACT_VERSION
    ...
    evaluations: tuple[HypothesisEvaluation, ...] = Field(
        default=(), max_length=M1201_MAX_EVALUATIONS
    )
    falsification_evaluations: tuple[FalsificationEvaluation, ...] = Field(
        default=(), max_length=M1201_MAX_FALSIFICATION_EVALUATIONS
    )
    findings: tuple[HypothesisFinding, ...] = Field(default=(), max_length=M1201_MAX_EVIDENCE)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["biomarker_panel"] = M1201_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> BiomarkerPanelHypothesisRegistryResult:
...
def expected_uncertainty(*, supported: bool) -> UncertaintyProfile:
    """Return all seven uncertainty dimensions, including safe abstention."""

    estimate = UncertaintyEstimate(
        state=EstimateState.ESTIMATED if supported else EstimateState.NOT_ESTIMABLE,
        probability=0.9 if supported else None,
        rationale=(
            "Caller-declared evidence tiers and falsification outcomes are explicit; "
            "no population calibration claim is made."
            if supported
            else "A quality, support, or hypothesis gate is not safely evaluable."
        ),
    )
...
def expected_provenance(
    ...
    """Project all seven caller-declared controls without traversing payloads."""
    ...
    return ProvenanceRecord(
        ...
        module_id=M1201_MODULE_ID,
        module_version=M1201_CONTRACT_VERSION,
...
__all__ = [
    ...
    "M1201_EVIDENCE_CLAIM",
    "M1201_GATE",
    "M1201_MAX_CANONICAL_REQUEST_BYTES",
    "M1201_MAX_CANONICAL_RESULT_BYTES",
    "M1201_MAX_EVIDENCE",
    "M1201_MAX_EVIDENCE_TIERS",
    "M1201_MAX_EXPLANATIONS",
    "M1201_MAX_FALSIFICATION_EVALUATIONS",
    "M1201_MAX_HYPOTHESES",
    "M1201_MAX_RULES",
    "M1201_MODULE_ID",
    "M1201_OPERATION",
