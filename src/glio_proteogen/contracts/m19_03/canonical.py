"""Canonical projections for the provisional M19-03 contract spine."""
...
def canonical_request_bytes(value: BaseModel | dict[str, Any]) -> bytes:
    """Return the exact replay bytes for a request, without lossy coercion."""
...
def canonical_result_payload_bytes(value: BaseModel | dict[str, Any]) -> bytes:
    """Return replay bytes after removing only the self-referential digest."""
...
def verify_request_digest(value: BaseModel | dict[str, Any], expected: Sha256Digest | str) -> bool:
    """Compare a caller-supplied request digest to the canonical request."""
...
def verify_result_digest(value: BaseModel | dict[str, Any], expected: Sha256Digest | str) -> bool:
    """Compare a caller-supplied result digest to the canonical payload."""
