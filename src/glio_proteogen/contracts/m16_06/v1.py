"""Provisional M16-06 reviewer discrepancy and adjudication contracts.

M16-06 owns a structured discrepancy queue, blinded review, escalation,
resolution and immutable history beneath the KINOPHOS object consumer.  The
public ABI is provisional pending Quality engineering owner confirmation.
"""
...
# PROVISIONAL ABI: inferred solely from the M16-06 dossier slice.
M1606_MODULE_ID: Final = "GLIO-PROTEOGEN-M16-06"
M1606_DOSSIER_SHA256: Final = (
    "sha256:0a6b200cbe073db13a4bcf315edc23ab97edfe6f500bc7ea2785f5e1c70da181"
)
M1606_DOSSIER_SLICE: Final = "GLIO-PROTEOGEN_240_Module_Dossier.md:5656-5696"
M1606_OPERATION: Final = "adjudicate_protein_rna_discordance_queue"
M1606_CONTRACT_VERSION: Final = "0.1.0-provisional"
M1606_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m16-06+json"
M1606_M1605_INPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m16-05+json"
M1606_PARENT: Final = "protein-RNA discordance"
M1606_OWNER: Final = "Quality engineering"
M1606_SAFETY_CLASS: Final = "S2"
M1606_GATE: Final = "G4"
M1606_PROVISIONAL_ABI: Final = True
M1606_MAX_QUEUE_ENTRIES: Final = 256
M1606_MAX_ASSIGNMENTS: Final = 256
M1606_MAX_HISTORY: Final = 1_024
M1606_MAX_EVIDENCE: Final = 64
M1606_MAX_FINDINGS: Final = 64
M1606_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1606_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1606_EVIDENCE_CLAIM: Final = (
    "Caller-declared M16-06 discrepancy, review, escalation and audit material; "
...
class DiscrepancyQueueEntry(FrozenModel):
    ...
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1606_MAX_EVIDENCE)
...
class ImmutableAuditEvent(FrozenModel):
    ...
    sequence: int = Field(ge=1, le=M1606_MAX_HISTORY)
    event_id: Identifier
...
class AdjudicationRecord(FrozenModel):
    ...
    entries: tuple[DiscrepancyQueueEntry, ...] = Field(
        min_length=1, max_length=M1606_MAX_QUEUE_ENTRIES
    )
    assignments: tuple[ReviewerAssignment, ...] = Field(
        min_length=1, max_length=M1606_MAX_ASSIGNMENTS
    )
    history: tuple[ImmutableAuditEvent, ...] = Field(min_length=1, max_length=M1606_MAX_HISTORY)
...
class ReviewWorkspaceConfiguration(FrozenModel):
    ...
    version: SemanticVersion
    maximum_queue_entries: int = Field(gt=0, le=M1606_MAX_QUEUE_ENTRIES)
    escalation_required_for_critical: Literal[True] = True
    immutable_history_required: Literal[True] = True
    locked: Literal[True] = True
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1606_MAX_EVIDENCE)
...
class AdjudicateProteinRnaDiscordanceQueueRequest(FrozenModel):
    """Provisional request bound to the M16-05 workspace object."""

    operation: Literal["adjudicate_protein_rna_discordance_queue"] = M1606_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1606_CONTRACT_VERSION
    ...
    source_artifacts: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M1606_MAX_EVIDENCE
    )
    supersedes_result_digest: Sha256Digest | None = None

    @model_validator(mode="after")
    def request_is_bound(self) -> AdjudicateProteinRnaDiscordanceQueueRequest:
        if self.upstream_result.media_type != M1606_M1605_INPUT_MEDIA_TYPE:
            raise ValueError("request must bind the provisional M16-05 workspace result")
        entry_ids = tuple(item.discrepancy_id for item in self.entries)
        assignment_ids = tuple(item.assignment_id for item in self.assignments)
        if len(entry_ids) != len(set(entry_ids)):
...
class ProteinRnaDiscordanceAdjudicationResult(FrozenModel):
    """Versioned adjudication record with safe abstention and audit history."""

    output_type: Literal["protein_rna_discordance_adjudication"] = (
        "protein_rna_discordance_adjudication"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1606_CONTRACT_VERSION
    request_digest: Sha256Digest
    result_digest: Sha256Digest
    request: AdjudicateProteinRnaDiscordanceQueueRequest
    status: QueueResultStatus
    record: AdjudicationRecord | None = None
    findings: tuple[QueueFinding, ...] = Field(default=(), max_length=M1606_MAX_FINDINGS)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["protein-RNA discordance"] = M1606_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> ProteinRnaDiscordanceAdjudicationResult:
...
__all__ = [
    ...
    "M1606_MAX_ASSIGNMENTS",
    "M1606_MAX_CANONICAL_REQUEST_BYTES",
    "M1606_MAX_CANONICAL_RESULT_BYTES",
    "M1606_MAX_EVIDENCE",
    "M1606_MAX_FINDINGS",
    "M1606_MAX_HISTORY",
    "M1606_MAX_QUEUE_ENTRIES",
    "M1606_MODULE_ID",
    "M1606_OPERATION",
