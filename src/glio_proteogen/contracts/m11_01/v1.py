"""Provisional M11-01 biological hypothesis registry contracts.

The M11-01 dossier requires versioned hypotheses, competing explanations,
...
the ABI below is intentionally provisional pending Bioinformatics owner
confirmation.
"""
...
from glio_proteogen.contracts.m11_01.canonical import (
...
# PROVISIONAL ABI: inferred solely from the M11-01 dossier slice.
M1101_MODULE_ID: Final = "GLIO-PROTEOGEN-M11-01"
M1101_OPERATION: Final = "register_variant_peptide_hypotheses"
M1101_CONTRACT_VERSION: Final = "0.1.0-provisional"
M1101_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m11-01+json"
M1101_PARENT: Final = "variant_peptide"
M1101_OWNER: Final = "Bioinformatics"
M1101_SAFETY_CLASS: Final = "S2"
M1101_GATE: Final = "G0"
M1101_PROVISIONAL_ABI: Final = True
M1101_MAX_HYPOTHESES: Final = 256
M1101_MAX_EXPLANATIONS: Final = 16
M1101_MAX_RULES: Final = 16
M1101_MAX_EVIDENCE_TIERS: Final = 8
M1101_MAX_EVALUATIONS: Final = M1101_MAX_HYPOTHESES
M1101_MAX_FALSIFICATION_EVALUATIONS: Final = M1101_MAX_HYPOTHESES * M1101_MAX_RULES
M1101_MAX_EVIDENCE: Final = 64
M1101_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1101_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1101_EVIDENCE_CLAIM: Final = (
    "Caller-declared M11-01 hypothesis, falsification, and evidence-tier material; "
...
class CompetingExplanation(FrozenModel):
    ...
    required_evidence: tuple[EvidenceReference, ...] = Field(
        min_length=1, max_length=M1101_MAX_EVIDENCE
    )
...
class EvidenceTier(FrozenModel):
    tier: int = Field(ge=1, le=M1101_MAX_EVIDENCE_TIERS)
    label: NonEmptyStr
    rationale: NonEmptyStr
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1101_MAX_EVIDENCE)
...
class BiologicalHypothesis(FrozenModel):
    ...
    target_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M1101_MAX_HYPOTHESES)
    competing_explanations: tuple[CompetingExplanation, ...] = Field(
        min_length=1, max_length=M1101_MAX_EXPLANATIONS
    )
    falsification_rules: tuple[FalsificationRule, ...] = Field(
        min_length=1, max_length=M1101_MAX_RULES
    )
    evidence_tiers: tuple[EvidenceTier, ...] = Field(
        min_length=1, max_length=M1101_MAX_EVIDENCE_TIERS
    )
    prohibited_interpretations: tuple[NonEmptyStr, ...] = Field(min_length=1, max_length=32)
    status: HypothesisStatus = HypothesisStatus.PROPOSED
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1101_MAX_EVIDENCE)
...
class HypothesisRegistry(FrozenModel):
    ...
    hypotheses: tuple[BiologicalHypothesis, ...] = Field(
        min_length=1, max_length=M1101_MAX_HYPOTHESES
    )
...
class RegisterVariantPeptideHypothesesRequest(FrozenModel):
    """Provisional request for registering a complete hypothesis registry."""

    operation: Literal["register_variant_peptide_hypotheses"] = M1101_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1101_CONTRACT_VERSION
    ...
    @model_validator(mode="after")
    def request_is_closed(self) -> RegisterVariantPeptideHypothesesRequest:
        ids = tuple(item.hypothesis_id for item in self.hypotheses)
        if len(ids) != len(set(ids)):
...
class VariantPeptideHypothesisRegistryResult(FrozenModel):
    """Versioned registry with complete evaluations and explicit abstention."""

    output_type: Literal["variant_peptide_hypothesis_registry"] = (
        "variant_peptide_hypothesis_registry"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1101_CONTRACT_VERSION
    ...
    evaluations: tuple[HypothesisEvaluation, ...] = Field(
        default=(), max_length=M1101_MAX_EVALUATIONS
    )
    falsification_evaluations: tuple[FalsificationEvaluation, ...] = Field(
        default=(), max_length=M1101_MAX_FALSIFICATION_EVALUATIONS
    )
    findings: tuple[HypothesisFinding, ...] = Field(default=(), max_length=M1101_MAX_EVIDENCE)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["variant_peptide"] = M1101_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> VariantPeptideHypothesisRegistryResult:
...
def expected_provenance(
    ...
    return ProvenanceRecord(
        ...
        module_id=M1101_MODULE_ID,
        module_version=M1101_CONTRACT_VERSION,
...
__all__ = [
    ...
    "M1101_EVIDENCE_CLAIM",
    "M1101_GATE",
    "M1101_MAX_CANONICAL_REQUEST_BYTES",
    "M1101_MAX_CANONICAL_RESULT_BYTES",
    "M1101_MAX_EVIDENCE",
    "M1101_MAX_EVIDENCE_TIERS",
    "M1101_MAX_EXPLANATIONS",
    "M1101_MAX_FALSIFICATION_EVALUATIONS",
    "M1101_MAX_HYPOTHESES",
    "M1101_MAX_RULES",
    "M1101_MODULE_ID",
    "M1101_OPERATION",
