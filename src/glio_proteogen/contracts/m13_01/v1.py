"""Provisional M13-01 biological hypothesis registry contracts.

The M13-01 dossier requires versioned hypotheses, competing explanations,
...
the ABI below is intentionally provisional pending Quality engineering owner
confirmation.
"""
...
# PROVISIONAL ABI: inferred solely from the M13-01 dossier slice.
M1301_MODULE_ID: Final = "GLIO-PROTEOGEN-M13-01"
M1301_OPERATION: Final = "register_proteotype_hypotheses"
M1301_CONTRACT_VERSION: Final = "0.1.0-provisional"
M1301_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m13-01+json"
M1301_PARENT: Final = "proteotype"
M1301_OWNER: Final = "Quality engineering"
M1301_SAFETY_CLASS: Final = "S2"
M1301_GATE: Final = "G0"
M1301_PROVISIONAL_ABI: Final = True
M1301_MAX_HYPOTHESES: Final = 256
M1301_MAX_EXPLANATIONS: Final = 16
M1301_MAX_RULES: Final = 16
M1301_MAX_EVIDENCE_TIERS: Final = 8
M1301_MAX_EVALUATIONS: Final = M1301_MAX_HYPOTHESES
M1301_MAX_FALSIFICATION_EVALUATIONS: Final = M1301_MAX_HYPOTHESES * M1301_MAX_RULES
M1301_MAX_EVIDENCE: Final = 64
M1301_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1301_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1301_EVIDENCE_CLAIM: Final = (
    "Caller-declared M13-01 hypothesis, falsification, and evidence-tier material; "
...
class CompetingExplanation(FrozenModel):
    ...
    required_evidence: tuple[EvidenceReference, ...] = Field(
        min_length=1, max_length=M1301_MAX_EVIDENCE
    )
...
class EvidenceTier(FrozenModel):
    tier: int = Field(ge=1, le=M1301_MAX_EVIDENCE_TIERS)
    label: NonEmptyStr
    rationale: NonEmptyStr
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1301_MAX_EVIDENCE)
...
class BiologicalHypothesis(FrozenModel):
    ...
    target_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M1301_MAX_HYPOTHESES)
    competing_explanations: tuple[CompetingExplanation, ...] = Field(
        min_length=1, max_length=M1301_MAX_EXPLANATIONS
    )
    falsification_rules: tuple[FalsificationRule, ...] = Field(
        min_length=1, max_length=M1301_MAX_RULES
    )
    evidence_tiers: tuple[EvidenceTier, ...] = Field(
        min_length=1, max_length=M1301_MAX_EVIDENCE_TIERS
    )
    prohibited_interpretations: tuple[NonEmptyStr, ...] = Field(min_length=1, max_length=32)
    status: HypothesisStatus = HypothesisStatus.PROPOSED
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1301_MAX_EVIDENCE)
...
class HypothesisRegistry(FrozenModel):
    ...
    hypotheses: tuple[BiologicalHypothesis, ...] = Field(
        min_length=1, max_length=M1301_MAX_HYPOTHESES
    )
...
class RegisterProteotypeHypothesesRequest(FrozenModel):
    """Provisional request for registering a complete hypothesis registry."""

    operation: Literal["register_proteotype_hypotheses"] = M1301_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1301_CONTRACT_VERSION
    ...
    @model_validator(mode="after")
    def request_is_closed(self) -> RegisterProteotypeHypothesesRequest:
        ids = tuple(item.hypothesis_id for item in self.hypotheses)
        if len(ids) != len(set(ids)):
...
class ProteotypeHypothesisRegistryResult(FrozenModel):
    """Versioned registry with complete evaluations and explicit abstention."""

    output_type: Literal["proteotype_hypothesis_registry"] = "proteotype_hypothesis_registry"
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1301_CONTRACT_VERSION
    ...
    evaluations: tuple[HypothesisEvaluation, ...] = Field(
        default=(), max_length=M1301_MAX_EVALUATIONS
    )
    falsification_evaluations: tuple[FalsificationEvaluation, ...] = Field(
        default=(), max_length=M1301_MAX_FALSIFICATION_EVALUATIONS
    )
    findings: tuple[HypothesisFinding, ...] = Field(default=(), max_length=M1301_MAX_EVIDENCE)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["proteotype"] = M1301_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> ProteotypeHypothesisRegistryResult:
...
def expected_uncertainty(*, supported: bool) -> UncertaintyProfile:
    """Return explicit seven-axis uncertainty for every outcome path."""

    estimate = UncertaintyEstimate(
        state=EstimateState.ESTIMATED if supported else EstimateState.NOT_ESTIMABLE,
        probability=0.9 if supported else None,
        rationale=(
            "Evidence tiers and falsification outcomes are explicit; no population "
            "calibration claim is made."
            if supported
            else "A hypothesis, falsification rule, or control is not safely evaluable."
        ),
    )
...
def expected_provenance(
    ...
    """Project all seven caller controls without inferring identity or consent."""
    ...
    return ProvenanceRecord(
        ...
        module_id=M1301_MODULE_ID,
        module_version=M1301_CONTRACT_VERSION,
...
__all__ = [
    ...
    "M1301_MAX_EVIDENCE",
    "M1301_MAX_EVIDENCE_TIERS",
    "M1301_MAX_EXPLANATIONS",
    "M1301_MAX_FALSIFICATION_EVALUATIONS",
    "M1301_MAX_HYPOTHESES",
    "M1301_MAX_RULES",
    "M1301_MODULE_ID",
    "M1301_OPERATION",
    "M1301_OUTPUT_MEDIA_TYPE",
    "M1301_OWNER",
    "M1301_PARENT",
    "M1301_PROVISIONAL_ABI",
    "M1301_SAFETY_CLASS",
