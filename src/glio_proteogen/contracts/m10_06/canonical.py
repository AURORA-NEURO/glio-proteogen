"""Canonical projections for the provisional M10-06 contract spine."""
...
def verify_result_digest(value: BaseModel | dict[str, Any]) -> bool:
    """Verify a result receipt against its canonical payload."""

    document = _dump(value)
    recorded = document.get("result_digest")
    return isinstance(recorded, str) and recorded == result_payload_digest(document)
