"""Provisional M17-01 upstream contract resolver contracts.

M17-01 owns typed upstream discovery and compatibility beneath
Metabolomic/lipidomic integration.  Candidate consent, intended use,
support, provenance and uncertainty remain visible; unknown or unsupported
inputs never become negative findings.  The public ABI is provisional pending
Scientific engineering owner confirmation.
"""
...
M1701_PARENT: Final = "variant peptide"
M1701_OWNER: Final = "Scientific engineering"
M1701_SAFETY_CLASS: Final = "S2"
M1701_GATE: Final = "G0"
M1701_PROVISIONAL_ABI: Final = True
M1701_MAX_CANDIDATES: Final = 128
M1701_MAX_RULES: Final = 64
M1701_MAX_DECISIONS: Final = M1701_MAX_CANDIDATES
M1701_MAX_EVIDENCE: Final = 64
M1701_MAX_FINDINGS: Final = 64
M1701_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1701_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1701_EVIDENCE_CLAIM: Final = (
    "Caller-declared M17-01 upstream compatibility, consent, support and "
...
class CompatibilityRule(FrozenModel):
    ...
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1701_MAX_EVIDENCE)
...
class ResolverConfiguration(FrozenModel):
    ...
    rules: tuple[CompatibilityRule, ...] = Field(min_length=1, max_length=M1701_MAX_RULES)
    accepted_intended_uses: tuple[NonEmptyStr, ...] = Field(min_length=1, max_length=64)
    require_granted_consent: Literal[True] = True
    locked: Literal[True] = True
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1701_MAX_EVIDENCE)
...
class ValidatedUpstreamBundle(FrozenModel):
    ...
    version: SemanticVersion
    candidates: tuple[UpstreamCandidate, ...] = Field(min_length=1, max_length=M1701_MAX_CANDIDATES)
...
class ResolveVariantPeptideUpstreamContractsRequest(FrozenModel):
    """Provisional request for typed upstream discovery and compatibility."""

    operation: Literal["resolve_variant_peptide_upstream_contracts"] = M1701_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1701_CONTRACT_VERSION
    request_id: Identifier
    context: ExecutionContext
    candidates: tuple[UpstreamCandidate, ...] = Field(min_length=1, max_length=M1701_MAX_CANDIDATES)
    configuration: ResolverConfiguration
    source_artifacts: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M1701_MAX_EVIDENCE
    )
    supersedes_result_digest: Sha256Digest | None = None

    @model_validator(mode="after")
    def request_is_closed(self) -> ResolveVariantPeptideUpstreamContractsRequest:
        candidate_ids = tuple(item.candidate_id for item in self.candidates)
        if len(candidate_ids) != len(set(candidate_ids)):
...
class VariantPeptideUpstreamResolutionResult(FrozenModel):
    """Validated upstream bundle and compatibility report with safe abstention."""

    output_type: Literal["variant_peptide_upstream_resolution"] = (
        "variant_peptide_upstream_resolution"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1701_CONTRACT_VERSION
    ...
    compatibility_report: CompatibilityReport
    findings: tuple[ResolverFinding, ...] = Field(default=(), max_length=M1701_MAX_FINDINGS)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["variant peptide"] = M1701_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> VariantPeptideUpstreamResolutionResult:
...
__all__ = [
    ...
    "M1701_MAX_CANONICAL_RESULT_BYTES",
    "M1701_MAX_DECISIONS",
    "M1701_MAX_EVIDENCE",
    "M1701_MAX_FINDINGS",
    "M1701_MAX_RULES",
