"""Semantic canonicalization for M03-06 protein-inference harmonization."""
...
def _dump(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):  # pragma: no cover - contract roots are objects.
        raise TypeError("M03-06 canonical object root must be a mapping")
    return decoded
...
def normalized_unit_receipt(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    return _dump(value)


def unit_binding_digest(
    values: list[BaseModel | dict[str, Any]] | tuple[BaseModel | dict[str, Any], ...],
) -> Sha256Digest:
    return sha256_digest(_sort(tuple(normalized_unit_receipt(item) for item in values)))


def normalized_artifact_receipt(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    data = _dump(value)
    data["units"] = _sort(tuple(normalized_unit_receipt(item) for item in data["units"]))
    return data
...
def normalized_invariant(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    data = _dump(value)
    data["left_unit_ids"] = tuple(sorted(data["left_unit_ids"]))
    data["right_unit_ids"] = tuple(sorted(data["right_unit_ids"]))
    return data
...
def configuration_digest(value: BaseModel | dict[str, Any]) -> Sha256Digest:
    return sha256_digest({"protein_inference_harmonization_policy": normalized_policy(value)})
...
def normalized_stage_transformation(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    ...
    data["clipped_unit_ids"] = tuple(sorted(data["clipped_unit_ids"]))
    return data
...
def normalized_finding(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    data = _dump(value)
    for field in ("stage_ids", "unit_ids", "invariant_ids"):
        data[field] = tuple(sorted(data[field]))
    return data
