"""Provisional M15-08 mechanism evidence dossier contracts.

The M15-08 dossier requires a review-ready, reconstructable chain from input
...
from glio_proteogen.contracts.m15_08.canonical import (
...
M1508_M1507_INPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m15-07+json"
M1508_PARENT: Final = "complex_activity"
M1508_OWNER: Final = "Clinical science"
M1508_SAFETY_CLASS: Final = "S2"
M1508_GATE: Final = "G3"
M1508_PROVISIONAL_ABI: Final = True
M1508_MAX_LINKS: Final = 512
M1508_MAX_COUNTER_EVIDENCE: Final = 256
M1508_MAX_VALIDATION_ROUTES: Final = 128
M1508_MAX_PROHIBITED_INTERPRETATIONS: Final = 64
M1508_MAX_EVIDENCE: Final = 64
M1508_MAX_DIAGNOSTICS: Final = 128
M1508_MAX_FINDINGS: Final = 64
M1508_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M1508_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
M1508_EVIDENCE_CLAIM: Final = (
    "Caller-declared M15-08 mechanism evidence dossier material; issuer authority "
...
class MechanismEvidenceLink(FrozenModel):
    ...
    predecessor_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M1508_MAX_LINKS)
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1508_MAX_EVIDENCE)
...
class CounterEvidenceRecord(FrozenModel):
    ...
    impact: NonEmptyStr
    challenges_link_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M1508_MAX_LINKS)
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M1508_MAX_EVIDENCE)
...
class ClaimCeiling(FrozenModel):
    ...
    prohibited_interpretations: tuple[NonEmptyStr, ...] = Field(
        min_length=1, max_length=M1508_MAX_PROHIBITED_INTERPRETATIONS
    )
...
class MechanismDossierConfiguration(FrozenModel):
    ...
    source_manifest: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M1508_MAX_EVIDENCE
    )
    locked: Literal[True] = True
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M1508_MAX_EVIDENCE)
...
class MechanismEvidenceDossier(FrozenModel):
    ...
    version: SemanticVersion
    links: tuple[MechanismEvidenceLink, ...] = Field(min_length=1, max_length=M1508_MAX_LINKS)
    counter_evidence: tuple[CounterEvidenceRecord, ...] = Field(
        min_length=1, max_length=M1508_MAX_COUNTER_EVIDENCE
    )
    validation_routes: tuple[ValidationRoute, ...] = Field(
        min_length=1, max_length=M1508_MAX_VALIDATION_ROUTES
    )
...
class AssembleComplexActivityMechanismDossierRequest(FrozenModel):
    """Provisional request ABI bound to the M15-07 upstream adjudication."""

    operation: Literal["assemble_complex_activity_mechanism_dossier"] = M1508_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M1508_CONTRACT_VERSION
    ...
    @model_validator(mode="after")
    def request_is_bound(self) -> AssembleComplexActivityMechanismDossierRequest:
        if self.upstream_result.media_type != M1508_M1507_INPUT_MEDIA_TYPE:
            raise ValueError("request must bind the provisional M15-07 adjudication result")
        keys = tuple(
...
class ComplexActivityMechanismDossierResult(FrozenModel):
    """Review-ready mechanism dossier with explicit claim ceiling and abstention."""

    output_type: Literal["complex_activity_mechanism_evidence_dossier"] = (
        "complex_activity_mechanism_evidence_dossier"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M1508_CONTRACT_VERSION
    ...
    diagnostics: tuple[MechanismDossierDiagnostic, ...] = Field(
        min_length=1, max_length=M1508_MAX_DIAGNOSTICS
    )
    findings: tuple[MechanismDossierFindingCode, ...] = Field(
        default=(), max_length=M1508_MAX_FINDINGS
    )
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["complex_activity"] = M1508_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> ComplexActivityMechanismDossierResult:
...
__all__ = [
    ...
    "M1508_MAX_CANONICAL_RESULT_BYTES",
    "M1508_MAX_COUNTER_EVIDENCE",
    "M1508_MAX_DIAGNOSTICS",
    "M1508_MAX_EVIDENCE",
    "M1508_MAX_FINDINGS",
    "M1508_MAX_LINKS",
    "M1508_MAX_PROHIBITED_INTERPRETATIONS",
    "M1508_MAX_VALIDATION_ROUTES",
    "M1508_MODULE_ID",
    "M1508_OPERATION",
