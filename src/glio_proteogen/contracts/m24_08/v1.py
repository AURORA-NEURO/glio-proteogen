...
protein sensitivity. The ABI is provisional pending Data engineering owner confirmation.
"""
...
M2408_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m23-08+json"
...
M2408_DOSSIER_SHA256: Final = (
    "sha256:0a6b200cbe073db13a4bcf315edc23ab97edfe6f500bc7ea2785f5e1c70da181"
)
M2408_DOSSIER_SLICE: Final = "GLIO-PROTEOGEN_240_Module_Dossier.md:8580-8620"
...
def _expected_gate_decision(
    requirements: tuple[GateRequirement, ...],
    benchmarks: tuple[BenchmarkOutcome, ...],
    residual_risks: tuple[ResidualRisk, ...],
    approvals: tuple[ApprovalRecord, ...],
) -> GateDecision:
    if any(not item.satisfied for item in requirements):
        return GateDecision.BLOCK
    if any(not item.passed for item in benchmarks):
        return GateDecision.BLOCK
    if any(
        ...
        return GateDecision.BLOCK
    if any(item.decision is not ApprovalDecision.APPROVE for item in approvals):
...
class SignedReleaseRecord(FrozenModel):
    ...
    benchmarks: tuple[BenchmarkOutcome, ...] = Field(min_length=1, max_length=M2408_MAX_BENCHMARKS)
...
class AdjudicateBiomarkerPanelEvidenceGateRequest(FrozenModel):
    ...
    @model_validator(mode="after")
    def request_is_closed(self) -> AdjudicateBiomarkerPanelEvidenceGateRequest:
...
class BiomarkerPanelEvidenceGateResult(FrozenModel):
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> BiomarkerPanelEvidenceGateResult:  # noqa: PLR0912
        ...
        if self.status is GateRunStatus.ADJUDICATED:
            ...
            if self.release_record.release_id != (
                "m2408.release." + self.request_digest.removeprefix("sha256:")
            ):
                raise ValueError("release record must bind the request digest")
            if self.release_record.version != self.request.configuration.version:
                raise ValueError("release record version must bind the request configuration")
            if self.release_record.evidence != self.evidence:
                raise ValueError("release record evidence must bind the result evidence")
            if self.release_record.decision != _expected_gate_decision(
                self.request.requirements,
                self.request.benchmarks,
                self.request.residual_risks,
                self.request.approvals,
            ):
                raise ValueError("release record decision must bind the request gate inputs")
            if self.release_record.signature_digest != release_record_signature_digest(
                self.request_digest, self.release_record
            ):
                raise ValueError("release record signature does not bind its canonical fields")
            ...
            if self.release_record.decision is not GateDecision.PASS and not self.findings:
                raise ValueError("blocked or review gate results must retain findings")
        elif (
