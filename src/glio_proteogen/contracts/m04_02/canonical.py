"""Semantic canonicalization for M04-02 identity-lineage contracts."""
...
from glio_proteogen.contracts.m04_01 import (
    normalized_result_payload as normalized_m0401_result_payload,
)
...
def normalized_identity_resolution(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(value, BaseModel):
        ...
        # after M04-02 strict validation, so normalize their semantic collections here.
        data = source
...
def normalized_protocol_result(value: BaseModel | dict[str, Any]) -> dict[str, Any]:
    """Canonicalize the embedded, already-digest-bound M04-01 result."""

    data = normalized_m0401_result_payload(value)
