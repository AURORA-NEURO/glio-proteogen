"""Provisional M22-05 subgroup, equity, and rare-context contracts."""
...
from glio_proteogen.contracts.m22_05.canonical import (
...
# PROVISIONAL ABI: inferred solely from dossier lines 7772-7812.
M2205_MODULE_ID: Final = "GLIO-PROTEOGEN-M22-05"
M2205_DOSSIER_SHA256: Final = (
    "sha256:0a6b200cbe073db13a4bcf315edc23ab97edfe6f500bc7ea2785f5e1c70da181"
)
M2205_DOSSIER_SLICE: Final = "GLIO-PROTEOGEN_240_Module_Dossier.md:7772-7812"
M2205_OPERATION: Final = "evaluate_protein_rna_discordance_subgroup_equity"
M2205_CONTRACT_VERSION: Final = "0.1.0-provisional"
M2205_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m22-05+json"
M2205_M2204_INPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m22-04+json"
M2205_PARENT: Final = "protein-RNA discordance"
M2205_OWNER: Final = "Computational biology"
M2205_SAFETY_CLASS: Final = "S3"
M2205_GATE: Final = "G3"
M2205_PROVISIONAL_ABI: Final = True
M2205_MAX_METRICS: Final = 256
M2205_MAX_EVIDENCE: Final = 64
M2205_MAX_FINDINGS: Final = 64
M2205_MAX_CANONICAL_REQUEST_BYTES: Final = 4 * 1024 * 1024
M2205_MAX_CANONICAL_RESULT_BYTES: Final = 8 * 1024 * 1024
_M2205_FRACTION_TOLERANCE: Final = 1e-12
M2205_EVIDENCE_CLAIM: Final = (
    "Caller-declared M22-05 subgroup performance, calibration, coverage and "
...
class SubgroupPerformance(FrozenModel):
    ...
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M2205_MAX_EVIDENCE)
...
class CoverageSummary(FrozenModel):
    ...
    def fraction_is_canonical(self) -> CoverageSummary:
        ...
        if abs(self.coverage_fraction - expected) > _M2205_FRACTION_TOLERANCE:
...
class SubgroupEvaluationReport(FrozenModel):
    ...
    version: SemanticVersion
    performance: tuple[SubgroupPerformance, ...] = Field(min_length=1, max_length=M2205_MAX_METRICS)
    calibration: tuple[CalibrationSummary, ...] = Field(min_length=1, max_length=M2205_MAX_METRICS)
    coverage: tuple[CoverageSummary, ...] = Field(min_length=1, max_length=M2205_MAX_METRICS)
...
class SubgroupFinding(FrozenModel):
    ...
    message: NonEmptyStr
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M2205_MAX_EVIDENCE)


class EvaluateProteinRnaDiscordanceSubgroupEquityRequest(FrozenModel):
    operation: Literal["evaluate_protein_rna_discordance_subgroup_equity"] = M2205_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M2205_CONTRACT_VERSION
    ...
    source_artifacts: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M2205_MAX_EVIDENCE
    )
    supersedes_result_digest: Sha256Digest | None = None

    @model_validator(mode="after")
    def request_is_bound(self) -> EvaluateProteinRnaDiscordanceSubgroupEquityRequest:
        if self.upstream_result.media_type != M2205_M2204_INPUT_MEDIA_TYPE:
            raise ValueError("request must bind the provisional M22-04 evaluator result")
...
class ProteinRnaDiscordanceSubgroupEvaluationResult(FrozenModel):
    output_type: Literal["protein_rna_discordance_subgroup_evaluation"] = (
        "protein_rna_discordance_subgroup_evaluation"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M2205_CONTRACT_VERSION
    request_digest: Sha256Digest
    result_digest: Sha256Digest
    request: EvaluateProteinRnaDiscordanceSubgroupEquityRequest
    status: EvaluationStatus
    report: SubgroupEvaluationReport | None = None
    findings: tuple[SubgroupFinding, ...] = Field(default=(), max_length=M2205_MAX_FINDINGS)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["protein-RNA discordance"] = M2205_PARENT
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> ProteinRnaDiscordanceSubgroupEvaluationResult:
        ...
        if self.provenance.module_id != M2205_MODULE_ID:
            raise ValueError("provenance module id must identify M22-05")
