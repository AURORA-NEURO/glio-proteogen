...
class UpstreamCandidate(FrozenModel):
    ...
    def compatible_candidate_is_safe(self) -> UpstreamCandidate:
        ...
        artifact_digests = (self.artifact.digest,) + (
...
class ValidatedUpstreamBundle(FrozenModel):
    ...
    def bundle_is_supported(self) -> ValidatedUpstreamBundle:
        ...
        if self.version != self.compatibility_report.version:
            raise ValueError("validated bundle version must match compatibility report")
        evidence_digests = tuple(item.reference.digest for item in self.evidence)
        if len(evidence_digests) != len(set(evidence_digests)):
...
class ProteinSubtypeUpstreamResolutionResult(FrozenModel):
    ...
    @model_validator(mode="after")
    def result_is_closed(self) -> ProteinSubtypeUpstreamResolutionResult:  # noqa: PLR0912
        ...
        if self.provenance.module_id != M2001_MODULE_ID:
            raise ValueError("result provenance must identify M20-01")
