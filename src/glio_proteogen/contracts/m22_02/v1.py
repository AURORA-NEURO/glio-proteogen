"""Provisional M22-02 synthetic truth and simulation generator contracts.
...
# PROVISIONAL ABI: inferred solely from dossier lines 7640-7680.
M2202_MODULE_ID: Final = "GLIO-PROTEOGEN-M22-02"
M2202_OPERATION: Final = "generate_protein_rna_discordance_synthetic_truth"
M2202_CONTRACT_VERSION: Final = "0.1.0-provisional"
M2202_OUTPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m22-02+json"
M2202_M2201_INPUT_MEDIA_TYPE: Final = "application/vnd.glio-proteogen.m22-01+json"
M2202_PARENT: Final = "protein-RNA discordance"
M2202_OWNER: Final = "Data engineering"
...
class SyntheticTruthCase(FrozenModel):
    ...
    evidence: tuple[EvidenceReference, ...] = Field(default=(), max_length=M2202_MAX_EVIDENCE)
...
class GenerationManifest(FrozenModel):
    ...
    case_ids: tuple[Identifier, ...] = Field(min_length=1, max_length=M2202_MAX_CASES)
    reproducibility_digest: Sha256Digest
    fixture_summary: tuple[NonEmptyStr, ...] = Field(
        min_length=1, max_length=M2202_MAX_FIXTURE_LABELS
    )
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M2202_MAX_EVIDENCE)
...
class SyntheticTruthCorpus(FrozenModel):
    ...
    version: SemanticVersion
    cases: tuple[SyntheticTruthCase, ...] = Field(min_length=1, max_length=M2202_MAX_CASES)
    manifest: GenerationManifest
    source_artifacts: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M2202_MAX_EVIDENCE
    )
    evidence: tuple[EvidenceReference, ...] = Field(min_length=1, max_length=M2202_MAX_EVIDENCE)
...
class GenerateProteinRnaDiscordanceSyntheticTruthRequest(FrozenModel):
    """Provisional request bound to the M22-01 reference curator result."""

    operation: Literal["generate_protein_rna_discordance_synthetic_truth"] = M2202_OPERATION
    contract_version: Literal["0.1.0-provisional"] = M2202_CONTRACT_VERSION
    request_id: Identifier
    context: ExecutionContext
    upstream_result: ArtifactReference
    configuration: GenerationConfiguration
    requested_case_count: int = Field(ge=1, le=M2202_MAX_CASES)
    source_artifacts: tuple[ArtifactReference, ...] = Field(
        min_length=1, max_length=M2202_MAX_EVIDENCE
    )
...
class ProteinRnaDiscordanceSyntheticTruthResult(FrozenModel):
    ...
    output_type: Literal["protein_rna_discordance_synthetic_truth"] = (
        "protein_rna_discordance_synthetic_truth"
    )
    result_id: Identifier
    result_version: Literal["0.1.0-provisional"] = M2202_CONTRACT_VERSION
    ...
    findings: tuple[GeneratorFinding, ...] = Field(default=(), max_length=M2202_MAX_FINDINGS)
    abstention_reason: NonEmptyStr | None = None
    parent_target: Literal["protein-RNA discordance"] = M2202_PARENT
