    ...
    def _provenance_is_closed(self) -> None:
        ...
        provenance_bindings = (
            (
                ...
                "m2704.activity." + self.request_digest.removeprefix("sha256:"),
                "activity identity",
            ),
            (self.provenance.actor_id, self.request.context.actor_id, "actor identity"),
            (self.provenance.module_id, M2704_MODULE_ID, "module identity"),
            (self.provenance.module_version, M2704_CONTRACT_VERSION, "module version"),
        ...
        for actual, expected, label in provenance_bindings:
            if actual != expected:
                raise ValueError(f"gateway provenance {label} does not bind the request")
