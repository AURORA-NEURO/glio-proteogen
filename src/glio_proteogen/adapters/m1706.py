"""Standalone provisional FastAPI and Typer adapters for M17-06."""
...
from glio_proteogen.contracts.m17_06.schema import ContractName, contract_json_schema
...
_SERVICE = m1706.M1706Service()
_INVALID_REQUEST: Final = "invalid M17-06 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M17-06 reviewer discrepancy adjudication",
    version="0.1.0-provisional",
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1706_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1706_MAX_CANONICAL_RESULT_BYTES,
)
m1706_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1706_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except m1706.M1706AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m17-06/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(cast("ContractName", name))
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M17-06 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M17-06/adjudicate")
async def adjudicate(request: Request) -> JSONResponse:
    ...
    try:
        ...
        raise _json_error(403, str(error)) from error
    except m1706.M1706ExportError as error:
        raise _json_error(422, "M17-06 adjudication failed") from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M17-06/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1706_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, m1706.M1706ReplayVerificationError) as error:
        raise _json_error(422, "M17-06 result verification failed") from error
...
def export_schema(name: Annotated[str, typer.Argument(help="M17-06 schema name.")]) -> None:
    try:
        typer.echo(
            json.dumps(contract_json_schema(cast("ContractName", name)), indent=2, sort_keys=True)
        )
    except (KeyError, ValueError) as error:
        typer.echo("unknown M17-06 schema", err=True)
        raise typer.Exit(code=2) from error


@m1706_app.command("adjudicate")
...
def verify_command(
    ...
    try:
        ...
        typer.echo("verification failed: M17-06 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1706_app"]
