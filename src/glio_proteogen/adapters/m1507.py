...
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1507_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1507_MAX_CANONICAL_RESULT_BYTES,
)
...
def _load_request(path: Path) -> AdjudicateComplexActivityPlausibilityRequest:
    try:
        raw = read_bounded(path, M1507_MAX_CANONICAL_REQUEST_BYTES)
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1507_MAX_CANONICAL_RESULT_BYTES)
