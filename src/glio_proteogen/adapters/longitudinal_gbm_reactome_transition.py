"""Bounded HTTP and CLI boundary for Reactome conditional transitions."""
...
LONGITUDINAL_GBM_REACTOME_TRANSITION_ROUTE_PREFIX: Final = (
    "/v1/research/longitudinal-gbm-reactome-transition"
)
LONGITUDINAL_GBM_REACTOME_TRANSITION_REQUEST_MAX_BYTES: Final = MAX_REQUEST_BYTES
LONGITUDINAL_GBM_REACTOME_TRANSITION_RESULT_MAX_BYTES: Final = MAX_RESULT_BYTES
LONGITUDINAL_GBM_REACTOME_TRANSITION_REPLAY_MAX_BYTES: Final = MAX_REPLAY_BYTES
LONGITUDINAL_GBM_REACTOME_TRANSITION_MAX_CONCURRENT_ANALYSES: Final = 2
LONGITUDINAL_GBM_REACTOME_TRANSITION_TIMEOUT_SECONDS: Final = 120.0
LONGITUDINAL_GBM_REACTOME_TRANSITION_RETRY_AFTER_SECONDS: Final = 1
_DISCONNECT_POLL_SECONDS: Final = 0.05
_CANCELLED_MESSAGE: Final = "Reactome transition computation was cancelled"
_TIMEOUT_MESSAGE: Final = "Reactome transition computation exceeded its deadline"
_ANALYSIS_ERROR_MESSAGE: Final = "Reactome transition analysis failed safely"
_REPLAY_ERROR_MESSAGE: Final = "Reactome transition replay failed safely"
_UNAVAILABLE_MESSAGE: Final = "Reactome transition model is unavailable"
_CLI_INPUT_ERROR_MESSAGE: Final = "input does not satisfy the Reactome transition contract"

_REQUEST_ADAPTER: Final = TypeAdapter(LongitudinalGbmReactomeTransitionRequest)
_REPLAY_ADAPTER: Final = TypeAdapter(ReactomeConditionalReplayVerificationRequest)
_ANALYSIS_SLOTS = BoundedSemaphore(
    LONGITUDINAL_GBM_REACTOME_TRANSITION_MAX_CONCURRENT_ANALYSES
)

router = APIRouter(
    prefix=LONGITUDINAL_GBM_REACTOME_TRANSITION_ROUTE_PREFIX,
    tags=["research-longitudinal-gbm-reactome-transition"],
)
cli = typer.Typer(
    no_args_is_help=True,
    pretty_exceptions_enable=False,
    help="Run and replay research-only Reactome conditional-transition concordance.",
)


class LongitudinalGbmReactomeTransitionCliError(typer.BadParameter):
    """Sanitized Reactome transition command-line validation error."""
...
async def _typed_body[T](
    ...
    try:
        ...
        raise _http_error(
            422,
            "request does not satisfy the Reactome transition contract",
        ) from None
    else:
        return typed
...
def install_longitudinal_gbm_reactome_transition_openapi(app: FastAPI) -> None:
    ...
    def openapi_with_replay_contract() -> dict[str, Any]:
        ...
        components["ReactomeConditionalReplayVerificationRequest"] = replay_schema
...
def mount_longitudinal_gbm_reactome_transition(app: FastAPI) -> None:
...
def _acquire_http_slot() -> None:
    if not _ANALYSIS_SLOTS.acquire(blocking=False):
        raise _http_error(
            429,
            "Reactome transition capacity is exhausted",
            **{
                "Retry-After": str(
                    LONGITUDINAL_GBM_REACTOME_TRANSITION_RETRY_AFTER_SECONDS
                )
            },
        )
...
def _execute(  # noqa: C901 - explicit boundary mapping is intentionally exhaustive.
    ...
    *,
    ...
    try:
        result = analyze_longitudinal_gbm_reactome_transition(
            request,
        ...
        raise _http_error(499, _CANCELLED_MESSAGE) from None
    except (ReactomeTransitionSourceIntegrityError, ReactomeConditionalModelIntegrityError):
        ...
        raise _http_error(
            422,
            "request could not be evaluated by the Reactome transition model",
        ) from None
    ...
    if len(canonical_json_bytes(result)) > LONGITUDINAL_GBM_REACTOME_TRANSITION_RESULT_MAX_BYTES:
        raise _http_error(500, "Reactome transition result exceeded its transport bound")
    receipt_size = len(canonical_json_bytes({"request": request, "result": result}))
    if receipt_size > LONGITUDINAL_GBM_REACTOME_TRANSITION_REPLAY_MAX_BYTES:
        raise _http_error(500, "Reactome transition receipt exceeded its replay bound")
    return result


def _execute_verification(
    request: ReactomeConditionalReplayVerificationRequest,
    cancellation: CancellationContext | None = None,
    *,
    admitted: bool = False,
) -> ReactomeConditionalReplayVerificationResult:
    ...
    try:
        result = verify_longitudinal_gbm_reactome_transition_replay(
            request,
            cancellation=cancellation,
        )
    except InferenceDeadlineExceededError:
    ...
    if (
        ...
        raise _http_error(500, "Reactome transition verification exceeded its bound")
    return result
...
@router.get(
    "/profile",
    response_model=LongitudinalGbmReactomeTransitionProfile,
    responses={500: _ERROR_RESPONSES[500], 503: _ERROR_RESPONSES[503]},
)
def profile(response: Response) -> LongitudinalGbmReactomeTransitionProfile:
...
@router.get(
    "/demo",
    response_model=LongitudinalGbmReactomeTransitionRequest,
    responses={500: _ERROR_RESPONSES[500], 503: _ERROR_RESPONSES[503]},
)
def demo(response: Response) -> LongitudinalGbmReactomeTransitionRequest:
...
@router.post(
    "/analyze",
    response_model=LongitudinalGbmReactomeTransitionResult,
    responses=_ERROR_RESPONSES,
    openapi_extra=_request_body_schema(
        {"$ref": "#/components/schemas/LongitudinalGbmReactomeTransitionRequest"}
    ),
)
async def analyze(request: Request, response: Response) -> LongitudinalGbmReactomeTransitionResult:
    """Run bounded deterministic conditional inference off the event loop."""

    _acquire_http_slot()
    cancellation = CancellationContext.with_timeout(
        LONGITUDINAL_GBM_REACTOME_TRANSITION_TIMEOUT_SECONDS
    )
    finished = asyncio.Event()
...
@router.post(
    "/verify",
    response_model=ReactomeConditionalReplayVerificationResult,
    responses=_ERROR_RESPONSES,
    openapi_extra=_request_body_schema(
        {"$ref": "#/components/schemas/ReactomeConditionalReplayVerificationRequest"}
    ),
)
async def verify(
    request: Request,
...
def _read_typed[T](path: Path, adapter: TypeAdapter[T], max_bytes: int) -> T:
    try:
        ...
        raise LongitudinalGbmReactomeTransitionCliError(_CLI_INPUT_ERROR_MESSAGE) from None
...
def cli_profile() -> None:
    """Print the source- and model-bound Reactome transition profile."""

    try:
        result = algorithm_profile()
    except Exception:  # noqa: BLE001 - sanitize model-integrity failures.
        raise LongitudinalGbmReactomeTransitionCliError(_UNAVAILABLE_MESSAGE) from None
    _emit(result)
...
def cli_demo() -> None:
    ...
    try:
        ...
        raise LongitudinalGbmReactomeTransitionCliError(_ANALYSIS_ERROR_MESSAGE) from None
    _emit(result)
...
def cli_analyze(
    ...
    try:
        result = analyze_longitudinal_gbm_reactome_transition(typed)
    ...
    if (
        ...
        raise LongitudinalGbmReactomeTransitionCliError(_ANALYSIS_ERROR_MESSAGE)
    if (
...
def cli_verify(
    ...
    try:
        result = verify_longitudinal_gbm_reactome_transition_replay(typed)
    except Exception:  # noqa: BLE001 - sanitize every numerical/model failure.
        raise LongitudinalGbmReactomeTransitionCliError(_REPLAY_ERROR_MESSAGE) from None
    if (
        len(canonical_json_bytes(result))
        > LONGITUDINAL_GBM_REACTOME_TRANSITION_RESULT_MAX_BYTES
    ):
        raise LongitudinalGbmReactomeTransitionCliError(_REPLAY_ERROR_MESSAGE)
    _emit(result)
...
__all__ = [
    ...
    "LONGITUDINAL_GBM_REACTOME_TRANSITION_TIMEOUT_SECONDS",
    "LongitudinalGbmReactomeTransitionCliError",
    "cli",
