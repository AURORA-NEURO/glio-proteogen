...
def _m1906_request_body() -> dict[str, object]:
    return {
        "requestBody": {
            "required": True,
            "content": {"application/json": {"schema": m1906_contract_json_schema("request")}},
        }
    }
...
def create_app(database_path: Path) -> FastAPI:  # noqa: PLR0915 - central route composition.
    ...
    @app.post(
        "/v1/modules/M19-06/adjudicate",
        response_model=ProteotypeAdjudicationResult,
        tags=["M19-06"],
        openapi_extra=_m1906_request_body(),
    )
    ...
    def monitor_m1908_translation_health(
        request: Annotated[MonitorProteotypeTranslationHealthRequest, Depends(_m1908_body)],
    ) -> ProteotypeTranslationMonitoringResult:
