"""Bounded HTTP and CLI boundary for Neftel bulk-protein program evidence."""
...
NEFTEL_PROGRAMS_ROUTE_PREFIX: Final = "/v1/research/neftel-protein-programs"
NEFTEL_PROGRAMS_REQUEST_MAX_BYTES: Final = MAX_REQUEST_BYTES
NEFTEL_PROGRAMS_RESULT_MAX_BYTES: Final = MAX_RESULT_BYTES
NEFTEL_PROGRAMS_REPLAY_MAX_BYTES: Final = MAX_REPLAY_BYTES
NEFTEL_PROGRAMS_MAX_CONCURRENT_ANALYSES: Final = 2
NEFTEL_PROGRAMS_TIMEOUT_SECONDS: Final = 25.0
NEFTEL_PROGRAMS_RETRY_AFTER_SECONDS: Final = 1
_DISCONNECT_POLL_SECONDS: Final = 0.05
_CANCELLED_MESSAGE: Final = "Neftel protein-program computation was cancelled"
_TIMEOUT_MESSAGE: Final = "Neftel protein-program computation exceeded its deadline"
_ANALYSIS_ERROR_MESSAGE: Final = "Neftel protein-program analysis failed safely"
_REPLAY_ERROR_MESSAGE: Final = "Neftel protein-program replay failed safely"
_CLI_INPUT_ERROR_MESSAGE: Final = "input does not satisfy the Neftel protein-program contract"

_REQUEST_ADAPTER: Final = TypeAdapter(ProteinProgramRequest)
_REPLAY_ADAPTER: Final = TypeAdapter(ReplayVerificationRequest)
_ANALYSIS_SLOTS = BoundedSemaphore(NEFTEL_PROGRAMS_MAX_CONCURRENT_ANALYSES)

router = APIRouter(
    prefix=NEFTEL_PROGRAMS_ROUTE_PREFIX,
    tags=["research-neftel-protein-programs"],
)
cli = typer.Typer(
    no_args_is_help=True,
    pretty_exceptions_enable=False,
    help="Run and replay research-only Neftel bulk-protein program evidence.",
)


class NeftelProgramsCliError(typer.BadParameter):
    """Sanitized Neftel protein-program command-line validation error."""
...
async def _typed_body[T](
    ...
    try:
        ...
        raise _http_error(
            422,
            "request does not satisfy the Neftel protein-program contract",
        ) from None
    else:
        return typed
...
_ERROR_RESPONSES: Final[dict[int | str, dict[str, Any]]] = {
    ...
    422: _error_response("Request or deterministic program inference is not evaluable"),
    429: _error_response(
...
def install_neftel_programs_openapi(app: FastAPI) -> None:
    ...
    def openapi_with_replay_contract() -> dict[str, Any]:
        ...
        components["NeftelReplayVerificationRequest"] = replay_schema
        operation = schema["paths"][f"{NEFTEL_PROGRAMS_ROUTE_PREFIX}/verify"]["post"]
        operation["requestBody"] = {
            "required": True,
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/NeftelReplayVerificationRequest"}
                }
            },
        }
...
def _acquire_http_slot() -> None:
    if not _ANALYSIS_SLOTS.acquire(blocking=False):
        raise _http_error(
            429,
            "Neftel protein-program capacity is exhausted",
            **{"Retry-After": str(NEFTEL_PROGRAMS_RETRY_AFTER_SECONDS)},
        )
...
def _execute(
    ...
    *,
    ...
    try:
        result = analyze_neftel_protein_programs(request, cancellation=cancellation)
    except InferenceDeadlineExceededError:
        raise _http_error(504, _TIMEOUT_MESSAGE) from None
    except InferenceCancelledError:
        raise _http_error(499, _CANCELLED_MESSAGE) from None
        ...
        raise _http_error(
            422,
            "request could not be evaluated by the Neftel protein-program model",
        ) from None
    ...
    if len(canonical_json_bytes(result)) > NEFTEL_PROGRAMS_RESULT_MAX_BYTES:
        raise _http_error(500, "Neftel protein-program result exceeded its transport bound")
    receipt_size = len(canonical_json_bytes({"request": request, "result": result}))
    if receipt_size > NEFTEL_PROGRAMS_REPLAY_MAX_BYTES:
        raise _http_error(500, "Neftel protein-program receipt exceeded its replay bound")
    return result
...
def _execute_verification(
    ...
    *,
    ...
    try:
        result = verify_neftel_protein_program_replay(
            request,
        ...
        if not admitted:
            _ANALYSIS_SLOTS.release()
    if len(canonical_json_bytes(result)) > NEFTEL_PROGRAMS_RESULT_MAX_BYTES:
        raise _http_error(500, "Neftel protein-program verification exceeded its bound")
    return result


def _analysis_headers(response: Response, result: ProteinProgramResult) -> None:
...
@router.get("/profile", response_model=NeftelAlgorithmProfile)
def profile(response: Response) -> NeftelAlgorithmProfile:
...
def demo(response: Response) -> ProteinProgramRequest:
    """Return a versioned synthetic AC-like bulk-protein request."""
...
async def analyze(request: Request, response: Response) -> ProteinProgramResult:
    """Run bounded deterministic Neftel program inference off the event loop."""

    _acquire_http_slot()
    cancellation = CancellationContext.with_timeout(NEFTEL_PROGRAMS_TIMEOUT_SECONDS)
    finished = asyncio.Event()
...
def _read_typed[T](path: Path, adapter: TypeAdapter[T], max_bytes: int) -> T:
    try:
        ...
        raise NeftelProgramsCliError(_CLI_INPUT_ERROR_MESSAGE) from None
...
def cli_profile() -> None:
    """Print the content-bound Neftel algorithm profile."""
...
@cli.command("demo")
def cli_demo() -> None:
    """Print the versioned synthetic bulk-protein request."""
...
def cli_analyze(
    ...
    typed = _read_typed(request, _REQUEST_ADAPTER, NEFTEL_PROGRAMS_REQUEST_MAX_BYTES)
    try:
        result = analyze_neftel_protein_programs(typed)
    except (TypeError, ValueError, ValidationError):
        raise NeftelProgramsCliError(_ANALYSIS_ERROR_MESSAGE) from None
    except Exception:  # noqa: BLE001 - sanitize unexpected model failures.
        raise NeftelProgramsCliError(_ANALYSIS_ERROR_MESSAGE) from None
    if len(canonical_json_bytes(result)) > NEFTEL_PROGRAMS_RESULT_MAX_BYTES:
        raise NeftelProgramsCliError(_ANALYSIS_ERROR_MESSAGE)
    if (
...
def cli_verify(
    ...
    typed = _read_typed(envelope, _REPLAY_ADAPTER, NEFTEL_PROGRAMS_REPLAY_MAX_BYTES)
    try:
        result = verify_neftel_protein_program_replay(typed)
    except (TypeError, ValueError, ValidationError):
        raise NeftelProgramsCliError(_REPLAY_ERROR_MESSAGE) from None
    except Exception:  # noqa: BLE001 - sanitize unexpected model failures.
        raise NeftelProgramsCliError(_REPLAY_ERROR_MESSAGE) from None
    if len(canonical_json_bytes(result)) > NEFTEL_PROGRAMS_RESULT_MAX_BYTES:
        raise NeftelProgramsCliError(_REPLAY_ERROR_MESSAGE)
    _emit(result)
...
__all__ = [
    "NEFTEL_PROGRAMS_MAX_CONCURRENT_ANALYSES",
    "NEFTEL_PROGRAMS_REPLAY_MAX_BYTES",
    "NEFTEL_PROGRAMS_REQUEST_MAX_BYTES",
    "NEFTEL_PROGRAMS_RESULT_MAX_BYTES",
    "NEFTEL_PROGRAMS_ROUTE_PREFIX",
    "NEFTEL_PROGRAMS_TIMEOUT_SECONDS",
    "NeftelProgramsCliError",
    "cli",
