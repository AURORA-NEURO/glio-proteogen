...
def create_app(service: M1705Service | None = None) -> FastAPI:
    """Create a small isolated API for the M17-05 contract."""

    operation = service or M1705Service()
    api = FastAPI(title="GLIO-PROTEOGEN M17-05", version="0.1.0-provisional")
    api.add_middleware(
        RequestSizeLimitMiddleware,
        max_bytes=M1705_MAX_CANONICAL_REQUEST_BYTES,
        result_max_bytes=M1705_MAX_CANONICAL_RESULT_BYTES,
    )

    @api.get("/v1/m17-05/schema/{name}")
    async def schema(name: str) -> JSONResponse:
        try:
            return JSONResponse(contract_json_schema(name))  # type: ignore[arg-type]
        except (KeyError, ValueError, TypeError) as error:
            raise HTTPException(status_code=404, detail="unknown M17-05 contract") from error
...
def _emit(result: object, output: str | None) -> None:
    ...
    target = Path(output)
    if target.exists():
        typer.echo("M17-05 output exists; refusing overwrite", err=True)
...
def export_schema(
    name: Annotated[ContractName, typer.Argument(help="M17-05 contract name.")],
) -> None:
