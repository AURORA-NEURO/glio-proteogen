...
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1408_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1408_MAX_CANONICAL_RESULT_BYTES,
)
...
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1408_MAX_CANONICAL_RESULT_BYTES)
...
def _load_request(path: Path) -> PublishProteinSubtypeMechanismDossierRequest:
    try:
        raw = read_bounded(path, M1408_MAX_CANONICAL_REQUEST_BYTES)
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1408_MAX_CANONICAL_RESULT_BYTES)
