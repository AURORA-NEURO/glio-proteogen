"""Standalone provisional M15-06 FastAPI and Typer adapters."""
...
_RESULT_ADAPTER = TypeAdapter(ComplexActivitySensitivitySimulationResult)
_SERVICE = M1506Service()
_INVALID_REQUEST: Final = "invalid M15-06 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(title="GLIO-PROTEOGEN M15-06 sensitivity simulator", version="0.1.0-provisional")
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1506_MAX_CANONICAL_RESULT_BYTES,
)
m1506_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> SimulateComplexActivityPerturbationsRequest:
    if (
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES)
        preflight_m1506_authorization(decoded)
    ...
    except M1506AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m15-06/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(name)  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M15-06 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M15-06/simulate")
...
@app.post("/v1/modules/M15-06/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES * 2)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1506ReplayVerificationError) as error:
        raise _json_error(422, "M15-06 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> SimulateComplexActivityPerturbationsRequest:
    try:
        raw = read_bounded(path, M1506_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES)
        preflight_m1506_authorization(decoded)
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1506_MAX_CANONICAL_RESULT_BYTES)


@m1506_app.command("export-schema")
def export_schema(name: Annotated[str, typer.Argument(help="M15-06 schema name.")]) -> None:
    try:
        typer.echo(json.dumps(contract_json_schema(name), indent=2, sort_keys=True))  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        typer.echo("unknown M15-06 schema", err=True)
        raise typer.Exit(code=2) from error


@m1506_app.command("simulate")
def simulate_command(
    ...
    except (M1506AuthorizationError, OSError, typer.BadParameter) as error:
        typer.echo(f"simulation failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1506_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1506_MAX_CANONICAL_REQUEST_BYTES * 2)
        ...
        typer.echo("verification failed: M15-06 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1506_app"]
