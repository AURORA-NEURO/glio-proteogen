"""Standalone provisional M11-05 API and CLI adapters.

The shared legacy adapters are intentionally untouched while this dossier ABI
is pending owner confirmation. Both surfaces use the same strict parse-once
service seam and never traverse opaque artifacts.
"""

from __future__ import annotations

import json
from pathlib import Path  # noqa: TC003 - Typer resolves the runtime path type.
from typing import Annotated, Final, cast

import typer
from fastapi import FastAPI, HTTPException, Request
from fastapi.responses import JSONResponse
from pydantic import TypeAdapter, ValidationError

from glio_proteogen.adapters.limits import RequestSizeLimitMiddleware, read_bounded
from glio_proteogen.contracts.m11_05 import (
    M1105_MAX_CANONICAL_REQUEST_BYTES,
    M1105_MAX_CANONICAL_RESULT_BYTES,
    ModelVariantPeptideLongitudinalEvolutionRequest,
    VariantPeptideLongitudinalEvolutionResult,
    contract_json_schema,
)
from glio_proteogen.kernel.canonical import canonical_json_bytes
from glio_proteogen.kernel.strict_json import (
    StrictJsonError,
    sanitized_validation_errors,
    strict_json_loads,
)

from ..modules.c11_protein_native_subtype.m11_05_longitudinal_evolution.engine import (
    M1105AuthorizationError,
    M1105ReplayVerificationError,
    preflight_m1105_authorization,
)
from ..modules.c11_protein_native_subtype.m11_05_longitudinal_evolution.service import (
    M1105Service,
)

_REQUEST_ADAPTER = TypeAdapter(ModelVariantPeptideLongitudinalEvolutionRequest)
_RESULT_ADAPTER = TypeAdapter(VariantPeptideLongitudinalEvolutionResult)
_SERVICE = M1105Service()
_INVALID_REQUEST: Final = "invalid M11-05 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M11-05 longitudinal and evolutionary model",
    version="0.1.0-provisional",
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1105_MAX_CANONICAL_RESULT_BYTES,
)
m1105_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)


def _json_error(status_code: int, detail: object) -> HTTPException:
    return HTTPException(status_code=status_code, detail=detail)


def _content_type(request: Request) -> None:
    if request.headers.get("content-type", "").partition(";")[0].strip().lower() != (
        "application/json"
    ):
        raise _json_error(415, "content-type must be application/json")


async def _validated_body(request: Request) -> ModelVariantPeptideLongitudinalEvolutionRequest:
    _content_type(request)
    try:
        body = await request.body()
        decoded = strict_json_loads(body, max_bytes=M1105_MAX_CANONICAL_REQUEST_BYTES)
        preflight_m1105_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(body, strict=True)
    except StrictJsonError as error:
        raise _json_error(422, "invalid JSON request") from error
    except ValidationError as error:
        raise _json_error(
            422,
            sanitized_validation_errors(error, location_prefix=("body",)),
        ) from error
    except M1105AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m11-05/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(cast("object", name))  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M11-05 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M11-05/evolve")
@app.post("/v1/m11-05/evolve")
async def evolve(request: Request) -> JSONResponse:
    validated = await _validated_body(request)
    try:
        result = _SERVICE._execute_validated(validated)
    except M1105AuthorizationError as error:
        raise _json_error(403, str(error)) from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M11-05/verify")
@app.post("/v1/m11-05/verify")
async def verify(request: Request) -> JSONResponse:
    _content_type(request)
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1105_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1105ReplayVerificationError) as error:
        raise _json_error(422, "M11-05 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> ModelVariantPeptideLongitudinalEvolutionRequest:
    try:
        raw = read_bounded(path, M1105_MAX_CANONICAL_REQUEST_BYTES)
        strict_json_loads(raw, max_bytes=M1105_MAX_CANONICAL_REQUEST_BYTES)
        return _REQUEST_ADAPTER.validate_json(raw, strict=True)
    except (OSError, StrictJsonError, ValidationError) as error:
        raise typer.BadParameter(_INVALID_REQUEST) from error


@m1105_app.command("export-schema")
def export_schema(
    name: Annotated[str, typer.Argument(help="M11-05 schema name.")],
) -> None:
    try:
        typer.echo(
            json.dumps(
                contract_json_schema(cast("object", name)),  # type: ignore[arg-type]
                indent=2,
                sort_keys=True,
            )
        )
    except (KeyError, ValueError) as error:
        typer.echo("unknown M11-05 schema", err=True)
        raise typer.Exit(code=2) from error


@m1105_app.command("evolve")
def evolve_command(
    request_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
    output: Annotated[Path | None, typer.Option("--output", "-o")] = None,
) -> None:
    if output is not None and output.exists():
        raise typer.BadParameter(_OUTPUT_EXISTS)
    try:
        request = _load_request(request_path)
        result = _SERVICE._execute_validated(request)
        payload = canonical_json_bytes(result).decode("utf-8")
        if output is None:
            typer.echo(payload)
        else:
            output.write_text(payload + "\n", encoding="utf-8", newline="\n")
    except (M1105AuthorizationError, OSError, typer.BadParameter) as error:
        typer.echo(f"evolution failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1105_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = read_bounded(result_path, M1105_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(raw, max_bytes=M1105_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(raw, strict=True)
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1105ReplayVerificationError) as error:
        typer.echo("verification failed: M11-05 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1105_app"]
