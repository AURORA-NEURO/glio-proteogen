"""Standalone provisional M11-04 FastAPI and Typer adapters.
...
_RESULT_ADAPTER = TypeAdapter(VariantPeptideMechanismInferenceResult)
_SERVICE = M1104Service()
_INVALID_REQUEST: Final = "invalid M11-04 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M11-04 network/state/mechanism inference",
    version="0.1.0-provisional",
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1104_MAX_CANONICAL_RESULT_BYTES,
)
m1104_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> InferVariantPeptideMechanismRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1104_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except M1104MechanismAuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m11-04/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(name)  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M11-04 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M11-04/mechanism")
...
@app.post("/v1/modules/M11-04/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1104_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1104ReplayVerificationError) as error:
        raise _json_error(422, "M11-04 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> InferVariantPeptideMechanismRequest:
    try:
        raw = read_bounded(path, M1104_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1104_MAX_CANONICAL_REQUEST_BYTES)
        preflight_mechanism_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(raw, strict=True)
    except (OSError, StrictJsonError, ValidationError, M1104MechanismAuthorizationError) as error:
        raise typer.BadParameter(_INVALID_REQUEST) from error


@m1104_app.command("export-schema")
def export_schema(
    name: Annotated[str, typer.Argument(help="M11-04 schema name.")],
) -> None:
    try:
        typer.echo(json.dumps(contract_json_schema(name), indent=2, sort_keys=True))  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        typer.echo("unknown M11-04 schema", err=True)
        raise typer.Exit(code=2) from error


@m1104_app.command("infer")
def infer_command(
    ...
    except (M1104MechanismAuthorizationError, OSError, typer.BadParameter) as error:
        typer.echo(f"inference failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1104_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = read_bounded(result_path, M1104_MAX_CANONICAL_RESULT_BYTES)
        strict_json_loads(raw, max_bytes=M1104_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(raw, strict=True)
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1104ReplayVerificationError) as error:
        typer.echo("verification failed: M11-04 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1104_app"]
