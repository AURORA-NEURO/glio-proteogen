...
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1304_MAX_CANONICAL_REQUEST_BYTES * 2)
...
def _load_request(path: Path) -> InferProteotypeMechanismRequest:
    ...
    except (OSError, StrictJsonError, ValidationError, M1304MechanismAuthorizationError) as error:
...
def verify_command(
    ...
    try:
        ...
        strict_json_loads(raw, max_bytes=M1304_MAX_CANONICAL_REQUEST_BYTES * 2)
        result = _RESULT_ADAPTER.validate_json(raw, strict=True)
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1304ReplayVerificationError) as error:
