"""Standalone provisional FastAPI and Typer adapters for M16-07."""
...
from glio_proteogen.contracts.m16_07.schema import ContractName, contract_json_schema
...
_SERVICE = M1607Service()
_INVALID_REQUEST: Final = "invalid M16-07 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(title="GLIO-PROTEOGEN M16-07 downstream typed export", version="0.1.0-provisional")
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1607_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1607_MAX_CANONICAL_RESULT_BYTES,
)
m1607_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> ExportProteinRnaDiscordanceDownstreamContractRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1607_MAX_CANONICAL_REQUEST_BYTES)
        preflight_export_authorization(decoded)
    ...
    except M1607AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m16-07/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(cast("ContractName", name))
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M16-07 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M16-07/export")
async def export(request: Request) -> JSONResponse:
    ...
    try:
        ...
        raise _json_error(403, str(error)) from error
    except M1607InferenceError as error:
        raise _json_error(422, "M16-07 downstream export failed") from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M16-07/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1607_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1607ReplayVerificationError) as error:
        raise _json_error(422, "M16-07 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> ExportProteinRnaDiscordanceDownstreamContractRequest:
    try:
        raw = read_bounded(path, M1607_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1607_MAX_CANONICAL_REQUEST_BYTES)
        preflight_export_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (
        OSError,
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1607_MAX_CANONICAL_RESULT_BYTES)


@m1607_app.command("export-schema")
def export_schema(name: Annotated[str, typer.Argument(help="M16-07 schema name.")]) -> None:
    try:
        typer.echo(
            json.dumps(contract_json_schema(cast("ContractName", name)), indent=2, sort_keys=True)
        )
    except (KeyError, ValueError) as error:
        typer.echo("unknown M16-07 schema", err=True)
        raise typer.Exit(code=2) from error


@m1607_app.command("export")
def export_command(
    ...
    except (M1607AuthorizationError, M1607InferenceError, OSError, typer.BadParameter) as error:
        typer.echo(f"export failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1607_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1607_MAX_CANONICAL_RESULT_BYTES)
        ...
        typer.echo("verification failed: M16-07 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1607_app"]
