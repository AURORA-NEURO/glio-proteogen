"""Standalone provisional FastAPI and Typer adapters for M15-01."""
...
from glio_proteogen.contracts.m15_01.schema import ContractName, contract_json_schema
...
_SERVICE = M1501Service()
_INVALID_REQUEST: Final = "invalid M15-01 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M15-01 biological hypothesis registry", version="0.1.0-provisional"
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1501_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1501_MAX_CANONICAL_RESULT_BYTES,
)
m1501_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> RegisterComplexActivityHypothesesRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1501_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except M1501AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m15-01/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(cast("ContractName", name))
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M15-01 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M15-01/register")
async def register(request: Request) -> JSONResponse:
    ...
    try:
        ...
        raise _json_error(403, str(error)) from error
    except M1501InferenceError as error:
        raise _json_error(422, "M15-01 hypothesis registration failed") from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M15-01/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1501_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1501ReplayVerificationError) as error:
        raise _json_error(422, "M15-01 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> RegisterComplexActivityHypothesesRequest:
    try:
        raw = read_bounded(path, M1501_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1501_MAX_CANONICAL_REQUEST_BYTES)
        preflight_hypothesis_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (
        OSError,
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1501_MAX_CANONICAL_RESULT_BYTES)


@m1501_app.command("export-schema")
def export_schema(name: Annotated[str, typer.Argument(help="M15-01 schema name.")]) -> None:
    try:
        typer.echo(
            json.dumps(contract_json_schema(cast("ContractName", name)), indent=2, sort_keys=True)
        )
    except (KeyError, ValueError) as error:
        typer.echo("unknown M15-01 schema", err=True)
        raise typer.Exit(code=2) from error


@m1501_app.command("register")
def register_command(
    ...
    except (M1501AuthorizationError, M1501InferenceError, OSError, typer.BadParameter) as error:
        typer.echo(f"registration failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1501_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1501_MAX_CANONICAL_RESULT_BYTES)
        ...
        typer.echo("verification failed: M15-01 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1501_app"]
