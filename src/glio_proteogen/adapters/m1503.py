...
def _load_request(path: Path) -> ConstructComplexActivityMechanisticFeaturesRequest:
    ...
    except (OSError, StrictJsonError, ValidationError, M1503AuthorizationError) as error:
...
def verify_command(
    ...
    try:
        ...
        strict_json_loads(raw, max_bytes=M1503_MAX_CANONICAL_REQUEST_BYTES * 2)
        result = _RESULT_ADAPTER.validate_json(raw, strict=True)
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1503ReplayVerificationError) as error:
