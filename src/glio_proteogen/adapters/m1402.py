"""Standalone provisional FastAPI and Typer adapters for M14-02."""
...
_SERVICE = M1402Service()
_INVALID_REQUEST: Final = "invalid M14-02 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(title="GLIO-PROTEOGEN M14-02 context stratifier", version="0.1.0-provisional")
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1402_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1402_MAX_CANONICAL_RESULT_BYTES,
)
m1402_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> StratifyProteinSubtypeContextRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1402_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except M1402AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m14-02/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(name)  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M14-02 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M14-02/stratify")
async def stratify(request: Request) -> JSONResponse:
    ...
    try:
        ...
        raise _json_error(403, str(error)) from error
    except M1402InferenceError as error:
        raise _json_error(422, "M14-02 context stratification failed") from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M14-02/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1402_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1402ReplayVerificationError) as error:
        raise _json_error(422, "M14-02 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> StratifyProteinSubtypeContextRequest:
    try:
        raw = read_bounded(path, M1402_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1402_MAX_CANONICAL_REQUEST_BYTES)
        preflight_context_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (
        OSError,
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1402_MAX_CANONICAL_RESULT_BYTES)


@m1402_app.command("export-schema")
def export_schema(name: Annotated[str, typer.Argument(help="M14-02 schema name.")]) -> None:
    try:
        typer.echo(json.dumps(contract_json_schema(name), indent=2, sort_keys=True))  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        typer.echo("unknown M14-02 schema", err=True)
        raise typer.Exit(code=2) from error


@m1402_app.command("stratify")
def stratify_command(
    ...
    except (M1402AuthorizationError, M1402InferenceError, OSError, typer.BadParameter) as error:
        typer.echo(f"stratification failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1402_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1402_MAX_CANONICAL_RESULT_BYTES)
        ...
        typer.echo("verification failed: M14-02 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1402_app"]
