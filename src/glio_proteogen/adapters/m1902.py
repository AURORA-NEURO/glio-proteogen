"""Strict FastAPI and Typer adapters for provisional M19-02."""
...
from glio_proteogen.contracts.m19_02.schema import ContractName, contract_json_schema
...
_REQUEST_ADAPTER = TypeAdapter(AlignProteotypeSourcesRequest)
_RESULT_ADAPTER = TypeAdapter(ProteotypeAlignmentResult)
_SERVICE = m1902.M1902Service()
...
async def _validated_body(request: Request) -> AlignProteotypeSourcesRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1902_MAX_CANONICAL_REQUEST_BYTES)
        m1902.preflight_m1902_authorization(decoded)
    ...
    except m1902.M1902AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m19-02/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(cast("ContractName", name))
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M19-02 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M19-02/align")
async def align(request: Request) -> JSONResponse:
    validated = await _validated_body(request)
    try:
        result = _SERVICE.align(validated)
    except (ValueError, ValidationError) as error:
        raise _json_error(422, "M19-02 alignment failed") from error
    return JSONResponse(result.model_dump(mode="json"))


@app.post("/v1/modules/M19-02/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1902_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.replay(result)
    except (StrictJsonError, ValidationError, m1902.M1902ReplayError) as error:
        raise _json_error(422, "M19-02 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> AlignProteotypeSourcesRequest:
    try:
        raw = read_bounded(path, M1902_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1902_MAX_CANONICAL_REQUEST_BYTES)
        m1902.preflight_m1902_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (
        OSError,
        ...
        StrictJsonError,
        ValidationError,
        m1902.M1902AuthorizationError,
    ) as error:
        raise typer.BadParameter("invalid M19-02 request") from error  # noqa: TRY003


def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1902_MAX_CANONICAL_RESULT_BYTES)


@m1902_app.command("export-schema")
def export_schema(name: Annotated[str, typer.Argument(help="M19-02 schema name.")]) -> None:
    try:
        typer.echo(
            json.dumps(contract_json_schema(cast("ContractName", name)), indent=2, sort_keys=True)
        )
    except (KeyError, ValueError) as error:
        typer.echo("unknown M19-02 schema", err=True)
        raise typer.Exit(code=2) from error


@m1902_app.command("align")
def align_command(
    ...
    try:
        ...
        typer.echo("alignment failed: M19-02 request is invalid", err=True)
        raise typer.Exit(code=1) from error


@m1902_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1902_MAX_CANONICAL_RESULT_BYTES)
        ...
        StrictJsonError,
        ValidationError,
        m1902.M1902ReplayError,
    ) as error:
        typer.echo("verification failed: M19-02 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["align", "app", "m1902_app", "schema", "verify"]
