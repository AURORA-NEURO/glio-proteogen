...
def _parse_bytes(payload: bytes) -> object:
    parsed = strict_json_loads(payload, max_bytes=M1805_MAX_CANONICAL_REQUEST_BYTES)
...
def present_command(
    ...
    try:
        result = M1805Service().execute(_load_path(request))
        _emit(result, output)
    except M1805AuthorizationError as error:
...
def verify_command(
    ...
    try:
        verified = M1805Service().verify(_load_path(result))
    except Exception as error:
        typer.echo(_sanitized(error), err=True)
        raise typer.Exit(code=1) from error
