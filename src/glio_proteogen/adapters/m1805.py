"""FastAPI and Typer adapters for provisional M18-05."""
...
def _sanitized(error: Exception) -> str:
    return f"M18-05 request rejected: {type(error).__name__}"
...
def create_app(service: M1805Service | None = None) -> FastAPI:
    """Create an isolated M18-05 API."""

    operation = service or M1805Service()
    api = FastAPI(title="GLIO-PROTEOGEN M18-05", version="0.1.0-provisional")
    api.add_middleware(
        RequestSizeLimitMiddleware,
        max_bytes=M1805_MAX_CANONICAL_REQUEST_BYTES,
        result_max_bytes=M1805_MAX_CANONICAL_RESULT_BYTES,
    )

    @api.get("/v1/m18-05/schema/{name}")
    async def schema(name: str) -> JSONResponse:
        try:
            return JSONResponse(contract_json_schema(name))  # type: ignore[arg-type]
        except (KeyError, ValueError, TypeError) as error:
            raise HTTPException(status_code=404, detail="unknown M18-05 contract") from error

    @api.post("/v1/modules/M18-05/present")
    async def present(request: Request) -> JSONResponse:
        try:
            result = operation.execute(
                _parse_bytes(await request.body(), max_bytes=M1805_MAX_CANONICAL_REQUEST_BYTES)
            )
        except M1805AuthorizationError as error:
            raise HTTPException(status_code=403, detail="M18-05 authorization denied") from error
        except Exception as error:
            raise HTTPException(status_code=422, detail=_sanitized(error)) from error
        return JSONResponse(result.model_dump(mode="json"))

    @api.post("/v1/modules/M18-05/verify")
    async def verify(request: Request) -> JSONResponse:
        try:
            result = operation.verify(
                _parse_bytes(await request.body(), max_bytes=M1805_MAX_CANONICAL_RESULT_BYTES)
            )
        except M1805ReplayVerificationError as error:
            raise HTTPException(
                status_code=422, detail="M18-05 replay verification failed"
            ) from error
        except Exception as error:
            raise HTTPException(status_code=422, detail=_sanitized(error)) from error
        return JSONResponse(result.model_dump(mode="json"))
...
app = create_app()
cli = typer.Typer(help="Provisional M18-05 workflow presentation service.")
...
def _emit(result: object, output: str | None) -> None:
    ...
    target = Path(output)
    if target.exists():
        typer.echo("M18-05 output exists; refusing overwrite", err=True)
...
def export_schema(
    name: Annotated[ContractName, typer.Argument(help="M18-05 contract name.")],
) -> None:
...
def present_command(
    ...
    try:
        result = M1805Service().execute(
            _load_path(request, max_bytes=M1805_MAX_CANONICAL_REQUEST_BYTES)
        )
        _emit(result, output)
    except M1805AuthorizationError as error:
        typer.echo("M18-05 authorization denied", err=True)
...
def verify_command(
    ...
    try:
        verified = M1805Service().verify(
            _load_path(result, max_bytes=M1805_MAX_CANONICAL_RESULT_BYTES)
        )
    except Exception as error:
        typer.echo(_sanitized(error), err=True)
        raise typer.Exit(code=1) from error
