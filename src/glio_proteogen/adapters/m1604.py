...
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1604_MAX_CANONICAL_REQUEST_BYTES * 2)
...
def _load_request(path: Path) -> AdaptProteinRnaDiscordanceIntendedUseRequest:
    ...
    except (OSError, StrictJsonError, ValidationError, M1604AuthorizationError) as error:
...
def verify_command(
    ...
    try:
        ...
        strict_json_loads(raw, max_bytes=M1604_MAX_CANONICAL_REQUEST_BYTES * 2)
        result = _RESULT_ADAPTER.validate_json(raw, strict=True)
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1604ReplayVerificationError) as error:
