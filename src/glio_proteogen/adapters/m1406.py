...
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1406_MAX_CANONICAL_REQUEST_BYTES * 2)
...
def _load_request(path: Path) -> SimulateProteinSubtypePerturbationsRequest:
    ...
    except (OSError, StrictJsonError, ValidationError, M1406SensitivityAuthorizationError) as error:
...
def verify_command(
    ...
    try:
        ...
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1406ReplayVerificationError) as error:
