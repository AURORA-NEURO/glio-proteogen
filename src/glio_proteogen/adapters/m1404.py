"""Standalone provisional M14-04 FastAPI and Typer adapters.
...
from glio_proteogen.modules.c14_microenvironment_protein_deconvolution.m14_04_network_state_mechanism_inference import (  # noqa: E501
...
_RESULT_ADAPTER = TypeAdapter(ProteinSubtypeMechanismInferenceResult)
_SERVICE = M1404Service()
_INVALID_REQUEST: Final = "invalid M14-04 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M14-04 network/state/mechanism inference",
    version="0.1.0-provisional",
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1404_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1404_MAX_CANONICAL_RESULT_BYTES,
)
m1404_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> InferProteinSubtypeMechanismRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1404_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except M1404MechanismAuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m14-04/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(name)  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M14-04 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M14-04/mechanism")
...
@app.post("/v1/modules/M14-04/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1404_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1404ReplayVerificationError) as error:
        raise _json_error(422, "M14-04 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> InferProteinSubtypeMechanismRequest:
    try:
        raw = read_bounded(path, M1404_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1404_MAX_CANONICAL_REQUEST_BYTES)
        preflight_mechanism_authorization(decoded)
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1404_MAX_CANONICAL_RESULT_BYTES)


@m1404_app.command("export-schema")
def export_schema(
    name: Annotated[str, typer.Argument(help="M14-04 schema name.")],
) -> None:
    try:
        typer.echo(json.dumps(contract_json_schema(name), indent=2, sort_keys=True))  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        typer.echo("unknown M14-04 schema", err=True)
        raise typer.Exit(code=2) from error


@m1404_app.command("infer")
def infer_command(
    ...
    except (M1404MechanismAuthorizationError, OSError, typer.BadParameter) as error:
        typer.echo(f"inference failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1404_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1404_MAX_CANONICAL_REQUEST_BYTES * 2)
        ...
        typer.echo("verification failed: M14-04 result is invalid", err=True)
        raise typer.Exit(code=1) from error
