"""Standalone provisional M13-05 FastAPI and Typer adapters.
...
_REQUEST_ADAPTER = TypeAdapter(ModelProteotypeLongitudinalEvolutionRequest)
_RESULT_ADAPTER = TypeAdapter(ProteotypeLongitudinalEvolutionResult)
_SERVICE = M1305Service()
_INVALID_REQUEST: Final = "invalid M13-05 request"
_OUTPUT_EXISTS: Final = "output already exists"

app = FastAPI(
    title="GLIO-PROTEOGEN M13-05 longitudinal and evolutionary model",
    version="0.1.0-provisional",
)
app.add_middleware(
    RequestSizeLimitMiddleware,
    max_bytes=M1305_MAX_CANONICAL_REQUEST_BYTES,
    result_max_bytes=M1305_MAX_CANONICAL_RESULT_BYTES,
)
m1305_app = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)
...
async def _validated_body(request: Request) -> ModelProteotypeLongitudinalEvolutionRequest:
    ...
    try:
        ...
        decoded = strict_json_loads(body, max_bytes=M1305_MAX_CANONICAL_REQUEST_BYTES)
    ...
    except M1305AuthorizationError as error:
        raise _json_error(403, str(error)) from error


@app.get("/v1/m13-05/schema/{name}")
def schema(name: str) -> JSONResponse:
    try:
        document = contract_json_schema(name)  # type: ignore[arg-type]
    except (KeyError, ValueError) as error:
        raise _json_error(404, "unknown M13-05 contract schema") from error
    return JSONResponse(document)


@app.post("/v1/modules/M13-05/longitudinal")
...
@app.post("/v1/modules/M13-05/verify")
async def verify(request: Request) -> JSONResponse:
    ...
    try:
        body = await request.body()
        strict_json_loads(body, max_bytes=M1305_MAX_CANONICAL_RESULT_BYTES)
        result = _RESULT_ADAPTER.validate_json(body, strict=True)
        verified = _SERVICE.verify(result)
    except (StrictJsonError, ValidationError, M1305ReplayVerificationError) as error:
        raise _json_error(422, "M13-05 result verification failed") from error
    return JSONResponse(verified.model_dump(mode="json"))


def _load_request(path: Path) -> ModelProteotypeLongitudinalEvolutionRequest:
    try:
        raw = read_bounded(path, M1305_MAX_CANONICAL_REQUEST_BYTES)
        decoded = strict_json_loads(raw, max_bytes=M1305_MAX_CANONICAL_REQUEST_BYTES)
        preflight_longitudinal_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (
        OSError,
...
def _read_result(path: Path) -> bytes:
    return read_bounded(path, M1305_MAX_CANONICAL_RESULT_BYTES)


@m1305_app.command("export-schema")
def export_schema(
    name: Annotated[str, typer.Argument(help="M13-05 schema name.")],
) -> None:
    try:
        typer.echo(
        ...
        typer.echo("unknown M13-05 schema", err=True)
        raise typer.Exit(code=2) from error


@m1305_app.command("infer")
def infer_command(
    ...
    except (M1305AuthorizationError, OSError, typer.BadParameter) as error:
        typer.echo(f"inference failed: {error}", err=True)
        raise typer.Exit(code=1) from error


@m1305_app.command("verify")
def verify_command(
    result_path: Annotated[Path, typer.Argument(exists=True, readable=True)],
) -> None:
    try:
        raw = _read_result(result_path)
        strict_json_loads(raw, max_bytes=M1305_MAX_CANONICAL_RESULT_BYTES)
        ...
        typer.echo("verification failed: M13-05 result is invalid", err=True)
        raise typer.Exit(code=1) from error
    typer.echo(canonical_json_bytes(verified).decode("utf-8"))


__all__ = ["app", "m1305_app"]
