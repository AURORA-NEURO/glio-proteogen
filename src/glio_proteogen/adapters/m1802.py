"""FastAPI and Typer adapters for provisional M18-02."""
...
def create_app(service: M1802Service | None = None) -> FastAPI:
    """Create an isolated M18-02 API."""

    operation = service or M1802Service()
    api = FastAPI(title="GLIO-PROTEOGEN M18-02", version="0.1.0-provisional")
    api.add_middleware(
        RequestSizeLimitMiddleware,
        max_bytes=M1802_MAX_CANONICAL_REQUEST_BYTES,
        result_max_bytes=M1802_MAX_CANONICAL_RESULT_BYTES,
    )

    @api.get("/v1/m18-02/schema/{name}")
    async def schema(name: str) -> JSONResponse:
        try:
            return JSONResponse(contract_json_schema(name))  # type: ignore[arg-type]
        except (KeyError, ValueError, TypeError) as error:
            raise HTTPException(status_code=404, detail="unknown M18-02 contract") from error
...
def _emit(result: object, output: str | None) -> None:
    ...
    target = Path(output)
    if target.exists():
        typer.echo("M18-02 output exists; refusing overwrite", err=True)
...
def export_schema(
    name: Annotated[ContractName, typer.Argument(help="M18-02 contract name.")],
) -> None:
