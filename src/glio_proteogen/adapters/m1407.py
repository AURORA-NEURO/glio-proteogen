...
def _load_request(path: Path) -> AdjudicateProteinSubtypePlausibilityRequest:
    try:
        ...
        preflight_plausibility_authorization(decoded)
        return _REQUEST_ADAPTER.validate_json(canonical_json_bytes(decoded), strict=True)
    except (OSError, StrictJsonError, ValidationError, M1407AuthorizationError) as error:
...
def verify_command(
    ...
    try:
        ...
        verified = _SERVICE.verify(result)
    except (OSError, StrictJsonError, ValidationError, M1407ReplayVerificationError) as error:
