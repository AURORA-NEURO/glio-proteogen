"""Bounded benchmark for the public M18-03 fusion engine."""
...
def run_benchmark() -> BenchmarkReport:
    ...
    for _ in range(ITERATIONS):
        ...
        if result != warmup:
            raise RuntimeError("M18-03 benchmark result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[(95 * len(ordered) - 1) // 100]
    mean = fmean(samples)
    return BenchmarkReport(
        module_id="GLIO-PROTEOGEN-M18-03",
        contract_version="0.1.0-provisional",
        workload="component_specific_reliability_weighted_fusion",
        timed_boundary="M1803Engine.adapt_only",
