"""Benchmark only public M03-07 routing on one prepared genuine chain."""
...
from evals.m03_07.run import build_scenario
from glio_proteogen.contracts.m03_07 import ProteinInferenceSupportDisposition
...
def run_benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    """Prepare genuine M03-04/M03-06 results before timing only M03-07."""

    if iterations < 1:
        raise InvalidIterationCountError
    request = build_scenario().request
    warmup = route_protein_inference_support(request)
    dimension_count = sum(len(assessment.dimensions) for assessment in warmup.envelope_assessments)
    if (
        warmup.disposition is not ProteinInferenceSupportDisposition.SUPPORTED
