...
def run() -> dict[str, object]:
    ...
    checks = {
        "no_fabricated_stream": result.status is TelemetryStatus.ABSTAINED
        and result.telemetry_stream is None
        and result.dashboards == ()
        and result.alert is None,
        "observations_abstained": result.safe_failure_report is not None
        and result.safe_failure_report.trigger == "telemetry_observations_missing",
