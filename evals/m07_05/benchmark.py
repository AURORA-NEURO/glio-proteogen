"""Small deterministic benchmark for M07-05 constraint integration."""
...
def benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    ...
    for _ in range(iterations):
        ...
        if built.canonical_bytes != expected.canonical_bytes:
            raise RuntimeError("constraint result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[min(len(ordered) - 1, (95 * len(ordered) - 1) // 100)]
    return BenchmarkReport(
        module_id="GLIO-PROTEOGEN-M07-05",
        contract_version="0.1.0-provisional",
        workload="hard and soft proteotype constraint integration",
        timed_boundary="M0705ConstraintEngine.integrate",
        ...
        constraint_count=len(request.constraint_set.constraints),
        feature_count=len(request.feature_artifacts),
