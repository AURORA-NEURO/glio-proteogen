"""Frozen caller-declared M22-03 benchmark fixture."""
...
def _evidence(name: str) -> EvidenceReference:
    return EvidenceReference(
        ...
        role="evidence",
        claim="Frozen M22-03 benchmark fixture evidence.",
    )


def _context(request_id: str) -> ExecutionContext:
    artifact = _artifact("m2203.context")
    accepted = UpstreamDecisionReference(
        decision_id="m2203.accepted",
    ...
    return ExecutionContext(
        ...
        actor_id="m2203.fixture.actor",
        occurred_at=datetime(2026, 8, 16, tzinfo=UTC),
        references=ContextReferences(
            approved_configuration=accepted,
            identity_lineage=IdentityLineageReference(
                decision_id="m2203.identity",
...
def build_request() -> RunProteinRnaDiscordanceInternalBenchmarkRequest:
    """Return the frozen normal benchmark request."""

    upstream = _artifact("m2202.synthetic.truth", M2203_M2202_INPUT_MEDIA_TYPE)
    return RunProteinRnaDiscordanceInternalBenchmarkRequest(
        request_id="m2203.fixture.request",
        context=_context("m2203.fixture.request"),
        upstream_result=upstream,
        split=LockedSplit(
            split_id="m2203.locked.split",
            version="1.0.0",
            train_examples=10,
            validation_examples=5,
            test_examples=5,
            random_seed=17,
            evidence=(_evidence("m2203.split.evidence"),),
        ),
        baseline_runs=(
            _baseline("m2203.simple", BaselineKind.SIMPLE),
            _baseline("m2203.mature", BaselineKind.MATURE),
        ),
        ablations=(
            ComponentAblation(
                ablation_id="m2203.pathway.ablation",
                ...
                compute_units=10.0,
                status=ValidationStatus.PASS,
                evidence=(_evidence("m2203.ablation.evidence"),),
            ),
        ),
        comparisons=(
            ComputeMatchedComparison(
                comparison_id="m2203.simple.vs.mature",
                reference_run_id="m2203.simple",
                candidate_run_id="m2203.mature",
                ...
                candidate_score=0.7,
                status=ValidationStatus.PASS,
                evidence=(_evidence("m2203.comparison.evidence"),),
            ),
        ),
        source_artifacts=(upstream, _artifact("m2203.benchmark.material")),
    )


def denied_request() -> RunProteinRnaDiscordanceInternalBenchmarkRequest:
