"""Executable evaluator and adversarial evidence for M19-08."""
...
SCENARIO_PATH: Final = ROOT / "tests" / "fixtures" / "m19_08" / "scenarios.json"
MODULE_ID: Final = "GLIO-PROTEOGEN-M19-08"
...
def _consent_denied_request() -> MonitorProteotypeTranslationHealthRequest:
    request = _request()
...
def _scenario(name: str) -> MonitorProteotypeTranslationHealthRequest:  # noqa: PLR0911
    if name == "healthy":
        return _request()
    if name == "degraded":
        return _request(
            telemetry=(
                _request().telemetry[0].model_copy(update={"status": ObservationStatus.WARNING}),
            )
        )
    ...
    if name == "upstream_media_rejected":
        upstream = _request().upstream_result.model_copy(update={"media_type": "application/json"})
        return _request().model_copy(update={"upstream_result": upstream})
    if name == "replay_tamper":
        return _request()
    raise ValueError(f"unknown M19-08 evaluator scenario: {name}")  # noqa: TRY003
...
def evaluate() -> EvaluationReport:
    ...
    for name in names:
        try:
            result = engine.infer(_scenario(name))
        except (m1908.M1908AuthorizationError, ValidationError):
    ...
    healthy = engine.infer(_scenario("healthy"))
    checks.extend(
        (
            EvalCheck(
    ...
    rollback = engine.infer(_scenario("rollback_required"))
    checks.append(
        EvalCheck(
    ...
    tampered = engine.infer(_scenario("replay_tamper")).model_copy(
        update={"human_review_required": True}
    )
    replay_denied = False
    try:
        engine.replay(tampered)
    except m1908.M1908ReplayVerificationError:
        replay_denied = True
    ...
    for request, expected_state in (
        (_scenario("rollback_required"), "rollback_required"),
        (
            _request(
                support_drift=(
                    _request()
    ...
    try:
        engine.infer(_consent_denied_request())
    except m1908.M1908AuthorizationError:
        adversarial_passed += 1
    try:
        engine.infer(_scenario("upstream_media_rejected"))
