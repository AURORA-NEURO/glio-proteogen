"""Frozen caller-declared M21-01 scenarios.

The fixture intentionally carries no issuer or biological truth claim. It
only exercises the locked package shape, seven-control preflight, and safe
abstention paths required by the provisional dossier boundary.
"""
...
def evidence(name: str) -> EvidenceReference:
    return EvidenceReference(
        ...
        role="evidence",
        claim="Frozen caller-declared benchmark evidence.",
    )
...
def _uncertainty() -> UncertaintyProfile:
    unavailable = UncertaintyEstimate(
        ...
        rationale="M21-01 does not infer uncertainty from fixture content.",
    )
...
def build_request() -> CurateComplexActivityReferenceTruthRequest:
    references = (
    ...
    return CurateComplexActivityReferenceTruthRequest(
        ...
        endpoint=EndpointDefinition(
            ...
            name="Frozen complex activity endpoint",
...
def pending_request() -> CurateComplexActivityReferenceTruthRequest:
...
def denied_request() -> CurateComplexActivityReferenceTruthRequest:
    request = build_request()
...
__all__ = ["build_request", "denied_request", "pending_request"]
