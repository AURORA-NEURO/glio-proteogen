"""Bounded benchmark for the public M17-08 monitor."""
...
def run_benchmark() -> BenchmarkReport:
    ...
    for _ in range(ITERATIONS):
        ...
        if result != warmup:
            raise RuntimeError("M17-08 benchmark result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[(95 * len(ordered) - 1) // 100]
    mean = fmean(samples)
    return BenchmarkReport(
        module_id="GLIO-PROTEOGEN-M17-08",
        contract_version="0.1.0-provisional",
        workload="translation_health_monitoring",
        timed_boundary="M1708Engine.adapt_only",
