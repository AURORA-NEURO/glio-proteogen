"""Executable evaluator and adversarial evidence for M17-08."""
...
SCENARIO_PATH: Final = ROOT / "tests" / "fixtures" / "m17_08" / "scenarios.json"
MODULE_ID: Final = "GLIO-PROTEOGEN-M17-08"
...
def _consent_denied_request() -> MonitorVariantPeptideTranslationHealthRequest:
    request = _request()
...
def _scenario(name: str) -> MonitorVariantPeptideTranslationHealthRequest:  # noqa: PLR0911
    if name == "healthy":
        return _request()
    if name == "degraded":
        return _request(telemetry_status=ObservationStatus.WARNING)
    if name == "rollback_required":
        return _request(telemetry_status=ObservationStatus.FAIL)
    ...
    if name == "replay_tamper":
        return _request()
    raise ValueError(f"unknown M17-08 evaluator scenario: {name}")  # noqa: TRY003
...
def evaluate() -> EvaluationReport:
    ...
    engine = m1708.M1708Engine()
    scenario_oracles_passed = True
    for name in names:
        try:
            result = engine.adapt(_scenario(name))
        except (m1708.M1708AuthorizationError, ValidationError):
    ...
    for request, expected_state in (
        (_request(telemetry_status=ObservationStatus.FAIL), "rollback_required"),
        (_request(support_status=ObservationStatus.FAIL), "rollback_required"),
        (_request(workflow_status=ObservationStatus.FAIL), "rollback_required"),
    ...
    except m1708.M1708AuthorizationError:
