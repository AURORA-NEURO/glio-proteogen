"""Frozen caller-declared M23-03 benchmark fixture."""
...
def _evidence(name: str) -> EvidenceReference:
    return EvidenceReference(
        ...
        role="evidence",
        claim="Frozen M23-03 internal benchmark fixture evidence.",
    )


def _context(request_id: str) -> ExecutionContext:
    artifact = _artifact("m2303.context")
    accepted = UpstreamDecisionReference(
        decision_id="m2303.accepted",
    ...
    return ExecutionContext(
        ...
        actor_id="m2303.fixture.actor",
        occurred_at=datetime(2026, 8, 16, tzinfo=UTC),
        references=ContextReferences(
            approved_configuration=accepted,
            identity_lineage=IdentityLineageReference(
                decision_id="m2303.identity",
...
def build_request() -> RunVariantPeptideInternalBenchmarkRequest:
    """Return the frozen normal benchmark request."""

    upstream = _artifact("m2303.m2302.synthetic.truth", M2303_M2302_INPUT_MEDIA_TYPE)
    return RunVariantPeptideInternalBenchmarkRequest(
        request_id="m2303.fixture.request",
        context=_context("m2303.fixture.request"),
        upstream_result=upstream,
        split=LockedSplit(
            split_id="m2303.locked.split",
            version="1.0.0",
            train_examples=10,
            validation_examples=5,
            test_examples=5,
            random_seed=17,
            evidence=(_evidence("m2303.split.evidence"),),
        ),
        baseline_runs=(
            _baseline("m2303.simple", BaselineKind.SIMPLE),
            _baseline("m2303.mature", BaselineKind.MATURE),
        ),
        ablations=(
            ComponentAblation(
                ablation_id="m2303.pathway.ablation",
                ...
                evidence=(_evidence("m2303.ablation.evidence"),),
            ),
        ),
        comparisons=(
            ComputeMatchedComparison(
                comparison_id="m2303.simple.vs.mature",
                reference_run_id="m2303.simple",
                candidate_run_id="m2303.mature",
                ...
                status=ValidationStatus.PASS,
                evidence=(_evidence("m2303.comparison.evidence"),),
            ),
        ),
        source_artifacts=(upstream, _artifact("m2303.benchmark.material")),
    )


def denied_request() -> RunVariantPeptideInternalBenchmarkRequest:
...
def not_evaluable_request() -> RunVariantPeptideInternalBenchmarkRequest:
