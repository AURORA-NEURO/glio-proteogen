"""Benchmark only public M03-04 computation on one fully prepared genuine chain."""
...
def run_benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    """Prepare M01-02 through M03-03 and the ledger before timing only M03-04."""

    if iterations < 1:
        raise InvalidIterationCountError
    request = build_scenario().request
    warmup = compute_protein_inference_quality(request)
    if (
        warmup.disposition is not ProteinInferenceQualityDisposition.QUALIFIED
        or len(warmup.metrics) != len(ProteinInferenceQualityMetricCode)
    ):
    ...
    return BenchmarkReport(
        ...
        workload="genuine_m0102_m0301_m0302_m0303_prepared_m0304_quality",
        timed_boundary="compute_protein_inference_quality_only",
        iterations=iterations,
        metric_count=len(warmup.metrics),
