"""Benchmark only public M03-06 harmonization on one fully prepared genuine chain."""
...
from evals.m03_06.run import build_scenario
from glio_proteogen.contracts.m03_06 import ProteinInferenceHarmonizationDisposition
...
def run_benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    """Prepare M01-02 through M03-05 and the support ledger before timing M03-06."""

    if iterations < 1:
        raise InvalidIterationCountError
    request = build_scenario().request
    ledger = request.support_ledger
    warmup = harmonize_protein_inference_support(request)
    manifest = warmup.transformation_manifest
    if (
        ledger is None
        or manifest is None
        or warmup.disposition is not ProteinInferenceHarmonizationDisposition.ACCEPTED
    ...
    return BenchmarkReport(
        ...
        workload="genuine_m0102_through_m0305_prepared_fixed_point_support_ledger",
        timed_boundary="harmonize_protein_inference_support_only",
