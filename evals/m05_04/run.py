"""Executable evidence and genuine public builders for M05-04 quality computation."""
...
import glio_proteogen.modules.c05_ptm_localization.m05_04_quality_metrics.engine as m0504_engine
from evals.m05_03.run import (
    _genuine_scenario as build_m0503_genuine_scenario,
)
from evals.m05_03.run import (
    _with_document_updates as with_m0503_document_updates,
)
...
from glio_proteogen.contracts.m05_03 import (
    result_payload_digest as m0503_result_payload_digest,
)
...
MODULE_ID: Final = "GLIO-PROTEOGEN-M05-04"
SCENARIO_PATH: Final = Path("tests/fixtures/m05_04/scenarios.json")
...
EXPECTED_HOSTILE_DICT_SUBCLASS_COUNT: Final = 2
...
class Scenario:
    """One strict metadata-only M05-04 computation request."""
...
class _InvalidFixtureError(RuntimeError):
    def __init__(self) -> None:
        super().__init__("M05-04 fixture root must be an exact JSON object")


def _oid(namespace: str, label: object) -> str:
    digest = sha256_digest({"m0504_fixture": str(label)}).removeprefix("sha256:")
...
def _reference(label: str, *, media_type: str) -> ArtifactReference:
    return ArtifactReference(
        ...
        version="1.0.0",
        digest=sha256_digest({"m0504_evidence": label}),
        media_type=media_type,
    )


def _thresholds() -> tuple[PtmLocalizationQualityThreshold, ...]:
    return tuple(
        PtmLocalizationQualityThreshold(
            metric_code=code,
            direction=(
                PtmLocalizationQualityMetricDirection.AT_MOST
                if code is PtmLocalizationQualityMetricCode.DETECTION_LIMIT_BURDEN
                else PtmLocalizationQualityMetricDirection.AT_LEAST
            ),
            pass_threshold_ppm=(
                0 if code is PtmLocalizationQualityMetricCode.DETECTION_LIMIT_BURDEN else 666_667
            ),
...
def _profile_for(
    source: ValidatedPtmLocalizationRawInput,
    ...
    *,
    ...
    return PtmLocalizationAssayQualityProfile(
        profile_id=_oid("profile", f"{role.value}-{ordinal}"),
        version=version,
        role=role,
        assay_kind=(
            cast("MassSpectrometryProteomeInputDocument", document).assay_kind
            if role is PtmLocalizationRawInputRole.MASS_SPECTROMETRY_PROTEOME
            else None
        ),
        support_domain=(
            cast("MassSpectrometryProteomeInputDocument", document).support_domain
            if role is PtmLocalizationRawInputRole.MASS_SPECTROMETRY_PROTEOME
            else None
        ),
        ...
        approved_unit_system_versions=(document.unit_system_version if canonical else version,),
...
def _counts() -> PtmLocalizationQualityRoleCounts:
    return PtmLocalizationQualityRoleCounts(
...
def _states() -> PtmLocalizationQualityRoleFactStates:
    observed = PtmLocalizationQualityObservationState.OBSERVED
    return PtmLocalizationQualityRoleFactStates(
...
def _fact(source: ValidatedPtmLocalizationRawInput) -> PtmLocalizationQualityRoleFacts:
    document = source.document
    role = source.role
    return PtmLocalizationQualityRoleFacts(
...
def _ledger(
    ...
    return PtmLocalizationQualityFactLedger.model_validate(payload, strict=True)
...
def _request_from_raw_result(
    ...
    """Bind one genuine M05-03 result into the exact M05-04 request graph."""
    ...
    return ComputePtmLocalizationQualityMetricsRequest(
...
def _base_request(
    ...
    m0503 = (
        build_m0503_scenario()
        if upstream_case_id == "canonical_four_role_documents_validated"
        else build_m0503_genuine_scenario(upstream_case_id)
    )
    raw_result = ingest_ptm_localization_raw_inputs(m0503.request, m0503.artifacts_by_role)
...
def build_scenario_request(
    ...
    """Build one strict request through genuine public M01-02 to M05-03 operations."""

    upstream_cases = {
        "canonical_four_role_quality_qualified": "canonical_four_role_documents_validated",
        "quarantined_upstream_zero_ledger_traversal": "upstream_protocol_quarantined",
        "abstained_upstream_zero_ledger_traversal": "upstream_identity_unresolved",
    }
    try:
...
def build_maximum_scenario() -> Scenario:
    ...
    return Scenario(request=_base_request(M0504_MAX_PROFILES))
...
def _inventory(corpus: dict[str, Any]) -> tuple[list[str], tuple[int, ...], EvalCheck]:
    ...
    closed = (
        ...
        and corpus.get("expected_group_count") == EXPECTED_GROUP_COUNT
        and corpus.get("expected_total_case_count") == EXPECTED_CASE_COUNT
        and tuple(cast("list[int]", corpus.get("expected_case_allocation"))) == EXPECTED_ALLOCATION
        ...
        and all(
            ...
            and group.get("expected_case_count") == len(cast("list[object]", group["case_ids"]))
            for group in groups
        )
    )
    return (
...
def _request_payload(
    request: ComputePtmLocalizationQualityMetricsRequest,
    *,
    policy: PtmLocalizationQualityPolicy | None = None,
    ledger: PtmLocalizationQualityFactLedger | object | None = ...,
) -> dict[str, object]:
    ...
    if ledger is not ...:
        payload["fact_ledger"] = (
            ...
            if isinstance(ledger, PtmLocalizationQualityFactLedger)
            else ledger
        )
...
def _with_policy(
    ...
    return ComputePtmLocalizationQualityMetricsRequest.model_validate_json(
...
def _with_fact(
    request: ComputePtmLocalizationQualityMetricsRequest,
    fact: PtmLocalizationQualityRoleFacts,
) -> ComputePtmLocalizationQualityMetricsRequest:
    ...
    payload["ledger_digest"] = fact_ledger_digest(payload)
    resealed = PtmLocalizationQualityFactLedger.model_validate_json(
...
def _mutated_fact(
    ...
    *,
    ...
    return PtmLocalizationQualityRoleFacts.model_validate_json(
        canonical_json_bytes(payload), strict=True
    )
...
def _with_threshold(
    request: ComputePtmLocalizationQualityMetricsRequest,
    code: PtmLocalizationQualityMetricCode,
    **updates: object,
) -> ComputePtmLocalizationQualityMetricsRequest:
    ...
    replacement = PtmLocalizationAssayQualityProfile.model_validate_json(
...
def _codes(result: PtmLocalizationQualityResult) -> set[PtmLocalizationQualityFindingCode]:
...
def _safe_failure_rejects_without_ledger_access(
    ...
    rejected = False
    try:
        compute_ptm_localization_quality_metrics(payload)
...
def _semantic_checks() -> dict[str, EvalCheck]:  # noqa: C901, PLR0912, PLR0915
    ...
    result = compute_ptm_localization_quality_metrics(request)
    ...
    add(
        "exact_m0503_full_result_replay",
        ...
        "embedded full M05-03 result and receipt replay",
    )
    add(
    ...
    add(
        ...
        len(result.assay_quality) == M0504_ROLE_COUNT
        and all(len(item.metrics) == M0504_METRIC_COUNT for item in result.assay_quality)
        and len(metrics) == M0504_COMPUTED_METRIC_COUNT,
    ...
    add(
        ...
        and result.parent_target == "variant_peptide"
    ...
    completeness = _metric(result, PtmLocalizationQualityMetricCode.RAW_INPUT_COMPLETENESS)
    add(
        ...
        and completeness.status is PtmLocalizationQualityMetricStatus.PASS,
    ...
    zero_result = compute_ptm_localization_quality_metrics(_with_fact(request, zero_fact))
    zero_metric = _metric(
        zero_result,
    ...
    add(
        ...
        and zero_metric.status is PtmLocalizationQualityMetricStatus.NOT_EVALUABLE,
    ...
    censored_result = compute_ptm_localization_quality_metrics(_with_fact(request, censored_fact))
    ...
    add(
        ...
        censored_metric.observation_state is PtmLocalizationQualityObservationState.CENSORED
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityRoleFacts, fact_payload),
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityRoleCounts, count_payload),
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityFactLedger, ledger_payload),
        "stale ledger self-digest is rejected",
    )
    mismatched_fact = _mutated_fact(request, input_id=_oid("input", "wrong-binding"))
    mismatch_result = compute_ptm_localization_quality_metrics(_with_fact(request, mismatched_fact))
    add(
        "role_fact_binding_mismatch_quarantines",
        mismatch_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and mismatch_result.assay_quality == ()
        and _codes(mismatch_result)
    ...
    reordered = ComputePtmLocalizationQualityMetricsRequest.model_validate_json(
        canonical_json_bytes(reordered_payload), strict=True
    )
    add(
        "semantic_role_order_full_equality",
        compute_ptm_localization_quality_metrics(reordered) == result,
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityPolicy, overlap_payload),
    ...
    missing_profile_result = compute_ptm_localization_quality_metrics(
        _with_fact(request, no_control_fact)
    )
    add(
        "missing_role_profile_abstains",
        missing_profile_result.disposition is PtmLocalizationQualityDisposition.ABSTAINED
    ...
    version_cases = (
        ...
        (
            "unit_system_version_mismatch_quarantines",
    ...
    for case_id, field, code in version_cases:
        version_result = compute_ptm_localization_quality_metrics(
            _with_profile_update(request, **{field: ("9.9.9",)})
        )
        add(
            case_id,
            version_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
            and code in _codes(version_result),
            f"codes={sorted(item.value for item in _codes(version_result))}",
        )
    applicability_result = compute_ptm_localization_quality_metrics(
        _with_profile_update(request, assay_kind="targeted_mass_spectrometry")
    )
    add(
        "proteome_assay_kind_mismatch_abstains",
        applicability_result.disposition is PtmLocalizationQualityDisposition.ABSTAINED
        and PtmLocalizationQualityFindingCode.ASSAY_PROFILE_UNSUPPORTED
        in _codes(applicability_result),
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityThreshold, threshold_payload),
    ...
    add(
        ...
        _rejects(PtmLocalizationAssayQualityProfile, profile_payload),
    ...
    add(
        ...
        result.disposition is PtmLocalizationQualityDisposition.QUALIFIED
        and all(item.status is PtmLocalizationQualityMetricStatus.PASS for item in metrics),
    ...
    warning_result = compute_ptm_localization_quality_metrics(warning_request)
    add(
        "required_metric_warning_quarantines",
        warning_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and PtmLocalizationQualityFindingCode.REQUIRED_METRIC_WARNING in _codes(warning_result),
    ...
    failure_result = compute_ptm_localization_quality_metrics(failure_request)
    add(
        "required_metric_failure_quarantines",
        failure_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and PtmLocalizationQualityFindingCode.METRIC_THRESHOLD_FAILED in _codes(failure_result),
        f"disposition={failure_result.disposition.value}",
    )
    optional_warning_result = compute_ptm_localization_quality_metrics(
        _with_threshold(
            request,
            PtmLocalizationQualityMetricCode.RAW_INPUT_COMPLETENESS,
            pass_threshold_ppm=700_000,
    ...
    add(
        ...
        optional_warning_result.disposition is PtmLocalizationQualityDisposition.QUALIFIED
        and optional_warning_result.support.reason_code
        == "ptm_localization_quality_qualified_with_optional_warning",
        optional_warning_result.support.reason_code,
    )
    optional_failure_result = compute_ptm_localization_quality_metrics(
        _with_threshold(
            request,
            PtmLocalizationQualityMetricCode.RAW_INPUT_COMPLETENESS,
            pass_threshold_ppm=800_000,
    ...
    add(
        ...
        optional_failure_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and PtmLocalizationQualityFindingCode.METRIC_THRESHOLD_FAILED
        in _codes(optional_failure_result),
        f"disposition={optional_failure_result.disposition.value}",
    )
    state_results: dict[str, PtmLocalizationQualityResult] = {}
    for state in ("missing", "unsupported"):
        ...
        state_results[state] = compute_ptm_localization_quality_metrics(
            _with_fact(request, state_fact)
        )
    add(
        "required_metric_missing_abstains",
        state_results["missing"].disposition is PtmLocalizationQualityDisposition.ABSTAINED
        and PtmLocalizationQualityFindingCode.REQUIRED_METRIC_MISSING
        in _codes(state_results["missing"]),
        f"disposition={state_results['missing'].disposition.value}",
    )
    add(
        "required_metric_unsupported_abstains",
        state_results["unsupported"].disposition is PtmLocalizationQualityDisposition.ABSTAINED
        and PtmLocalizationQualityFindingCode.REQUIRED_METRIC_UNSUPPORTED
    ...
    add(
        ...
        zero_result.disposition is PtmLocalizationQualityDisposition.ABSTAINED
        and PtmLocalizationQualityFindingCode.REQUIRED_METRIC_NOT_EVALUABLE in _codes(zero_result),
    ...
    precedence_result = compute_ptm_localization_quality_metrics(
        _with_fact(failure_request, precedence_fact)
    )
    add(
        "quarantine_precedes_abstention",
        precedence_result.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and PtmLocalizationQualityFindingCode.METRIC_THRESHOLD_FAILED in _codes(precedence_result)
        and PtmLocalizationQualityFindingCode.REQUIRED_METRIC_MISSING in _codes(precedence_result),
    ...
    quarantined = compute_ptm_localization_quality_metrics(quarantined_request)
    abstained_request = build_scenario_request("abstained_upstream_zero_ledger_traversal")
    abstained = compute_ptm_localization_quality_metrics(abstained_request)
    add(
        "quarantined_upstream_zero_ledger_traversal",
        quarantined_request.fact_ledger is None
        and quarantined.disposition is PtmLocalizationQualityDisposition.QUARANTINED
        and quarantined.assay_quality == ()
        and _safe_failure_rejects_without_ledger_access(quarantined_request),
        "genuine quarantined M05-03 yields zero metrics and never traverses a supplied ledger",
    )
    add(
        "abstained_upstream_zero_ledger_traversal",
        abstained_request.fact_ledger is None
        and abstained.disposition is PtmLocalizationQualityDisposition.ABSTAINED
        and abstained.assay_quality == ()
        and _safe_failure_rejects_without_ledger_access(abstained_request),
        "genuine abstained M05-03 yields zero metrics and never traverses a supplied ledger",
    )
    stale_bindings = (
    ...
    for case_id, role, path in stale_bindings:
        ...
        add(
            case_id,
            _rejects(ComputePtmLocalizationQualityMetricsRequest, stale),
    ...
    upstream["result_id"] = "result.m0503." + ("f" * 64)
    upstream["result_digest"] = m0503_result_payload_digest(upstream)
    add(
        "resigned_m0503_full_result_forgery_rejected",
        _rejects(ComputePtmLocalizationQualityMetricsRequest, resigned),
    ...
    duplicate_fact = PtmLocalizationQualityRoleFacts.model_validate_json(
        canonical_json_bytes(duplicate_payload), strict=True
    )
    duplicate_result = compute_ptm_localization_quality_metrics(_with_fact(request, duplicate_fact))
    ...
    add(
        ...
        and missing_metric.status is PtmLocalizationQualityMetricStatus.NOT_EVALUABLE,
        "missing observation remains nonnumeric and not evaluable",
    )
    add(
        "safe_failure_minimum_12_evidence",
        len(quarantined.evidence) == M0504_MIN_EVIDENCE
        and len(abstained.evidence) == M0504_MIN_EVIDENCE,
        f"quarantined={len(quarantined.evidence)};abstained={len(abstained.evidence)}",
    )
    maximum_request = build_maximum_scenario().request
    maximum = compute_ptm_localization_quality_metrics(maximum_request)
    ...
    add(
        "maximum_45_evidence",
        maximum.disposition is PtmLocalizationQualityDisposition.QUALIFIED
        and len(maximum_request.policy.profiles) == M0504_MAX_PROFILES
        and maximum_threshold_count == M0504_MAX_PROFILES * M0504_METRIC_COUNT
        and maximum_ledger is not None
        and len(maximum_ledger.role_facts) == M0504_ROLE_COUNT
        and len(maximum.assay_quality) == M0504_ROLE_COUNT
        and maximum_metric_count == M0504_COMPUTED_METRIC_COUNT
        and len(maximum.evidence) == M0504_MAX_EVIDENCE,
        (
    ...
    canary = "CANARY_M0504_EXTERNAL_CONTENT_DO_NOT_REFLECT"
    canary_bytes = canary.encode()
    canary_base = build_m0503_scenario()
    ...
    canary_scenario = with_m0503_document_updates(
        canary_base,
        {
            canary_role: {
    ...
    canary_raw_result = ingest_ptm_localization_raw_inputs(
        canary_scenario.request,
        canary_scenario.artifacts_by_role,
    )
    canary_result = compute_ptm_localization_quality_metrics(
        _request_from_raw_result(canary_raw_result, M0504_ROLE_COUNT)
    )
    ...
    add(
        ...
        and len(result.limitations) == M0504_LIMITATION_COUNT,
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityResult, metric_forgery),
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityResult, finding_forgery),
    ...
    add(
        ...
        _rejects(PtmLocalizationQualityResult, receipt_forgery)
        and _rejects(PtmLocalizationQualityResult, digest_forgery),
    ...
    for role, state in denied_states.items():
        ...
        except PtmLocalizationQualityAuthorizationError:
            denied = True
        add(
    ...
    try:
        preflight_ptm_localization_quality_authorization(_UntouchedMapping())
    except PtmLocalizationQualityAuthorizationError:
    ...
    hostile_dict_subclasses_rejected = 0
    for hostile in (exploding, fatal):
        try:
            preflight_ptm_localization_quality_authorization(hostile)
        except PtmLocalizationQualityAuthorizationError:
            hostile_dict_subclasses_rejected += 1
    exception_failed_closed = False
    with patch.object(m0504_engine, "_member", side_effect=RuntimeError("ordinary-exception")):
        try:
            preflight_ptm_localization_quality_authorization(request)
        except PtmLocalizationQualityAuthorizationError:
            exception_failed_closed = True
    base_exception_propagated = False
    with patch.object(m0504_engine, "_member", side_effect=KeyboardInterrupt("fatal-exception")):
        try:
            preflight_ptm_localization_quality_authorization(request)
    ...
    add(
        ...
        hostile_dict_subclasses_rejected == EXPECTED_HOSTILE_DICT_SUBCLASS_COUNT
        and exception_failed_closed
        and base_exception_propagated,
        (
            f"dict_subclasses_rejected={hostile_dict_subclasses_rejected};"
    ...
    add(
        ...
        duplicate_rejected
        and _rejects(ComputePtmLocalizationQualityMetricsRequest, unknown)
        and _rejects(ComputePtmLocalizationQualityMetricsRequest, coercion),
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        with TestClient(create_app(root / "api.sqlite3")) as client:
            ...
            api_schema_results = tuple(
                (
                    name,
                    client.get(f"/v1/contracts/M05-04/{name}/schema"),
                )
        ...
        cli_schema_results = tuple(
            (
                name,
                CliRunner().invoke(cli_app, ["ptm-localization-quality", "export-schema", name]),
            )
            for name in schema_registry
        )
        api_result = (
            PtmLocalizationQualityResult.model_validate_json(api_parity.content, strict=True)
...
def run_evaluation() -> dict[str, object]:
    ...
    declared, allocation, inventory = _inventory(corpus)
    evaluated = _semantic_checks()
    scenario_checks = [evaluated[item] for item in declared if item in evaluated]
    ...
    executable_coverage = EvalCheck(
        ...
        detail=(
            ...
            f"allocation={allocation};substantive={len(executed)}"
        ),
    )
    checks = [inventory, *scenario_checks, executable_coverage]
    return {
        "module_id": MODULE_ID,
