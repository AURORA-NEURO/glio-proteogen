"""Benchmark only public M05-04 computation at the maximum supported metadata shape."""
...
from evals.m05_04.run import build_representative_quality_fixture
...
def run_benchmark() -> BenchmarkReport:
    ...
    warmup = compute_ptm_localization_quality_metrics(request)
    ...
    if (
        ledger is None
        or len(request.policy.profiles) != M0504_MAX_PROFILES
        or threshold_count != M0504_MAX_PROFILES * M0504_METRIC_COUNT
        or len(ledger.role_facts) != M0504_ROLE_COUNT
        or warmup.disposition is not PtmLocalizationQualityDisposition.QUALIFIED
        or len(warmup.assay_quality) != M0504_ROLE_COUNT
        or metric_count != M0504_COMPUTED_METRIC_COUNT
        or len(warmup.evidence) != M0504_MAX_EVIDENCE
        or len(warmup.limitations) != M0504_LIMITATION_COUNT
    ...
    return BenchmarkReport(
        ...
        passed=(
            request_bytes <= M0504_MAX_CANONICAL_REQUEST_BYTES
