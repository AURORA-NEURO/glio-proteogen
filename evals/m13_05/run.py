"""Run the locked, deterministic M13-05 evaluation matrix."""
...
SCENARIO_PATH: Final = (
    Path(__file__).parents[2] / "tests" / "fixtures" / "m13_05" / "scenarios.json"
)
...
def _artifact(label: str, media_type: str = "application/json") -> ArtifactReference:
    return ArtifactReference(
        ...
        version="1.0.0",
        digest=sha256_digest({"m1305-eval": label}),
...
def build_scenario_request(
    ...
    configuration = EvolutionModelConfiguration(
        configuration_id="configuration.m1305",
        version="1.0.0",
    ...
    return ModelProteotypeLongitudinalEvolutionRequest(
        ...
        network_state_result=_artifact("m1304-result", M1305_M1304_RESULT_MEDIA_TYPE),
...
def run_evaluator() -> dict[str, object]:
    ...
    if case_ids != EXPECTED_CASE_IDS:
        raise ValueError("M13-05 fixture case IDs are not locked")
    ...
    except M1305AuthorizationError:
        denied = True
