"""Run the locked, deterministic M12-04 evaluation matrix."""
...
SCENARIO_PATH: Final = (
    Path(__file__).parents[2] / "tests" / "fixtures" / "m12_04" / "scenarios.json"
)
...
def _digest(label: str) -> str:
    return sha256_digest({"m1204_fixture": label})
...
def build_scenario_request(
    ...
    *,
    ...
    """Build a typed request entirely from caller-declared references."""
...
def run_evaluator() -> dict[str, object]:
    ...
    if case_ids != EXPECTED_CASE_IDS:
        raise ValueError("M12-04 fixture case IDs are not locked")
    ...
    try:
        engine.verify(posterior.model_copy(update={"result_digest": _digest("tampered")}))
    except ValueError:
    ...
    except M1204MechanismAuthorizationError:
        denied = True
