"""Run the locked, deterministic M14-04 evaluation matrix."""
...
SCENARIO_PATH: Final = (
    Path(__file__).parents[2] / "tests" / "fixtures" / "m14_04" / "scenarios.json"
)
...
def _digest(label: str) -> str:
    return sha256_digest({"m1404_fixture": label})
...
def build_scenario_request(
    ...
    *,
    ...
    configuration = MechanismInferenceConfiguration(
        configuration_id="configuration.m1404",
        version="1.0.0",
        method=method,
    ...
    return InferProteinSubtypeMechanismRequest(
        request_id="request.m1404",
        context=context,
        hypothesis_registry_result=_artifact("m1101-result", M1404_M1401_RESULT_MEDIA_TYPE),
...
def run_evaluator() -> dict[str, object]:
    ...
    if case_ids != EXPECTED_CASE_IDS:
        raise ValueError("M14-04 fixture case IDs are not locked")
    ...
    except M1404MechanismAuthorizationError:
        denied = True
