"""Small deterministic benchmark for the M07-02 constructor boundary."""
...
def benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    ...
    for _ in range(iterations):
        ...
        if built.canonical_bytes != expected.canonical_bytes:
            raise RuntimeError("representation result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[min(len(ordered) - 1, (95 * len(ordered) - 1) // 100)]
    return BenchmarkReport(
        module_id="GLIO-PROTEOGEN-M07-02",
        contract_version="0.1.0-provisional",
        workload="two-feature leakage-safe representation construction",
        timed_boundary="M0702RepresentationEngine.construct",
