"""Small deterministic benchmark for the M06-07 calibration boundary."""
...
def benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    ...
    expected = engine.calibrate(request)
    samples: list[int] = []
    for _ in range(iterations):
        ...
        if built.canonical_bytes != expected.canonical_bytes:
            raise RuntimeError("calibration result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[min(len(ordered) - 1, (95 * len(ordered) - 1) // 100)]
    return BenchmarkReport(
        module_id="GLIO-PROTEOGEN-M06-07",
        contract_version="0.1.0-provisional",
        workload="single locked calibration and selective-prediction request",
        timed_boundary="M0607CalibrationEngine.calibrate",
        ...
        stratum_count=len(request.policy.strata),
        estimate_count=len(request.uncertainty_result.request.constraint_result.estimates),
