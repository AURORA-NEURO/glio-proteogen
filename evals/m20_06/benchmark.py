"""Benchmark the bounded M20-06 adjudication workload."""
...
from glio_proteogen.contracts.m20_06 import M2006_MODULE_ID
...
def run_benchmark() -> BenchmarkReport:
    ...
    for _ in range(ITERATIONS):
        ...
        if result != warmup:
            raise RuntimeError("M20-06 benchmark result was not deterministic")  # noqa: TRY003
    ordered = sorted(samples)
    p95 = ordered[(95 * len(ordered) - 1) // 100]
    mean = fmean(samples)
    return BenchmarkReport(
        module_id=M2006_MODULE_ID,
        workload="bounded_blinded_protein_subtype_adjudication_queue",
