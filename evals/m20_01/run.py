"""Executable evaluator for M20-01 typed upstream resolution."""
...
from glio_proteogen.contracts.m20_01 import CompatibilityStatus
...
if TYPE_CHECKING:
    from glio_proteogen.contracts.m20_01 import ResolveProteinSubtypeUpstreamContractsRequest

ROOT: Final = Path(__file__).resolve().parents[2]
SCENARIO_PATH: Final = ROOT / "tests" / "fixtures" / "m20_01" / "scenarios.json"
MODULE_ID: Final = "GLIO-PROTEOGEN-M20-01"
...
def _scenario(name: str) -> ResolveProteinSubtypeUpstreamContractsRequest:
    ...
    raise ValueError(f"unknown M20-01 evaluator scenario: {name}")  # noqa: TRY003
...
def evaluate() -> EvaluationReport:
    ...
    engine = m2001.M2001Engine()
    scenario_passed = True
    results = {}
    for name in scenario_names:
        try:
            result = engine.resolve(_scenario(name))
        except m2001.M2001AuthorizationError:
    ...
    checks.extend(
        (
            EvalCheck(
                ...
                mixed.parent_target == "protein subtype" and mixed.emits_parent is False,
                "resolver does not emit the parent protein subtype",
            ),
            EvalCheck(
