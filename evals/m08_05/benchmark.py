"""Deterministic benchmark for the M08-05 integration boundary."""
...
def benchmark(iterations: int = DEFAULT_ITERATIONS) -> BenchmarkReport:
    ...
    request = _request("conservation_hold", "soft force_violation")
    engine = M0805ConstraintIntegrator()
    expected = engine.integrate(request)
    samples: list[int] = []
    for _ in range(iterations):
        ...
        if built.canonical_bytes != expected.canonical_bytes:
            raise RuntimeError("M08-05 result was not deterministic")  # noqa: TRY003
    ...
    return BenchmarkReport(
        ...
        workload="hard and soft transcript-protein constraint integration",
        timed_boundary="M0805ConstraintIntegrator.integrate",
        ...
        constraint_count=len(request.policy.constraints),
