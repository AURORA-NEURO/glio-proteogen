...
from tools import verify_m2502_release as verifier
...
def _write_package_receipt(root: Path, wheel: Path, sdist: Path, *, members: int = 2) -> None:
    evidence = root / "release-evidence" / "m25_02"
    package = {
...
def _fixture_release(tmp_path: Path) -> tuple[Path, Path, Path]:
    source = Path(__file__).parents[2] / "release-evidence" / "m25_02"
    evidence = tmp_path / "release-evidence" / "m25_02"
    ...
    artifacts.mkdir()
    wheel = artifacts / "m2502-fixture.whl"
    ...
    sdist = artifacts / "m2502-fixture.tar.gz"
    sdist.write_bytes(b"synthetic M25-02 source distribution")
...
def test_m2502_release_evidence_is_internally_consistent() -> None:
    ...
    wheel = root / "dist-m25-02" / "glio_proteogen-0.1.0-py3-none-any.whl"
    sdist = root / "dist-m25-02" / "glio_proteogen-0.1.0.tar.gz"
...
def test_m2502_release_verifier_accepts_self_contained_receipts_and_artifacts(
    tmp_path: Path,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("evaluation.json", "dossier_sha256", "sha256:" + ("0" * 64), "authority"),
        ("evaluation.json", "parent_target", "patient", "dependency"),
        ("evaluation.json", "scenario_count", 10, "evaluation"),
        ("benchmark.json", "p95_ns", verifier.P95_BUDGET_NS + 1, "benchmark"),
        ("coverage.json", "branch_enabled", False, "coverage"),
    ],
)
def test_m2502_release_verifier_fails_closed_on_evidence_tamper(
    tmp_path: Path,
    filename: str,
    field: str,
    replacement: object,
    ...
    path = root / "release-evidence" / "m25_02" / filename
...
def test_m2502_release_verifier_binds_package_bytes(tmp_path: Path) -> None:
    root, wheel, sdist = _fixture_release(tmp_path)
...
def test_m2502_release_verifier_rejects_digest_valid_non_zip_wheel(tmp_path: Path) -> None:
    root, wheel, sdist = _fixture_release(tmp_path)
...
def test_m2502_release_verifier_rejects_malformed_package_record(tmp_path: Path) -> None:
    root, wheel, sdist = _fixture_release(tmp_path)
    package_path = root / "release-evidence" / "m25_02" / "package.json"
    package = json.loads(package_path.read_text(encoding="utf-8"))
    package["wheel"] = "forged"
...
def test_m2502_release_verifier_rejects_non_object_receipt(tmp_path: Path) -> None:
    root, _, _ = _fixture_release(tmp_path)
    (root / "release-evidence" / "m25_02" / "evaluation.json").write_text("[]", encoding="utf-8")

    with pytest.raises(TypeError, match="must contain a JSON object"):
        verifier.verify(root)


def test_m2502_release_verifier_cli_reports_success_and_failure(
    tmp_path: Path,
