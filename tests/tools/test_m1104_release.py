"""Release-evidence verifier tests for M11-04."""
...
import pytest
from tools.verify_m1104_release import (
    M1104ReleaseVerificationError,
    _verify_benchmark,
    _verify_evaluation,
    verify_release,
)
...
def test_evaluation_rejects_wrong_fixture_digest() -> None:
    value = json.loads(
        (Path("release-evidence/m11_04/evaluation.json")).read_text(encoding="utf-8")
    )
    value["fixture_sha256"] = "sha256:" + "0" * 64
    with pytest.raises(M1104ReleaseVerificationError):
