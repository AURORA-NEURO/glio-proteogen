...
from tools.verify_m1401_release import verify_release


def _evidence_path() -> Path:
    return Path(__file__).parents[2] / "release-evidence" / "m14_01"


def test_m1401_release_evidence_verifies() -> None:
    report = verify_release(_evidence_path())
    assert report["module_id"] == "GLIO-PROTEOGEN-M14-01"
    assert report["evaluation"] == "passed"


def test_m1401_release_verifier_rejects_fixture_tampering(tmp_path: Path) -> None:
...
def test_m1401_release_verifier_rejects_package_tampering(tmp_path: Path) -> None:
    source = _evidence_path()
    for name in ("evaluation.json", "benchmark.json", "package.json"):
        (tmp_path / name).write_text((source / name).read_text(encoding="utf-8"), encoding="utf-8")
