"""Release-evidence verifier tests for M13-04."""
...
import pytest
from tools.verify_m1304_release import (
    M1304ReleaseVerificationError,
    _verify_benchmark,
    _verify_evaluation,
    verify_release,
)
...
def test_release_evidence_verifies_current_artifacts() -> None:
    ...
    assert report["module_id"] == "GLIO-PROTEOGEN-M13-04"
...
def test_evaluation_rejects_wrong_fixture_digest() -> None:
    value = json.loads(
        (Path("release-evidence/m13_04/evaluation.json")).read_text(encoding="utf-8")
    )
    value["fixture_sha256"] = "sha256:" + "0" * 64
    with pytest.raises(M1304ReleaseVerificationError):
