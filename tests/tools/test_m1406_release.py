"""Release-evidence verifier tests for M14-06."""
...
import pytest
from tools.verify_m1406_release import (
    M1406ReleaseVerificationError,
    _verify_benchmark,
    _verify_evaluation,
    verify_release,
)
...
def test_release_evidence_verifies_current_artifacts() -> None:
    ...
    assert report["module_id"] == "GLIO-PROTEOGEN-M14-06"
...
def test_evaluation_rejects_wrong_fixture_digest() -> None:
    value = json.loads(
        (Path("release-evidence/m14_06/evaluation.json")).read_text(encoding="utf-8")
    )
    value["fixture_sha256"] = "sha256:" + "0" * 64
    with pytest.raises(M1406ReleaseVerificationError):
