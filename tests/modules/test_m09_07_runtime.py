"""Runtime, replay, and gate tests for M09-07."""

from __future__ import annotations

from glio_proteogen.contracts.m09_07 import CalibrationCandidate, CalibrationStatus
...
def test_missing_candidate_abstains_for_review() -> None:
    result = m0907.M0907Service().execute(_request())
...
def test_scope_gate_and_public_operation_wrapper() -> None:
    ...
    result = m0907.calibrate_complex_activity_selective_prediction(request)
    assert result.findings == ("scope_not_supported",)
    assert m0907.M0907Service.verify(result) is True
