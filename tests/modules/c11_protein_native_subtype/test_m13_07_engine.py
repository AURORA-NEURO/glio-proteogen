"""Runtime, replay, and hostile-input coverage for M13-07."""
...
adjudicate_proteotype_plausibility = m1307.adjudicate_proteotype_plausibility
preflight_plausibility_authorization = m1307.preflight_plausibility_authorization
verify_plausibility_replay = m1307.verify_plausibility_replay
...
def _request(
    *,
    ...
    return AdjudicateProteotypePlausibilityRequest(
        ...
        mechanism_inference_result=_artifact("mechanism-result", 10, M1307_M1306_RESULT_MEDIA_TYPE),
...
def test_supported_controls_produce_high_adjudication() -> None:
    result = adjudicate_proteotype_plausibility(_request())
    ...
    assert result.provenance.module_id == "GLIO-PROTEOGEN-M13-07"
...
def test_blocking_control_abstains_without_negative_inference(outcome: ControlOutcome) -> None:
    result = M1307PlausibilityEngine().adjudicate(_request(outcome=outcome))
...
def test_conflict_is_preserved_and_requires_review() -> None:
    result = adjudicate_proteotype_plausibility(_request(conflict=True))
    assert result.status is PlausibilityAdjudicationStatus.ABSTAINED
    assert result.grade is None
...
def test_unsupported_upstream_control_abstains_before_execution() -> None:
    ...
    with pytest.raises(PlausibilityAuthorizationError):
        adjudicate_proteotype_plausibility(request)


def test_json_plugin_is_parse_once_and_runs_only_capability() -> None:
    service = M1307Service()
    plugin = M1307Plugin(service)
...
def test_request_digest_is_stable_and_result_digest_is_bound() -> None:
    first = adjudicate_proteotype_plausibility(_request())
    second = adjudicate_proteotype_plausibility(_request())
    assert canonical_request_digest(first.request) == canonical_request_digest(second.request)
    assert first.result_digest == second.result_digest
    with pytest.raises(ValueError, match="result"):
        M1307PlausibilityEngine().verify(
    ...
    with pytest.raises(ValueError, match="negative control"):
        AdjudicateProteotypePlausibilityRequest(
            ...
            mechanism_inference_result=_artifact(
                "mechanism-result", 10, M1307_M1306_RESULT_MEDIA_TYPE
            ),
            controls=controls,
