"""Adversarial contract, replay, and adapter cases for M11-07."""
...
from glio_proteogen.adapters import m1107
...
from glio_proteogen.modules.c11_protein_native_subtype import (
    m11_07_plausibility_adjudicator as module,
)
...
def test_contract_rejects_wrong_binding_duplicates_and_missing_control_kinds() -> None:
    ...
    with pytest.raises(ValueError, match="bind"):
        AdjudicateVariantPeptidePlausibilityRequest.model_validate(wrong_media)
    ...
    with pytest.raises(ValueError, match="unique"):
        AdjudicateVariantPeptidePlausibilityRequest.model_validate(duplicate_ids)
    ...
    with pytest.raises(ValueError, match="six required"):
        AdjudicateVariantPeptidePlausibilityRequest.model_validate(missing_kind)
...
def test_contract_rejects_invalid_negative_control_and_no_negative_control() -> None:
    ...
    with pytest.raises(ValueError, match="negative control"):
        AdjudicateVariantPeptidePlausibilityRequest.model_validate(no_negative)


def test_contract_result_closure_rejects_outcome_and_direction_tampering() -> None:
    result = module.adjudicate_variant_peptide_plausibility(_request())
...
def test_contract_closure_rejects_duplicate_evaluations_conflicts_and_states() -> None:
    ...
    result = module.adjudicate_variant_peptide_plausibility(request)
    ...
    failed = module.adjudicate_variant_peptide_plausibility(_request(outcome=ControlOutcome.FAILED))
    ...
    conflict = module.adjudicate_variant_peptide_plausibility(_request(conflict=True))
...
def test_canonical_dict_projection_and_hostile_mapping_fail_closed() -> None:
    ...
    with pytest.raises(module.PlausibilityAuthorizationError):
        module.adjudicate_variant_peptide_plausibility(UserDict())
    with pytest.raises(module.PlausibilityAuthorizationError):
...
def test_typed_request_plain_value_rejects_non_string_nested_keys() -> None:
    ...
    with pytest.raises(ValueError, match="contract"):
        module.adjudicate_variant_peptide_plausibility(invalid)


def test_replay_request_and_result_digest_failures() -> None:
    engine = module.M1107PlausibilityEngine()
    request = _request()
...
def test_plugin_typed_path_descriptor_and_invalid_strict_json() -> None:
    plugin = module.M1107Plugin(module.M1107Service())
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M11-07"
    assert plugin.validate(_request()).request.request_id == "request-m1107"
    with pytest.raises(module.PlausibilityAuthorizationError):
        plugin.validate('{"request_id":1}')
    invalid_json = (
        _request().model_dump_json().replace('"request_id":"request-m1107"', '"request_id":1')
    )
    with pytest.raises(ValueError, match="contract"):
...
def test_adapter_error_branches_and_cli_rejections(tmp_path, monkeypatch) -> None:
    with pytest.raises(m1107._CliParameterError):
        m1107._read_json(tmp_path / "missing.json")
    oversized = tmp_path / "oversized.json"
    oversized.write_text("{}", encoding="utf-8")
    monkeypatch.setattr(m1107, "M1107_MAX_CANONICAL_REQUEST_BYTES", 1)
    with pytest.raises(m1107._CliParameterError):
        m1107._read_json(oversized)
    with pytest.raises(m1107._CliParameterError):
        m1107._write_json(tmp_path / "missing" / "result.json", {})
    with pytest.raises(m1107.HTTPException):
        m1107.export_schema("unknown")

    request_path = tmp_path / "invalid.json"
    request_path.write_text("{}", encoding="utf-8")
    response = CliRunner().invoke(m1107.m1107_app, ["adjudicate", str(request_path)])
    assert response.exit_code != 0


def test_adapter_api_rejects_bad_schema_payloads_and_tamper() -> None:
    with TestClient(m1107.app) as client:
        assert client.get("/v1/modules/M11-07/schema/unknown").status_code == _NOT_FOUND
        assert (
            client.post("/v1/modules/M11-07/plausibility", content=b"not-json").status_code
            == _BAD_REQUEST
        )
        assert client.post("/v1/modules/M11-07/verify", json=[]).status_code == _UNPROCESSABLE
        assert (
            client.post("/v1/modules/M11-07/verify", json={"request": {}}).status_code == _FORBIDDEN
        )
    request = _request()
    result = module.adjudicate_variant_peptide_plausibility(request)
...
def test_adapter_remaining_size_validation_and_cli_error_paths(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(m1107.app) as client:
        assert (
            client.post("/v1/modules/M11-07/plausibility", content=b"{} ").status_code
            == _PAYLOAD_TOO_LARGE
        )
        assert (
            client.post("/v1/modules/M11-07/verify", content=b"{} ").status_code
            == _PAYLOAD_TOO_LARGE
        )
    monkeypatch.setattr(m1107, "M1107_MAX_CANONICAL_REQUEST_BYTES", 4 * 1024 * 1024)
    with TestClient(m1107.app) as client:
        assert (
            client.post("/v1/modules/M11-07/verify", json={"request": "wrong"}).status_code
            == _UNPROCESSABLE
        )
        assert (
            client.post(
    ...
    runner = CliRunner()
    assert runner.invoke(m1107.m1107_app, ["export-schema", "output"]).exit_code == 0
    invalid_path = tmp_path / "invalid.json"
    invalid_path.write_text("{}", encoding="utf-8")
    assert runner.invoke(m1107.m1107_app, ["adjudicate", str(invalid_path)]).exit_code != 0
    result_path = tmp_path / "result.json"
    result_path.write_text("{}", encoding="utf-8")
    assert (
        runner.invoke(m1107.m1107_app, ["verify", str(invalid_path), str(result_path)]).exit_code
        != 0
    )
    assert runner.invoke(m1107.m1107_app, ["export-schema", "unknown"]).exit_code != 0
    request_path = tmp_path / "request.json"
    request_path.write_text(_request().model_dump_json(), encoding="utf-8")
    assert runner.invoke(m1107.m1107_app, ["adjudicate", str(request_path)]).exit_code == 0
...
def test_api_validation_error_handler(monkeypatch) -> None:
    ...
    monkeypatch.setattr(m1107, "_validate_json_request", fail)
    with TestClient(m1107.app) as client:
        response = client.post(
