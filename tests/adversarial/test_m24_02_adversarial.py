"""Deep adversarial coverage for M24-02 replay and safety boundaries."""
...
def test_plugin_rejects_duplicate_keys_before_contract_parse() -> None:
    plugin = m2402.M2402Plugin(m2402.M2402Service())
...
def test_hostile_mapping_fails_closed_before_material_traversal() -> None:
    ...
    with pytest.raises(m2402.M2402AuthorizationError):
        m2402.preflight_m2402_authorization(ExplodingMapping())
    with pytest.raises(m2402.M2402AuthorizationError):
        m2402.M2402Service().validate_request(ExplodingMapping())
...
def test_service_rejects_unknown_fields_and_wrong_upstream_media() -> None:
    ...
    with pytest.raises(ValidationError):
        m2402.M2402Service().validate_request(payload)
    ...
    with pytest.raises(ValidationError, match="M24-01"):
        m2402.M2402Service().validate_request(wrong_media)
...
def test_result_replay_rejects_request_mutation() -> None:
    ...
    with pytest.raises(m2402.M2402ReplayError, match="request digest"):
...
def test_api_rejects_nonobject_duplicate_and_tampered_verify_payloads() -> None:
    ...
    non_object = client.post("/v1/modules/M24-02/verify", json=[])
    ...
    tampered = client.post("/v1/modules/M24-02/verify", json=result)
...
def test_api_denies_failed_support_without_leaking_details() -> None:
    ...
    client = TestClient(m2402.create_app(m2402.M2402Service()))
    response = client.post("/v1/modules/M24-02/generate", json=denied.model_dump(mode="json"))
...
def test_public_entrypoint_matches_engine_and_json_boundary() -> None:
    request = _request()
    service = m2402.M2402Service()
    direct = m2402.generate_biomarker_panel_synthetic_truth(request)
