...
def test_surface_rejects_observation_disposition_and_ood_mismatches() -> None:
    request = build_request()
    scenarios = request.scenarios
    observations = tuple(_observation(request, item.scenario_id) for item in scenarios)

    disposition_mismatch = observations[0].model_copy(
        update={"disposition": ChallengeDisposition.ABSTAIN_UNSUPPORTED}
    )
    with pytest.raises(ValueError, match="disposition must match"):
        _surface(request, scenarios, (disposition_mismatch, *observations[1:])).surface_is_closed()

    unsupported_band = observations[0].model_copy(update={"ood_band": OODBand.OUT_OF_DOMAIN})
    with pytest.raises(ValueError, match="supported OOD bands"):
        _surface(request, scenarios, (unsupported_band, *observations[1:])).surface_is_closed()

    unsupported_request = build_request(disposition=ChallengeDisposition.ABSTAIN_UNSUPPORTED)
    unsupported_scenarios = unsupported_request.scenarios
    unsupported_observations = tuple(
        _observation(unsupported_request, item.scenario_id) for item in unsupported_scenarios
    )
    novel_index = next(
        index
        for index, scenario in enumerate(unsupported_scenarios)
        if scenario.kind is ChallengeKind.NOVEL_STATE
    )
    unsupported_in_domain = unsupported_observations[novel_index].model_copy(
        update={
            "disposition": ChallengeDisposition.ABSTAIN_UNSUPPORTED,
            "ood_band": OODBand.IN_DOMAIN,
            "within_envelope": True,
        }
    )
    unsupported_observations = (
        *unsupported_observations[:novel_index],
        unsupported_in_domain,
        *unsupported_observations[novel_index + 1 :],
    )
    with pytest.raises(ValueError, match="unsupported observations"):
        _surface(
            unsupported_request, unsupported_scenarios, unsupported_observations
        ).surface_is_closed()
...
def test_result_validator_closes_digest_provenance_and_findings() -> None:
    ...
    wrong_id = result.model_copy(update={"result_id": "result.wrong-again"})
    wrong_id = wrong_id.model_copy(update={"result_digest": result_payload_digest(wrong_id)})
