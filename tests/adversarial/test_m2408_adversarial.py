"""Adversarial boundary and replay tests for M24-08."""
...
from glio_proteogen.modules.c21_reference_material.m24_08_evidence_gate_release_adjudicator import (
    engine as m2408_engine,
)
from tests.contract.test_m2408_deep import _evidence, _request
...
def test_plugin_rejects_duplicate_json_keys_before_contract_parse() -> None:
    plugin = M2408Plugin(M2408Service())
...
def test_service_fails_closed_on_unknown_control_mapping() -> None:
    with pytest.raises(M2408AuthorizationError):
        M2408Service().validate_request({"context": {"references": {}}})
...
def test_contract_rejects_duplicate_requirement_id_and_unlocked_config() -> None:
    ...
    with pytest.raises(ValidationError, match="identifiers"):
        AdjudicateBiomarkerPanelEvidenceGateRequest.model_validate(payload)
...
def test_contract_rejects_source_artifact_digest_substitution() -> None:
    ...
    with pytest.raises(ValidationError, match="unique artifact IDs"):
        AdjudicateBiomarkerPanelEvidenceGateRequest.model_validate(duplicate_payload)


def test_api_replay_rejects_nonobject_and_tampered_result() -> None:
    request = _request()
    client = TestClient(m2408_api.create_app())
    response = client.post("/v1/modules/M24-08/verify", json=["not-an-object"])
    ...
    tampered = client.post("/v1/modules/M24-08/verify", json=result)
...
def test_cli_unknown_schema_and_missing_input_are_sanitized(tmp_path: Path) -> None:
    runner = CliRunner()
    unknown = runner.invoke(m2408_cli.app, ["export-schema", "unknown"])
    assert unknown.exit_code != 0
    assert "unknown M24-08 contract" in unknown.output
    missing = runner.invoke(m2408_cli.app, ["validate", str(tmp_path / "missing.json")])
...
def test_public_entry_point_and_preflight_hostile_mapping_fail_closed() -> None:
    request = _request()
    assert (
        adjudicate_biomarker_panel_evidence_gate(request).result_digest
        == M2408EvidenceGateEngine().adjudicate(request).result_digest
    )
    ...
    with pytest.raises(M2408AuthorizationError):
        M2408Service().validate_request(BrokenMapping())


def test_plugin_descriptor_and_replay_boundary() -> None:
    service = M2408Service()
    plugin = M2408Plugin(service)
    assert plugin.descriptor.module_id == "GLIO-PROTEOGEN-M24-08"
...
def test_plugin_direct_validation_and_seal_mutation_fail_closed() -> None:
    plugin = M2408Plugin()
    request = _request()
...
def test_cli_schema_output_and_read_validation_errors(tmp_path: Path) -> None:
    ...
    invalid = runner.invoke(m2408_cli.app, ["validate", str(bad_request)])
    assert invalid.exit_code != 0
    bad_result = tmp_path / "bad-result.json"
    bad_result.write_text("{}", encoding="utf-8")
    invalid_result = runner.invoke(m2408_cli.app, ["verify", str(bad_result)])
    assert invalid_result.exit_code != 0


def test_cli_authorization_output_abstention_and_replay_failures(
    ...
    assert runner.invoke(m2408_cli.app, ["validate", str(denied_path)]).exit_code != 0
    assert runner.invoke(m2408_cli.app, ["adjudicate", str(denied_path)]).exit_code != 0

    output = tmp_path / "result.json"
    adjudicated = runner.invoke(
        m2408_cli.app,
        ["adjudicate", str(request_path), "--output", str(output)],
    )
    assert adjudicated.exit_code == 0
    assert (
        runner.invoke(m2408_cli.app, ["adjudicate", str(request_path), "-o", str(output)])
        .exit_code
        != 0
    )
    assert runner.invoke(m2408_cli.app, ["verify", str(output)]).exit_code == 0

    valid_result = M2408EvidenceGateEngine().adjudicate(request)
    ...
    monkeypatch.setattr(m2408_cli, "_SERVICE", ReplayFailure())
    assert runner.invoke(m2408_cli.app, ["verify", str(output)]).exit_code != 0

    class ReplayMismatch:
        def replay(self, result: object) -> BiomarkerPanelEvidenceGateResult:
            del result
            return valid_result.model_copy(update={"result_digest": "sha256:" + "f" * 64})

    monkeypatch.setattr(m2408_cli, "_SERVICE", ReplayMismatch())
    mismatch = runner.invoke(m2408_cli.app, ["verify", str(output)])
    assert mismatch.exit_code == 1
    assert '"verified": false' in mismatch.stdout

    abstained_candidate = valid_result.model_copy(
        update={
            "status": GateRunStatus.ABSTAINED,
            "release_record": None,
            "abstention_reason": "provisional review",
            "support_decision": valid_result.support_decision.model_copy(
                update={"status": SupportStatus.UNSUPPORTED}
            ),
        }
    )
    abstained = abstained_candidate.model_copy(
        update={"result_digest": result_payload_digest(abstained_candidate)}
    )
    abstained = BiomarkerPanelEvidenceGateResult.model_validate(abstained, strict=True)
    ...
    monkeypatch.setattr(m2408_cli, "_SERVICE", AbstainingService())
    assert runner.invoke(m2408_cli.app, ["adjudicate", str(request_path)]).exit_code == 1
...
def test_api_auth_and_json_parse_errors_are_sanitized() -> None:
    ...
    for route in ("validate", "adjudicate"):
        response = client.post(
            f"/v1/modules/M24-08/{route}",
            content=denied.model_dump_json(),
            headers={"content-type": "application/json"},
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
    malformed = client.post("/v1/modules/M24-08/verify", content=b"not-json")
...
def test_signed_record_and_result_closures_reject_missing_bindings() -> None:
    result = M2408EvidenceGateEngine().adjudicate(_request())
    assert result.release_record is not None
    record_payload = result.release_record.model_dump(mode="python")
    record_payload["requirements"] = record_payload["requirements"][:-1]
    with pytest.raises(ValidationError, match="cover every required"):
        SignedReleaseRecord.model_validate(record_payload)

    for update, message in (
        (
            {"release_record": result.release_record.model_copy(update={"version": "9.9.9"})},
            "version",
        ),
        (
            {
                "release_record": result.release_record.model_copy(
                    update={"evidence": (_evidence("tampered-release-evidence"),)}
                )
            },
            "evidence",
        ),
    ):
        candidate_model = result.model_copy(update=update)
        candidate = candidate_model.model_dump(mode="python")
        candidate["result_digest"] = result_payload_digest(candidate_model)
        with pytest.raises(ValidationError, match=message):
            BiomarkerPanelEvidenceGateResult.model_validate(candidate, strict=True)

    blocked = M2408EvidenceGateEngine().adjudicate(_request(satisfied=False))
    candidate_model = blocked.model_copy(update={"findings": ()})
    candidate = candidate_model.model_dump(mode="python")
    candidate["result_digest"] = result_payload_digest(candidate_model)
    with pytest.raises(ValidationError, match="retain findings"):
        BiomarkerPanelEvidenceGateResult.model_validate(candidate, strict=True)


def test_abstained_result_requires_safe_support_status() -> None:
    result = M2408EvidenceGateEngine().adjudicate(_request())
    candidate_model = result.model_copy(
        update={
            "status": GateRunStatus.ABSTAINED,
            "release_record": None,
...
def test_resigned_result_cannot_bypass_release_record_closure() -> None:
    result = M2408EvidenceGateEngine().adjudicate(_request())
    assert result.release_record is not None
    ...
    for update, message in (
        (
            {
                "release_record": result.release_record.model_copy(
                    update={"limitations": ("tampered limitation",)}
                )
            },
            "signature",
        ),
        (
            {
                "release_record": result.release_record.model_copy(
                    update={"decision": GateDecision.BLOCK}
                )
            },
            "decision",
        ),
        (
            {
                "release_record": result.release_record.model_copy(
                    update={"release_id": "m2408.release.forged"}
                )
            },
            "request digest",
        ),
    ):
        candidate = result.model_copy(update=update).model_dump(mode="python")
        candidate["result_digest"] = result_payload_digest(candidate)
        with pytest.raises(ValidationError, match=message):
...
def test_replay_checks_each_digest_identifier_and_recomputed_result() -> None:
    ...
    cases = (
        ...
        result.model_copy(update={"result_id": "gate.m2408.forged"}),
    ...
    other_request = _request(request_id="m2408.request.other")
...
def test_replay_defensive_guards_reject_inconsistent_engine_helpers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = M2408EvidenceGateEngine()
    result = engine.adjudicate(_request())
    real_request_digest = canonical_request_digest
    real_result_identifier = result_identifier
    real_result_payload_digest = result_payload_digest

    monkeypatch.setattr(m2408_engine, "canonical_request_digest", lambda _: "sha256:" + "0" * 64)
    with pytest.raises(M2408ReplayError):
        engine.replay(result)

    monkeypatch.setattr(m2408_engine, "canonical_request_digest", real_request_digest)
    monkeypatch.setattr(m2408_engine, "result_identifier", lambda _: "gate.m2408.forged")
    with pytest.raises(M2408ReplayError):
        engine.replay(result)

    monkeypatch.setattr(m2408_engine, "result_identifier", real_result_identifier)
    monkeypatch.setattr(m2408_engine, "result_payload_digest", lambda _: "sha256:" + "f" * 64)
    with pytest.raises(M2408ReplayError):
        engine.replay(result)

    monkeypatch.setattr(m2408_engine, "result_payload_digest", real_result_payload_digest)

    def mismatching_adjudication(
        _engine: M2408EvidenceGateEngine, request: object
    ) -> BiomarkerPanelEvidenceGateResult:
        del request
        return result.model_copy(update={"limitations": (result.limitations[0],)})

    monkeypatch.setattr(M2408EvidenceGateEngine, "adjudicate", mismatching_adjudication)
    with pytest.raises(M2408ReplayError):
        engine.replay(result)
...
def test_individual_schema_route_and_cli_stdout_adjudication(tmp_path: Path) -> None:
    client = TestClient(m2408_api.create_app())
    schema = client.get("/v1/modules/M24-08/schemas/request")
    assert schema.status_code == HTTPStatus.OK
    assert schema.json()["$id"].endswith(":request")

    path = tmp_path / "m2408-adversarial-request.json"
