"""Adversarial runtime and interface closure for M24-03."""
...
import pytest
from evals.m24_03.fixture import denied_request
from fastapi.testclient import TestClient
from pydantic import ValidationError
from typer.testing import CliRunner

from glio_proteogen.contracts.m24_03 import BaselineRun, BenchmarkDossier, ValidationStatus
...
from tests.contract.test_m24_03_hardening import _request
...
def test_preflight_rejects_non_mapping_and_missing_context() -> None:
    with pytest.raises(M2403AuthorizationError):
        preflight_m2403_authorization(object())
    with pytest.raises(M2403AuthorizationError):
        preflight_m2403_authorization({"context": None})


def test_service_accepts_canonical_json_and_engine_public_entrypoint() -> None:
    request = _request()
    assert (
        M2403Service().validate_request(request.model_dump_json()).request_id == request.request_id
    )
    assert run_biomarker_panel_internal_benchmark(request).status.value == "completed"
...
def test_contract_nested_identity_closures_reject_duplicates() -> None:
    ...
    dossier = M2403Service().generate(request).dossier
...
def test_preflight_mapping_exception_fails_closed() -> None:
    ...
    with pytest.raises(M2403AuthorizationError):
        preflight_m2403_authorization(ExplodingMapping())
...
def test_plugin_json_path_is_strict_and_replays() -> None:
    plugin = M2403Plugin(M2403Service())
    ...
    with pytest.raises((ValidationError, M2403AuthorizationError)):
...
def test_fastapi_verify_rejects_tampered_result_without_traceback() -> None:
    ...
    response = client.post("/v1/modules/M24-03/verify", json=result)
    assert response.status_code == _HTTP_UNPROCESSABLE
    assert "Traceback" not in response.text


def test_fastapi_known_schema_and_sanitized_auth_and_json_errors() -> None:
    client = TestClient(create_app(M2403Service()))
    assert client.get("/v1/modules/M24-03/schemas/request").status_code == _HTTP_OK
    denied_json = denied_request().model_dump_json()
    validate = client.post("/v1/modules/M24-03/validate", content=denied_json)
    benchmark = client.post("/v1/modules/M24-03/benchmark", content=denied_json)
    assert validate.status_code == benchmark.status_code == _HTTP_UNPROCESSABLE
    malformed = client.post("/v1/modules/M24-03/verify", content=b"not-json")
...
def test_typer_sanitizes_auth_unknown_schema_and_replay_errors(
    ...
    result_path = tmp_path / "result.json"
    result_path.write_text(M2403Service().generate(_request()).model_dump_json(), encoding="utf-8")
