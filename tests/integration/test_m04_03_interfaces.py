"""Black-box schema, CLI, filesystem, and parity checks for M04-03."""
...
def test_api_and_cli_export_identical_m04_03_schemas(tmp_path: Path, name: str) -> None:
    with TestClient(create_app(tmp_path / f"schema-{name}.sqlite3")) as client:
        response = client.get(f"/v1/contracts/M04-03/{name}/schema")
        absent = client.post("/v1/modules/M04-03/raw-ingestion", json={})
    cli = CliRunner().invoke(cli_app, ["proteoform-raw", "export-schema", name])
...
def _write_capsule(root: Path) -> tuple[Path, Path, Path, ProteoformRawInputValidationResult]:
...
def test_library_service_plugin_and_cli_emit_exact_parity(tmp_path: Path) -> None:
    ...
    scenario = build_scenario()
    service = M0403Service()
    plugin = M0403Plugin(service)
    token = plugin.validate(M0403Submission(scenario.request, scenario.artifacts_by_role))
    ...
    assert cli.exit_code == 0, cli.output
    cli_result = ProteoformRawInputValidationResult.model_validate_json(
...
def test_cli_accepts_exact_four_mib_request_and_rejects_plus_one_before_source(
    ...
    exact = serialized + (b" " * (M0403_MAX_CANONICAL_REQUEST_BYTES - len(serialized)))
    assert len(exact) == M0403_MAX_CANONICAL_REQUEST_BYTES
...
def test_cli_authorization_denial_precedes_source_classification(
    ...
    assert result.exit_code == CLI_USAGE_ERROR
    assert result.output.startswith("proteoform raw ingestion failed:")
...
def test_cli_source_reads_are_bounded_and_size_caps_precede_open(
    ...
    snapshots = cli_module._load_proteoform_raw_files(source_directory, scenario.request)
    ...
    with pytest.raises(ValueError, match="source is unavailable"):
        cli_module._load_proteoform_raw_files(source_directory, scenario.request)
    assert read_limits == []
...
def test_cli_atomic_output_rejects_parent_reparse_swap(
    ...
    if os.name == "nt":
        original_rename = cli_module._windows_rename_proteoform_raw_output
    ...
    with pytest.raises(ValueError, match="new regular file"):
        cli_module._write_proteoform_raw_result(output_path, b"must-not-escape")

    assert publication_succeeded is (os.name != "nt")
    assert not (escape / output_path.name).exists()
    assert tuple(escape.glob(".m0403-*.tmp")) == ()
    ...
    assert tuple(received_parent.glob(".m0403-*.tmp")) == ()
...
def test_cli_atomic_output_cleans_transient_parent_swap(  # noqa: PLR0915
    ...
    assert not (escape / output_path.name).exists()
    assert tuple(escape.glob(".m0403-*.tmp")) == ()
    assert tuple(output_parent.glob(".m0403-*.tmp")) == ()
...
def test_cli_atomic_output_rolls_back_late_publication_failure(
    ...
    with pytest.raises(ValueError, match="new regular file"):
        cli_module._write_proteoform_raw_result(output_path, b"must-be-rolled-back")
