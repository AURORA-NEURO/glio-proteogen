"""HTTP and CLI parity tests for M13-07."""
...
from glio_proteogen.adapters.m1307 import app, m1307_app
...
def test_api_schema_and_adjudication() -> None:
    ...
    with TestClient(app) as client:
        schema = client.get("/v1/modules/M13-07/schema/request")
...
def test_api_verify_and_cli_adjudicate_verify(tmp_path) -> None:
    ...
    verify = runner.invoke(m1307_app, ["verify", str(request_path), str(result_path)])
    ...
    with TestClient(app) as client:
        response = client.post("/v1/modules/M13-07/verify", json=envelope)
    assert response.status_code == _OK
...
def test_cli_schema_no_overwrite(tmp_path) -> None:
    ...
    runner = CliRunner()
    first = runner.invoke(m1307_app, ["export-schema", "output", "--output", str(output)])
