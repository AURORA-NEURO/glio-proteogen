"""FastAPI and Typer parity tests for M08-01."""
...
from glio_proteogen.contracts.m08_01 import canonical_request_digest
...
def test_api_schema_and_validation_parity(tmp_path: Path) -> None:
    candidate = request()
    with TestClient(create_app(tmp_path / "events.sqlite")) as client:
        schema = client.get("/v1/contracts/M08-01/request/schema")
...
def test_cli_schema_and_result_are_canonical(tmp_path: Path) -> None:
    candidate = request()
    request_path = tmp_path / "request.json"
    output_path = tmp_path / "result.json"
    request_path.write_bytes(canonical_json_bytes(candidate.model_dump(mode="json")))
    runner = CliRunner()
    schema = runner.invoke(app, ["transcript-protein-state", "export-schema", "request"])
    result = runner.invoke(
        app,
        [
            "transcript-protein-state",
            "validate",
            str(request_path),
            "--output",
            str(output_path),
...
def test_cli_rejects_duplicate_json_key(tmp_path: Path) -> None:
    request_path = tmp_path / "request.json"
    output_path = tmp_path / "result.json"
