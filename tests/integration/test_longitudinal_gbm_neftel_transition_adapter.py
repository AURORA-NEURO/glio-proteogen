"""HTTP/CLI lifecycle and hard guards for Neftel program transitions."""
...
from glio_proteogen.adapters import longitudinal_gbm_neftel_transition as adapter
...
_PREFIX = adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_ROUTE_PREFIX
...
_EXPECTED_PROFILE_ID = "kncc-neftel-program-transition/1.0.0"
...
def _app() -> FastAPI:
    app = FastAPI()
    adapter.mount_longitudinal_gbm_neftel_transition(app)
    adapter.install_longitudinal_gbm_neftel_transition_openapi(app)
    return app
...
def test_exact_limits_and_real_service_receipt(real_receipt: Receipt) -> None:
    ...
    assert adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_REQUEST_MAX_BYTES == 2 * _MEBIBYTE
    assert adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_RESULT_MAX_BYTES == 4 * _MEBIBYTE
    assert adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_REPLAY_MAX_BYTES == 8 * _MEBIBYTE
    assert (
        adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_MAX_CONCURRENT_ANALYSES == _EXPECTED_CONCURRENCY
    )
    assert adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_TIMEOUT_SECONDS == _EXPECTED_TIMEOUT_SECONDS
    assert request.series_id == DEMO_ID
    assert len(result.transitions) == _EXPECTED_TRANSITIONS
    assert verification.verified is True


def test_central_cli_mounts_neftel_transition_lane() -> None:
    ...
    assert payload["series_id"] == DEMO_ID
    assert payload["profile_id"] == _EXPECTED_PROFILE_ID
...
def test_fail_closed_mount_preflights_profile(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        adapter,
        "algorithm_profile",
        partial(_raise_failure, NeftelConditionalModelIntegrityError(_SENSITIVE)),
    )
    with pytest.raises(NeftelConditionalModelIntegrityError, match=_SENSITIVE):
        adapter.mount_longitudinal_gbm_neftel_transition(app)
...
def test_http_lifecycle_openapi_and_forgery(
    ...
    assert profile_response.json()["profile_id"] == _EXPECTED_PROFILE_ID
    ...
    replay = schema["components"]["schemas"]["NeftelProgramReplayVerificationRequest"]
    assert {item["$ref"] for item in replay["properties"]["result"]["anyOf"]} == {
        "#/components/schemas/LongitudinalGbmNeftelTransitionResult",
        "#/components/schemas/UnverifiedLongitudinalGbmNeftelTransitionResult",
    }
...
def test_transport_metadata_media_duplicate_and_oversize() -> None:
    ...
    with TestClient(_app()) as client:
        ...
        oversized = client.post(
            f"{_PREFIX}/analyze",
            content=b'{"padding":"'
            + b"x" * adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_REQUEST_MAX_BYTES
            + b'"}',
            headers={"content-type": "application/json"},
        )
    ...
    with pytest.raises(HTTPException) as captured:
        asyncio.run(
            adapter._typed_body(
                ...
                adapter.LONGITUDINAL_GBM_NEFTEL_TRANSITION_REQUEST_MAX_BYTES,
                cancellation,
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (NeftelTransitionSourceIntegrityError(_SENSITIVE), _HTTP_UNAVAILABLE),
        (NeftelConditionalModelIntegrityError(_SENSITIVE), _HTTP_UNAVAILABLE),
        (NeftelConditionalInferenceError(_SENSITIVE), _HTTP_INTERNAL_ERROR),
        (InferenceCancelledError(_SENSITIVE), _HTTP_CLIENT_CLOSED),
        (InferenceDeadlineExceededError(_SENSITIVE), _HTTP_TIMEOUT),
...
def test_direct_capacity_success_and_result_receipt_bounds(
    ...
    with monkeypatch.context() as context:
        context.setattr(adapter, "LONGITUDINAL_GBM_NEFTEL_TRANSITION_RESULT_MAX_BYTES", 1)
    ...
    with monkeypatch.context() as context:
        ...
        context.setattr(adapter, "LONGITUDINAL_GBM_NEFTEL_TRANSITION_REPLAY_MAX_BYTES", 1)
...
def test_verification_failures_are_sanitized(
    ...
    request, result, _verification = real_receipt
    envelope = NeftelProgramReplayVerificationRequest(request=request, result=result)
    slot = _TrackingSlot()
    monkeypatch.setattr(adapter, "_ANALYSIS_SLOTS", slot)
    monkeypatch.setattr(
        adapter,
...
@pytest.mark.parametrize(
    ...
    [
        (
            "profile",
            "algorithm_profile",
            NeftelConditionalModelIntegrityError(_SENSITIVE),
            _HTTP_UNAVAILABLE,
        ),
        ...
        (
            "demo",
            "synthetic_demo_request",
            NeftelTransitionSourceIntegrityError(_SENSITIVE),
            _HTTP_UNAVAILABLE,
        ),
...
def test_cli_failures_bounds_false_replay_and_text_stream(
    ...
    for target, command in (
        ...
        with monkeypatch.context() as context:
            ...
            with pytest.raises(adapter.LongitudinalGbmNeftelTransitionCliError):
                command()
    ...
    stream = StringIO()
    monkeypatch.setattr(
        "glio_proteogen.adapters.longitudinal_gbm_neftel_transition.sys.stdout",
        stream,
    )
