...
from glio_proteogen.modules.c08_transcript_protein_discordance.m08_07_calibration_selective_prediction.cli import (  # noqa: E501
    app as cli_app,
)
...
def test_plugin_rejects_invalid_json_and_forged_capability() -> None:
    ...
    token = plugin.validate(json.dumps(document, separators=(",", ":")))
    forged = m0807.ValidatedM0807Request(request=token.request, _seal=object())
    with pytest.raises(TypeError, match="validated request token"):
        plugin.run(forged)
