"""API, CLI, and plugin parity tests for M07-02."""
...
from glio_proteogen.contracts.m07_02 import contract_json_schema, contract_json_schemas
...
def test_api_and_cli_export_identical_schema_inventory() -> None:
    ...
    with TestClient(m0702_api.create_app()) as client:
        for name in contract_json_schemas():
            api = client.get(f"/v1/modules/M07-02/schemas/{name}")
            cli = runner.invoke(m0702_cli.app, ["export-schema", name])
...
def test_api_and_cli_validate_identical_request(tmp_path) -> None:
    ...
    with TestClient(m0702_api.create_app()) as client:
        api = client.post("/v1/modules/M07-02/validate", content=encoded)
    cli = CliRunner().invoke(m0702_cli.app, ["validate", str(request_path)])
    assert api.status_code == _HTTP_OK
    assert cli.exit_code == 0
    assert api.json() == json.loads(cli.stdout)
...
def test_api_and_cli_construct_identical_canonical_result(tmp_path) -> None:
    ...
    request_path.write_bytes(encoded)
    with TestClient(m0702_api.create_app()) as client:
        api = client.post("/v1/modules/M07-02/construct", content=encoded)
    cli = CliRunner().invoke(m0702_cli.app, ["construct", str(request_path)])
    assert api.status_code == _HTTP_OK
    assert cli.exit_code == 0
...
def test_plugin_parse_once_requires_validated_token() -> None:
    ...
    assert isinstance(validated, ValidatedM0702Request)
    ...
    assert plugin.construct(request).canonical_bytes == plugin.execute(request).canonical_bytes
...
def test_duplicate_json_keys_are_rejected_without_secret_leak(tmp_path) -> None:
    ...
    with TestClient(m0702_api.create_app()) as client:
        api = client.post("/v1/modules/M07-02/validate", content=payload)
    cli = CliRunner().invoke(m0702_cli.app, ["validate", str(request_path)])
...
def test_unknown_schema_and_invalid_request_are_sanitized(tmp_path) -> None:
    ...
    runner = CliRunner()
    unknown = runner.invoke(m0702_cli.app, ["export-schema", "unknown"])
    invalid = runner.invoke(m0702_cli.app, ["validate", str(invalid_path)])
    with TestClient(m0702_api.create_app()) as client:
        api_unknown = client.get("/v1/modules/M07-02/schemas/unknown")
        api_invalid = client.post("/v1/modules/M07-02/validate", content=b"{}")
...
def test_cli_output_is_non_overwriting_and_input_handler_is_sanitized(tmp_path) -> None:
    request_path = tmp_path / "request.json"
    request_path.write_bytes(canonical_json_bytes(_request().model_dump(mode="json")))
    output = tmp_path / "result.json"
    first = CliRunner().invoke(
        m0702_cli.app, ["construct", str(request_path), "--output", str(output)]
    )
    ...
    with patch.object(
        m0702_api.M0702Service,
        "construct",
        side_effect=RepresentationInputError("result_digest"),
    ), TestClient(m0702_api.create_app()) as client:
        rejected = client.post(
            "/v1/modules/M07-02/construct",
            content=canonical_json_bytes(_request().model_dump(mode="json")),
        )
    assert rejected.status_code == _HTTP_UNPROCESSABLE
    assert rejected.json() == {"detail": "M07-02 input rejected"}
