"""Transport and lifecycle coverage for the caller-owned GBM RNA mixture lane."""
...
def test_demo_analyze_verify_lifecycle_and_headers() -> None:
    with TestClient(_app()) as client:
        profile = client.get(f"{adapter.MIXTURE_ROUTE_PREFIX}/profile")
        demo = client.get(f"{adapter.MIXTURE_ROUTE_PREFIX}/demo")
        analysis = client.post(
            f"{adapter.MIXTURE_ROUTE_PREFIX}/analyze",
            json=demo.json(),
        )
        verification = client.post(
            f"{adapter.MIXTURE_ROUTE_PREFIX}/verify",
    ...
    assert analysis.json()["support"] == "limited"
...
def test_openapi_contains_all_operations_and_nested_replay_schema() -> None:
    schema = _app().openapi()
    prefix = adapter.MIXTURE_ROUTE_PREFIX
    ...
    assert verify_body["content"]["application/json"]["schema"]["properties"]["request"]
    assert verify_body["content"]["application/json"]["schema"]["properties"]["result"]
    ...
    with TestClient(_app()) as client:
        ...
        oversized = client.post(
            ...
            headers={
                ...
                "content-length": str(adapter.MIXTURE_REQUEST_MAX_BYTES + 1),
            },
        )
...
def test_modified_receipt_is_rejected() -> None:
    with TestClient(_app()) as client:
        ...
        result = client.post(f"{adapter.MIXTURE_ROUTE_PREFIX}/analyze", json=demo).json()
