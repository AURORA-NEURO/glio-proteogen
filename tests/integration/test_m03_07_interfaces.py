"""Public parity and hostile authorization boundaries for M03-07."""
...
PRIVATE_CANARY: Final = "PRIVATE_M0307_INTERFACE_CANARY"
...
def canonical_request() -> RouteProteinInferenceSupportRequest:
    return build_scenario_request()


def _payload(request: RouteProteinInferenceSupportRequest) -> dict[str, Any]:
    return copy.deepcopy(request.model_dump(mode="json"))


def test_library_engine_service_and_plugin_return_equal_result(
    canonical_request: RouteProteinInferenceSupportRequest,
) -> None:
    request = canonical_request
    library = route_protein_inference_support(request)
    engine = M0307ProteinInferenceSupportRouterEngine().route(request)
    service = M0307Service()
    service_result = service.execute(request)
    plugin = M0307Plugin(service)
...
def test_every_service_denial_precedes_hostile_downstream_traversal(
    ...
    with pytest.raises(ProteinInferenceSupportAuthorizationError) as caught:
        M0307Service.validate_request(_HostileRequest(context))

    assert PRIVATE_CANARY not in str(caught.value)


@pytest.mark.parametrize("name", SCHEMA_NAMES)
def test_api_and_cli_export_identical_m03_07_schemas(tmp_path: Path, name: str) -> None:
    with TestClient(create_app(tmp_path / f"schema-{name}.sqlite3")) as client:
        response = client.get(f"/v1/contracts/M03-07/{name}/schema")
    cli = CliRunner().invoke(
        cli_app,
        ["protein-inference-support", "export-schema", name],
    )
    ...
    assert response.json()["$id"] == (
        f"urn:aurora-neuro:glio-proteogen:GLIO-PROTEOGEN-M03-07:1.0.0:{name}"
    )
...
def test_api_and_cli_return_the_same_result_as_public_operation(
    ...
    expected = route_protein_inference_support(canonical_request)
    ...
    cli = CliRunner().invoke(
        cli_app,
        ["protein-inference-support", "route", str(request_path)],
    )
...
def test_every_api_and_cli_denial_is_sanitized_before_prerequisite_validation(
    ...
    assert response.json() == {
        "detail": "upstream controls do not authorize protein-inference support routing"
    }
...
def test_api_and_cli_reject_every_non_strict_json_class_without_disclosure(
    ...
    if mutation in {"duplicate", "nonfinite"}:
        ...
        operation = '"operation":"route_protein_inference_support"'
...
def test_api_and_cli_distinguish_exact_four_mib_from_first_byte_past_limit(
    ...
    exact = b"{" + b" " * (M0307_MAX_CANONICAL_REQUEST_BYTES - 1)
    oversized = exact + b" "
    ...
    exact_cli = CliRunner().invoke(
        cli_app,
        ["protein-inference-support", "route", str(exact_path)],
    )
    oversized_cli = CliRunner().invoke(
        cli_app,
        ["protein-inference-support", "route", str(oversized_path)],
    )

    assert len(exact) == M0307_MAX_CANONICAL_REQUEST_BYTES
    assert len(oversized) == M0307_MAX_CANONICAL_REQUEST_BYTES + 1
...
def test_invalid_schema_name_is_rejected_by_api_and_cli(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(tmp_path / "schema-invalid.sqlite3")) as client:
        response = client.get(f"/v1/contracts/M03-07/{invalid_name}/schema")
    cli = CliRunner().invoke(
        cli_app,
        ["protein-inference-support", "export-schema", invalid_name],
    )
