...
def test_fastapi_service_authorization_error_is_sanitized(monkeypatch) -> None:
    class AuthorizationService:
        def _execute_validated(self, request):
            raise adapter_module.M1205AuthorizationError
...
def test_typer_schema_infer_verify_no_overwrite_and_invalid_files(tmp_path) -> None:
    runner = CliRunner()
    schema = runner.invoke(m1205_app, ["export-schema", "request"])
    assert schema.exit_code == 0 and json.loads(schema.stdout)["$schema"]
    ...
    assert inferred.exit_code == 0 and result_path.exists()
    ...
    assert stdout_inferred.exit_code == 0 and "modeled" in stdout_inferred.stdout
