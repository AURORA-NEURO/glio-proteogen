"""Black-box checks for M02-05 schema, API, and CLI parity."""
...
def test_api_and_cli_export_identical_schemas(tmp_path: Path, name: str) -> None:
    with TestClient(create_app(tmp_path / f"schema-{name}.sqlite3")) as client:
        response = client.get(f"/v1/contracts/M02-05/{name}/schema")
    cli = CliRunner().invoke(
        cli_app,
        ["identification-artifacts", "export-schema", name],
    )
...
def test_api_cli_and_service_emit_identical_results(tmp_path: Path) -> None:
    ...
    expected = M0205Service().execute(request)
    ...
    cli = CliRunner().invoke(
        cli_app,
        ["identification-artifacts", "detect", str(request_path)],
    )

    assert response.status_code == HTTP_OK
    assert cli.exit_code == 0, cli.output
    assert (
        IdentificationArtifactDetectionResult.model_validate_json(
            response.content,
            strict=True,
        )
        == expected
    ...
    assert (
        IdentificationArtifactDetectionResult.model_validate_json(cli.stdout, strict=True)
        == expected
    )


def test_denied_consent_precedes_hostile_signal_traversal(tmp_path: Path) -> None:
    request = build_scenario_request().model_dump(mode="json")
    request["context"]["references"]["consent"]["state"] = "withheld"
    sentinel = "PRIVATE_SIGNAL_CANARY"
    request["signals"] = [{"target_id": sentinel}]
    request_path = tmp_path / "denied.json"
    request_path.write_text(json.dumps(request), encoding="utf-8")

    with TestClient(create_app(tmp_path / "denied.sqlite3")) as client:
        response = client.post("/v1/modules/M02-05/artifacts", json=request)
    cli = CliRunner().invoke(
        cli_app,
        ["identification-artifacts", "detect", str(request_path)],
    )
