"""Black-box transport parity and strict boundary checks for M05-02."""
...
def test_api_and_cli_export_identical_m05_02_schemas(tmp_path: Path, name: str) -> None:
    with TestClient(create_app(tmp_path / f"schema-{name}.sqlite3")) as client:
        response = client.get(f"/v1/contracts/M05-02/{name}/schema")
    cli = CliRunner().invoke(cli_app, ["m05-02-export-schema", name])
...
def test_library_engine_service_plugin_api_and_cli_emit_exact_parity(tmp_path: Path) -> None:
    ...
    request_path = tmp_path / "ptm-lineage.json"
    request_path.write_text(payload, encoding="utf-8")
    service = M0502Service()
    plugin = M0502Plugin(service)
    ...
    cli = CliRunner().invoke(cli_app, ["m05-02-reconcile", str(request_path)])

    assert response.status_code == HTTP_OK, response.text
    assert cli.exit_code == 0, cli.output
    api_result = PtmLocalizationIdentityLineageResolution.model_validate_json(
        response.content, strict=True
    )
    cli_result = PtmLocalizationIdentityLineageResolution.model_validate_json(
        cli.stdout, strict=True
    )
    expected = reconcile_ptm_localization_identity_lineage(request)
    assert expected == M0502Engine().reconcile(request)
...
def test_api_and_cli_deny_each_control_before_claim_validation(
    tmp_path: Path,
    control: str,
    state: str,
) -> None:
...
def test_api_and_cli_reject_request_above_four_mib(tmp_path: Path) -> None:
    oversized = b"{" + (b" " * M0502_MAX_CANONICAL_REQUEST_BYTES)
...
def test_api_rejects_wrong_media_type_and_unknown_schema(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(tmp_path / "media.sqlite3")) as client:
        ...
        schema = client.get("/v1/contracts/M05-02/not-a-contract/schema")
