"""HTTP, OpenAPI, deployment-catalog, and CLI lifecycle for kinase transitions."""
...
def test_central_api_demo_analyze_verify_lifecycle(tmp_path: Path) -> None:
    with TestClient(create_app(tmp_path / "events.sqlite3")) as client:
        ...
        assert not profile.json()["quality_gates"]["same_assay_independent_evidence_gate_passed"]
        assert profile.json()["source_provenance"]["sphinks_license"] == "CC-BY-4.0"

        demo = client.get(f"{PREFIX}/demo")
        assert demo.status_code == HTTP_OK
        assert demo.json()["series_id"] == DEMO_ID
        ...
        assert analysis.json()["infers_kinase_activity"] is False
        assert analysis.json()["independent_evidence"] is False
        first = analysis.json()["transitions"][0]
        chek2 = next(item for item in first["kinase_signatures"] if item["kinase"] == "CHEK2")
        assert chek2["selection_state"] == "selected_unstable"
        assert chek2["support"] == "limited"
...
def test_openapi_and_route_limits_are_exact(tmp_path: Path) -> None:
    ...
    assert analyze_schema["requestBody"]["content"]["application/json"]["schema"] == {
        "$ref": "#/components/schemas/LongitudinalGbmKinaseTransitionRequest"
    }
    verify_schema = schema["paths"][f"{PREFIX}/verify"]["post"]
    assert verify_schema["requestBody"]["content"]["application/json"]["schema"] == {
        "$ref": ("#/components/schemas/LongitudinalGbmKinaseTransitionReplayVerificationRequest")
    }
...
def test_v2_catalog_discovers_every_kinase_transition_operation(tmp_path: Path) -> None:
...
def test_cli_demo_analyze_and_verify(tmp_path: Path) -> None:
    runner = CliRunner()
    demo = runner.invoke(cli_app, ["longitudinal-gbm-kinase-transition", "demo"])
    assert demo.exit_code == 0
    request_path = tmp_path / "request.json"
    request_path.write_text(demo.stdout, encoding="utf-8")
    analysis = runner.invoke(
        cli_app,
        ["longitudinal-gbm-kinase-transition", "analyze", str(request_path)],
    )
    ...
    verification = runner.invoke(
        cli_app,
        ["longitudinal-gbm-kinase-transition", "verify", str(receipt)],
    )
