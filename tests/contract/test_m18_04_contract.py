"""Focused contract/schema smoke for provisional M18-04."""
...
def test_provisional_schemas_preserve_intended_use_boundaries() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["upstreamInputMediaType"].endswith("m18-03+json")
        and schema["x-glio-contract"]["parentTarget"] == "biomarker panel"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1804_OUTPUT_MEDIA_TYPE
    assert M1804_PROVISIONAL_ABI is True
...
def test_registration_keeps_claim_ceiling_and_display_semantics_typed() -> None:
    ...
    registration = IntendedUseRegistration(
        ...
        audience="Research bioinformatics reviewer",
        evidence_tier=2,
        ...
        display_semantics=DisplaySemantics(
            ...
            safe_default="Show bounded research context.",
            evidence=evidence,
        ),
        evidence=evidence,
    )
    assert registration.intended_use is IntendedUseKind.RESEARCH
