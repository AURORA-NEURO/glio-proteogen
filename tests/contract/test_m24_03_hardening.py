"""Deep contract and replay closure for provisional M24-03."""
...
def _decision(role: str) -> UpstreamDecisionReference:
    return UpstreamDecisionReference(
        decision_id=f"m2403.{role}.decision",
    ...
    return ExecutionContext(
        ...
        references=ContextReferences(
            ...
            consent=ConsentReference(
                decision_id="m2403.consent.decision",
    ...
    return BenchmarkDossier(
        ...
        ablations=(
            ComponentAblation(
                ...
                component="stoichiometric-factorization",
...
def _request(request_id: str = "request-1") -> RunBiomarkerPanelInternalBenchmarkRequest:
    dossier = _dossier()
    upstream = _artifact("m2402-result", M2403_M2402_INPUT_MEDIA_TYPE)
    return RunBiomarkerPanelInternalBenchmarkRequest(
        request_id=request_id,
        context=_context(request_id),
        upstream_result=upstream,
        split=dossier.split,
        baseline_runs=dossier.baselines,
        ablations=dossier.ablations,
...
def test_request_binds_context_media_and_unique_source_closure() -> None:
    ...
    with pytest.raises(ValidationError, match="exactly one declared M24-02"):
        request.__class__.model_validate(
            request.model_dump(mode="python") | {"source_artifacts": (_artifact("unrelated"),)},
            strict=True,
        )
