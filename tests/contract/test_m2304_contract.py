...
def test_provisional_schemas_preserve_variant_peptide_transport_boundaries() -> None:
    ...
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M2304_OUTPUT_MEDIA_TYPE
    assert M2304_PROVISIONAL_ABI is True


def test_transport_validation_keeps_dimension_and_identity_typed() -> None:
    validation = TransportValidation(
        validation_id="validation-1",
        dimension=TransportDimension.PLATFORM,
        source_domain="platform-A",
        target_domain="platform-B",
        assay_or_platform="cross-instrument quantification",
    ...
    assert validation.dimension is TransportDimension.PLATFORM
