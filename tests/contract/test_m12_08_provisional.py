"""Focused contract/schema smoke for provisional M12-08."""
...
def test_provisional_schemas_require_reconstructable_dossier_controls() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["primaryArchitecture"] == "bayesian_graph_baseline_stack"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1208_OUTPUT_MEDIA_TYPE
    assert M1208_PROVISIONAL_ABI is True
