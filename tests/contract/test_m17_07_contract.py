"""Focused contract/schema smoke for provisional M17-07."""
...
def test_provisional_schemas_preserve_typed_export_boundaries() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["upstreamInputMediaType"].endswith("m17-06+json")
        and schema["x-glio-contract"]["parentTarget"] == "variant peptide"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1707_OUTPUT_MEDIA_TYPE
    assert M1707_PROVISIONAL_ABI is True
