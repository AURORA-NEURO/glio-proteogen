"""Strict contract/schema smoke for the provisional M07-07 scaffold."""
...
def test_schema_inventory_is_explicitly_provisional() -> None:
    ...
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M0707_OUTPUT_MEDIA_TYPE


def test_calibration_selective_prediction_and_runtime_import_smoke() -> None:
    stratum = CalibrationStratum(
        stratum_id="stratum.m0707.site",
    ...
    prediction_set = CalibratedPredictionSet(
        prediction_set_id="prediction-set.m0707.smoke",
        feature_id="feature.m0707.smoke",
        labels=("high", "low"),
    ...
    assert M0707Service is not None
    assert M0707CalibrationEngine is not None
