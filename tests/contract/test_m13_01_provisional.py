"""Focused contract/schema smoke for provisional M13-01."""
...
def test_provisional_schemas_require_hypothesis_controls() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["primaryArchitecture"] == "multi_block_pls"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1301_OUTPUT_MEDIA_TYPE
    assert M1301_PROVISIONAL_ABI is True
...
def test_hypothesis_without_competing_explanation_is_rejected() -> None:
    with pytest.raises(ValueError, match="at least 1 item"):
        BiologicalHypothesis(
            hypothesis_id="h1",
            version="1.0.0",
            statement="A candidate proteotype has a biological mechanism.",
            mechanism_class="proteotype",
            target_ids=("proteotype",),
