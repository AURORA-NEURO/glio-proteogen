...
import pytest
from evals.m13_01.benchmark import run_benchmark
from evals.m13_01.run import build_scenario_request, run_evaluator
from fastapi.testclient import TestClient
from pydantic import ValidationError
from typer.testing import CliRunner

from glio_proteogen.adapters.m1301 import app, m1301_app
...
def test_all_provisional_schemas_have_strict_metadata_and_unique_ids() -> None:
    ...
    for schema in schemas.values():
        ...
        assert metadata["outputMediaType"] == M1301_OUTPUT_MEDIA_TYPE
...
def test_unsafe_or_unknown_paths_abstain_without_registry(case_id: str) -> None:
    result = m1301_runtime.M1301HypothesisEngine().register(build_scenario_request(case_id))
...
def test_denied_control_preflight_happens_before_hypothesis_evaluation() -> None:
    ...
    with pytest.raises(m1301_runtime.M1301HypothesisAuthorizationError):
        m1301_runtime.M1301HypothesisEngine().register(denied)
...
def test_unknown_fields_and_duplicate_hypotheses_are_rejected() -> None:
    ...
    with pytest.raises(ValidationError):
        RegisterProteotypeHypothesesRequest.model_validate(payload, strict=True)
    ...
    with pytest.raises(ValidationError):
        RegisterProteotypeHypothesesRequest.model_validate(duplicate_payload, strict=True)


def test_result_replay_rejects_digest_and_nested_tampering() -> None:
    engine = m1301_runtime.M1301HypothesisEngine()
    result = engine.register(build_scenario_request())
    with pytest.raises(m1301_runtime.M1301ReplayVerificationError):
...
def test_result_contract_rejects_replay_closure_breaks(field: str) -> None:
    result = m1301_runtime.M1301HypothesisEngine().register(build_scenario_request())
    ...
    with pytest.raises(ValidationError):
        ProteotypeHypothesisRegistryResult.model_validate(forged, strict=True)
...
def test_duplicate_nested_ids_and_unsafe_result_states_are_rejected() -> None:
    ...
    with pytest.raises(ValidationError):
        RegisterProteotypeHypothesesRequest.model_validate(
    ...
    with pytest.raises(ValidationError):
        ProteotypeHypothesisRegistryResult.model_validate(
...
def test_canonical_dict_projection_and_replay_disabled_paths() -> None:
    ...
    result = m1301_runtime.M1301HypothesisEngine().register(request)
    ...
    assert m1301_runtime.M1301HypothesisEngine().verify(result, replay=False) == result


def test_service_public_seams_and_plugin_hostile_token_inputs() -> None:
    request = build_scenario_request()
    service = m1301_runtime.M1301Service()
    assert service.validate_request(request) == request
    assert service.execute(request).status is HypothesisStatus.SUPPORTED
    assert (
        m1301_runtime.register_proteotype_hypotheses(request).status is HypothesisStatus.SUPPORTED
    )
    plugin = m1301_runtime.M1301Plugin(service)
...
def test_http_schema_register_verify_and_sanitized_failures() -> None:
    ...
    response = client.get("/v1/m13-01/schema/request")
    assert response.status_code == HTTP_OK
    assert response.json()["$id"].endswith(":request")
    assert client.get("/v1/m13-01/schema/nope").status_code == HTTP_NOT_FOUND
    response = client.post(
        "/v1/modules/M13-01/hypotheses", content=body, headers={"content-type": "application/json"}
    )
    assert response.status_code == HTTP_OK
    result_payload = response.json()
    response = client.post("/v1/modules/M13-01/verify", json=result_payload)
    assert response.status_code == HTTP_OK
    assert response.json()["result_digest"] == result_payload["result_digest"]
    assert (
        client.post(
            "/v1/modules/M13-01/hypotheses", content=body, headers={"content-type": "text/plain"}
        ).status_code
    ...
    assert (
        client.post(
            "/v1/modules/M13-01/verify", content=b"{}", headers={"content-type": "application/json"}
        ).status_code
        == HTTP_UNPROCESSABLE
    )
    assert (
        client.post(
            "/v1/modules/M13-01/verify", content=b"{}", headers={"content-type": "text/plain"}
        ).status_code
        == HTTP_UNSUPPORTED_MEDIA
    )
...
def test_cli_register_verify_schema_and_no_overwrite(tmp_path: Path) -> None:
    ...
    export = runner.invoke(m1301_app, ["export-schema", "request"])
    assert export.exit_code == 0
    assert '"$id"' in export.stdout
    assert runner.invoke(m1301_app, ["export-schema", "invalid"]).exit_code == CLI_USAGE_ERROR
    registered = runner.invoke(
        m1301_app, ["register", str(request_path), "--output", str(result_path)]
    )
    ...
    verified = runner.invoke(m1301_app, ["verify", str(result_path)])
    assert verified.exit_code == 0
    assert "result_digest" in verified.stdout
    request_path.write_text('{"duplicate": 1, "duplicate": 2}', encoding="utf-8")
    assert runner.invoke(m1301_app, ["register", str(request_path)]).exit_code != 0
