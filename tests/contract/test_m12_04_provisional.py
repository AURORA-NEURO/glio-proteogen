"""Focused contract/schema smoke for provisional M12-04."""
...
def test_provisional_schemas_require_counter_evidence_and_abstention() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["primaryArchitecture"] == "variant_peptide_graph"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1204_OUTPUT_MEDIA_TYPE
    assert M1204_PROVISIONAL_ABI is True
