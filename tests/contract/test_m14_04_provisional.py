"""Focused contract/schema smoke for provisional M14-04."""
...
def test_provisional_schemas_require_counter_evidence_and_abstention() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["primaryArchitecture"] == "ptm_aware_state_model"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1404_OUTPUT_MEDIA_TYPE
    assert M1404_PROVISIONAL_ABI is True
