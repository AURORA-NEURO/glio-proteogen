"""Adversarial M09-07 contract closure tests.
...
from glio_proteogen.contracts.m09_07.canonical import verify_result_replay
...
def _configuration() -> CalibrationConfiguration:
    return CalibrationConfiguration(
        configuration_id="configuration.m0907",
        version="1.0.0",
        ...
        nominal_coverage=M0907_NOMINAL_COVERAGE,
...
def _request(source_artifacts: tuple[object, ...] | None = None):
    ...
    return CalibrateComplexActivitySelectivePredictionRequest(
        request_id="request.m0907",
        context=context,
        uncertainty_result=_artifact(20, media_type="application/vnd.glio-proteogen.m09-06+json"),
...
def test_request_and_result_digest_closures_reject_tampering() -> None:
    request = _request()
    with pytest.raises(ValueError, match="M09-06"):
    ...
    with pytest.raises(ValueError, match="request digest"):
        ComplexActivitySelectivePredictionResult.model_validate(payload, strict=True)
    payload = result.model_dump()
