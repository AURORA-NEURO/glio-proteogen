"""Focused contract/schema smoke for provisional M11-01."""
...
def test_provisional_schemas_require_hypothesis_controls() -> None:
    ...
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M1101_OUTPUT_MEDIA_TYPE
    assert M1101_PROVISIONAL_ABI is True
...
def test_hypothesis_without_competing_explanation_is_rejected() -> None:
    with pytest.raises(ValueError, match="at least 1 item"):
        BiologicalHypothesis(
            hypothesis_id="h1",
            version="1.0.0",
            statement="A candidate variant peptide has a biological mechanism.",
            mechanism_class="variant_peptide",
            target_ids=("variant-peptide",),
