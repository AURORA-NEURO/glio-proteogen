"""Focused contract/schema smoke for provisional M24-04."""
...
def test_provisional_schemas_preserve_biomarker_panel_transport_boundaries() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["ptmAwareStateModelRequired"]
        and schema["x-glio-contract"]["isoformAwareQuantificationAvailable"]
    ...
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M2404_OUTPUT_MEDIA_TYPE
...
def test_transport_validation_keeps_dimension_and_identity_typed() -> None:
    validation = TransportValidation(
        ...
        dimension=TransportDimension.SPECIMEN,
        source_domain="specimen-A",
        target_domain="specimen-B",
        assay_or_platform="PTM-aware quantification",
    ...
    assert validation.dimension is TransportDimension.SPECIMEN
