"""Contract, runtime, adapter, and adversarial coverage for M14-04."""
...
import pytest
from evals.m14_04.run import build_scenario_request, run_evaluator
from fastapi.testclient import TestClient
from typer.testing import CliRunner

import glio_proteogen.modules.c14_microenvironment_protein_deconvolution.m14_04_network_state_mechanism_inference.engine as engine_module
...
def test_estimate_invariants_reject_invalid_posterior_and_state() -> None:
    counter = M1404MechanismEngine().infer(build_scenario_request()).estimates[0].counter_evidence
...
def test_request_and_result_closure_reject_forged_payloads() -> None:
    ...
    with pytest.raises(ValueError, match="provisional M14-01"):
        InferProteinSubtypeMechanismRequest.model_validate(forged_request, strict=True)
    ...
    with pytest.raises(ValueError, match="abstained result"):
        ProteinSubtypeMechanismInferenceResult.model_validate(invalid_abstention, strict=True)
    ...
    with pytest.raises(ValueError, match="human review"):
        ProteinSubtypeMechanismInferenceResult.model_validate(no_review, strict=True)
...
def test_method_matrix_is_deterministic_and_safe(method: str) -> None:
    result = M1404MechanismEngine().infer(build_scenario_request(method))
...
def test_counter_evidence_gate_abstains_when_source_refs_are_empty(
    ...
    normal = M1404MechanismEngine().infer(build_scenario_request())
    ...
    with pytest.raises(ValueError):
        M1404MechanismEngine()._result(request)


def test_authorization_runs_before_typed_traversal() -> None:
    with pytest.raises(M1404MechanismAuthorizationError):
        M1404MechanismEngine().infer(build_scenario_request(accepted=False))
    with pytest.raises(M1404MechanismAuthorizationError):
        M1404MechanismEngine().infer({"context": {"references": {}}})
    ...
    with pytest.raises(M1404MechanismAuthorizationError):
        M1404MechanismEngine().infer(Exploding())
...
def test_replay_and_tamper_detection(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(M1404ReplayVerificationError):
    ...
    original_infer = engine_module.M1404MechanismEngine.infer
    monkeypatch.setattr(
        engine_module.M1404MechanismEngine,
        "infer",
    ...
    assert (
        engine_module.infer_protein_subtype_mechanism(build_scenario_request()).status
...
def test_plugin_is_parse_once_and_token_bound() -> None:
    service = M1404Service()
    plugin = M1404Plugin(service)
    request = build_scenario_request()
    token = plugin.validate(canonical_json_bytes(request))
    assert isinstance(token, ValidatedM1404Request)
    assert plugin.run(token).status is MechanismInferenceStatus.INFERRED
    with pytest.raises(TypeError):
        plugin.run(ValidatedM1404Request(request=request, _seal=object()))
    with pytest.raises(TypeError):
    ...
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M14-04"
...
def test_http_schema_infer_verify_and_sanitized_errors() -> None:
    ...
    assert client.get("/v1/m14-04/schema/request").status_code == 200
    assert client.get("/v1/m14-04/schema/nope").status_code == 404
    response = client.post("/v1/modules/M14-04/mechanism", json=payload)
    assert response.status_code == 200
    result_payload = response.json()
    verified = client.post("/v1/modules/M14-04/verify", json=result_payload)
    assert verified.status_code == 200
    assert (
        client.post(
            "/v1/modules/M14-04/mechanism", content=b"{}", headers={"content-type": "text/plain"}
        ).status_code
        == 415
    )
    assert (
        client.post(
            "/v1/modules/M14-04/mechanism",
            content=b"{",
    ...
    assert client.post("/v1/modules/M14-04/mechanism", json=invalid).status_code == 422
...
def test_http_denies_controls_and_rejects_tamper() -> None:
    ...
    assert (
        client.post("/v1/modules/M14-04/mechanism", json=denied.model_dump(mode="json")).status_code
        == 403
    )
...
def test_http_service_authorization_failure_is_sanitized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(M1404Service, "_execute_validated", denied)
    client = TestClient(app)
    ...
    result = M1404MechanismEngine().infer(build_scenario_request()).model_dump(mode="json")
    result["result_digest"] = "sha256:" + "f" * 64
    assert client.post("/v1/modules/M14-04/verify", json=result).status_code == 422
    assert (
        client.post(
            "/v1/modules/M14-04/verify", content=b"{}", headers={"content-type": "text/plain"}
        ).status_code
        == 415
    )
...
def test_cli_export_infer_verify_and_no_overwrite(tmp_path: Path) -> None:
    ...
    assert runner.invoke(m1404_app, ["export-schema", "request"]).exit_code == 0
    inferred = runner.invoke(m1404_app, ["infer", str(request_path), "--output", str(result_path)])
    assert inferred.exit_code == 0
    assert runner.invoke(m1404_app, ["infer", str(request_path)]).exit_code == 0
    assert (
        runner.invoke(
            m1404_app, ["infer", str(request_path), "--output", str(result_path)]
        ).exit_code
        != 0
    )
    assert runner.invoke(m1404_app, ["verify", str(result_path)]).exit_code == 0
    result_path.write_text("{", encoding="utf-8")
    assert runner.invoke(m1404_app, ["verify", str(result_path)]).exit_code != 0
    assert runner.invoke(m1404_app, ["export-schema", "bad"]).exit_code == 2
...
def test_strict_json_duplicate_keys_are_rejected(tmp_path: Path) -> None:
    ...
    result = runner.invoke(m1404_app, ["infer", str(request_path)])
