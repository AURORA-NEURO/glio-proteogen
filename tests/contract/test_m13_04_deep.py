...
def test_typed_proteotype_solver_backtracks_objective_increase(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    base = build_scenario_request()
    request = base.model_copy(
        update={
            "configuration": base.configuration.model_copy(
                update={"model_family": M1304_GLIOMA_MODEL_FAMILY, "bootstrap_replicates": 8}
            ),
            "typed_observations": (
                MechanismObservation(
                    observation_id="obs.egfr",
