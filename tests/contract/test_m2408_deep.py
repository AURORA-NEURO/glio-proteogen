"""Deep M24-08 contract closure and adversarial schema tests."""
...
def _evidence(label: str = "evidence") -> EvidenceReference:
    return EvidenceReference(
        ...
        role="evidence",
        claim="Caller-declared M24-08 gate evidence.",
    )
...
def _controls() -> ContextReferences:
    def decision(label: str) -> UpstreamDecisionReference:
        return UpstreamDecisionReference(
            decision_id=f"m2408.decision.{label}",
    ...
    return ContextReferences(
        ...
        identity_lineage=IdentityLineageReference(
            decision_id="m2408.decision.identity",
        ...
        consent=ConsentReference(
            decision_id="m2408.decision.consent",
...
def _requirements(*, satisfied: bool = True) -> tuple[GateRequirement, ...]:
    return tuple(
        GateRequirement(
            requirement_id=f"m2408.requirement.{category.value}",
            category=category,
...
def _benchmarks(*, passed: bool = True) -> tuple[BenchmarkOutcome, ...]:
    return (
        BenchmarkOutcome(
            benchmark_id="m2408.benchmark.locked",
...
def _approvals(
    *, decision: ApprovalDecision = ApprovalDecision.APPROVE
    ...
    return (
        ApprovalRecord(
            approval_id="m2408.approval.release",
...
def _obligations() -> tuple[PostReleaseObligation, ...]:
    return (
        PostReleaseObligation(
            obligation_id="m2408.obligation.review",
...
def _configuration() -> GateConfiguration:
    return GateConfiguration(
        configuration_id="m2408.configuration.locked",
        version="0.1.0",
...
def _request(
    *,
    ...
    request_id: str = "m2408.request.1",
) -> AdjudicateBiomarkerPanelEvidenceGateRequest:
    inputs = tuple(
        _artifact(label)
        for label in (
...
def test_authority_schema_and_all_requirement_categories_are_locked() -> None:
    ...
    assert M2408_DOSSIER_SHA256.startswith("sha256:")
    assert M2408_DOSSIER_SLICE.endswith("8580-8620")
    assert all(
        schema["x-glio-contract"]["dossierSha256"] == M2408_DOSSIER_SHA256
...
def test_exact_source_artifacts_and_context_identity_are_closed() -> None:
    ...
    data["source_artifacts"][-1]["artifact_id"] = "m2408.forged"
    with pytest.raises(ValueError, match="source artifacts"):
        AdjudicateBiomarkerPanelEvidenceGateRequest.model_validate_json(
            canonical_json_bytes(data),
            strict=True,
        )
    data = request.model_dump(mode="json")
    data["context"]["request_id"] = "m2408.mismatch"
...
def test_benchmark_and_record_status_closures_are_explicit() -> None:
    with pytest.raises(ValueError, match="passed flag"):
        BenchmarkOutcome(
            benchmark_id="m2408.invalid-benchmark",
    ...
    with pytest.raises(ValueError, match="unsatisfied"):
        SignedReleaseRecord(
            release_id="m2408.release.invalid",
            version="0.1.0",
