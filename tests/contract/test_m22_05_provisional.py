"""Focused contract/schema smoke for provisional M22-05."""
...
def test_provisional_schemas_require_subgroup_equity_controls() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["upstreamInputMediaType"].endswith("m22-04+json")
        and schema["x-glio-contract"]["parentTarget"] == "protein-RNA discordance"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M2205_OUTPUT_MEDIA_TYPE
    assert M2205_PROVISIONAL_ABI is True
