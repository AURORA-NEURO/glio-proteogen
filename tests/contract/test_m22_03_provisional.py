"""Focused contract/schema smoke for provisional M22-03."""
...
def test_provisional_schemas_require_locked_benchmark_controls() -> None:
    ...
    assert all(
        schema["x-glio-contract"]["upstreamInputMediaType"].endswith("m22-02+json")
        and schema["x-glio-contract"]["parentTarget"] == "protein-RNA discordance"
        for schema in schemas.values()
    )
    assert schemas["output"]["x-glio-contract"]["outputMediaType"] == M2203_OUTPUT_MEDIA_TYPE
    assert M2203_PROVISIONAL_ABI is True
