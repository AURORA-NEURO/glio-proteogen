...
def test_fastapi_sanitizes_validation_auth_and_replay_errors() -> None:
    ...
    invalid = client.post(
        "/v1/modules/M19-01/resolve",
        content=canonical_json_bytes(request.model_copy(update={"candidates": ()})),
        headers={"content-type": "application/json"},
    )
    ...
    assert client.get("/v1/m19-01/schema/not-a-schema").status_code == HTTP_NOT_FOUND
    assert (
        client.post(
            "/v1/modules/M19-01/verify", content=b"{}", headers={"content-type": "text/plain"}
        ).status_code
        == HTTP_UNSUPPORTED_MEDIA_TYPE
    )
    assert (
...
def test_fastapi_resolution_failure_is_sanitized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("glio_proteogen.adapters.m1901._SERVICE.resolve", _fail)
    response = client.post(
        "/v1/modules/M19-01/resolve",
...
def test_typer_schema_and_verify_commands_are_available(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_typer_resolve_verify_and_no_overwrite_are_executable(tmp_path: Path) -> None:
    runner = CliRunner()
    request_path = tmp_path / "request.json"
    request_path.write_bytes(canonical_json_bytes(_request()))
    result_path = tmp_path / "result.json"
    stdout_result = runner.invoke(m1901_app, ["resolve", str(request_path)])
    assert stdout_result.exit_code == 0
    resolved = runner.invoke(
        m1901_app,
        ["resolve", str(request_path), "--output", str(result_path)],
    )
    assert resolved.exit_code == 0
    assert result_path.exists()
    verified = runner.invoke(m1901_app, ["verify", str(result_path)])
    ...
    invalid_schema = runner.invoke(m1901_app, ["export-schema", "not-a-schema"])
    assert invalid_schema.exit_code == CLI_SCHEMA_ERROR
    result_path.write_text("{}", encoding="utf-8")
    invalid_verify = runner.invoke(m1901_app, ["verify", str(result_path)])
    assert invalid_verify.exit_code == 1
    invalid_request = tmp_path / "invalid.json"
    invalid_request.write_text("{}", encoding="utf-8")
    invalid_resolve = runner.invoke(m1901_app, ["resolve", str(invalid_request)])
    assert invalid_resolve.exit_code == 1
