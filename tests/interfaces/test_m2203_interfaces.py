"""FastAPI, Typer, and plugin parity tests for provisional M22-03."""
...
def test_fastapi_validate_benchmark_verify_and_sanitized_errors() -> None:
    ...
    schemas = client.get("/v1/modules/M22-03/schemas")
    ...
    assert client.post("/v1/modules/M22-03/validate", json=body).status_code == _HTTP_OK
    generated = client.post("/v1/modules/M22-03/benchmark", json=body)
    assert generated.status_code == _HTTP_OK
    verified = client.post("/v1/modules/M22-03/verify", json={"result": generated.json()})
    assert verified.status_code == _HTTP_OK
    assert verified.json()["verified"] is True
    assert client.get("/v1/modules/M22-03/schemas/unknown").status_code == _HTTP_NOT_FOUND
    assert (
        client.post("/v1/modules/M22-03/validate", content=b"[]").status_code == _HTTP_UNPROCESSABLE
    )
    malformed = client.post("/v1/modules/M22-03/verify", content=b"[")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M22-03/verify", json={}).status_code == _HTTP_UNPROCESSABLE
...
def test_fastapi_denied_controls_are_sanitized() -> None:
    ...
    client = TestClient(create_app(M2203Service()))
    validate_response = client.post("/v1/modules/M22-03/validate", json=denied)
    benchmark_response = client.post("/v1/modules/M22-03/benchmark", json=denied)
    assert validate_response.status_code == _HTTP_UNPROCESSABLE
    assert benchmark_response.status_code == _HTTP_UNPROCESSABLE
    assert "Traceback" not in benchmark_response.text
...
def test_plugin_is_strict_parse_once_and_requires_token() -> None:
    ...
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M22-03"
...
def test_typer_sanitizes_bad_inputs_and_replay_failures(
    ...
    result_path.write_bytes(canonical_json_bytes(M2203Service().generate(_request())))
