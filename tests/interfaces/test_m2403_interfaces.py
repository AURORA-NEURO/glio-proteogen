"""FastAPI, Typer, and plugin parity tests for provisional M24-03."""
...
def test_fastapi_schema_validate_benchmark_and_verify_parity() -> None:
    client = TestClient(create_app(M2403Service()))
    schemas = client.get("/v1/modules/M24-03/schemas")
    assert schemas.status_code == _HTTP_OK
    assert len(schemas.json()) == _SCHEMA_COUNT
    request_json = _request().model_dump_json()
    validated = client.post("/v1/modules/M24-03/validate", content=request_json)
    assert validated.status_code == _HTTP_OK
    benchmark = client.post("/v1/modules/M24-03/benchmark", content=request_json)
    assert benchmark.status_code == _HTTP_OK
    result = benchmark.json()
    verified = client.post("/v1/modules/M24-03/verify", json={"result": result})
...
def test_fastapi_sanitizes_non_object_and_unknown_schema_errors() -> None:
    client = TestClient(create_app(M2403Service()))
    malformed = client.post("/v1/modules/M24-03/validate", content=b"[]")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert "Traceback" not in malformed.text
    unknown = client.get("/v1/modules/M24-03/schemas/unknown")
    assert unknown.status_code == _HTTP_NOT_FOUND
    replay = client.post("/v1/modules/M24-03/verify", content=b"[]")
    assert replay.status_code == _HTTP_UNPROCESSABLE


def test_typer_export_validate_benchmark_and_verify_no_overwrite(tmp_path: Path) -> None:
    runner = CliRunner()
    ...
    verify = runner.invoke(cli_app, ["verify", str(result_path)])
    assert verify.exit_code == 0
    assert json.loads(verify.stdout)["verified"] is True


def test_typer_rejects_hostile_json_without_leaking_validation_details(tmp_path: Path) -> None:
    runner = CliRunner()
    hostile = tmp_path / "hostile.json"
    hostile.write_bytes(b"[]")
    result = runner.invoke(cli_app, ["validate", str(hostile)])
