"""FastAPI, Typer, and plugin parity tests for provisional M23-03."""
...
def test_fastapi_schema_validate_benchmark_and_verify_parity() -> None:
    ...
    schemas = client.get("/v1/modules/M23-03/schemas")
    assert schemas.status_code == _HTTP_OK
    assert len(schemas.json()) == _SCHEMA_COUNT
    request_json = _request().model_dump_json()
    validated = client.post("/v1/modules/M23-03/validate", content=request_json)
...
def test_fastapi_sanitizes_non_object_and_unknown_schema_errors() -> None:
    client = TestClient(create_app(M2303Service()))
    malformed = client.post("/v1/modules/M23-03/validate", content=b"[]")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert "Traceback" not in malformed.text
    unknown = client.get("/v1/modules/M23-03/schemas/unknown")
    assert unknown.status_code == _HTTP_NOT_FOUND
    replay = client.post("/v1/modules/M23-03/verify", content=b"[]")
