...
def test_api_schema_surface_is_closed() -> None:
    ...
    response = client.get("/v1/modules/M25-04/schemas")
    named = client.get("/v1/modules/M25-04/schemas/request")
...
def test_api_denied_and_malformed_requests_are_sanitized() -> None:
    ...
    denied_validate = client.post(
        "/v1/modules/M25-04/validate", json=denied_request().model_dump(mode="json")
    )
    denied = client.post(
