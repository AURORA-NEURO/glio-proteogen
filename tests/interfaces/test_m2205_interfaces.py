"""FastAPI, Typer, and plugin parity tests for provisional M22-05."""
...
def test_fastapi_validate_evaluate_verify_and_sanitized_errors() -> None:
    ...
    schemas = client.get("/v1/modules/M22-05/schemas")
    ...
    assert client.post("/v1/modules/M22-05/validate", json=body).status_code == _HTTP_OK
    generated = client.post("/v1/modules/M22-05/evaluate", json=body)
    assert generated.status_code == _HTTP_OK
    verified = client.post("/v1/modules/M22-05/verify", json={"result": generated.json()})
    assert verified.status_code == _HTTP_OK
    assert verified.json()["verified"] is True
    assert client.get("/v1/modules/M22-05/schemas/unknown").status_code == _HTTP_NOT_FOUND
    assert (
        client.post("/v1/modules/M22-05/validate", content=b"[]").status_code == _HTTP_UNPROCESSABLE
    )
    malformed = client.post("/v1/modules/M22-05/verify", content=b"[")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M22-05/verify", json={}).status_code == _HTTP_UNPROCESSABLE
    assert (
        client.post("/v1/modules/M22-05/verify", content=b"[]").status_code == _HTTP_UNPROCESSABLE
    )
...
def test_fastapi_unsupported_coverage_is_explicit_abstention() -> None:
    ...
    payload = request.model_copy(update={"coverage": (unsupported,)}).model_dump(mode="json")
    response = TestClient(create_app(M2205Service())).post(
        "/v1/modules/M22-05/evaluate", json=payload
    )
...
def test_fastapi_denied_controls_are_sanitized() -> None:
    ...
    client = TestClient(create_app(M2205Service()))
    validate_response = client.post("/v1/modules/M22-05/validate", json=denied)
    evaluate_response = client.post("/v1/modules/M22-05/evaluate", json=denied)
...
def test_plugin_is_strict_parse_once_and_requires_token() -> None:
    ...
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M22-05"
...
def test_typer_sanitizes_bad_inputs_and_replay_failures(
    ...
    result_path.write_bytes(canonical_json_bytes(M2205Service().evaluate(_request())))
