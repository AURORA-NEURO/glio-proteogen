"""FastAPI, Typer, and plugin parity tests for provisional M22-07."""
...
def test_fastapi_validate_evaluate_verify_and_sanitized_errors() -> None:
    ...
    schemas = client.get("/v1/modules/M22-07/schemas")
    ...
    assert client.post("/v1/modules/M22-07/validate", json=body).status_code == _HTTP_OK
    generated = client.post("/v1/modules/M22-07/evaluate", json=body)
    assert generated.status_code == _HTTP_OK
    verified = client.post("/v1/modules/M22-07/verify", json={"result": generated.json()})
    assert verified.status_code == _HTTP_OK
    assert verified.json()["verified"] is True
    assert client.get("/v1/modules/M22-07/schemas/unknown").status_code == _HTTP_NOT_FOUND
    assert (
        client.post("/v1/modules/M22-07/validate", content=b"[]").status_code == _HTTP_UNPROCESSABLE
    )
    malformed = client.post("/v1/modules/M22-07/verify", content=b"[")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M22-07/verify", json={}).status_code == _HTTP_UNPROCESSABLE
...
def test_fastapi_not_evaluable_dimension_is_explicit_abstention() -> None:
    ...
    response = TestClient(m2207.create_app(m2207.M2207Service())).post(
        "/v1/modules/M22-07/evaluate", json=request
    )
...
def test_fastapi_denied_controls_are_sanitized() -> None:
    ...
    client = TestClient(m2207.create_app(m2207.M2207Service()))
    validate_response = client.post("/v1/modules/M22-07/validate", json=denied)
    evaluate_response = client.post("/v1/modules/M22-07/evaluate", json=denied)
...
def test_plugin_is_strict_parse_once_and_requires_token() -> None:
    ...
    validated = plugin.validate(
        m2207.HumanFactorsEvaluationSubmission(request=request.model_dump_json())
    )
    result = plugin.run(validated)
    assert plugin.replay(result).result_digest == result.result_digest
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M22-07"
...
def test_typer_export_validate_evaluate_verify_and_no_overwrite(tmp_path: Path) -> None:
    ...
    assert (
        runner.invoke(
            m2207.app, ["export-schema", "request", "--output", str(schema_path)]
        ).exit_code
        == 0
    )
    assert (
    ...
    assert runner.invoke(m2207.app, ["validate", str(request_path)]).exit_code == 0
    assert runner.invoke(m2207.app, ["evaluate", str(request_path)]).exit_code == 0
    assert (
        runner.invoke(
...
def test_typer_sanitizes_bad_inputs_and_replay_failures(
    ...
    runner = CliRunner()
    assert runner.invoke(m2207.app, ["export-schema", "unknown"]).exit_code != 0
    bad_request = tmp_path / "bad-request.json"
    bad_request.write_bytes(b"[]")
    assert runner.invoke(m2207.app, ["validate", str(bad_request)]).exit_code != 0
    assert runner.invoke(m2207.app, ["evaluate", str(bad_request)]).exit_code != 0
    bad_result = tmp_path / "bad-result.json"
    bad_result.write_bytes(b"[]")
    assert runner.invoke(m2207.app, ["verify", str(bad_result)]).exit_code != 0
    result_path = tmp_path / "valid-result.json"
    result_path.write_bytes(canonical_json_bytes(m2207.M2207Service().evaluate(_request())))
