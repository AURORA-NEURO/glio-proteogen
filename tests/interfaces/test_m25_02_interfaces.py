"""FastAPI, Typer, and plugin parity tests for provisional M25-02."""
...
def test_fastapi_validate_generate_verify_and_sanitized_errors() -> None:
    request = _request()
    client = TestClient(create_app(M2502Service()))
    schemas = client.get("/v1/modules/M25-02/schemas")
    assert schemas.status_code == _HTTP_OK
    assert set(schemas.json()) == _CONTRACT_NAMES
    body = request.model_dump(mode="json")
    assert client.post("/v1/modules/M25-02/validate", json=body).status_code == _HTTP_OK
    generated = client.post("/v1/modules/M25-02/generate", json=body)
    assert generated.status_code == _HTTP_OK
    verified = client.post("/v1/modules/M25-02/verify", json={"result": generated.json()})
    assert verified.status_code == _HTTP_OK
    assert verified.json()["verified"] is True
    assert client.get("/v1/modules/M25-02/schemas/unknown").status_code == _HTTP_NOT_FOUND
    assert client.get("/v1/modules/M25-02/schemas/request").status_code == _HTTP_OK
    malformed = client.post("/v1/modules/M25-02/verify", content=b"[")
    assert malformed.status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M25-02/validate", json=[]).status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M25-02/verify", json=[]).status_code == _HTTP_UNPROCESSABLE
    assert client.post("/v1/modules/M25-02/verify", json={}).status_code == _HTTP_UNPROCESSABLE
...
def test_fastapi_denied_controls_are_sanitized() -> None:
    ...
    response = TestClient(create_app(M2502Service())).post(
        "/v1/modules/M25-02/generate", json=denied
    )
    assert response.status_code == _HTTP_UNPROCESSABLE
    validate = TestClient(create_app(M2502Service())).post(
        "/v1/modules/M25-02/validate", json=denied
    )
...
def test_plugin_is_strict_parse_once_and_requires_token() -> None:
    request = _request()
    plugin = M2502Plugin(M2502Service())
    ...
    assert plugin.descriptor().module_id == "GLIO-PROTEOGEN-M25-02"
...
def test_typer_export_generate_verify_and_no_overwrite(tmp_path: Any) -> None:
    ...
    assert M2502Service.export_json(M2502Service().generate(_request()))
...
def test_typer_sanitizes_bad_inputs_and_replay_failures(
    ...
    result_path.write_bytes(canonical_json_bytes(M2502Service().generate(_request())))
