...
def test_fastapi_enforces_request_ceiling_before_parsing() -> None:
    client = TestClient(create_app(M2404Service()))
    oversized = b"{" + b"x" * M2404_MAX_CANONICAL_REQUEST_BYTES + b"}"
    response = client.post("/v1/modules/M24-04/validate", content=oversized)
    assert response.status_code == _HTTP_PAYLOAD_TOO_LARGE
    assert "request body exceeds the byte limit" in response.text
