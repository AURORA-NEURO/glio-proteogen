"""Runtime, service, plugin, and replay tests for provisional M23-03."""
...
from tests.contract.test_m23_03_hardening import _artifact, _request
...
def test_preflight_fails_closed_for_denied_or_malformed_controls() -> None:
    ...
    with pytest.raises(M2303AuthorizationError):
        M2303Service().generate(request.model_copy(update={"context": denied_context}))
    with pytest.raises(M2303AuthorizationError):
        preflight_m2303_authorization({"context": {}})


def test_wrong_upstream_media_is_rejected_at_contract_boundary() -> None:
    request = _request()
    with pytest.raises(ValidationError, match="M23-02"):
        M2303Service().generate(
...
def test_replay_rejects_result_tampering_and_plugin_requires_capability() -> None:
    ...
    with pytest.raises(M2303ReplayError, match="payload digest"):
        service.replay(result.model_copy(update={"result_digest": "sha256:" + "0" * 64}))
    plugin = M2303Plugin(service)
    validated = plugin.validate(BenchmarkSubmission(request=_request()))
    assert isinstance(validated, ValidatedM2303Request)
